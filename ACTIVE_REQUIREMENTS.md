...
- Status: merged to develop and deployed to development; RED captured for catalog sales spec template contract and frontend sales spec rows; targeted catalog/BOM/costing/sales/production Go packages, frontend helper tests, frontend build, full Go suite, changed verifier, diff check, API write smoke, API route smoke, and browser UI smoke are green.
