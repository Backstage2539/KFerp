...
- Status: follow-up verified locally after deployed PR-505; RED captured for template-owned inventory unit, frontend payload/UI, catalog effective-unit priority, and the sales-spec-template UI conversion-table cleanup. Current follow-up GREEN: targeted catalog/BOM/costing/sales/production Go packages, targeted frontend helper/source tests, frontend build, browser layout/save acceptance with a local mock, changed verifier, and diff check. Merge, deploy, API/browser smoke pending.
