...
- Current follow-up branch: codex/missing-product-bom-create-flow-20260604
...
- Current follow-up scope: 商品档案行没有生产 BOM 时，商品 BOM列表不能只显示“无生产 BOM / 未维护”和禁用按钮；前端提供 `创建BOM`，自动创建生产 BOM、生成 V001 已发布版本、绑定商品，再进入配方明细维护。
- DEV:
  ...
  - DEV-406-MISSING-PRODUCT-BOM-CREATE：缺少生产 BOM 的商品档案行显示 `创建BOM`；创建时复用 `/api/production-boms` 和 `/api/products/:id/production-bom-binding`，不再让用户停在不可处理的禁用状态。
...
- Current follow-up notes: 截图中的“无生产 BOM / 未维护”行是 `/api/bom/list` 返回的商品档案行，`product_id>0` 但 `production_bom_id=0`，不是未绑定生产 BOM 档案。旧前端按 `production_bom_id>0` 禁用勾选、版本、复制和失效，导致用户看到不可处理的死行。本轮改为创建并绑定生产 BOM。Deploy evidence: Docker build ran `go test ./...`; containers running; unauthenticated `/app/` returned 303 to `/app/orders`; authenticated `/app/vue-shell` returned 200; requirement API exposes `PR-406-BOM-PRODUCT-ALIAS-LAYOUT`; authenticated `/app/api/bom/list` and `/app/api/production-boms?status=all` returned 200; remote source includes `createProductionBomForProductRow`, `defaultProductionBomNameForProduct`, and `production-bom-binding`; app logs show normal startup on `:8080`.
