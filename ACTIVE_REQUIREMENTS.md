...
- Status: follow-up verified locally after deployed PR-505; RED captured for template-owned inventory unit, frontend payload/UI, and catalog effective-unit priority. Current follow-up GREEN: targeted catalog/BOM/costing/sales/production Go packages, targeted frontend helper tests, frontend build, changed verifier, and diff check. Merge, deploy, API/browser smoke pending.
- Scope: 原 `单位模板` 改名并改义为 `销售规格模板`。销售规格模板维护唯一库存单位、规格名称、销售单位、净含量、默认规格和启用状态；父 SKU/商品档案只选择销售规格模板，不再单独维护库存单位；父 SKU 保存或模板变更后自动派生子 SKU，子 SKU 不手工新增、不配置库存单位，库存单位实时继承销售规格模板。
- DEV:
  ...
  - DEV-505-TEMPLATE-INVENTORY-UOM：销售规格模板保存 `inventory_unit`；商品档案新增/配置只读展示 `库存单位：来自销售规格模板`，子 SKU、BOM、价格表、录单和生产查询读取子 SKU 销售单位 + 模板有效库存单位。
