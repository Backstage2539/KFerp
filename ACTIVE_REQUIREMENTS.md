...
- Restart guard: production acceptance-record restart exposed 24 legacy product bindings recreated by startup compatibility backfills. Real-PostgreSQL and source-contract RED/GREEN now require both PR-403 repair and the system library backfill to select only active target BOMs, so intentionally inactive cutover sources cannot be rebound; production reconciliation and restart verification are part of final acceptance.
