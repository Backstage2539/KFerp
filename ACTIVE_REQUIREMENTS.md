...
- Status: doing；production 目标内部员工账号已通过既有账号启停 API 恢复登录并留下原有审计，正在实现小程序上拉品牌标识和管理员当前账号防自停用
...
- DEV:
  ...
  - DEV-591-DOCS-DEVELOPMENT-DELIVERY（doing）：同步需求、验收、小程序员工 ERP、客户门户、设置审计手册、种子和支持合同；通过定向 Go/Vue/miniapp、构建及开发部署门禁后合入并部署 development。
- Verifier:
...
- Deployment: production 目标账号已通过既有账号启停 API 即时恢复并确认审计动作；功能分支、develop 合入和 development 部署仍在进行。production 代码部署、微信上传、提审和发布未执行。
