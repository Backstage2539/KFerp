...
- Deployment: merged to `origin/develop` at `fe034bac60dbeaee3158ff199e1f66d160e1e472` and deployed to development only. Source backup `/opt/stacks/erp/orderapp.backup.deploy-20260825122143-fe034bac60db`; rollback image `kferp-orderapp-rollback:development-20260825122143-fe034bac60db`; external login HTTP 200, protected API unauthenticated HTTP 401, PostgreSQL healthy. Production was not touched.
...
- Verifier:
  ...
  - Review/acceptance: restored-production-clone apply/idempotency/rollback/dependency-abort GREEN; live production 31/30/1/31/26/5 counts, special recipes, old-binding removal, audit trace and idempotency GREEN; both startup compatibility paths are guarded and a second production restart retained zero old bindings; Van acceptance remains pending
- Deployment: development runs the final guard at `develop@663c563088d23fc9f0c45afd477fed1f60b4a5f5` with source backup `/opt/stacks/erp/orderapp.backup.deploy-20260824152409-663c563088d2` and rollback image `kferp-orderapp-rollback:development-20260824152409-663c563088d2`; production runs it at `main@c2fb95cd1393b5f661f2e5a1d72e3b0ec4aa488f` with source backup `/opt/stacks/erp-production/orderapp.backup.deploy-20260824151259-c2fb95cd1393` and rollback image `kferp-orderapp-rollback:production-20260824151259-c2fb95cd1393`. Both login smokes returned HTTP 200 and production protected API returned 401 without credentials.
- Production data: locked preview was `ready` with zero unfinished dependencies; verified custom-format backup `/opt/stacks/erp-production/backups/pr606-pre-cutover-20260824143425.dump` (2,256,127 bytes, 1,955 restore-list items, SHA-256 `81c881dee5bef781fa484962ad8748f5a58eae41eaf3848eb42913dad5d27642`) preceded the single-transaction apply. Final state after audited compatibility cleanup and a second restart is 31 valid semi-finished materials/replacement BOMs, 26 published defaults/can-manufacture and 5 empty drafts; all 31 source BOMs are inactive and old product bindings/output bindings/configs are zero. Reapply wrote no changes, the apply audit count remains one, and the exact 24-row restart reconciliation has one audit.
- Last update: 2026-08-24 Asia/Shanghai
...
- Restart guard: production acceptance-record restart exposed 24 legacy product bindings recreated by startup compatibility backfills. Real-PostgreSQL and source-contract RED/GREEN now require both PR-403 repair and the system library backfill to select only active target BOMs. Both guards are deployed; the 24 rows were removed in one count-asserted transaction with `repair_restart_reintroduced_bindings` audit, and a subsequent production restart kept all three old binding surfaces at zero.
