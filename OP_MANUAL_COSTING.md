...
6. 在产品豆单的“豆单版本列表”查看官方、我的客户豆单或指定客户豆单版本；需要客户自己的豆单时先在 SKU设置 选择客户并按需开启公共 SKU/公共商品分类引用，再回到产品豆单选择该客户。
...
- 创建客户专属 SKU 时先选择产品形态：熟豆可维护烘焙度并可复制 BOM；生豆会展示生豆属性和绑定熟豆；挂耳会展示每袋克重和每盒袋数。基础产品下拉会按当前形态筛选公共 SKU。
- 在“商品分类 · 客户SKU”中打开“是否使用公共商品分类”会把公共分类作为只读引用显示；在“客户SKU列表 · 客户 SKU”中打开“是否使用公共SKU”会把公共 SKU 作为只读引用显示。两个开关关闭后，公共分类和公共 SKU 会从当前客户视图移除。
- 公共分类和公共 SKU 不再复制成客户自己的数据；开关只保存引用状态，并写入操作日志。客户真正需要差异化时，使用“客户专属 SKU”或新增客户自己的分类。
- 客户SKU列表可按形态、名称、一级分类和二级分类筛选；列表中的“备注”用于记录 SKU 级说明，公共引用行只读展示备注；筛选会先匹配当前归属下全部 SKU，再按分页展示，切换筛选条件后回到第一页。
