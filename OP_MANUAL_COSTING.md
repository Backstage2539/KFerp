...
- 在价格来源抽屉里修改生豆成本、出成率或利润率只影响当前临时试算，不会保存。需要长期生效时，出成率回到 BOM 或产品出成率维护，商用熟豆利润率回到 SKU设置 的梯度模板维护，挂耳倍率回到挂耳供应价模板维护。
...
- 进入产品豆单后，发布归属默认是棵凡官方豆单；选择“指定客户豆单”并选择客户后，价格试算、豆单预览和版本列表使用该客户上下文。
- 选择公共/官方归属时，产品豆单展示公共SKU的试算、公共豆单和公共分类。
- 选择指定客户豆单时，产品豆单展示公共SKU加该客户SKU范围内的客户自己的价格试算；生成豆单默认保存到该客户归属。
- 客户自己的豆单会读取产品豆单中的客户选择，后续发布或保存草稿时要确认客户正确，避免把客户自己的价格试算和样式保存到错误客户。
...
- 参数解释不清：打开价格来源抽屉查看公式步骤；如果源参数错误，按参数来源回到成本参数设置、BOM/产品出成率、梯度模板或挂耳供应价模板保存后再重新试算。
