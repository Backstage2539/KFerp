...
4. 进入客户履约账户，在“选择客户”中按客户名、公司、联系人或电话搜索客户，并点击载入账户；下拉框只展示外部用户已配置**手机号+密码且已启用登录**、且能力模板暴露 ERP 工作台的批发客户。
...
- 客户下拉框只出现已配置手机号+密码且启用登录的外部用户，且能力模板暴露 ERP 工作台的批发客户；零售、电商、未绑定客户、绑定账号已禁用登录的客户和历史绑定指向非工作台模板或无法识别模板的客户不出现。
