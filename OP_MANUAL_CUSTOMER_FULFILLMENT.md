...
- 客户能看到什么，主要由客户类型、能力模板、外部用户绑定和客户专属 SKU/豆单决定；内部员工能操作什么，主要由 ERP 角色和页面权限决定。
- 客户账户模式默认隐藏内部“履约运营台”入口；客户只进入客户侧履约工作台或小程序，不进入内部运营台。

## 合并后的阅读地图
```mermaid
flowchart LR
  A["客户档案/客户类型"] --> B["门户配置/能力模板"]
  B --> C{"客户入口"}
  C -->|批发履约| D["外部用户/客户履约工作台"]
  C -->|零售商城| E["小程序商城"]
  D --> F["客户 SKU/豆单/BOM"]
  F --> G{"业务来源"}
...
| 角色 | 可操作范围 | 权限边界 | 重点入口 |
| --- | --- | --- | --- |
| 管理员/老板 | 配置客户类型、能力模板、外部用户、界面设置、权限、操作日志，必要时查看全流程 | 不要把客户外部账号当内部员工账号维护；客户能力模板变更会实时影响客户 | 客户档案、客户门户配置、能力模板、界面设置、员工维护、操作日志 |
...
- `settings.write`：界面设置等系统配置；保存配置必须写入操作日志。
...
| 场景 | 操作流程 | 核对点 |
| --- | --- | --- |
| 新客户开通 | 客户档案设为批发客户 → 客户门户配置选择能力模板 → 创建外部用户、设置密码并启用登录 → 保存配置 → 顶部客户账户或客户履约运营台选择客户 | PR-311-CUSTOMER-PORTAL-ACCOUNT-PROFILE：客户出现在顶部“客户账户”当前客户下拉和履约运营台选择器；客户侧登录后只看到模板允许的入口 |
...
## 新用户上手清单
1. 先用一个测试批发客户完成开通：客户档案、能力模板、外部用户、登录启用。
...
## 还需要继续手册化的内容
- 首次开通检查表：把客户类型、模板、外部用户、登录、专属 SKU、豆单、BOM、默认仓库、结算能力做成一页勾选清单。
...
5. 进入客户履约账户，在“选择客户”中按客户名、公司、联系人或电话搜索客户，并点击载入账户；下拉框只展示存在 active 外部用户且已配置手机号、已设置密码并启用登录的批发客户。客户账户模式已选择客户时，本页会自动载入该客户。
5. 客户账户模式下 SKU设置、BOM、产品豆单、仓库库存和费用管理都锁定顶部当前客户；不要在这些页面内另选客户。公共 SKU/BOM 只作为公共引用，客户自己的配置只归当前客户。
5. 外部用户不在本页维护；如需创建账号、重置密码或启停登录，回到工厂总览的“客户门户配置”的“外部用户”区域处理。保存后顶部“客户账户”当前客户下拉会自动刷新；客户门户配置的“打开客户档案”可直接打开当前客户资料抽屉。外部用户不进入员工维护页，不配置内部角色。
...
12. 在客户履约账户手工提交代加工工单或代发订单前，也要确认客户已开通对应能力；未开通对应能力的客户不能通过内部 API 绕过页面写入工单或代发订单。手工提交订单信息时按“一个收件信息 + 多行商品”录入，逐行选择商品/规格或单位/数量，页面会实时展示阶梯价、行小计，并实时计算订单合计（商品金额 + 运费）。曲奇拼配等 KG 梯度商品按该行总 KG 选档，1000g 及以上保存元/kg 单价并按总 KG 计算小计；挂耳商品按袋或盒下单，盒价按挂耳已发布价格梯度换算。
