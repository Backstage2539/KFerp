...
- 客户能看到什么，主要由客户门户开关、客户门户能力模板、外部用户绑定和客户专属 SKU/豆单决定；客户类型只是分类口径，渠道客户也必须开通门户并绑定能力模板后才具备工作台能力。
- 客户账户模式默认隐藏内部“履约运营台”入口；客户只进入客户侧履约工作台或小程序，不进入内部运营台。

## 相关手册怎么分工
| 手册 | 解决的问题 | 主要读者 |
| --- | --- | --- |
| 本手册：客户履约手册 | 内部运营如何载入客户、导入 Excel、提交代发/加工、调整托管库存、查看履约订单、生成月结 | 履约运营、仓库、财务、管理员 |
| 客户门户手册 | 如何配置客户能力模板、外部用户、客户侧工作台、小程序、商城、账单和客户可见范围 | 管理员、客户成功、实施人员 |
| 订单销售手册 | 履约订单进入 ERP 后如何在订单列表、订单详情、销售单、出库单、失效订单和发货资料中继续处理 | 销售、履约运营、仓库、财务 |

补充阅读：客户专属 SKU、公共 SKU 引用和豆单版本看“成本核价手册”和“物料、库存与 BOM 手册”；生豆产品、绑定熟豆 BOM 和生豆豆单看“生豆销售手册”。

## 角色权限速查
| 角色 | 可操作范围 | 权限边界 | 重点入口 |
| --- | --- | --- | --- |
| 管理员/老板 | 配置客户类型、客户门户开关、客户门户能力模板、外部用户、界面设置、权限、操作日志，必要时查看全流程 | 不要把客户外部账号当内部员工账号维护；客户能力模板变更会实时影响客户 | 客户档案、客户门户配置、客户门户能力模板、界面设置、员工维护、操作日志 |
| 客户成功/实施 | 帮客户开通门户、确认模板能力、准备试单、核对客户是否能登录和看到正确入口 | 不能绕过模板给零售商城客户开放 ERP 工作台；不能把一个外部用户 active 绑定多个客户 | 客户门户配置、客户履约手册、客户门户手册 |
| 履约运营 | 选择客户、导入三类 Excel、应用批次、手工提交订单/代发/加工、查看履约客户订单和费用 | 进入客户履约运营台需要内部 `stock.write`；客户未开通对应能力时不能提交或导入对应业务 | 客户履约运营台、订单列表“履约客户订单” |
...
| 场景 | 操作流程 | 核对点 |
| --- | --- | --- |
| 新客户开通 | 客户档案打开“开通客户门户/工作台” → 客户门户配置选择能力模板 → 创建外部用户、设置密码并启用登录 → 保存配置 → 客户履约运营台选择客户 | 客户出现在履约运营台选择器；客户侧登录后只看到模板允许的入口 |
...
## 入口
- 客户管理：客户档案、客户门户配置、客户门户能力模板、客户履约手册。
...
## 标准操作
1. 第一次导入前，确认客户档案存在且已打开“开通客户门户/工作台”，并在客户门户配置中启用代加工、托管库存、代发、结算等能力。
2. 为客户库存准备客户仓库时，进入“仓库库存”，在“客户仓库”分类中选中仓库，点击“仓库设置”绑定客户；取消绑定也在同一抽屉处理。
3. 客户自己的产品作为客户专属 SKU 管理，不放进公共产品列表，也不能直接共用其他客户 SKU。
4. 进入客户履约账户，在“选择客户”中按客户名、公司、联系人或电话搜索客户，并点击载入账户；下拉框只展示已开通门户/工作台、能力模板暴露工作台或下单能力、存在 active 外部用户且已配置手机号、已设置密码并启用登录的客户。
...
21. 导入批次、解析结果、费用、库存和底部履约客户订单等列表底部统一显示总条数、总页数和当前页，支持跳转到指定页，并可切换每页显示条数。
...
- 客户下拉框只出现已开通门户/工作台、模板具备工作台或下单能力、存在 active 外部用户且已配置手机号、已设置密码并启用登录的客户；未开通门户、未配置手机号、未设置密码、登录已停用或仅剩 inactive 绑定的客户不出现。
