...
## 适用范围
客户门户配置、批发客户门户能力、外部用户管理交接、小程序入口模式、小程序主题、商城能力、商城商品上架、图片上传、小程序商城下单和 ERP 订单承接。
...
2. 进入客户门户配置，找到目标批发客户。
3. 如果客户需要外部用户登录，在客户门户配置行点击“去履约运营台管理”，进入客户履约运营台后创建外部用户、设置密码并启用登录。只有带 ERP 工作台权限或视图的模板才需要外部用户；一个客户只能保留一个启用绑定，同一外部用户不能 active 绑定第二个客户。
...
12. 如果客户无法授权手机号，切到“密码登录”，使用外部用户的用户名/手机号和密码登录。账号必须先在客户履约运营台创建、设置密码、启用登录并绑定到当前客户。
...
- 外部用户账号使用客户履约运营台维护的同一用户名/手机号和密码；小程序后端通过 `/api/mini/login/password` 校验后，只签发客户小程序 mini token，不创建内部 ERP 后台会话。
...
- 账号必须满足：员工记录启用、账号类型为渠道客户、登录未禁用、密码正确、存在 active 客户门户 ERP 绑定、绑定客户未停用。创建、重置密码和启停登录都在客户履约运营台完成。
...
- 禁用登录的外部用户不能通过客户履约运营台继续作为有效绑定；历史 active 绑定如果对应账号已禁用，会从客户工作台上下文中失效。
...
- 客户门户配置页支持跳转到客户履约运营台管理外部用户；外部用户创建后能在履约运营台看到账号名称、手机号、登录状态和绑定状态。同一账号不能重复 active 绑定给其他客户，禁用登录的外部用户不能继续作为有效工作台绑定。
...
- 客户门户配置页找不到绑定账号下拉框：这是新逻辑；点击“去履约运营台管理”，在客户履约运营台维护外部用户。
- 外部用户提示模板不支持 ERP 工作台或 `ERP workbench unavailable for capability template`：检查客户是否选择了“零售商城客户”模板；零售商城客户不需要 ERP 工作台账号，也不能从客户履约账户内部 API 绑定。
