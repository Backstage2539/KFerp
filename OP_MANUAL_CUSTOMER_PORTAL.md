...
- 订单单价由后端按当前客户、商品、规格、数量、商品默认价和小批量阶梯价统一计算；客户侧只填写收件信息、商品、规格、件数和备注，不配置运费或优惠。
...
- 客户小程序订单单价异常偏低：检查是否有人构造请求传入 `unit_price`、`shipping_amount` 或优惠字段；后端必须忽略这些客户侧价格字段并按商品默认价/阶梯价重算，若仍异常，核对商品价格档和客户模板配置。
