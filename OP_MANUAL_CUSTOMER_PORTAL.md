...
2. 客户门户配置属于工厂内部设置。进入工厂总览后打开客户门户配置，按客户名、联系人或手机号搜索客户；不要从客户账户模式维护门户和能力模板。
3. 如果客户需要外部用户登录，直接在客户门户配置行的“外部用户”区域创建账号、设置密码并启用登录。外部用户配置了手机号、密码并启用登录后，该客户会出现在 ERP 顶部“客户账户”的当前客户下拉和客户履约运营台选择器；创建、重置密码或启停登录后系统会自动刷新顶部客户列表，不需要整页刷新。一个客户只能保留一个启用绑定，同一外部用户不能 active 绑定第二个客户。
