...
2. 进入客户门户配置，找到目标批发客户。
3. 在 ERP账号 中选择已启用登录的渠道客户账号并绑定；绑定成功后当前行会显示账号名称和手机号。只有带 ERP 工作台权限或视图的模板才能启用绑定；一个客户只能保留一个启用绑定，同一账号不能绑定第二个客户。
...
11. 客户打开小程序后，登录页不再展示微信一键登录；客户使用 ERP 渠道客户账号输入用户名/手机号和密码登录。账号必须先在用户权限页设为渠道客户、设置密码并启用登录，再在客户门户配置中绑定到客户。
12. 客户登录成功后，如果配置为商城首页，会直接进入商城；重新登录或切换当前客户后，小程序也必须按当前客户的小程序首页模式进入服务首页或商城首页。商城页顶部保留“我的订单”，用于查看商城订单记录。
13. 首页、商城页和服务页顶部只保留“个人中心”账号入口；客户进入个人中心后可查看当前客户、切换绑定客户、点击“切换用户”回到登录页输入另一套 ERP 账号密码，或点击“退出登录”清除本机小程序会话。
14. 如果客户不是商城首页模式，可在服务首页点击商城下单或我的订单。
15. 客户浏览商品、加入购物车、增减数量，填写收件人、手机号、收货地址和备注后提交订单。
16. 提交成功后，小程序提示订单号，ERP 订单列表出现来源为 `mall` 的订单。

## 小程序 ERP 账号密码登录
- 小程序客户入口只面向 ERP 渠道客户账号，不面向内部员工账号；内部员工即使密码正确，也不能登录客户小程序。
- ERP 渠道客户账号使用用户权限页维护的同一用户名/手机号和密码；小程序后端通过 `/api/mini/login/password` 校验后，只签发客户小程序 mini token，不创建内部 ERP 后台会话。
- 登录页密码输入框必须以掩码显示；如果开发者工具或真机上看到明文密码，先确认小程序包已更新到包含 `password` 输入属性的版本。
- 账号必须满足：员工记录启用、账号类型为渠道客户、登录未禁用、密码正确、存在 active 客户门户 ERP 绑定、绑定客户未停用。
...
- 公共 SKU 代发模板账号只处理代发信息，不显示加工工单；服务端只允许绑定客户具备 `direct_ship` 能力时提交代发信息。
- 同一渠道客户账号只能绑定一个客户，操作人员不要把一个账号给多个客户共用。
- 禁用登录的渠道客户账号不会出现在 ERP 账号选择器中，也不能通过 API 新增启用绑定；历史 active 绑定如果对应账号已禁用，会从客户门户配置和客户工作台上下文中失效。
...
- 需要给新客户 B 做差异配置时，先在“客户能力模板设置”手动点击“复制模板”，输入新模板 key 和名称，复制出的模板会作为子模板缩进显示，并可折叠；再修改子模板并绑定客户 B。
- 不再使用的模板应标记为“模板失效”。失效模板仍在模板设置页可见，便于排查历史引用，但不会出现在客户门户配置的可选模板中。
- 如果客户当前引用了失效模板，客户门户配置页会显示“模板已失效”，保存、应用或绑定 ERP 工作台时会返回 `capability template invalid`；管理员必须重新选择一个启用中的模板后保存。
...
- ERP账号 绑定后能看到账号名称和手机号；同一账号不能重复绑定给其他客户，禁用登录的渠道账号不能被选中或继续作为有效工作台绑定。
...
- 代加工客户账号能看到加工工单和代发入口；公共 SKU 代发客户账号只看到代发入口，不能提交加工工单。
...
- 小程序登录页只显示 ERP 账号密码登录，不再展示微信一键登录；启用登录且已绑定客户门户的 ERP 渠道客户账号可登录，内部员工账号和未绑定账号不能登录。
...
- 商城首页模式客户仍可从商城页顶部进入“我的订单”，查看已提交商城订单记录。
- 商城订单进入 ERP 订单列表后，可沿用现有生产、发货、销售单和财务流程。

## 常见问题
- 客户仍进入订单处理页：检查客户门户配置的小程序首页是否保存为商城。
- 零售商城客户具备商城能力但仍进入订单处理页：检查 `/api/mini/login/password`、`/api/mini/me` 或 `/api/mini/services/orders` 返回的 `miniapp_entry_mode` 是否为 `mall`；若仍为 `services`，需要修复客户门户 profile 或重新登录刷新会话。商城页进入“我的订单”等服务页时也必须保持 `mall`。
- 商城客户看不到订单记录入口：确认小程序已更新到包含商城页“我的订单”入口的版本，并确认客户具备商城下单能力。
- 客户门户配置找不到客户：检查客户档案中的客户类型是否为批发客户。
- 绑定账号不可选：先到用户权限页把账号类型设为渠道客户，设置密码，并确认账号登录未被停用；禁用登录的渠道客户账号不会出现在绑定下拉框。
- 绑定账号提示 `login-enabled channel customer account required`：说明所选账号不是启用登录的渠道客户账号；先在用户权限页启用该渠道账号，或改选其他已启用登录的渠道客户账号。
- 绑定账号提示模板不支持 ERP 工作台或 `ERP workbench unavailable for capability template`：检查客户是否选择了“零售商城客户”模板；零售商城客户不需要 ERP 工作台账号，也不能从客户履约账户内部 API 绑定。
...
- 保存客户门户配置提示模板无效：重新选择系统提供的能力模板；如果要手工配置能力，清空模板 key 后再保存。
