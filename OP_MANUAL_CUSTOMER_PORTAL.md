...
11. 客户打开小程序登录页，优先点击“手机号快捷登录”并授权微信手机号；如果无法授权，也可以切到“密码登录”，使用已绑定渠道客户 ERP 账号的手机号和密码登录。
12. 客户登录小程序后，如果配置为商城首页，会直接进入商城；重新登录或切换当前客户后，小程序也必须按当前客户的小程序首页模式进入服务首页或商城首页。商城页顶部保留“我的订单”，用于查看商城订单记录。
13. 如果客户不是商城首页模式，可在服务首页点击商城下单或我的订单。
14. 客户浏览商品、加入购物车、增减数量，填写收件人、手机号、收货地址和备注后提交订单。
15. 提交成功后，小程序提示订单号，ERP 订单列表出现来源为 `mall` 的订单。
...
- 小程序密码登录失败：检查手机号是否属于已启用登录的渠道客户 ERP 账号、密码是否正确、账号是否被停用、客户门户配置是否已完成 ERP 账号绑定。
