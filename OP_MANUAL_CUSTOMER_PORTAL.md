...
- 账单入口专门进入结算页，加载当前客户自己的订单账单、费用明细和结算单；即使还没有生成费用明细或结算单，只要当前客户已有订单，也应先看到订单应收。
...
- 结算服务页只展示当前客户自己的订单账单、费用明细和结算单，不能显示其他客户的订单号、费用明细、结算单号或金额。
- 小程序登录页只显示 ERP 账号密码登录，不再展示微信一键登录；启用登录且已绑定客户门户的外部用户可登录，内部员工账号和未绑定账号不能登录。
- 小程序启动页不会停留在空白页；底部四个入口固定为“首页、订单、账单、我的”。首页不重复显示订单/账单，订单页能看到订单明细、销售单、出库单和物流信息，账单页显示当前客户订单应收、费用明细和结算单，我的页可退出登录和切换用户。
