...
7. 给生豆所属二级分类绑定梯度模板；产品豆单会按绑定熟豆 BOM 的成本输入和该模板生成 `green_bean_sale_tiers`。
8. SKU设置的客户 SKU 列表可按形态、名称、一级分类、二级分类过滤，确认生豆和熟豆在同一列表内区分清楚。
9. 进入 产品豆单，切换到“生豆豆单”，检查报价档、绑定 BOM、最新通过生产质检信息和 PDF 预览。
10. 发布或导出生豆豆单后，ERP 录单选择生豆产品；规格可选常用克数，也可选择“自定义克数”直接输入实际克数。如果该生豆 SKU 暂无自身生豆价目，录单会使用绑定熟豆 BOM 的启用梯度作为自动价兜底。
...
- ERP 录单中，客户可见生豆 SKU 如果没有自身启用梯度或可用的已发布生豆豆单价格，但已绑定熟豆 BOM，系统会继承绑定熟豆的启用梯度作为自动价兜底；订单行仍保存 `product_kind=green_bean`，价格来源标记为 `green_bean_bound_roasted_tier`。
...
- 客户生豆 SKU 没有自身生豆价目时，录单商品梯度能看到绑定熟豆梯度，保存后订单行仍是 `green_bean`，价格来源能追溯到绑定熟豆梯度。
...
- 生豆豆单价格不对：检查生豆 SKU 所属二级分类是否绑定正确梯度模板、绑定熟豆 BOM 是否正确、是否重新生成并发布生豆豆单；若录单使用兜底价，还要检查绑定熟豆是否有启用梯度。
