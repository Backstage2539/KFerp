...
- PR-355-PRODUCT-SUBTYPE-CONFIG-UNIT-RULES：产品子类型配置中可维护默认阶梯价模板、工序模板、产品价格表生成规则和轻量单位换算。操作入口是 SKU设置 → 商品分类 → 找到对应产品类型下的产品子类型行 → 点击“配置” → 填写库存单位、报价单位、录单单位、单位换算 JSON 和整数单位 → 点击“保存子类型配置”。例如“速溶咖啡 / 冻干速溶”可设置库存单位 kg、报价单位盒、录单单位盒、`unit_conversion_json={"盒":{"kg":0.2}}`、`integer_unit=true`，表示录单必须按整盒，后续生成产品价格表和录单解析按该单位规则读取。
- PR-356-CUSTOMER-PRODUCT-RULE-TEMPLATES：选择某个履约客户后，SKU设置 会显示“客户产品规则”面板。先新增客户产品规则模板，在模板明细中按产品子类型选择阶梯价模板、填写工序模板 ID、价格表生成规则 JSON 和单位规则 JSON；再把当前客户绑定到该模板。某个客户的某个产品子类型需要特殊规则时，在同一面板的“客户专属覆盖”单独选择产品子类型并覆盖阶梯价模板、工序模板或单位规则。解析优先级是客户专属覆盖 > 客户产品规则模板 > SKU 覆盖 > 产品子类型默认 > 产品类型默认 > 系统兜底。
- 客户产品规则模板适合“普通批发客户”“贴牌客户”“代加工客户”等批量规则；客户专属覆盖适合单个客户对某个产品子类型临时或长期差异，例如大客户冻干速溶使用专属盒装阶梯价，但工序仍继承模板。
