...
8. 仓库库存左侧按“普通仓库”和“客户仓库”分组展示，可展开或收起。只有客户仓库需要绑定客户；选中客户仓库后，在仓库合计右侧点击“仓库设置”，选择绑定客户或改为“不绑定客户”后保存。
9. 物料、批次、库存流水、出库日志和分配批次等列表底部统一显示总页数、当前页、跳转页和每页条数；筛选变化后从第一页重新查询。
...
- 库存调整后有调整单、流水和操作日志证据。
- 客户仓库绑定或取消绑定客户后，仓库库存页能看到绑定客户，操作日志能查到仓库 `customer_id` 变化。
