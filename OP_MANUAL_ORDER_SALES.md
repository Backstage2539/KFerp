...
- PR-328-SALES-ORDER-PAYMENT-CODE-SOFT-DISABLE：销售单设置中点击收款码“停用”只会隐藏该码，不会删除历史记录、素材记录或已上传图片；停用后新生成的销售单预览、PDF 和图片不再展示该码。
...
- 销售单收款码上传必须先填写标签；标签为空时不能写入收款码资产，补齐标签后再上传。
- 销售单收款码停用只隐藏，不删除数据库记录、素材记录或上传图片；停用后新预览和新下载文件不再展示该码。
...
- 收款码停用后找不到：这是正常隐藏状态，停用不会删除原始收款码记录和图片；如果要继续使用，需要重新上传或由管理员从数据库恢复 active 状态。
