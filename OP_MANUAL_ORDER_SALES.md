...
- 商品的 KG 梯度在价格提示中显示为 `kg` 区间；生豆豆单的销售单价仍按发布快照中的元/磅显示，例如 `1-59kg 23.49/磅`、`60kg+ 62/磅`，避免把 KG 区间误解为 KG 单价。
...
- 销售单收款码默认放在第 1 页右侧并放大显示；如果仍和商品表、说明或公章挤在一起，在“销售单设置”的“收款与说明版式 / 收款码位置和大小”中按 A4 毫米坐标调整 X、Y、宽和高，保存后刷新预览并重新生成 PDF 或图片。
- “收款与说明版式”控制收款方式、公账收款和个性化说明的文本框；文本太长时可加宽文本框、下移文本框或缩小高度范围内的内容，避免把收款码挤到下一页。
