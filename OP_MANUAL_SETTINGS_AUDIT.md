...
## 用户权限与账号
- 用户权限页只面向内部用户；在用户权限页为内部员工勾选角色并保存，菜单和 API 会按角色权限控制。
- 员工维护和用户权限列表会过滤外部用户，用户权限页不再提供账号类型切换，也不再出现渠道客户或客户业务角色。
