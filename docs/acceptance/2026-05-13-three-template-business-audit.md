...
| “可扩展性、可测试性、可维护性” | 模板契约测试、跨模块走查、支持守卫 PR-175 至 PR-268、PR/DEV 进度录入至 PR/DEV-268、真实 PostgreSQL 集成测试、acceptance 文档 | 已补，但不能替代未覆盖的 ERP 边缘矩阵。 |
...
| 操作手册和进度表 | `OP_MANUAL_CUSTOMER_PORTAL.md`、`OP_MANUAL_CUSTOMER_FULFILLMENT.md`、`OP_MANUAL_ORDER_SALES.md`、`OP_MANUAL_PRODUCTION.md`、`OP_MANUAL_FINANCE.md`、`orderapp-remote/docs/OP_MANUAL_CUSTOMER_PORTAL.md`、`orderapp-remote/docs/OP_MANUAL_CUSTOMER_FULFILLMENT.md`、`orderapp-remote/docs/OP_MANUAL_ORDER_SALES.md`、`orderapp-remote/docs/OP_MANUAL_PRODUCTION.md`、`orderapp-remote/docs/OP_MANUAL_FINANCE.md`、PR/DEV 进度录入至 PR/DEV-268；PR-215 及以前历史 UT/API/REV 表项保留 | 已按新口径更新。 |
...
- 进度表状态：`req_store.go` 当前覆盖三模板审计主线 PR/DEV-175 至 PR/DEV-268；按 Van 最新规则，PR/DEV 用于系统进度查看，UT/API/REV 不再按旧表流程新增，测试和验收证据保留在 Superpower/TDD 证据链。
...
- 集成基线：`git fetch origin` 后，`HEAD`、`origin/develop` 和 merge-base 均为 `33ffb240432574723147bcc6fe1b969e30af3a3e`，`git rev-list --left-right --count HEAD...origin/develop` 为 `0 0`；当前成果仍是未提交工作树变更，尚未 commit、push、开 PR、合并或部署。
...
### 剩余 ERP 点击矩阵目标
- 生产管理剩余点击：`workOrders`、`jobCards`、`qualityInspections`、`produceLogs`、`productionCosts` 仍需后续从页面入口补更多查看/筛选/异常态点击证据；当前重点证据集中在 `producePlan`、`produceRunning` 和 WIP 抽屉边界。
- 库存管理剩余点击：`warehouseInventory` 已补 WIP 抽屉关键边界，但 `stockOperations`、`stockOutboundLogs`、`purchase`、`materials` 和隐藏追溯页仍需更完整的页面点击矩阵。
- 商品与配方剩余点击：`productSettings`、`mallSettings`、`costing`、`bom` 已有单测/源码/部分 API 边界，但仍缺从 ERP 页面真实点击覆盖商品配置、商城选品、核价和 BOM 配方维护的完整矩阵。
