...
- 豆单版本列表入口放在产品豆单，可按官方/我的/指定客户和商用/零售/生豆查看。
...
- 浏览器冒烟：本地 mocked API 打开 `/vue-shell/?view=productSettings` 和 `/vue-shell/?view=costing`，DOM 中分别可见“SKU设置”和“产品豆单”；SKU设置不再出现“豆单版本列表”，产品豆单中可见“豆单版本列表”、发布归属、豆单类型、复制配置、撤回和测试版本 `V3.0.5`。
