...
describe('mini mall helpers', () => {
  ...
  it('replaces the mini-program batch form with the single-write direct shipment panel', () => {
    const servicePage = fs.readFileSync(path.join(currentDir, '..', 'pages', 'service', 'service.vue'), 'utf8')
    const directShipPanel = fs.readFileSync(path.join(currentDir, '..', 'components', 'CustomerDirectShipPanel.vue'), 'utf8')
    expect(servicePage).toContain('CustomerDirectShipPanel')
    expect(servicePage).not.toContain('directShipForm')
    expect(directShipPanel).toContain('idempotency_key')
    expect(directShipPanel).not.toContain('代发批次')
    expect(directShipPanel).not.toContain('导入代发地址')
  })
  ...
  it('uses customer-facing pickers instead of raw system ID fields for service order forms', () => {
    ...
    expect(orderingPanel).toContain('选择收件客户')
    expect(orderingPanel).toContain('价格由 ERP 自动计算')
    ...
    expect(processingPanel).not.toContain('投入物料')
