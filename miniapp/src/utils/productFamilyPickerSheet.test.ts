...
describe('shared product-family picker sheet', () => {
  ...
  it('keeps product, spec and quantity controls visible for every direct-shipment row', () => {
    ...
    expect(directShip).toContain('<text class="field-label">商品</text>')
    expect(directShip).toContain('<text class="field-label">规格</text>')
    expect(directShip).toContain('<text class="field-label">数量（')
    expect(directShip).toContain('@tap="openProductSelector(line.key)"')
    expect(directShip).not.toContain('openSpecSelector')
    expect(directShip).toContain('class="spec-choice-list"')
    expect(directShip).toContain('@tap="chooseInlineSpec(line, spec)"')
    ...
    expect(directShip).toContain('schedulePreview')
  })
})
