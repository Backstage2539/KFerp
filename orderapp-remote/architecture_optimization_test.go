...
func TestBomAPIRoutesUseApplicationService(t *testing.T) {
	body, err := os.ReadFile("bom_api.go")
	if err != nil {
		t.Fatal(err)
	}
	content := string(body)
...
func TestReactBomStaticEntrypointIsRemoved(t *testing.T) {
	...
	checks := []struct {
		...
		{path: "static_frontend_routes.go", forbidden: []string{"/bom-react", "bomReact"}},
		{path: "Dockerfile", forbidden: []string{"frontend/dist", "React BOM"}},
		{path: "operation_log.go", forbidden: []string{"/bom-react/assets/"}},
	}
