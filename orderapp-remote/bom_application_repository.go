...
func (r postgresBomRepository) List(ctx context.Context) ([]bomapp.ListItem, error) {
	...
	for rows.Next() {
		...
		item.YieldRate = resolveYieldRate(item.RoastLevel, fallback)
...
func (r postgresBomRepository) Products(ctx context.Context) ([]bomapp.Option, error) {
	opts, err := fetchOptions(ctx, r.pool, "SELECT id, name FROM "+r.schema+".products WHERE active=true ORDER BY name")
	if err != nil {
		return nil, err
	}
...
func (r postgresBomRepository) Materials(ctx context.Context) ([]bomapp.Option, error) {
	opts, err := fetchOptions(ctx, r.pool, "SELECT id, name FROM "+r.schema+".materials ORDER BY name")
	if err != nil {
		return nil, err
	}
	return bomOptionsToApp(opts), nil
...
func bomOptionsToApp(opts []Option) []bomapp.Option {
