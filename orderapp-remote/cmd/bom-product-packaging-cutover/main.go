...
func main() {
	...
	manifest, err := postgresbom.LoadPR607ProductionManifest()
	if err != nil {
		fatal(err)
	}
	...
	var result postgresbom.PR607ProductPackagingSummary
	switch normalizedMode {
	case "preview":
		result, err = repo.PreviewPR607ProductPackagingCutover(ctx)
	case "apply":
		result, err = repo.ApplyPR607ProductPackagingCutover(ctx, *actor)
	case "rollback":
		result, err = repo.RollbackPR607ProductPackagingCutover(ctx, *actor)
	default:
