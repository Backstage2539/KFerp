...
- 数据影响：价格试算是只读操作，不保存价格模板、商品价格表、订单或操作日志。修改当前选择不会回算或改写已发布价格表、历史订单和历史价格快照；正式报价仍以重新生成并发布的商品价格表为准。
...
- PR-585-BOM-SINGLE-MATERIAL-LOSS / PR-589：商品价格管理顶部提供 `价格试算`。点击后打开 `价格计算模板试算` 抽屉，先选择启用的价格计算模板，再选择主商品和具体销售规格，可临时录入加价率、税率和其他成本；报价单位由所选具体 SKU 自动确定。损耗不在价格模板或试算抽屉重复配置，只读取当前已发布 BOM 唯一的 `原料损耗比`。字段完整后系统自动试算，结果区查看 BOM+工序成本、试算单价、实际毛利率和公式步骤。试算是只读结果，不保存到模板、商品价格表、发布快照或订单；正式报价仍需回到商品价格表生成并发布。
...
- PR-577 / PR-585：试算选择到“已发布但没有组件”的生产 BOM，且同一 BOM 已有有组件草稿时，系统不会把该空版本显示为有效 0 元，也不会暗中读取草稿成本；页面会指出当前 published 版本和待发布 draft 版本。进入 `生产管理 → 生产 BOM`，先核对草稿组件、单位、商品形态和唯一的原料损耗，再由有权限人员发布草稿，然后返回重新试算。发布动作沿用现有操作日志；历史价格表和订单不重算。
