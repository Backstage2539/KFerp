...
## 梯度模板和价格来源
- PR-392 / PR-393：产品价格表生成和成本核算以“客户商品名 + 商品档案 + 商品档案配置”为主链路。客户商品名负责对外名称、编号、品牌和分类模板；商品档案负责库存、成本和生产；商品档案配置负责绑定商品配置模板、分类模板、生产 BOM 版本、工艺路线、预期损耗率和行业字段模板。具体分类归属通过“配置分类”抽屉维护，价格表分组优先读取新分类模板归属，旧展示分类只作为历史 fallback。维护入口是 商品与配方 → 商品档案 → 点击商品名。
- 价格表和 PDF 不再从 BOM 版本维护特殊属性，也不展示预期产出率维护字段。需要展示烘焙度、布料参数、鲜果成熟度等产品信息时，在商品档案的商品生产配置字段中逐行维护，并勾选进入价格表。
- 先在 商品配置模板 维护阶梯价模板：每个模板选择一个展示单位（元/磅、元/kg、元/盒、元/袋等），每个档位填写区间名、最小/最大数量和利润率；产品价格表展示单位默认继承阶梯价模板单位。某个产品要从磅改成盒，不在商品档案上直接改展示单位，而是复制或新建一个“盒”单位阶梯价模板并绑定到对应商品配置模板。
...
- PR-386 / PR-393：客户价格表发布时会同时冻结客户商品名和商品档案双快照，包括客户商品名 ID、客户展示名、客户商品编号、品牌、分类模板/分类归属、商品档案 ID/编号/名称、生产 BOM 版本和使用方式、价格单位、阶梯、特殊属性和价格来源 JSON。后续客户商品名改名、商品档案改名、BOM 升级或模板调整，都不会回改已发布价格表。
...
- PR-386 / PR-393：客户侧新增报价对象优先使用“客户商品名”。客户商品名绑定一个商品档案，维护客户展示名、客户商品编号、品牌、分类模板、排序和是否进入价格表；具体分类归属通过“配置分类”抽屉维护。工厂自营也作为一个内置客户-like 归属维护客户商品名。
...
- PR-382 / PR-393：历史 SKU 复制抽屉保留兼容，但新贴牌场景不要默认复制商品档案/BOM。客户只改名称、编号、品牌或客户侧分类时，创建客户商品名并绑定已有商品档案；配方、包装、生产方式、库存对象或成本口径变化时，才创建新商品档案和生产 BOM。
...
- PR-393：例如 `roast_level / 烘焙度`、`caffeine / 咖啡因`、`stick_spec / 条装规格`、服装布料克重、鲜果成熟度等，都作为商品生产配置字段维护。点击商品名打开商品档案配置抽屉后，选择行业字段模板，系统按模板字段生成普通表单；操作员只能填写字段值和“价格表展示”标记，不能在商品档案里临时新增或删除字段定义。发布产品价格表时，系统冻结客户商品名、商品档案、商品配置模板、生产 BOM 版本、工艺路线、预期损耗率和商品生产配置字段快照。旧 BOM 版本 `special_attrs_json`、旧 SKU `special_attrs_json` 和 `roast_level` 只作为 fallback，避免历史价格表、旧工单或旧 API 解释断层。
...
- 客户商品名不再依赖旧 Excel 豆单资料。只要客户商品名绑定有效商品档案，产品价格表客户范围会按客户商品名分类模板归属生成分组；客户商品名没有新分类归属时，才 fallback 商品档案分类模板或旧展示分类。标题沿用公共价格表逻辑，优先显示实际分类名。例如把客户商品名加入“定制咖啡熟豆”分类后，会显示在“1、定制咖啡熟豆”分组下，不会落到旧公共豆单“源产地精选”，也不会因为缺少旧豆单编码而不显示。
- 客户商品名还没有新分类归属时，产品价格表会把它放到“未分类”，不会继续沿用基础公共商品的旧 Excel 分类；需要正式分组时先回到 客户商品名 点击“配置分类”，维护客户侧分类项和客户商品名归类。
- 历史兼容 marker：客户商品名还没有展示分类时，产品价格表会把它放到“未分类”；客户只改名称、编号、品牌或展示分类时，创建客户商品名并绑定已有商品档案。PR-393 后“展示分类”由分类模板和配置分类抽屉承载；若旧分类消失，仍先检查 schema 修复是否已生效。
...
- 客户新增商品在客户价格表中不出现：先确认 客户商品名 中该客户商品名已启用、已绑定有效商品档案且勾选进入价格表；若是客户自有/客户定制熟豆，产品价格表会按客户商品名分类模板或商品档案分类模板自动生成条目，不需要旧 Excel 豆单资料。若分类消失，先在客户商品名页用“配置分类”核对该客户侧分类模板和归属。
