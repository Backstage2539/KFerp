...
## 梯度模板和价格来源
- PR-392 / PR-394：产品价格表生成和成本核算以“客户商品名 + 商品档案 + 商品档案配置”为主链路。客户商品名负责对外名称、编号和品牌；商品档案负责库存、成本和生产；商品档案配置负责绑定商品配置模板、生产 BOM 版本、工艺路线、预期损耗率和行业字段模板。分类模板是页面启用的分类视图，价格表分组优先读取客户商品名页当前启用模板下的客户商品归类；没有新归类时进入“未分类”，旧展示分类只作为历史 fallback。维护入口是 商品与配方 → 商品档案 → 点击商品名。
...
- PR-386 / PR-394：客户价格表发布时会同时冻结客户商品名和商品档案双快照，包括客户商品名 ID、客户展示名、客户商品编号、品牌、价格表分组/分类归属、商品档案 ID/编号/名称、生产 BOM 版本和使用方式、价格单位、阶梯、特殊属性和价格来源 JSON。后续客户商品名改名、商品档案改名、BOM 升级或模板调整，都不会回改已发布价格表。
...
- PR-386 / PR-394：客户侧新增报价对象优先使用“客户商品名”。客户商品名绑定一个商品档案，维护客户展示名、客户商品编号、品牌、排序和是否进入价格表；具体分类归属在客户商品名页启用的分类模板 Tab 内完成。工厂自营也作为一个内置客户-like 归属维护客户商品名。
- 创建商品档案时只填写名称、备注等基础信息，不选择分类模板、产品类型或产品子类型。新增商品档案不在新增表单里复制 BOM 或价格梯度；这些能力统一通过商品档案配置抽屉、BOM 配方维护和商品配置模板完成。
- PR-382 / PR-394：历史 SKU 复制抽屉保留兼容，但新贴牌场景不要默认复制商品档案/BOM。客户只改名称、编号、品牌或客户侧分类时，创建客户商品名并绑定已有商品档案；配方、包装、生产方式、库存对象或成本口径变化时，才创建新商品档案和生产 BOM。
