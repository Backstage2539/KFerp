...
- PR-456-PRICING-RULE-TRIAL-PR439-UNIT：试算抽屉删除 `重新试算` 按钮和 `售价后附加成本`。报价单位来自全局单位字典下拉，例如 `kg`、`lb`、`袋`。PR-459 后，缺 BOM/工序成本时不再按已发布售价反推成本。
...
- PR-459-PRICING-RULE-TRIAL-WATERFALL-BOM-DETAIL：试算结果区先显示价格瀑布，按 `BOM+工序成本 + 其他成本 + 损耗增加 + 加价增加 + 税额 + 取整调整 = 试算单价` 展示每个金额节点；未税模板的税额只做另计提示，不加进最终单价。`BOM+工序成本明细` 展示物料成本明细、工序成本明细、物料合计、工序合计和总计。缺 BOM/工序成本时，BOM 成本显示 0 和感叹号警告，不再反推历史发布售价。
- PR-460-PRICING-RULE-TRIAL-OUTPUT-BOM-OPERATION-SELECT：试算抽屉可选择 `试算BOM版本` 和 `工序`。BOM 版本只来自“生产 BOM 声明产出当前商品”的 active BOM / published 版本，默认选中最新发布版本，也可切换同一产出商品下其他 active/published 版本；工序下拉来自 active 工序模板。试算不按“商品绑定 BOM”反查，也不保留商品侧 BOM 绑定兜底逻辑；没有产出 BOM 明细时 `BOM+工序成本` 为 0 并提示维护生产 BOM。
...
- 需要确认某个模板套到某个商品后的价格时，在模板行点 `试算`，在 `价格计算模板试算` 抽屉选择商品，必要时选择客户范围、报价单位、试算BOM版本和工序，临时调整损耗、利润、税率或其他成本后系统会自动试算。结果区先看价格瀑布，确认 `BOM+工序成本`、其他成本、损耗增加、加价增加、税额和取整调整如何合计到试算单价；再展开 `BOM+工序成本明细` 查看物料和工序行；最后查看 `计算公式` 和公式步骤。这些内容只用于判断公式和成本来源，不能替代商品价格表发布。
...
- 价格计算模板试算提示无成本：先确认试算商品是否被有效生产 BOM 设为产出商品，并且该 BOM 有 published 版本和工序成本。PR-460 后试算只按 `production_boms.output_product_id` 查找产出 BOM，不按商品绑定 BOM 反查；无 BOM/工序成本时 `BOM+工序成本` 显示 0 和警告。模板停用或毛利低于最低毛利只影响当前查看，不保存任何业务数据。
