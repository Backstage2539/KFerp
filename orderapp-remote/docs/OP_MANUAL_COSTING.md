...
- PR-512-PRICING-RULE-TRIAL-SOURCE-COST / PR-514-WORKSTATION-COST-COMPONENTS / PR-516-STANDARD-COST-DEFAULT-CAPACITY：价格试算顶部会显示 `取整规则` 和来源，取整规则来自价格计算模板；`取整调整=0` 时不再显示醒目瀑布卡片。税率按 `临时税率 -> 价格计算模板非 0 税率 -> 财务设置全局税率 -> 0` 解析，税额卡片展示来源；价格计算模板选择不计税时税额强制为 0。`工艺路线` 是生产模板，路线工序可配置 `标准成本默认产能` 供标准制造成本试算使用；真实生产工序成本仍在生产计划/工单拆分选择本次实际工位产能后冻结。
...
- 在 商品价格管理 维护价格计算模板 / Pricing Rule：基础成本选择 `生产 BOM 成本（物料+工序）`，系统按“生产 BOM 产出当前商品”带入物料采购成本，并按工艺路线工序的 `标准成本默认产能` 或唯一匹配产能计算标准工序单位成本。额外费用在 `其他成本` 中按成本名/成本价格维护。继续配置损耗/出率、利润率或加价、税额/含税规则、取整规则、最低毛利提示、公式版本、试算说明和备注。模板试算可以临时选择商品、报价单位、试算 BOM 版本和工艺路线，但模板本身不绑定商品，也不保存数量档位。真实工序成本在生产计划/工单拆分中按 `工位小时成本 × 标准分钟/60 × 批次数` 冻结，例如 `5元/小时 × 60分钟/60 × 1批 = 5元`。
- 需要确认某个模板套到某个商品后的价格时，在商品价格管理顶部点 `价格试算`，在 `价格计算模板试算` 抽屉先选择启用的价格计算模板，再选择商品；必要时选择客户范围、报价单位、试算 BOM 版本和工艺路线，临时调整利润、税率或其他成本后系统会自动试算。`临时损耗率` 留空时不额外计损耗；只有显式输入时才显示并计算 `损耗增加`。结果区先看价格瀑布，确认 `BOM+工序成本`、其他成本、加价增加、税额和取整调整如何合计到试算单价；再展开 `BOM+工序成本折算明细` 查看 BOM 物料成本和标准工序成本来源：物料行显示 `BOM组成`、原料损耗和折算成本；工序行显示所用产能来源、工位、小时费率、标准分钟、标准产出和单位成本。如果同一工序有多条启用适用产能但路线未设置 `标准成本默认产能`，试算会提示 `请为工艺路线工序设置标准成本默认产能`，正式发布价格表会被拦截。最后查看 `计算公式` 和公式步骤。这些内容只用于判断公式和成本来源，不能替代商品价格表发布。
...
- 标准工序成本来自工艺路线工序的 `标准成本默认产能` 和工位小时费率，按 `小时费率 × 标准分钟 / 60 / 标准产出` 折算为 `元/库存单位`。这里使用库存单位，不使用销售规格或价格单位；若未设置默认且只有一个启用适用产能，系统可按 `唯一匹配产能` 试算，多个候选时必须先回工艺路线补默认。
- 价格计算模板只做利润、税率和取整，不维护产能批次数，也不定义 BOM 或工位成本。价格表发布时后端重新读取标准制造成本并固化快照，后续 BOM、工艺路线或工位成本变化不回改历史价格表。
