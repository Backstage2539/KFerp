...
- PR-577：试算选择到“已发布但没有组件”的生产 BOM，且同一 BOM 已有有组件草稿时，系统不会把该空版本显示为有效 0 元，也不会暗中读取草稿成本；页面会指出当前 published 版本和待发布 draft 版本。进入 `生产管理 → 生产 BOM`，先核对草稿组件、单位、商品形态、整体预期损耗和原料损耗，再由有权限人员发布草稿，然后返回重新试算。发布动作沿用现有操作日志；历史价格表和订单不重算。
