...
- 需要确认某个模板套到某个商品后的价格时，在模板行点 `试算`，在 `价格计算模板试算` 抽屉选择商品，必要时选择客户范围和报价单位，临时调整损耗、利润、税率或其他成本后系统会自动试算。结果区只用于判断公式和成本来源，不能替代商品价格表发布。
- 用上传 Excel 做供应售价核对时，先把 `物料成本` 对应到商品当前 BOM/物料成本，再把 `生产项目` 中进入加价基数的费用填入 `其他成本`。`供应售价` 不同档位的利润率/加价率逐档录入 `临时利润/加价` 后查看自动试算结果，检查公式步骤中的成本基数、加价后价格和最终单价。
