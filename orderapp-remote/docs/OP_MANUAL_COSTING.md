...
6. 在产品豆单的“豆单版本列表”查看官方、我的客户豆单或指定客户豆单版本；需要客户自己的豆单时先在 SKU设置 选择客户并按需复制公共 SKU/公共商品分类，再回到产品豆单选择该客户。
...
- SKU归属的客户下拉包含所有启用客户；即使客户还没有专属 SKU，也可以先选中客户并初始化客户自己的 SKU 和商品分类。
- 选中客户后，“新增公共产品”隐藏；“客户专属 SKU”使用顶部已选客户，不再重复选择客户。
- 在“商品分类 · 客户SKU”和“客户SKU列表 · 客户 SKU”中可开启“是否使用公共SKU”和“是否使用公共商品分类”，点击“复制公共配置”后，系统把当前公共 SKU、公共商品分类和分类归属复制为该客户自己的数据。
- 复制公共配置是初始化动作：已存在的同一公共基础 SKU 不重复创建，已存在的同名客户分类会复用；复制动作会写入操作日志。
- 客户SKU列表可按形态、名称、一级分类和二级分类筛选；筛选会先匹配当前归属下全部 SKU，再按分页展示，切换筛选条件后回到第一页。
