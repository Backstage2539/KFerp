...
- 需要确认某个模板套到某个商品后的价格时，在商品价格管理顶部点 `价格试算`，在 `价格计算模板试算` 抽屉先选择启用的价格计算模板，再选择商品；必要时选择客户范围、报价单位、试算 BOM 版本和工艺路线，临时调整利润、税率或其他成本后系统会自动试算。`临时损耗率` 留空时不额外计损耗；只有显式输入时才显示并计算 `损耗增加`。结果区先看价格瀑布，确认 `BOM+工序成本`、其他成本、加价增加、税额和取整调整如何合计到试算单价；再展开 `BOM+工序成本明细` 查看物料和工艺路线工序行，BOM 原料损耗折算在这里体现；最后查看 `计算公式` 和公式步骤。这些内容只用于判断公式和成本来源，不能替代商品价格表发布。
