...
## 客户上下文
- PR-500-UNIT-MODEL-CONSOLIDATION / PR-501-PRODUCT-UOM-SALES-CONVERSION：价格、录单和报价侧统一使用销售单位，库存、BOM、WIP 和成本消耗侧统一使用商品或物料库存单位；两者通过商品档案中的单位转换连接。商品档案库存单位决定 BOM 产出单位、生产入库单位和成本消耗口径；商品档案默认销售单位和销售单位换算决定价格表可选价格单位。商品价格管理只维护 Pricing Rule、成本、利润、税费和取整公式，不维护单位换算。商品价格表发布时把当时的商品档案库存单位和换算固化到快照；录单和订单继续读取已发布快照。设置里的单位模板只保留历史兼容和迁移排查，不作为普通商品设置库存单位或销售换算的主入口。
...
- 旧“单位规则维护在“商品配置模板”里”和“新增换算”只用于解释旧价格表或旧订单；新价格单位到库存单位的换算维护在商品档案，发布价格表时固化到已发布价格表快照中。
