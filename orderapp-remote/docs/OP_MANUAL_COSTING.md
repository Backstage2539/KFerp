...
## 标准操作
1. 先进入 `商品 / 商品价格管理`，在与 `价格计算模板` 并排的 `成本参数设置` Tab 检查基础换算、生产包装、零售税费/物流等参数；理论成本优先使用商品生产配置中的预期损耗率，系统内部按 `1 - 预期损耗率` 得到产出因子；商品报价由同页价格计算模板和商品价格表快照决定。
