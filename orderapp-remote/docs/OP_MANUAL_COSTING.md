...
## 梯度模板和价格来源
- PR-392：产品价格表生成和成本核算以“客户商品名 + 商品档案 + 商品档案配置”为主链路。客户商品名负责对外名称、编号、品牌和展示分类；商品档案负责库存、成本和生产；商品档案配置负责绑定商品配置模板、生产 BOM 版本、工艺路线、预期损耗率、行业字段模板和值。维护入口是 商品与配方 → 商品档案 → 点击商品名。
...
- PR-392：例如 `roast_level / 烘焙度`、`caffeine / 咖啡因`、`stick_spec / 条装规格`、服装布料克重、鲜果成熟度等，都作为商品生产配置字段维护。点击商品名打开商品档案配置抽屉后，选择行业字段模板或手工新增字段，按文本/长文本/数字/比例/选项/勾选/日期填写单位和值，并勾选“价格表展示”。发布产品价格表时，系统冻结客户商品名、商品档案、商品配置模板、生产 BOM 版本、工艺路线、预期损耗率和商品生产配置字段快照。旧 BOM 版本 `special_attrs_json`、旧 SKU `special_attrs_json` 和 `roast_level` 只作为 fallback，避免历史价格表、旧工单或旧 API 解释断层。
