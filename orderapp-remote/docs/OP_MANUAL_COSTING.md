...
- PR-452-PRICING-RULE-TRIAL：商品价格管理模板列表每行有 `试算`。点击后打开 `价格计算模板试算` 抽屉，选择商品和报价单位，可临时录入损耗率、利润/加价、税率和其他成本，点击 `重新试算` 查看 BOM+工序成本、损耗后成本、试算单价和公式步骤。试算是只读结果，不保存到模板、商品价格表、发布快照或订单；正式报价仍需回到商品价格表生成并发布。
- PR-453-PRICING-RULE-TRIAL-EXCEL-PARITY：做 Excel 供应售价对账时，`物料成本` 对应原料/BOM 成本；`生产项目` 中进入加价基数的电力、燃气、租金、生产损耗和人力等费用，录入为加价前生产项目成本，也就是试算抽屉里的 `其他成本`；包装、产品损耗、利润税额等加价后追加项，录入为 `售价后附加成本`。`供应售价` 的档位利润率/加价率录入到 `临时利润/加价`。结果区查看 `加价后价格`、`售价后附加成本`、`试算单价` 和公式步骤节点，用来核对 Excel，不会写入模板、商品价格表、发布快照或订单。
...
- PR-440-PRODUCT-GROUP-PRICE-REMODEL / PR-441-PRICE-LIST-TIER-TEMPLATE-MODES：产品价格表生成和成本核算以“商品档案 + 客户引用 + 分组管理 + Pricing Rule + 阶梯模板 + 价格表快照 + 生产追溯”为主链路。客户引用负责客户料号和客户显示名；商品档案负责库存和商品主数据；分组只负责组织、筛选、选品和模板继承；Pricing Rule 负责计算口径；生产 BOM 声明产出商品、版本和组件清单。
...
- 需要确认某个模板套到某个商品后的价格时，在模板行点 `试算`，在 `价格计算模板试算` 抽屉选择商品，必要时选择客户范围和报价单位，临时调整损耗、利润、税率或其他成本后点 `重新试算`。结果区只用于判断公式和成本来源，不能替代商品价格表发布。
- 用上传 Excel 做供应售价核对时，先把 `物料成本` 对应到商品当前 BOM/物料成本，再把 `生产项目` 拆成两段：进入加价基数的费用填入 `其他成本`，加价后才追加的费用填入 `售价后附加成本`。`供应售价` 不同档位的利润率/加价率逐档录入 `临时利润/加价` 后重新试算，检查公式步骤中的成本基数、加价后价格、售价后附加成本和最终单价。
...
## 客户上下文
- 商品与配方下的新业务入口是商品档案、分组管理、商品价格管理和商品价格表。商品档案维护 Item 主数据和客户引用；分组管理维护泛化分组和分组项树；商品价格管理维护 Pricing Rule；商品价格表维护 Price List / Item Price 平铺价格行。
...
- 版本列表、“生成价格表”抽屉和下方预览卡片中的分组来自分组管理和当次价格表分组快照。商品没有当前分组时进入未分组/未分类展示，不按旧字段拆成多个“其他”。
...
- 生成价格表不再提供“复制已有豆单配置”。商品档案、客户引用、分组和价格差异先在商品档案、分组管理、商品价格管理和商品价格表中处理，商品价格表按当前数据和本次抽屉设置直接生成。
...
- 客户价格表中商品不出现：先确认商品档案启用、分组和价格表选品包含该商品；客户侧展示名或料号在商品档案 `客户引用` 中维护。若分组消失，先在分组管理和价格表生成抽屉检查分组项是否启用、商品是否被选中。
...
- 价格计算模板试算提示无成本：先确认试算商品已维护有效生产 BOM 和工序成本；如果只是模板停用或毛利低于最低毛利，抽屉会显示预警但不保存任何业务数据。
