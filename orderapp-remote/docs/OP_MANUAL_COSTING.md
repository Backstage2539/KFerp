...
9. PR-389：价格表、成本试算和 PDF 展示的特殊属性优先读取商品档案当前绑定的 BOM 版本；旧商品没有绑定 BOM 版本或版本属性为空时，才 fallback 到旧 SKU 特殊属性字段。
10. 确认无误后发布价格表，让录单、报价和客户侧展示使用新版本。
...
- PR-382：历史 SKU 复制抽屉保留兼容，但新贴牌场景不要默认复制商品档案/BOM。客户只改名称、编号、品牌或展示分类时，创建客户商品名并绑定已有商品档案；配方、包装、生产方式、库存对象或成本口径变化时，才创建新商品档案和生产 BOM。
...
- PR-376/377/378 历史兼容：早期特殊 KV 曾在商品配置模板和 SKU 行维护，用于产品价格表展示。PR-389 后，新 UI 不再在商品管理编辑这些字段；商品配置模板保留阶梯价模板、工序模板、价格表生成规则和单位模板，特殊属性改到 BOM 版本维护。
- PR-389：BOM 版本携带 `special_attrs_schema_json` 和 `special_attrs_json`。例如 `roast_level / 烘焙度`、`caffeine / 咖啡因`、`stick_spec / 条装规格`、服装布料克重、鲜果成熟度等，都在“生产 BOM（配方库）→ BOM版本与特殊属性”里维护。发布 BOM 版本后，这些特殊属性随版本冻结；产品价格表发布时再冻结到价格表快照和 PDF。
- 已发布 BOM 版本只读。需要改特殊属性或下拉选项时，先复制为新版草稿，在草稿中编辑 JSON 字段定义和值，发布后再把商品档案绑定到新版本。旧 SKU 的 `special_attrs_json`、`roast_level` 仍作为 fallback，避免历史价格表、旧工单或旧 API 解释断层。
