...
- PR-512-PRICING-RULE-TRIAL-SOURCE-COST / PR-514-WORKSTATION-COST-COMPONENTS：价格试算顶部会显示 `取整规则` 和来源，取整规则来自价格计算模板；`取整调整=0` 时不再显示醒目瀑布卡片。税率按 `临时税率 -> 价格计算模板非 0 税率 -> 财务设置全局税率 -> 0` 解析，税额卡片展示来源；价格计算模板选择不计税时税额强制为 0。`工艺路线` 是生产模板，只决定工序顺序和质检/损耗要求；价格试算不从路线模板读取工位产能或计划工序成本，真实工序成本在生产计划/工单拆分选择工位产能后冻结。
...
- 在 商品价格管理 维护价格计算模板 / Pricing Rule：基础成本选择 `生产 BOM 成本（物料+工序）`，系统按“生产 BOM 产出当前商品”带入物料采购成本；工艺路线只作为试算/生产的路线模板选择，不提供工位产能成本。额外费用在 `其他成本` 中按成本名/成本价格维护。继续配置损耗/出率、利润率或加价、税额/含税规则、取整规则、最低毛利提示、公式版本、试算说明和备注。模板试算可以临时选择商品、报价单位、试算 BOM 版本和工艺路线，但模板本身不绑定商品，也不保存数量档位。真实工序成本在生产计划/工单拆分中按 `工位小时成本 × 标准分钟/60 × 批次数` 冻结，例如 `5元/小时 × 60分钟/60 × 1批 = 5元`。
- 需要确认某个模板套到某个商品后的价格时，在商品价格管理顶部点 `价格试算`，在 `价格计算模板试算` 抽屉先选择启用的价格计算模板，再选择商品；必要时选择客户范围、报价单位、试算 BOM 版本和工艺路线，临时调整利润、税率或其他成本后系统会自动试算。`临时损耗率` 留空时不额外计损耗；只有显式输入时才显示并计算 `损耗增加`。结果区先看价格瀑布，确认 `BOM+工序成本`、其他成本、加价增加、税额和取整调整如何合计到试算单价；再展开 `BOM+工序成本折算明细` 查看 BOM 物料成本和路线模板来源：`BOM组成` 是原始配方，`损耗后用量` 是原料损耗后的有效比例，`折算成本` 会随报价单位变成 `/kg`、`/lb` 等，BOM 原料损耗折算也在这里体现；工艺路线只说明生产路径，不提供路线工序成本。最后查看 `计算公式` 和公式步骤。这些内容只用于判断公式和成本来源，不能替代商品价格表发布。
...
- 标准工序成本来自工艺路线的标准工序参数和工位小时费率，按 `小时费率 × 标准工时 / 标准产出` 折算为 `元/库存单位`。这里使用库存单位，不使用销售规格或价格单位。
