...
4. 如需维护挂耳商品报价，按普通商品处理：在商品价格管理维护价格计算模板，在商品价格表选择阶梯价模板和价格计算模板生成并发布快照；不要再使用挂耳专用模板。
...
8. PR-440-PRODUCT-GROUP-PRICE-REMODEL：商品价格表的选品和继承来自分组管理；生成抽屉可按分组勾选商品，并在默认、父组、子组和商品行设置阶梯价模板和 Pricing Rule。
...
- PR-440-PRODUCT-GROUP-PRICE-REMODEL：商品价格管理现在维护 `价格计算模板 / Pricing Rule`，它是价格计算器模板，不绑定商品、不维护阶梯档位、不保存最终成交价。模板字段覆盖成本来源、BOM/耗材成本口径、工艺/工序成本口径、损耗/产出率、利润率或加价、税额/含税规则、取整规则、最低毛利提示和备注；模板修改只影响后续草稿或重新计算，不回改已发布快照。
- PR-440-PRODUCT-GROUP-PRICE-REMODEL：新版阶梯价模板属于商品价格表模块，只定义档位名、最小数量、最大数量、数量单位、排序、启停和备注；它不保存商品、最终价、成本公式或利润率。
- PR-440-PRODUCT-GROUP-PRICE-REMODEL：商品价格表生成时可选择分组作为选品/继承分组；默认、父组、子组和商品行都可以设置阶梯价模板和价格计算模板。系统按 `商品 > 子组 > 父组 > 默认` 分别解析阶梯价模板和 Pricing Rule，再展开为商品 + 档位的平铺价格行。
- PR-440-PRODUCT-GROUP-PRICE-REMODEL：价格表发布快照必须固化最终价、价格单位、库存换算、分组快照、阶梯价模板来源、Pricing Rule 版本、计算时成本来源、客户引用显示快照和人工调整标记。手动改价必须写操作日志。
...
- PR-440-PRODUCT-GROUP-PRICE-REMODEL：商品价格表负责选择 Pricing Rule 和阶梯价模板、生成草稿、展开平铺价格行、发布版本并形成快照。商品档案只读展示价格摘要，摘要内容来自已发布价格表快照，包括来源价格表版本、最终价、价格单位、档位和更新时间；没有快照时显示 `暂无价格表价格`。
...
- PR-440-PRODUCT-GROUP-PRICE-REMODEL：产品价格表生成和成本核算以“商品档案 + 客户引用 + 分组管理 + Pricing Rule + 阶梯价模板 + 价格表快照 + 生产追溯”为主链路。客户引用负责客户料号和客户显示名；商品档案负责库存和商品主数据；分组只负责组织、筛选、选品和模板继承；Pricing Rule 负责计算口径；生产 BOM 声明产出商品、版本和组件清单。
...
- 在 商品价格管理 维护价格计算模板 / Pricing Rule：配置成本来源、BOM/耗材成本口径、工艺/工序成本口径、损耗/产出率、利润率或加价、税额/含税规则、取整规则、最低毛利提示和备注。模板试算可以临时选择商品和数量，但模板本身不绑定商品。
- 需要阶梯价格时，在 商品价格表模块维护新版阶梯价模板。阶梯价模板只定义档位名、最小数量、最大数量、数量单位、排序、启停和备注；不同档位的最终价格由商品价格表按对应 Pricing Rule 和价格表行计算生成。
- 商品价格表生成抽屉会把默认、父组、子组和商品行的模板选择解析为平铺价格行，并随价格行冻结分组快照、阶梯价模板来源、Pricing Rule 版本、成本来源、客户引用显示快照、最终价、价格单位、库存单位和库存换算。缺少可发布价格、单位或换算时不得发布。
- 客户小程序转售豆单只能基于当前客户可见的已发布工厂供货价格表快照。需要开放给客户转售时，确认来源价格表快照的最终价、价格单位和库存换算能生成客户转售快照。
- PR-400 / PR-404 / PR-440：商品没有有效阶梯价模板、Pricing Rule 或可发布价格行时，产品价格表不得自动补默认档位或旧 Excel 档位。商品卡片应提示维护价格计算模板、阶梯价模板或价格表生成设置；不再引导普通用户去旧商品配置模板选择计价方式。
- PR-415 / PR-440：产品价格表候选、生成抽屉、预览卡片和 PDF 产品选择使用同一套候选过滤。旧 Excel 豆单 metadata code 缺失时不隐藏商品；挂耳、速溶或未分组商品仍进入候选。挂耳不再有专用价格模板或专用渲染枚举，新生成的商品价格表按通用阶梯价模板和 Pricing Rule 发布快照。
...
- 修改快速成本参数、Pricing Rule、阶梯价模板或价格表生成设置后，只会刷新未发布的成本试算和价格表草稿；已发布价格表、录单和客户下单价格不会静默改变，必须重新发布价格表后才生效。
...
- PR-440：客户价格表发布时会同时冻结商品档案、客户引用显示快照、分组快照、最终价、价格单位、库存换算、阶梯价模板来源、Pricing Rule 版本、成本来源、人工调整标记和生产来源追溯。后续客户引用改名、商品档案改名、BOM 升级、分组调整或 Pricing Rule 调整，都不会回改已发布价格表。
...
- 官方豆单是没有专属客户时的公共兜底；客户豆单只对归属客户生效，客户自己的历史版本可继续追溯，但新版本按当前商品档案、客户引用、分组、阶梯价模板和 Pricing Rule 直接生成。
...
- 挂耳商品如果需要按盒、袋或其他规格报价，在 `商品与配方 → 商品价格管理` 维护 Pricing Rule，在商品价格表选择阶梯价模板并生成价格表快照；价格单位和库存换算必须固化到已发布价格表。
...
- 商品价格档、零售价和按 Pricing Rule + 阶梯价模板生成的挂耳商品报价显示完整。
- 客户豆单的商用批发豆单标题与公共豆单一致，优先使用价格表分组快照；客户引用只影响客户料号和客户显示名。
- 绑定阶梯价模板和 Pricing Rule 的产品按价格档生成商用梯度价格，价格来源能解释 Pricing Rule 版本、最终价、价格单位和库存换算。
...
- 挂耳盒价数量不对：检查阶梯价模板、价格单位、库存换算和已发布价格表快照；盒/袋换算应在价格表快照中固化。
