...
- PR-455-PRICING-RULE-TRIAL-PR439-UNIT：试算抽屉删除 `重新试算` 按钮和 `售价后附加成本`。报价单位来自全局单位字典下拉，例如 `kg`、`lb`、`袋`。选择 `PR439-20260606182321 熟豆下单商品` 且当前没有 BOM/工序成本时，系统会读取已发布 `88.5/kg` 发布售价快照，按模板公式反推成本基数并显示 `发布售价快照反推成本基数` 预警；该 fallback 只用于查看公式节点，不保存任何业务数据。
