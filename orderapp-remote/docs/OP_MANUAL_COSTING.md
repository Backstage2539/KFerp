...
- 父商品行的“商品计价”可以设置继承给规格的计价方式或价格模板；每个具体规格另有独立“计价”入口，标题显示 `商品名 · 规格`。系统按 `规格 > 商品 > 子类 > 父类 > 价格表` 解析计价方式和模板。固定价金额只能在具体规格填写，不能在商品、分类或价格表层共用，也不能让 227g、454g、磅、1Kg 串用同一个固定价。
- 阶梯模板只定义销售规格件数，不再定义 kg、lb、g 重量。模板的 1、10、100 档在 454g 商品上分别显示为 1个454g、10个454g、100个454g；换成 227g 时仍是 1、10、100 个227g。
...
- 进入 `商品 → 商品价格表` 的“选择分类和产品”。同一父商品的有效规格横向排列并自动换行；每个规格项直接包含勾选框、规格名称、`默认规格` 标识和 `计价`。点击某个规格的计价后，配置区在整行规格下方全宽展开，关闭后恢复紧凑规格行。
...
- 商品价格表生成区当前按 `规格 > 商品 > 子类 > 父类 > 价格表` 解析计价方式和模板。商品或规格显示继承时，如果上层已经选择有效阶梯模板或价格计算模板，系统不会再提示 `未设置计价方式`；固定价金额不继承，必须在具体规格单独录入。
