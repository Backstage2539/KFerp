...
- PR-454-PRICING-RULE-TRIAL-EXCEL-PARITY：做 Excel 供应售价对账时，`物料成本` 对应原料/BOM 成本；`生产项目` 中进入加价基数的电力、燃气、租金、生产损耗和人力等费用，录入为加价前生产项目成本，也就是试算抽屉里的 `其他成本`；包装、产品损耗、利润税额等加价后追加项，录入为 `售价后附加成本`。`供应售价` 的档位利润率/加价率录入到 `临时利润/加价`。结果区查看 `加价后价格`、`售价后附加成本`、`试算单价` 和公式步骤节点，用来核对 Excel，不会写入模板、商品价格表、发布快照或订单。
