...
8. PR-441-PRICE-LIST-TIER-TEMPLATE-MODES：商品价格表的选品和继承来自分组管理；生成抽屉可按分组勾选商品，并在价格表、父类、子类和商品行设置 `按阶梯模板计算`、`按价格计算模板计算` 或 `固定价`。
...
- PR-441-PRICE-LIST-TIER-TEMPLATE-MODES：商品价格表生成时可选择分组作为选品/继承分组；价格表、父类、子类和商品行都可以设置计价模式。系统按 `商品 > 子类 > 父类 > 价格表` 解析计价模式，再按模式解析阶梯模板、价格计算模板或固定价。
