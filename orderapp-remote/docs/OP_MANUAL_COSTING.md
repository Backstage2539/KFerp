...
- PR-505-SALES-SPEC-TEMPLATE-DERIVED-SKU：最新商品档案以父 SKU 维护库存单位，并引用 `销售规格模板` 自动派生子 SKU；销售规格模板只维护 `227g袋装 / 袋 / 227 g` 这类规格明细，不维护库存单位或销售单位换算。价格表仍按具体子 SKU 发布价格行，价格单位默认来自子 SKU 销售单位，库存单位来自父 SKU。
...
- PR-505 后，销售规格模板不再维护换算；系统按子 SKU 销售单位和父 SKU 库存单位固化发布快照，规格克重和包装差异由子 SKU 规格字段和 BOM 成本表达。
...
- 挂耳商品如果需要按盒、袋或其他规格报价，先在商品档案维护父 SKU 库存单位并引用销售规格模板自动派生具体子 SKU，再在 `商品与配方 → 商品价格管理` 维护 Pricing Rule，并在商品价格表选择具体子 SKU 生成价格表快照；价格单位、父 SKU 库存单位和 SKU 快照由发布时固化到已发布价格表。
- 价格表预览、生成抽屉和 PDF 都按商品当前归类显示商品类型；分类名包含“挂耳”也不会切换到旧 `drip_wholesale_tiers`。
- 历史已发布的旧挂耳价格表和旧订单快照仍可读取、下载和追溯；新发布价格表不再使用挂耳专用模板生成价格。

## 客户上下文
- PR-500-UNIT-MODEL-CONSOLIDATION / PR-504-PARENT-PRODUCT-CHILD-SKU-UOM / PR-505-SALES-SPEC-TEMPLATE-DERIVED-SKU：价格、录单和报价侧使用子 SKU 销售单位，库存、BOM、WIP 和成本消耗侧使用子 SKU 继承到的父 SKU 库存单位；规格克重和包装差异由销售规格模板、SKU 快照和 BOM 表达。父 SKU 维护库存单位和销售规格模板，子 SKU 自动派生且不直接维护库存单位或单位换算。商品价格管理只维护 Pricing Rule、成本、利润、税费和取整公式，不维护单位换算。商品价格表发布时把当时的子 SKU、销售单位、父 SKU 库存单位和 SKU 快照固化到快照；录单和订单继续读取已发布快照。
...
- 创建商品档案时填写父 SKU 名称、库存单位、销售规格模板、行业字段和备注；销售规格模板必须选择，模板只维护规格名称、销售单位、净含量、默认规格和启用状态。不选择旧分类模板、产品类型、产品子类型、商品配置模板或旧阶梯价模板。新增商品档案不在新增表单里复制 BOM、价格表快照或客户引用关系；生产结构到生产 BOM 维护，价格到 Pricing Rule 和商品价格表维护。
...
- 旧“单位规则维护在“商品配置模板”里”只用于解释旧价格表或旧订单；当时的结构化界面通过“新增销售单位”维护 1 盒 = 0.2 kg。PR-505 后新业务不再把 `袋/227g` 当单位换算维护，父 SKU 维护库存单位，销售规格模板派生子 SKU，发布价格表时固化子 SKU 销售单位、父 SKU 库存单位和 SKU 快照。
...
- 挂耳盒价数量不对：检查父 SKU 是否引用了正确销售规格模板、子 SKU 销售单位是否正确、阶梯模板/价格单位是否匹配，以及已发布价格表快照是否固化了正确 SKU。盒/袋这类包装层级应通过销售规格模板派生出的子 SKU 和价格表快照核对，克重差异看子 SKU/BOM。
