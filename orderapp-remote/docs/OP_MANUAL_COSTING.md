...
- PR-468-PRICING-RULE-LIST-ACTIONS-UX：商品价格管理中，`价格试算` 是列表顶部动作，位于 `新建价格计算模板` 左侧。点击后在抽屉内选择启用的价格计算模板，再选择商品、报价单位、BOM 版本和工序试算；模板行不再单独放 `试算`。点击模板名称进入编辑，不再使用独立 `编辑模板` 按钮。停用模板会置灰但仍可复制，复制按钮不置灰，复制后生成启用的新价格计算模板。
...
- PR-452 / PR-468-PRICING-RULE-TRIAL：商品价格管理顶部提供 `价格试算`。点击后打开 `价格计算模板试算` 抽屉，先选择启用的价格计算模板，再选择商品和报价单位，可临时录入损耗率、利润/加价、税率和其他成本；字段完整后系统自动试算，结果区查看 BOM+工序成本、损耗后成本、试算单价和公式步骤。试算是只读结果，不保存到模板、商品价格表、发布快照或订单；正式报价仍需回到商品价格表生成并发布。
...
- PR-460-PRICING-RULE-TRIAL-WATERFALL-BOM-DETAIL：试算结果区先显示价格瀑布，按 `BOM+工序成本 + 其他成本 + 损耗增加 + 加价增加 + 税额 + 取整调整 = 试算单价` 展示每个金额节点；未税模板的税额只做另计提示，不加进最终单价。`BOM+工序成本明细` 展示物料成本明细、工序成本明细、物料合计、工序合计和总计。缺 BOM/工序成本时，BOM 成本显示 0 和感叹号警告，不再反推历史发布售价。
- PR-462-PRICING-RULE-TRIAL-OUTPUT-BOM-OPERATION-SELECT：试算抽屉可选择 `试算BOM版本` 和 `工序`。BOM 版本只来自“生产 BOM 声明产出当前商品”的 active BOM / published 版本，默认选中最新发布版本，也可切换同一产出商品下其他 active/published 版本；工序下拉来自 active 工序模板。试算不按“商品绑定 BOM”反查，也不保留商品侧 BOM 绑定兜底逻辑；没有产出 BOM 明细时 `BOM+工序成本` 为 0 并提示维护生产 BOM。
...
- PR-440-PRODUCT-GROUP-PRICE-REMODEL / PR-502-PRODUCT-UNIT-TEMPLATE-REFERENCE：商品档案不再维护报价单位、录单单位、商品配置模板、计价方式、固定价、成本加成或利润率覆盖；客户差异只在商品档案的客户引用子表维护客户料号和客户显示名，客户引用不参与价格、单位、BOM、库存或分组。PR-502 后商品档案维护单位模板引用，单位模板决定库存单位、默认销售单位和可销售单位换算。
...
- PR-440-PRODUCT-GROUP-PRICE-REMODEL / PR-502-PRODUCT-UNIT-TEMPLATE-REFERENCE：商品档案保存不再写入旧价格模板字段；独立客户商品写入口下线或只读兼容。旧商品配置模板、旧阶梯价模板和分类模板保留为历史兼容或迁移来源，主菜单和客户工作区不再展示这些旧入口；单位模板恢复为普通商品 UOM 主入口，不作为价格模板或换算公式来源。
...
- 在 商品价格管理 维护价格计算模板 / Pricing Rule：基础成本选择 `生产 BOM 成本（物料+工序）`，系统按“生产 BOM 产出当前商品”带入物料采购成本和工序成本；额外费用在 `其他成本` 中按成本名/成本价格维护。继续配置损耗/出率、利润率或加价、税额/含税规则、取整规则、最低毛利提示、公式版本、试算说明和备注。模板试算可以临时选择商品、报价单位、试算BOM版本和工序，但模板本身不绑定商品，也不保存数量档位。
- 需要确认某个模板套到某个商品后的价格时，在商品价格管理顶部点 `价格试算`，在 `价格计算模板试算` 抽屉先选择启用的价格计算模板，再选择商品；必要时选择客户范围、报价单位、试算BOM版本和工序，临时调整损耗、利润、税率或其他成本后系统会自动试算。结果区先看价格瀑布，确认 `BOM+工序成本`、其他成本、损耗增加、加价增加、税额和取整调整如何合计到试算单价；再展开 `BOM+工序成本明细` 查看物料和工序行；最后查看 `计算公式` 和公式步骤。这些内容只用于判断公式和成本来源，不能替代商品价格表发布。
...
- PR-469-PRICE-LIST-PUBLISH-NO-RESPONSE：点击 `发布价格表` 前先看按钮附近的阻断原因。空预览、缺版本号、缺客户、价格行不完整时，页面会显示阻断原因，不会再静默无反馈。BOM已失效不在顶部汇总提示，而是在具体商品行下显示；点击商品行里的 `去商品档案重新选择 BOM`，到商品档案重新选择可用 BOM。失效 BOM 不能重新启用，如需沿用旧结构，先在生产 BOM 复制成新 BOM 后再选择。
...
- 挂耳商品如果需要按盒、袋或其他规格报价，先在商品档案维护默认销售单位和销售单位到库存单位换算，再在 `商品与配方 → 商品价格管理` 维护 Pricing Rule，并在商品价格表选择阶梯模板和价格单位生成价格表快照；价格单位和库存换算由发布时的商品档案快照固化到已发布价格表。
- 价格表预览、生成抽屉和 PDF 都按商品当前归类显示商品类型；分类名包含“挂耳”也不会切换到旧 `drip_wholesale_tiers`。
- 历史已发布的旧挂耳价格表和旧订单快照仍可读取、下载和追溯；新发布价格表不再使用挂耳专用模板生成价格。

## 客户上下文
- PR-500-UNIT-MODEL-CONSOLIDATION / PR-501-PRODUCT-UOM-SALES-CONVERSION / PR-502-PRODUCT-UNIT-TEMPLATE-REFERENCE：价格、录单和报价侧统一使用销售单位，库存、BOM、WIP 和成本消耗侧统一使用商品或物料库存单位；两者通过商品有效单位规则连接。单位模板是普通商品 UOM 主数据模板，商品档案必须引用单位模板；普通商品档案不再直接维护库存单位、整数库存或销售单位换算。历史商品级覆盖只作为兼容读取。商品有效库存单位决定 BOM 产出单位、生产入库单位和成本消耗口径；商品有效默认销售单位和换算决定价格表可选价格单位。商品价格管理只维护 Pricing Rule、成本、利润、税费和取整公式，不维护单位换算。商品价格表发布时把当时的商品有效库存单位和换算固化到快照；录单和订单继续读取已发布快照。
- 报价单位和录单单位只作为历史兼容字段；新保存的单位模板会把销售单位同步写入旧字段，历史价格表、订单和成本快照不回改。
...
- 创建商品档案时填写名称、单位模板、行业字段和备注；单位模板必须选择，库存单位、默认销售单位、整数规则和销售单位换算到单位模板维护。不选择旧分类模板、产品类型、产品子类型、商品配置模板或旧阶梯价模板。新增商品档案不在新增表单里复制 BOM、价格表快照或客户引用关系；生产结构到生产 BOM 维护，价格到 Pricing Rule 和商品价格表维护。
...
- 旧商品配置模板、阶梯价模板页面的历史直达链接仍可能打开兼容界面；不要把这些链接作为新商品、新价格或新录单配置入口。单位模板页面仍可用于商品 UOM 模板维护。
...
- 旧“单位规则维护在“商品配置模板”里”只用于解释旧价格表或旧订单；当时的结构化界面通过“新增换算”维护 1 盒 = 0.2 kg。PR-502 后新价格单位到库存单位的换算维护在单位模板中，发布价格表时固化到已发布价格表快照中。
...
- 挂耳盒价数量不对：检查商品档案的默认销售单位、销售单位换算、阶梯模板、价格单位和已发布价格表快照；盒/袋换算应先在商品档案维护，并在发布价格表时固化到快照。
