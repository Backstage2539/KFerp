...
8. PR-441 / PR-445 / PR-447 / PR-448：商品价格表的选品和继承来自分组管理。生成抽屉可按分组勾选商品；选品区默认只显示分类/商品选择和计价、展示摘要。价格表默认计价在“生成规则”区设置；分类计价仍在分类头 A 位置，商品计价、标签和标红词仍在商品勾选行 B 位置，但列表内只显示 `继承分类` 或实际模板/方式；点击分类或商品行的 `计价 / 展示` 摘要后，在弹窗中修改配置。计价可选择 `按阶梯模板计算`、`按价格计算模板计算` 或 `固定价`。
...
- PR-441 / PR-445 / PR-447 / PR-448：商品价格表生成时可选择分组作为选品/继承分组；价格表默认、父类、子类和商品行都可以设置计价模式。价格表默认在生成规则区维护；选品区默认只显示分类/商品选择和计价、展示摘要。分类计价放在分类头 A 位置，商品行覆盖放在商品勾选行 B 位置；摘要继承态统一显示 `继承分类`，不在列表里展示父类/子类字样，点击 `计价 / 展示` 摘要后在弹窗中维护配置。没有生成价格行时不显示平铺价格行块；预览按当前勾选商品生成。系统按 `商品 > 子类 > 父类 > 价格表` 解析计价模式，再按模式解析阶梯模板、价格计算模板或固定价。
