...
- PR-461-PRICE-LIST-PICKER-TREE-PRICING-POPOVER：商品价格表选品区按父类、子类、商品逐级缩进；父类和子类标题可收缩/展开，收缩后不清空勾选。点击分类或商品行的 `计价` 摘要后，在按钮附近修改计价模式；菜单包含 `继承分类`、`按阶梯模板价计算`、`按价格模板计算`、`固定价`。选择继承会清掉当前分类或商品自己的覆盖，商品行 `展示` 仍在展示弹窗中维护。
- PR-441 / PR-445 / PR-447 / PR-449 / PR-459 / PR-461：商品价格表生成时使用商品档案所选商品分组作为选品/继承分组；价格表默认、父类、子类和商品行都可以设置计价模式。价格表默认在主页面 `Price List / Item Price 生成规则` 区维护；选品区默认只显示分类/商品选择和计价、展示摘要，父类、子类、商品逐级缩进，分类可收缩/展开。分类计价放在分类头 A 位置，商品行覆盖放在商品勾选行 B 位置；摘要继承态统一显示 `继承分类`，不在列表里展示父类/子类字样，点击 `计价` 摘要后在按钮附近修改计价模式，点击商品行 `展示` 摘要后在弹窗中维护标签和标红词。`计价模式规则` 只作为按钮弹窗展示，不在页面常驻。没有生成价格行时不显示平铺价格行块；预览按当前勾选商品生成。系统按 `商品 > 子类 > 父类 > 价格表` 解析计价模式，再按模式解析阶梯模板、价格计算模板或固定价。
...
- 需要确认某个模板套到某个商品后的价格时，在模板行点 `试算`，在 `价格计算模板试算` 抽屉选择商品，必要时选择客户范围和报价单位，临时调整损耗、利润、税率或其他成本后系统会自动试算。结果区先看价格瀑布，确认 `BOM+工序成本`、其他成本、损耗增加、加价增加、税额和取整调整如何合计到试算单价；再展开 `BOM+工序成本明细` 查看物料和工序行；最后查看 `计算公式` 和公式步骤。这些内容只用于判断公式和成本来源，不能替代商品价格表发布。
...
- 价格计算模板试算提示无成本：先确认试算商品已维护有效生产 BOM 和工序成本。PR-460 后系统不再按已发布售价快照反推成本；无 BOM/工序成本时 `BOM+工序成本` 显示 0 和警告。模板停用或毛利低于最低毛利只影响当前查看，不保存任何业务数据。
