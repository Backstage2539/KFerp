...
- PR-460-PRICING-RULE-TRIAL-WATERFALL-BOM-DETAIL：试算结果区先显示价格瀑布，按 `BOM+工序成本 + 其他成本 + 加价增加 + 税额 + 取整调整 = 试算单价` 展示金额节点；只有本次显式输入 `临时损耗率` 且损耗金额大于 0 时，才在 `其他成本` 后额外显示 `损耗增加`。未税模板的税额只做另计提示，不加进最终单价。`BOM+工序成本明细` 展示物料成本明细、工序成本明细、物料合计、工序合计和总计。缺 BOM/工序成本时，BOM 成本显示 0 和感叹号警告，不再反推历史发布售价。
...
- PR-511-BOM-MATERIAL-LOSS-BOM-LEVEL：BOM+工序成本明细读取生产 BOM 版本级 `原料损耗比`，并按物料比例组件形成明细快照。当物料行 `比例 % = 40%` 且 `原料损耗比 = 20%` 时，材料成本按有效比例 `ratio / (1 - 原料损耗比) = 40% / (1 - 20%) = 50%` 计入；`合计比例` 仍显示净配方比例，不把损耗并入比例合计。商品价格管理试算默认把这部分损耗归入 `BOM+工序成本`，不会再把商品/BOM 默认损耗二次套到 `BOM+工序成本` 上。
...
- 需要确认某个模板套到某个商品后的价格时，在商品价格管理顶部点 `价格试算`，在 `价格计算模板试算` 抽屉先选择启用的价格计算模板，再选择商品；必要时选择客户范围、报价单位、试算BOM版本和工序，临时调整利润、税率或其他成本后系统会自动试算。`临时损耗率` 留空时不额外计损耗；只有显式输入时才显示并计算 `损耗增加`。结果区先看价格瀑布，确认 `BOM+工序成本`、其他成本、加价增加、税额和取整调整如何合计到试算单价；再展开 `BOM+工序成本明细` 查看物料和工序行，BOM 原料损耗折算在这里体现；最后查看 `计算公式` 和公式步骤。这些内容只用于判断公式和成本来源，不能替代商品价格表发布。
