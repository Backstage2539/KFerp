...
6. 在产品价格表的“已发布价格表”按“价格表归属”查看公共或具体履约客户的版本；需要客户自己的报价时优先在商品管理维护客户商品名并绑定商品档案。
7. 生成或发布价格表前确认价格表归属：工厂自营/公共归属用于工厂报价，客户归属用于指定客户。
8. PR-387-VIEW-CONTEXT：如果顶部“当前视图”是某个客户，产品价格表会把价格表归属锁定为该客户；如果当前视图是订单，系统先派生订单客户，再按该客户客户商品名生成候选。
8. 确认无误后发布价格表，让录单、报价和客户侧展示使用新版本。

## 梯度模板和价格来源
- 先在 商品管理 维护阶梯价模板：每个模板选择一个展示单位（元/磅、元/kg、元/盒、元/袋等），每个档位填写区间名、最小/最大数量和利润率；产品价格表展示单位默认继承阶梯价模板单位。某个产品要从磅改成盒，不在商品配置里改展示单位，而是复制或新建一个“盒”单位阶梯价模板并绑定到对应商品配置。
...
- 若某个商品档案需要长期使用不同利润率，在 商品管理 的商品档案列表填写该行“产品级利润率覆盖”；系统会用该产品利润率覆盖产品子类型梯度模板利润率，清空后恢复继承产品子类型模板。只改变某客户对外价格时，优先通过客户商品名进入该客户价格表发布，不直接改公共商品档案利润率。
...
- 需要长期调整原料成本、预期损耗率、预期产出率或利润率时，按参数来源回到成本参数设置、BOM 配方维护、商品管理 的梯度模板或挂耳供应价模板维护。
...
- PR-386：客户价格表发布时会同时冻结客户商品名和商品档案双快照，包括客户商品名 ID、客户展示名、客户商品编号、品牌、展示分类、商品档案 ID/编号/名称、生产 BOM 版本和使用方式、价格单位、阶梯、特殊属性和价格来源 JSON。后续客户商品名改名、商品档案改名、BOM 升级或模板调整，都不会回改已发布价格表。
...
## 客户上下文
- 商品管理 顶部仍通过归属上下文维护商品档案、商品分类和商品配置，不再内嵌价格表或价格试算。
- PR-386：客户侧新增报价对象优先使用“客户商品名”。客户商品名绑定一个商品档案，维护客户展示名、客户商品编号、品牌、展示分类、排序和是否进入价格表；工厂自营也作为一个内置客户-like 归属维护客户商品名。
- 创建商品档案时只填写名称、备注、产品类型和产品子类型。只有选中产品子类型时，系统才会把新商品档案挂到该子类型；不选择产品子类型时，商品会进入“停车场”，不会参与产品价格表生成，后续可从停车场拖入正确子类型。新增商品档案不在新增表单里复制 BOM 或价格梯度；这些能力统一通过商品档案行资料、BOM 配方维护和商品配置完成。
- 历史 SKU 复制流程保留兼容，但新贴牌场景不要默认复制商品档案/BOM。客户只改名称、编号、品牌或展示分类时，创建客户商品名并绑定已有商品档案；配方、包装、生产方式、库存对象或成本口径变化时，才创建新商品档案和生产 BOM。
...
- PR-362-PRODUCT-CONFIG-TEMPLATE / PR-386：生成客户范围产品价格表前，先在 商品管理 确认客户商品名已绑定商品档案，且商品档案所属产品子类型绑定了正确“商品配置”。商品配置统一承载阶梯价模板、工序模板、价格表生成规则和单位规则；客户要差异化时，复制公共商品配置为客户配置后再绑定到客户产品子类型。成本核算页切到某个客户范围时，后续会按客户商品名绑定的商品档案和商品配置预览价格表，避免生成时仍使用公共默认规则。
...
- PR-376/377：商品配置模板可以维护“产品信息字段（特殊属性KV）”，也就是原“特殊KV定义”，用于处理只属于某类产品的关键属性，例如 `roast_level / 烘焙度`、`caffeine / 咖啡因`、`stick_spec / 条装规格`。新增 KV 时填写 Key、名称、类型和选项，勾选“展示到价格表/PDF”后，商品档案中填写的值会进入产品价格表预览、发布快照、公共页面和 PDF；不勾选则只作为内部商品属性保存。旧的 `roast_level` 兼容字段仍可读取旧数据，但商品管理不再单独提供硬编码烘焙度下拉。若商品档案列表特殊属性列显示“未配置字段”，点击“配置字段”会切到对应商品配置模板的字段配置区。
...
- PR-382 / PR-385 / PR-386：历史 SKU 复制抽屉仍可用于兼容旧客户 SKU。复制后的商品默认跟随来源商品档案当前有效生产 BOM，成本核算和产品价格表会按这个有效 BOM 计算，不会因为本地没有 BOM 明细而缺成本。客户需要专属配方时，在 BOM 配方维护中点击“复制为单独维护 BOM”，之后成本和价格表改用该商品档案的单独生产 BOM。
- SKU 列表字段较多时，列表本身支持横向滚动。产品类型、产品子类型和特殊属性列会保持单行展示，避免“咖啡烘焙豆”被挤成一个字一行；需要查看更多列时在 SKU 表格区域左右滑动。
- 速溶盒装示例：先在物料档案维护“速溶咖啡条装原料”，单位用“条”，默认采购价填每条成本，备注 1 条 10g；在 BOM 配方维护中给“速溶盒装”添加该物料，消耗单位选“个/盒”，用量填 10，再按需添加盒子、标签等 `个/盒` 包材；商品档案自己的 BOM 出品率设为 100%。然后在 商品管理 → 商品配置 → 单位模板 新建“速溶盒装单位”，成品库存单位、报价单位和录单单位都选“盒”，需要重量统计时增加 `1 盒 = 0.1 kg` 或 `1 盒 = 10 条` 换算并开启整数单位。商品配置模板选择该单位模板和“盒”单位阶梯价模板，价格表生成规则可选“成本加成”并填写加成比例，也可选“固定单价”并填写每盒售价。生成并发布“速溶咖啡”产品价格表后，录单会显示 `元/盒`。
...
- 旧 `list_type` 仍作为兼容和内部渲染模式保留：生豆仍使用生豆价档，挂耳仍使用袋/盒价档，其他产品类型默认走普通产品报价渲染。新增产品类型不要再改代码加固定选项，先在 商品管理 维护产品类型和产品子类型。
...
- 客户商品名绑定的商品档案如果跟随默认生产 BOM，产品价格表使用跟随 BOM 的成本；点击“复制为单独维护 BOM”后，产品价格表和成本核算使用该商品档案单独维护的生产 BOM。来源说明只用于追溯，不会随来源商品档案改名或来源 BOM 升级而改写。
...
- 客户商品名不再依赖旧 Excel 豆单资料。只要客户商品名绑定的商品档案已挂到产品类型/子类型，产品价格表客户范围会按该客户商品名的展示分类或商品档案分类生成分组；标题沿用公共价格表逻辑，优先显示实际分类名。例如挂到“咖啡豆 / 定制咖啡熟豆”的商品会显示在“1、定制咖啡熟豆”分组下，不会落到旧公共豆单“源产地精选”，也不会因为缺少旧豆单编码而不显示。
- 客户商品名还没有展示分类时，产品价格表会把它放到“未分类”，不会继续沿用基础公共商品的旧 Excel 分类；需要正式分组时先回到 商品管理 维护客户商品名展示分类或商品档案产品子类型。
...
- 客户新增商品在客户价格表中不出现：先确认 商品管理 中该客户商品名已启用、已绑定有效商品档案且勾选进入价格表；若是客户自有/客户定制熟豆，产品价格表会按客户商品名展示分类或商品档案分类自动生成条目，不需要旧 Excel 豆单资料。若刚勾选再取消“是否使用公共商品分类”，客户自有分类仍应保留；若分类消失，应检查后端 schema 修复是否已重新启用仍有 active 子分类的父分类。
- 旧客户 SKU 是否应该进入新价格表：如果只是贴牌、客户命名、客户编号、展示分类或客户专属价格差异，到 商品管理 → 客户商品名 用“旧客户 SKU 收敛检查”预填客户商品名并绑定来源商品档案；新价格表会按客户商品名生成。旧客户 SKU 不会被自动删除，历史价格表和历史订单仍按原快照追溯。
