...
## 客户上下文
- PR-500-UNIT-MODEL-CONSOLIDATION：价格、录单和报价侧统一使用销售单位，库存、BOM、WIP 和成本消耗侧统一使用商品或物料库存单位；两者通过单位转换连接。商品档案库存单位决定 BOM 产出单位、生产入库单位和成本消耗口径；销售单位、价格单位和换算只在商品价格管理、价格表快照、录单和订单快照中固化。设置里的单位模板只保留历史兼容和迁移排查，不作为普通商品设置库存单位的主入口。
