...
商品价格试算不再选择销售规格。先选主商品，再选其默认已发布 BOM、版本和 BOM 规格；结果单位、成本明细和公式均以所选 BOM 规格库存单位为准（例如 454 袋显示“元/袋”）。若请求带入子 SKU 或与 BOM 规格不一致的报价单位，系统会拒绝并提示到 BOM 配置维护。
...
试算成本取匹配 BOM 规格，平铺价格行最终价右侧显示商品档案单位。BOM 的报价/库存单位仅写入成本来源快照，不再在快照列显示“商品档案单位”换算说明；快照列用“计算规则”和“状态”说明来源和是否自动计算。
