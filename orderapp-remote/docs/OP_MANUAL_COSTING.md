...
- 创建商品档案时填写名称、单位模板、行业字段和备注；同类商品优先引用单位模板，例外 SKU 再打开高级单位覆盖维护库存单位、默认销售单位和换算。不选择旧分类模板、产品类型、产品子类型、商品配置模板或旧阶梯价模板。新增商品档案不在新增表单里复制 BOM、价格表快照或客户引用关系；生产结构到生产 BOM 维护，价格到 Pricing Rule 和商品价格表维护。
...
- 旧“单位规则维护在“商品配置模板”里”只用于解释旧价格表或旧订单；当时的结构化界面通过“新增换算”维护 1 盒 = 0.2 kg。PR-502 后新价格单位到库存单位的换算维护在单位模板或商品档案高级覆盖中，发布价格表时固化到已发布价格表快照中。
