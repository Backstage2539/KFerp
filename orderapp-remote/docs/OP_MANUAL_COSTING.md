...
- PR-403：商品价格表遇到 `初晓2.5kg装` 这类没有配置计价方式的商品时，会在商品卡片上显示“未设置计价方式：商品价格表不会生成阶梯报价”。看到该提示后，从主菜单进入 `商品与配方 → 商品配置和分类模板 → 商品配置模板`，在该商品引用的配置模板里选择 `计价方式` 下拉；如果还没有可选阶梯，先进入 `商品与配方 → 阶梯价模板` 维护阶梯价模板，再回到商品配置模板绑定。
