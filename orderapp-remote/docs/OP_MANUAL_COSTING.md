...
- 商品与配方下的新业务入口是商品档案、商品价格管理和商品价格表；分组模板入口是独立的 `设置 / 分组模板`，不在系统设置页面内。商品档案维护 Item 主数据和客户引用；分组模板维护大类和小类；商品价格管理维护 Pricing Rule；商品价格表维护 Price List / Item Price 平铺价格行。
...
- 客户价格表中商品不出现：先确认商品档案启用、所选分组模板和价格表选品包含该商品；客户侧展示名或料号在商品档案 `客户引用` 中维护。若分类消失，先在独立 `设置 / 分组模板` 页面和价格表生成抽屉检查分类项是否启用、商品是否被选中。
