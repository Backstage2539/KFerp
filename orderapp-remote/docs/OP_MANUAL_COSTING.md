...
- PR-512-PRICING-RULE-TRIAL-SOURCE-COST / PR-514-WORKSTATION-COST-COMPONENTS：价格试算顶部会显示 `取整规则` 和来源，取整规则来自价格计算模板；`取整调整=0` 时不再显示醒目瀑布卡片。税率按 `临时税率 -> 价格计算模板非 0 税率 -> 财务设置全局税率 -> 0` 解析，税额卡片展示来源；价格计算模板选择不计税时税额强制为 0。`BOM+工序成本` 中的工序成本来自当前 `工艺路线` 每道工序自动折算的 `计划工序成本`，不是旧工序模板，也不是商品销售单位。
...
- 商品价格表预览和生成 PDF 的商品卡片按当前分类内顺序编号，例如 `1.1`、`1.2`；编号不再沿用旧 bean-list 元数据里的商品 ID 后缀。卡片只展示商品名、去重后的价格表行业字段/属性和报价，长商品名自动换行；同一行左右卡片按该行最高卡片等高对齐，行高仍随内容自适应，单条报价块按文字高度收缩，不用固定头部高度或底部自动撑空白；旧豆单的 `出品建议`、`风味`、`特点` 和 `批发价` 标签不进入商品价格表卡片。
...
- 在 商品价格管理 维护价格计算模板 / Pricing Rule：基础成本选择 `生产 BOM 成本（物料+工序）`，系统按“生产 BOM 产出当前商品”带入物料采购成本和工艺路线计划工序成本；工艺路线计划工序成本由 `工位/设备` 的机器/人工/其他小时成本和 `工位产能` 的标准批量、标准分钟自动折算，例如 `5元/小时 × 60分钟/60 ÷ 5kg = 1元/kg`。额外费用在 `其他成本` 中按成本名/成本价格维护。继续配置损耗/出率、利润率或加价、税额/含税规则、取整规则、最低毛利提示、公式版本、试算说明和备注。模板试算可以临时选择商品、报价单位、试算 BOM 版本和工艺路线，但模板本身不绑定商品，也不保存数量档位。
