...
- PR-511-BOM-MATERIAL-LOSS-BOM-LEVEL：BOM+工序成本明细读取生产 BOM 版本级 `原料损耗比`，并按物料比例组件形成明细快照。当物料行 `比例 % = 40%` 且 `原料损耗比 = 20%` 时，材料成本按有效比例 `ratio / (1 - 原料损耗比) = 40% / (1 - 20%) = 50%` 计入；`合计比例` 仍显示净配方比例，不把损耗并入比例合计。
...
- PR-508 后，理论成本和试算中的 BOM 物料成本还会读取 BOM 行级 `原料损耗比`；该字段只放大对应物料需求，不替代商品生产配置的整体预期损耗率。
