...
| 商品/成本维护 | 维护商品档案、客户商品、商品配置模板、生产 BOM、价格表归属、客户专属价格表版本和价格 | 不直接改客户订单金额；客户专属数据不能泄露给其他客户；客户商品不承载 BOM，商品档案只反查 BOM | 商品档案、客户商品、商品配置和分类模板、生产 BOM、产品价格表、成本核价手册 |
...
2. 在 客户商品 选择该客户，确认客户商品是否已绑定正确商品档案；贴牌只改客户商品，配方、包装、库存对象或生产方式变化才回到 商品档案 创建新商品档案，并在 生产 BOM 中把该商品设为产出。客户需要一次性上架多款现有商品时，点过滤行右侧“新建客户商品”打开抽屉，再切到“批量添加商品档案”模式，在商品列表顶部搜索商品档案；系统会跳过同客户已存在的绑定并记录创建日志。客户商品单个新增和批量新增都不填写客户商品编号、不选择分类模板，也不在抽屉内设置“进入价格表”，编号由系统生成。需要分组时，在客户商品页 Tab 行右侧用“增加分类”可搜索下拉启用分类模板，再勾选客户商品，用旁边“移动到分类”可搜索下拉确认归类；移动到其他大类、分类项或未分类会直接覆盖旧归类。客户商品页顶部可搜索、按启用/停用/全部过滤，也可勾选多条后“批量失效”；停用会逐条写操作日志。绑定商品档案已停用时，列表会标红提示“绑定商品已失效”。
...
4. 在 产品价格表 把“价格表归属”切到该客户，确认对应商品类型的价格表已发布。商品类型来自客户商品当前归属的分类模板名称；未归类统一进入“其他”。报价配置优先读取客户商品选择的商品配置模板，其次是绑定商品档案的商品配置模板，再其次是客户商品所属分类项或分类模板引用的商品配置模板。客户商品如果没有有效价格，客户侧下单和内部履约提交会返回价格未发布错误，不会按 0 元静默下单。客户需要在手机端维护展示分类时，从小程序 个人中心 → 我的商品 → 商品分类 新增、改名、删除、上移/下移或归类商品；第一次修改公共分类时系统会派生客户专属分类模板，公共模板不被改写。
5. 在 商品档案 检查商品配置模板、工艺路线、预期损耗率，以及该商品是否被上层 BOM 作为组件使用；在 生产 BOM 检查是否有 BOM 产出该商品、组件版本是否有效。挂耳、盒装速溶和半成品尤其要核对产出 BOM 与组件商品。
...
- 商品与配方：商品档案、客户商品、商品配置模板、生产 BOM，用于维护真实 SKU、客户展示名、模板规则和配方库。
...
- 客户发布商品价格表失败：先检查当前客户是否有 `bean_list` 能力、来源是否是该客户可见的工厂供货价格表、阶梯价模板是否已授权客户转售使用，以及来源价格单位是否能匹配所选模板。手机端已去掉覆盖档位和单品价，只支持统一加价、倍率加价和标签/说明展示。
