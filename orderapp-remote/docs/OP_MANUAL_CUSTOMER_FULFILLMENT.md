...
| 场景 | 操作流程 | 核对点 |
| --- | --- | --- |
| 新客户开通 | 客户档案设为批发客户 → 客户门户配置选择能力模板 → 创建外部用户、设置密码并启用登录 → 保存配置 → 客户履约运营台选择客户 | 客户出现在履约运营台选择器；客户侧登录后只看到模板允许的入口 |
...
5. 外部用户不在本页维护；如需创建账号、重置密码或启停登录，回到工厂总览的“客户门户配置”的“外部用户”区域处理。外部用户不进入员工维护页，不配置内部角色。
