...
2. 在 客户商品名 选择该客户，确认客户商品名是否已绑定正确商品档案；贴牌只改客户商品名，配方、包装、库存对象或生产方式变化才回到 商品档案 创建新商品档案并维护商品生产配置。客户需要一次性上架多款现有商品时，使用“批量添加商品档案”，系统会跳过同客户已存在的绑定并记录创建日志；客户商品名单个新增和批量新增都不填写客户商品编号、不选择分类模板，编号由系统生成。需要分组时，在客户商品名页点击“增加分类”启用分类模板 Tab：在“全部/未分类”Tab 用“移动到分类”移入大类，在大类 Tab 用“移动到子类”移入分类项；移动到其他大类或子类会直接覆盖旧归类。客户商品名页顶部可搜索、按启用/停用/全部过滤，也可勾选多条后“批量停用”；停用会逐条写操作日志。
...
- 客户重传同一商品后出现新旧两个商品：新流程先核对客户商品名绑定的商品档案是否一致。客户商品编号是系统生成编号，不作为客户外部编号手工维护；历史排查中仍可参考“客户商品编号是否保持一致”来识别旧客户 SKU 数据。只是客户展示名、品牌或客户侧分类变化时，应在 客户商品名 更新或批量创建客户商品名并绑定原商品档案；配方、包装、生产方式、库存对象或成本口径变化时，才在 商品档案 创建新商品档案并通过商品名打开配置抽屉维护商品生产配置。旧客户 SKU 可通过“旧客户 SKU 收敛检查”只读识别，系统不会自动删除或回改历史订单。
