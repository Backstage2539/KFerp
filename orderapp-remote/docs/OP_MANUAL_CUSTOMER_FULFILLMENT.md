...
2. 在 客户商品名 选择该客户，确认客户商品名是否已绑定正确商品档案；贴牌只改客户商品名，配方、包装、库存对象或生产方式变化才回到 商品档案 创建新商品档案并维护商品生产配置。客户需要一次性上架多款现有商品时，使用“批量添加商品档案”，系统会跳过同客户已存在的绑定并记录创建日志；客户商品名单个新增和批量新增都不填写客户商品编号、不选择分类模板，编号由系统生成。需要分组时，在客户商品名页点击“增加分类”启用分类模板 Tab：在“全部/未分类”Tab 用“移动到分类”移入大类，在大类 Tab 用“移动到子类”移入分类项；移动到其他大类或子类会直接覆盖旧归类。
3. 在 商品价格表 把“价格表归属”切到该客户，确认对应商品类型的价格表已发布。商品类型来自客户商品当前归属的分类模板名称；未归类统一进入“其他”。客户商品名如果没有有效价格，客户侧下单和内部履约提交会返回价格未发布错误，不会按 0 元静默下单。
