...
2. 在 客户商品名 选择该客户，确认客户商品名是否已绑定正确商品档案；贴牌只改客户商品名，配方、包装、库存对象或生产方式变化才回到 商品档案 创建新商品档案并维护商品生产配置。客户需要一次性上架多款现有商品时，点过滤行右侧“新建客户商品”打开抽屉，再切到“批量添加商品档案”模式；系统会跳过同客户已存在的绑定并记录创建日志。客户商品名单个新增和批量新增都不填写客户商品编号、不选择分类模板，编号由系统生成。需要分组时，在客户商品名页 Tab 行右侧用“增加分类”可搜索下拉启用分类模板，再勾选客户商品，用旁边“移动到分类”可搜索下拉确认归类；移动到其他大类或分类项会直接覆盖旧归类。客户商品名页顶部可搜索、按启用/停用/全部过滤，也可勾选多条后“批量失效”；停用会逐条写操作日志。绑定商品档案已停用时，列表会标红提示“绑定商品已失效”。
