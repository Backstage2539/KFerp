...
1. 在“商品 → 商品价格表”选择具体客户和商品类型，完成客户价格表发布；再在发布版本区域分别维护“该类别的一件代发价格表”和“该类别的商品下单价格表”。指定保存具体发布记录，发布新版不会自动替换。
...
- **一件代发 / 现货下单**：页面按“订单日期 → 收件客户 → 当前价格表 → 商品明细 → 商品估算合计 → 备注 → 提交”录入。商品选择复用员工录单的搜索和分类，规格弹层列出当前发布 BOM 的全部真实名称并标出默认规格；数量变化后自动显示服务端核定的只读单价、小计和合计。客户不能填写单价、运费、优惠或切换价格表。提交时服务端再次校验绑定发布版本、数量档位和报价签名；价格或版本变化会要求重新提交。成功后生成正常 ERP 订单并保留收件、价格表版本和成交价格快照；同一请求重试不重复下单。库存不足仍可下单，后续只走该订单已有生产流程。
...
- **现货下单 / 代发录单**：使用与员工录单相同的商品搜索、分类、规格、多行数量和单位逻辑。先选订单日期和收件客户，再逐行选择商品、真实规格和数量。价格表由 ERP 在商品价格表页按客户、商品类型和入口指定，客户侧不能选表或改价；金额自动核算并只读展示。
