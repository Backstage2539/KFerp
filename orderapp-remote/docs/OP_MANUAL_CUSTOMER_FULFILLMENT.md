...
- **一件代发 / 现货下单**：页面按“订单日期 → 收件客户 → 当前价格表 → 商品明细 → 商品估算合计 → 备注 → 提交”录入。商品选择复用员工录单的搜索和分类；选中商品后，录单行内直接列出指定发布价格表中的全部真实 BOM 规格，点击规格即可切换，不显示“默认规格”占位文字。数量变化后自动显示服务端核定的只读单价、小计和合计。客户不能填写单价、运费、优惠或切换价格表。提交时服务端再次校验绑定发布版本、规格、数量档位和报价签名；价格或版本变化会要求重新提交。成功后生成正常 ERP 订单并保留收件、价格表版本和成交价格快照；同一请求重试不重复下单。库存不足仍可下单，后续只走该订单已有生产流程。
- **生产工单**：默认先看申请列表，点“新建工单”后选择代加工商品、规格、数量、期望完成日期和备注。预览分别显示生产配置是否有效、物料是否齐套及缺料明细；缺料仍可提交，申请直接进入“待排产”，提交时不占用物料。网络重试使用同一提交标识时返回原申请，不重复生成待排产需求；同一标识的商品、规格、数量、期望日期或备注发生变化时会提示冲突并要求重新提交。商品停用、客户归属不符、未标记或 BOM/规格缺失会阻止提交。列表和详情显示申请数量、真实生产计划/工单状态、部分完成及实际完工入库数量。
