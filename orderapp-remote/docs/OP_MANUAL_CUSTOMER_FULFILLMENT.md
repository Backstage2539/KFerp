...
5. 进入客户履约账户，在“选择客户”中按客户名、公司、联系人或电话搜索客户，并点击载入账户；下拉框展示 active 且已开通门户/工作台的客户，不再按 `customer_type=wholesale` 硬编码，也不要求已经创建外部账号或密码。客户账户模式已选择客户时，本页会自动载入该客户；如果该客户未绑定有效能力模板，页面只显示空工作台或基础信息，提交代发、代加工、结算等动作仍会被能力校验拒绝。
