...
| 商品/成本维护 | 维护商品档案、客户商品名、商品生产配置、生产 BOM、价格表归属、客户专属价格表版本和价格 | 不直接改客户订单金额；客户专属数据不能泄露给其他客户；客户商品名不承载 BOM | 商品档案、客户商品名、商品配置和分类模板、生产 BOM、商品价格表、成本核价手册 |
...
2. 在 客户商品名 选择该客户，确认客户商品名是否已绑定正确商品档案；贴牌只改客户商品名，配方、包装、库存对象或生产方式变化才回到 商品档案 创建新商品档案并维护商品生产配置。客户需要一次性上架多款现有商品时，点过滤行右侧“新建客户商品”打开抽屉，再切到“批量添加商品档案”模式，在商品列表顶部搜索商品档案；系统会跳过同客户已存在的绑定并记录创建日志。客户商品名单个新增和批量新增都不填写客户商品编号、不选择分类模板，也不在抽屉内设置“进入价格表”，编号由系统生成。需要分组时，在客户商品名页 Tab 行右侧用“增加分类”可搜索下拉启用分类模板，再勾选客户商品，用旁边“移动到分类”可搜索下拉确认归类；移动到其他大类、分类项或未分类会直接覆盖旧归类。客户商品名页顶部可搜索、按启用/停用/全部过滤，也可勾选多条后“批量失效”；停用会逐条写操作日志。绑定商品档案已停用时，列表会标红提示“绑定商品已失效”。
3. 如果客户报价需要展示与工厂商品不同的行业字段，例如客户自己的烘焙度描述、包装规格或展示文案，在客户商品名列表的“客户行业字段”列点“设置”。字段定义来自绑定商品档案的行业字段模板；留空则继承商品档案值。客户商品名字段只影响销售展示和客户价格表，不改变生产 BOM、工艺路线或库存对象。
4. 在 商品价格表 把“价格表归属”切到该客户，确认对应商品类型的价格表已发布。商品类型来自客户商品当前归属的分类模板名称；未归类统一进入“其他”。客户商品名如果没有有效价格，客户侧下单和内部履约提交会返回价格未发布错误，不会按 0 元静默下单。
...
- 客户重传同一商品后出现新旧两个商品：新流程先核对客户商品名绑定的商品档案是否一致。客户商品编号是系统生成编号，不作为客户外部编号手工维护；历史排查中仍可参考“客户商品编号是否保持一致”来识别旧客户 SKU 数据。只是客户展示名、品牌或客户侧分类变化时，应在 客户商品名 更新或批量创建客户商品名并绑定原商品档案；配方、包装、生产方式、库存对象或成本口径变化时，才在 商品档案 创建新商品档案并通过商品名打开配置抽屉维护商品生产配置。旧客户 SKU 只作为历史兼容追溯，不再在客户商品名页面提供“旧客户 SKU 收敛检查”入口。
