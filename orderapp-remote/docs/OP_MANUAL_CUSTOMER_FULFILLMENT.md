...
## 适用范围
本手册把原“工作台模式手册、客户门户手册、客户履约手册”归并为一个客户履约操作手册，覆盖客户开通、门户和外部用户、工作台切换、客户商品和豆单、Excel 导入、客户下单、内部履约、库存发货、费用月结和操作追溯。
...
- PR-376-CHANNEL-PORTAL-WORKBENCH-SWITCH：客户能看到什么，主要由客户档案“开通客户门户/工作台”开关、能力模板、外部用户绑定和客户专属 SKU/豆单决定；客户类型只描述业务身份，不直接授予门户或工作台能力。内部员工能操作什么，主要由 ERP 角色和页面权限决定。
- 客户账户模式默认隐藏内部“履约运营台”入口；客户只进入客户侧履约工作台或小程序，不进入内部运营台。

## 合并后的阅读地图
```mermaid
flowchart LR
  A["客户档案/开关/客户类型"] --> B["门户配置/能力模板"]
  B --> C{"客户入口"}
...
| --- | --- | --- |
| 工作台模式 | 入口、工作台与客户上下文、结果校验、常见问题 | 管理员、履约运营、渠道客户 |
| 客户门户与商城 | 客户开通、能力模板、外部用户、小程序和商城、客户侧权限 | 管理员、客户成功、实施人员 |
| 客户履约账户 | Excel 导入、托管库存、代发/代加工、履约客户订单、费用月结 | 履约运营、仓库、财务 |

补充阅读：客户专属 SKU、公共 SKU 引用和豆单版本看“成本核价手册”和“物料、库存与 BOM 手册”；生豆产品、绑定熟豆 BOM 和生豆豆单看“生豆销售手册”；履约订单进入 ERP 后的销售单、出库单、失效订单和发货资料看“订单销售手册”。
...
| 角色 | 可操作范围 | 权限边界 | 重点入口 |
| --- | --- | --- | --- |
| 管理员/老板 | 配置客户类型、能力模板、外部用户、全局设置、权限、操作日志，必要时查看全流程 | 不要把客户外部账号当内部员工账号维护；客户能力模板变更会实时影响客户 | 客户档案、客户门户配置、能力模板、全局设置、员工维护、操作日志 |
| 客户成功/实施 | 帮客户开通门户、确认模板能力、准备试单、核对客户是否能登录和看到正确入口 | 不能绕过模板给零售商城客户开放 ERP 工作台；不能把一个外部用户 active 绑定多个客户 | 客户门户配置、客户履约手册 |
...
| 商品/成本维护 | 维护客户专属 SKU、公共 SKU 引用、BOM、豆单范围、客户专属豆单版本和价格 | 不直接改客户订单金额；客户专属数据不能泄露给其他客户 | SKU设置、BOM配方维护、产品豆单、成本核价手册 |
| 仓库/生产 | 核对客户托管库存、成品库存、生产计划、发货、销售单和出库单 | 不配置客户账号、能力模板或结算；库存和发货变更要能在操作日志追溯 | 仓库库存、库存作业、生产计划、订单列表 |
...
关键权限口径：
- `customers.write`：客户门户配置、能力模板、外部用户绑定等客户开通类操作。
...
| 场景 | 操作流程 | 核对点 |
| --- | --- | --- |
| 新客户开通 | 客户档案选择客户类型并打开“开通客户门户/工作台” → 选择能力模板 → 创建外部用户、设置密码并启用登录 → 保存配置 → 顶部客户账户或客户履约运营台选择客户 | PR-376-CHANNEL-PORTAL-WORKBENCH-SWITCH：只有 active、已开通门户/工作台、模板暴露工作台或下单能力的客户才进入履约候选；客户侧登录后只看到模板允许的入口 |
| 商品和报价准备 | SKU设置选择履约客户 → 维护客户专属 SKU 或公共 SKU 引用 → BOM 配方维护 → 产品豆单选择客户范围并发布价格 | 客户下单商品只包含公共可见商品和当前客户专属商品；价格来自正确豆单版本 |
| 客户侧下单 | 客户登录工作台或小程序 → 选择现货/代发/代加工 → 填收件信息和商品行 → 提交订单 | 客户不能填写单价、运费或优惠；服务端按客户、商品、规格和豆单重新计价 |
...
## 新用户上手清单
1. 先用一个测试客户完成开通：客户档案选择客户类型（可用渠道客户验证）、打开“开通客户门户/工作台”、选择能力模板、维护外部用户并启用登录。
2. 在 SKU设置 选择该客户，确认客户是否使用公共 SKU/公共商品分类，是否有客户专属 SKU。
3. 在产品豆单把“豆单范围”切到该客户，确认熟豆、生豆、挂耳价格都已发布或明确不启用。
4. 在 BOM 配方维护检查该客户要下单的产品是否有有效 BOM；挂耳和生豆尤其要核对绑定关系。
...
## 入口
- 内部员工顶部工作台模式：切到“客户账户”，选择当前客户后再处理该客户的履约、订单、SKU、豆单、BOM、库存和财务。
- 渠道客户账号登录：不展示工作台切换，左侧显示“工作台”和“费用相关”；“工作台”进入自己绑定客户的履约运营台。
- 工厂总览不再展示客户履约账户入口；客户门户配置和能力模板仍从工厂总览的设置入口维护。
- 设置：客户履约手册、客户门户配置。
- 商品与配方：SKU设置，用于维护客户专属 SKU。
...
## 标准操作
1. 第一次导入前，确认客户档案存在、客户启用、已打开“开通客户门户/工作台”，并绑定包含代加工、托管库存、代发、结算等能力的模板。渠道客户可以用“渠道代发/现货下单”模板做现货或代发场景。
2. 为代加工客户准备独立代加工仓，避免客户成品和自营库存混在一起。
3. 客户自己的产品作为客户专属 SKU 管理，不放进公共产品列表，也不能直接共用其他客户 SKU。
4. 如果本次要连续处理同一个客户，先在 ERP 顶部切到“客户账户”并选择当前客户；系统会把该客户带入履约运营台、SKU设置、产品豆单、BOM、仓库库存、订单列表、费用管理、经营报告和结账相关，避免每个页面重复选择。生产计划/开始生产不进入客户账户模式，排产和开工统一切回工厂总览处理。
5. 进入客户履约账户，在“选择客户”中按客户名、公司、联系人或电话搜索客户，并点击载入账户；下拉框只展示 active、已开通门户/工作台、能力模板暴露 ERP 工作台或下单能力的客户，不再按 `customer_type=wholesale` 硬编码。客户账户模式已选择客户时，本页会自动载入该客户。
5. 客户账户模式下 SKU设置、BOM、产品豆单、仓库库存和费用管理都锁定顶部当前客户；不要在这些页面内另选客户。公共 SKU/BOM 只作为公共引用，客户自己的配置只归当前客户。
5. PR-311-CUSTOMER-PORTAL-ACCOUNT-PROFILE：外部用户不在本页维护；如需创建账号、重置密码或启停登录，回到工厂总览的“客户门户配置”的“外部用户”区域处理。保存后顶部“客户账户”当前客户下拉会自动刷新；客户门户配置的“打开客户档案”可直接打开当前客户资料抽屉。外部用户不进入员工维护页，不配置内部角色。
...
12. 在客户履约账户手工提交代加工工单或代发订单前，也要确认客户已开通对应能力；未开通对应能力的客户不能通过内部 API 绕过页面写入工单或代发订单。手工提交订单信息时按“一个收件信息 + 多行商品”录入，渠道客户的终端收件人写订单收件字段，不新增客户档案；历史收件信息从该渠道客户历史订单聚合，可下拉搜索选择。逐行选择商品/规格或单位/数量后，页面会实时展示阶梯价、行小计，并实时计算订单合计（商品金额 + 运费）。曲奇拼配等 KG 梯度商品按该行总 KG 选档，1000g 及以上保存元/kg 单价并按总 KG 计算小计；挂耳商品按袋或盒下单，盒价按挂耳已发布价格梯度换算。
...
21. 如果管理员修改客户引用的能力模板，履约客户账号刷新工作台后会立即按新模板展示功能；模板被标记失效时，该客户工作台和绑定流程会报错，管理员需要重新绑定启用中的模板。
...
- PR-376-CHANNEL-PORTAL-WORKBENCH-SWITCH：客户下拉框只出现 active、已打开“开通客户门户/工作台”、能力模板暴露 ERP 工作台或下单能力的客户；未开通、模板不暴露工作台/下单能力、登录已停用或仅剩 inactive 绑定的客户不出现。渠道客户满足这些条件后可作为履约客户。
...
## 常见问题
- 页面没有客户数据：确认客户档案是否存在、客户是否启用、是否打开“开通客户门户/工作台”、是否已经在下拉框选择客户并点击载入账户；如果客户有历史 ERP 绑定但仍不出现，检查绑定账号是否仍启用登录、能力模板是否暴露 ERP 工作台或下单能力且模板键能被系统识别。
