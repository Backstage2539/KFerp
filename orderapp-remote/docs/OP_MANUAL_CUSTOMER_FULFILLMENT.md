...
| 商品/成本维护 | 维护商品档案、客户商品名、生产 BOM、价格表归属、客户专属价格表版本和价格 | 不直接改客户订单金额；客户专属数据不能泄露给其他客户 | 商品管理、BOM配方维护、产品价格表、成本核价手册 |
...
| 商品和报价准备 | 商品管理维护商品档案 → 为客户创建客户商品名并绑定商品档案 → 按需维护生产 BOM → 产品价格表选择价格表归属并发布价格 | 客户下单商品只包含当前客户启用的客户商品名；价格来自正确价格表版本，生产和成本追溯到商品档案 |
...
2. 在 商品管理 选择该客户，确认客户商品名是否已绑定正确商品档案；贴牌只改客户商品名，配方、包装、库存对象或生产方式变化才创建新商品档案。
3. 在 产品价格表 把“价格表归属”切到该客户，确认对应产品类型的价格表已发布。客户商品名如果没有有效价格，客户侧下单和内部履约提交会返回价格未发布错误，不会按 0 元静默下单。
4. 在 BOM 配方维护检查该客户商品名绑定的商品档案是否有有效生产 BOM；挂耳和生豆尤其要核对绑定关系。
...
- 商品与配方：商品管理，用于维护商品档案、客户商品名和商品配置。
...
4. 如果本次要连续处理同一个客户，先在 ERP 顶部切到“客户账户”并选择当前客户；系统会把该客户带入履约运营台、商品管理、产品价格表、BOM、仓库库存、订单列表、费用管理、经营报告和结账相关，避免每个页面重复选择。生产计划/开始生产不进入客户账户模式，排产和开工统一切回工厂总览处理。
...
5. 客户账户模式下 商品管理、BOM、产品价格表、仓库库存和费用管理都锁定顶部当前客户；不要在这些页面内另选客户。客户商品名只归当前客户；生产 BOM 和商品档案用于内部执行追溯。
...
- 客户重传同一 SKU 后出现新旧两个商品：先核对客户商品编号是否保持一致。只是客户展示名、编号、品牌或展示分类变化时，应在 商品管理 → 客户商品名 更新或新建客户商品名并绑定原商品档案；配方、包装、生产方式、库存对象或成本口径变化时，才创建新商品档案。旧客户 SKU 可通过“旧客户 SKU 收敛检查”只读识别，系统不会自动删除或回改历史订单。
