...
- **一件代发**：粘贴并解析收货信息，选择指定价格表目录内的商品、规格和数量，点击“核对价格”。页面显示价格表名称、发布版本、单价、小计和总额。提交时服务端再次校验价格表、数量档位和报价签名；价格或版本变化会要求重新核价。成功后生成一张正常 ERP 订单并保留代发关联记录、发布版本和成交价格快照；同一请求重试返回原订单，不重复下单。库存不足仍可下单，后续只走该订单已有生产流程，不额外创建第二份生产需求。新式 ERP 订单不能沿用旧库存申请的直接取消逻辑，需要按 ERP 订单流程处理。
...
- **个人中心**：查看当前客户、客户联系人、业务联系人、服务说明和账号操作；在“收件地址”维护当前客户所有账号共用的地址簿。一件代发和商品下单可选地址，也可临时修改本次信息或明确保存为新地址。
...
- **我的价格表**：列表显示名称、版本、发布日期。点击“预览”才加载普通价格表格，展示商品、规格、数量阶梯、计价单位和单价；再次点击收起。不生成图片、PDF，也不提供下载。
- **商品下单 / 代发录单**：使用与员工录单相同的商品搜索、分类、规格、多行数量和单位逻辑。价格表由 ERP 在商品价格表页按客户、商品类型和入口指定，客户侧不显示选表控件；客户不能修改成交单价、优惠、付款或发货状态。
- **连续录单**：填写商品和收件信息后选择“保存并继续”或“保存并查看订单”。继续录单保留客户、日期和所选价格表，清空商品、收件信息及本单备注。失败时保留输入；同一请求重试返回同一单号。
