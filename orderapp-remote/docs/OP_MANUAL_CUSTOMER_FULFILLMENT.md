...
| 场景 | 操作流程 | 核对点 |
| --- | --- | --- |
| 新客户开通 | 客户档案选择客户类型并打开“开通客户门户/工作台” → 选择客户门户能力模板 → 创建外部用户、设置密码并启用登录 → 保存配置 → 顶部客户账户或客户履约运营台选择客户 | PR-379-CHANNEL-PORTAL-WORKBENCH-SWITCH：只有 active、已开通门户/工作台、模板暴露工作台或下单能力的客户才进入履约候选；客户侧登录后只看到模板允许的入口 |
...
## 新用户上手清单
1. 先用一个测试客户完成开通：客户档案选择客户类型（可用渠道客户验证）、打开“开通客户门户/工作台”、选择客户门户能力模板、维护外部用户并启用登录。
...
2. 如客户需要独立库存仓，进入 库存管理 → 仓库库存，选择仓库并绑定客户。门户客户配置只展示绑定后的客户仓库，不再维护代加工仓库字段。
...
5. 进入客户履约账户，在“选择客户”中按客户名、公司、联系人或电话搜索客户，并点击载入账户；下拉框只展示 active、已开通门户/工作台、客户门户能力模板暴露 ERP 工作台或下单能力的客户，不再按 `customer_type=wholesale` 硬编码。客户账户模式已选择客户时，本页会自动载入该客户。
...
- PR-379-CHANNEL-PORTAL-WORKBENCH-SWITCH：客户下拉框只出现 active、已打开“开通客户门户/工作台”、客户门户能力模板暴露 ERP 工作台或下单能力的客户；未开通、模板不暴露工作台/下单能力、登录已停用或仅剩 inactive 绑定的客户不出现。渠道客户满足这些条件后可作为履约客户。
...
## 常见问题
- 页面没有客户数据：确认客户档案是否存在、客户是否启用、是否打开“开通客户门户/工作台”、是否已经在下拉框选择客户并点击载入账户；如果客户有历史 ERP 绑定但仍不出现，检查绑定账号是否仍启用登录、客户门户能力模板是否暴露 ERP 工作台或下单能力且模板键能被系统识别。
