...
> PR-600 当前口径：新建商品和已完成 cutover 的商品在商城、现货下单、代发、代加工和客户库存中统一使用父商品 + 当前默认已发布 BOM 规格，数量按规格库存单位 1:1。旧子 SKU 与克重换算只用于 legacy 商品和历史履约快照。
...
1. 在“产品价格表 → 价格表配置”为可用于代发的命名价格表勾选“适用于一件代发”，完成发布。该标记、价格表稳定标识、版本和归属会进入发布快照及操作日志。
2. 在“客户管理 → 门户客户配置”打开客户，按商品类型选择该客户的一件代发指定价格表并保存。小程序只能读取 ERP 指定表，不能自行换表；没有配置、未发布、失效或同一商品类型选择多表时会明确报错。
...
- **个人中心**：查看当前客户、常用收件人、客户联系人、业务联系人、服务说明和账号操作。联系人信息来自 ERP 当前客户档案及负责人。
...
- **现货录单 / 代发录单**：使用与员工录单相同的商品、规格、命名价格表选择及阶梯计价表单。商品与有效规格必须在客户有权使用的已发布价格表内，公共商品继续遵循客户现有配置。客户不能修改成交单价、优惠、付款或发货状态。
