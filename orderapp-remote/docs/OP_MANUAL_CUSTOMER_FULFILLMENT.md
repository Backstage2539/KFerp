...
| 客户自有销售豆单 | 工厂先发布供货豆单 → 客户小程序“我的豆单”复制供货快照 → 选择授权阶梯模板和商品 → 加价并发布销售豆单 → 下载 PDF/长图分享 | 客户销售豆单只给客户的下游客户看；不影响客户向工厂下单、ERP 录单默认价格、结算、费用中心或履约计价 |
...
4. 在 产品价格表 把“价格表归属”切到该客户，确认对应商品类型的价格表已发布。商品类型来自客户商品当前归属的分类模板名称；未归类统一进入“其他”。报价配置优先读取客户商品选择的商品配置模板，其次是绑定商品档案的商品配置模板，再其次是客户商品所属分类项或分类模板引用的商品配置模板。客户商品如果没有有效价格，客户侧下单和内部履约提交会返回价格未发布错误，不会按 0 元静默下单。
...
- 客户说“销售豆单价格”和“下单价格”不一致：先确认她查看的是客户自有销售豆单还是工厂供货豆单。客户自有销售豆单是 `customer_resale` 分享快照，只给她的下游客户看；客户向工厂下单仍按 `factory_supply` 价格表/豆单版本计价。
- 客户发布销售豆单失败：先检查当前客户是否有 `bean_list` 能力、来源是否是该客户可见的工厂供货豆单、阶梯价模板是否已授权客户转售使用，以及来源价格单位是否能匹配所选模板。
