...
- 客户侧网页版工作台的历史履约订单指标继续展示当前客户订单应收；代加工客户小程序费用中心不复用该订单指标，只展示 ERP 已确认推送的工单费用账单。
...
## 小程序代发、生产工单、库存与账单闭环（PR-581）

```mermaid
flowchart LR
  A["客户粘贴地址并选择客户仓 SKU"] --> B["FIFO 预览与库存预留"]
  B --> C["按仓拆 ERP 履约子单"]
  D["客户选择多种目标 SKU"] --> E["有效 BOM 汇总客户和工厂库存/WIP"]
  E --> F["生产申请进入待排产"]
  F --> G["真实工单与工序卡执行"]
  G --> H["完工进入客户成品仓"]
  G --> I["按已发布费用模板生成账单"]
...
- 客户发货申请使用幂等键；一次请求只产生一个申请。系统在同一事务建立子订单、批次分配、未发货预留和操作日志；任一步失败均不留下部分订单或库存占用。取消未发货申请会释放全部预留，已发货批次不重复释放。
- 可发商品只统计当前客户绑定成品仓，冻结批次、其他客户仓及已有预留不可用。跨仓时客户端仍显示一个申请，详情按包裹展示各仓子单和物流。
- 生产申请支持多目标 SKU，同 SKU/规格重复行合并。提交前服务端重新锁定并汇总整张申请的物料；客户自有库存/WIP优先，工厂库存/WIP其次，其他客户库存永不参与。不足时零写入。
- 客户提交的是待排产需求，ERP 仍通过生产计划下达真实工单、工序卡和完工入库。状态从真实计划、工单和工序卡派生；不同客户、目标仓或申请明细不得合并到同一生产需求。
- 完工成品固定进入申请冻结的客户成品仓。客户库存详情只暴露商品、规格、批次、生产日期、仓库、入库时间、数量和质量，不暴露成本、供应商或操作人。
- 代加工账单只能选择已完工且未计费的真实工单和已发布费用模板。客户自有物料消耗不收费，只有工厂提供物料的实际消耗成本进入物料费；已确认/已付款账单不重算，修正走冲销或调整行。

## 发货中心查询与库存批量生产（PR-582）

- 入口与角色：代加工客户登录小程序后，从底部进入“发货中心”，或从首页进入“我的库存”。需要客户已开通相应履约、库存和生产能力，并已绑定成品仓。
...
- “我的库存”顶部可按商品名称搜索；勾选多个商品规格后点击“生成生产工单（N）”，系统只打开一张生产工单申请并预填全部所选规格，数量仍由客户逐项填写和进行 BOM 试算。
