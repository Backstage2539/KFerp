...
4. 如果本次要连续处理同一个客户，先在 ERP 顶部“当前视图”选择客户；系统会把该客户带入履约运营台、商品管理、产品价格表、BOM、仓库库存、订单列表、费用管理、经营报告和结账相关，避免每个页面重复选择。生产计划/开始生产不进入客户专属页面，排产和开工统一切回工厂总览处理。
...
7. 客户视图下 商品管理、BOM、产品价格表、仓库库存和费用管理都锁定顶部当前客户；不要在这些页面内另选客户。客户商品名只归当前客户；生产 BOM 和商品档案用于内部执行追溯。
