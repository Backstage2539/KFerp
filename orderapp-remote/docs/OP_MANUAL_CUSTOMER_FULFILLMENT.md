...
## 客户独立商品、供料方式与客户仓（PR-627）

1. 在客户账户下进入“商品档案”，点击“新建客户商品”创建客户独立商品；如果只是贴牌，在同一入口选择“引用公共商品”，只填写该客户的展示名称、货号等字段。引用不会复制商品 ID、BOM 或工艺路线，客户显示名只在当前客户范围内生效。
2. 在客户商品行设置“供料方式”：`工厂供料`沿用工厂原料、包材和普通生产流程，`客户来料`只允许使用该客户库存。供料方式是商品与客户引用的明确配置，不按客户类型或是否开通原料仓自动推断。
3. 在“商品价格表”选择当前客户作为归属，按有效客户引用选择商品并发布版本。专属商品和公共商品可以在同一张客户价格表中发布；客户工作台的“我的价格表”只展示当前客户已发布的工厂供货表。没有当前客户有效报价时，录单会提示未报价，不会静默采用其他客户版本。
4. 在“门户客户配置”按需点击“开通原料仓”“开通包材仓”“开通成品仓”。同一客户、同一仓库类型重复点击保持幂等；物理仓库可以相同，系统按仓库绑定客户和库存货主隔离数量，本期不维护货架或库位。
5. 客户材料或成品到仓后，在库存作业选择 `customer_receipt` 手工补录，填写客户、仓库、商品、批次和数量。补录会同时形成批次、中央余额、库存流水和操作日志；客户来料不会增加工厂公共材料余额。
6. 客户工作台的库存卡片和明细只读取“当前账号绑定客户 + 已绑定该客户的有效仓库 + 相同库存货主”。没有绑定仓库时库存为空；绑定工厂仓或其他客户仓不会扩大可见范围。工作台可在“我的价格表”查看版本、发布日期、变更说明和内容预览。
7. 客户来料不足时只显示客户缺口，即使工厂有同种材料也不会自动借用。内部人员补录客户库存后重新校验可生产量；客户来料生产的领料、耗用、退料、完工和发货都必须保持该客户货主，批发客户的工厂供料商品不要求客户原料仓。
