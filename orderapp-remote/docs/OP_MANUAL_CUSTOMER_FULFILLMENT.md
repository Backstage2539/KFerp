...
4. 进入客户履约账户，在“选择客户”中按客户名、公司、联系人或电话搜索客户，并点击载入账户；下拉框只展示已绑定启用登录的 ERP 履约账号、且能力模板暴露 ERP 工作台的批发客户。
5. 选择导入类型：代加工工单、代发清单或结算单。一个 Excel 只按一种类型导入。
6. 上传 Excel 后先解析导入，查看有效行、错误行、工单数、代发订单数和费用明细数。
7. 表头、日期、数量、金额或产品名异常时，先修改 Excel 后重新解析。
8. 导入批次列表中有错误时，点击“查看错误行”，按表名、行号、类型和错误原因修正 Excel 后重新解析。
9. 确认“当前可应用批次”和“应用预览”，再点击“应用当前类型最新批次”；解析和预览都不改变业务数据，只有应用才写入库存、工单、订单和费用。
10. 应用导入批次前确认客户已开通对应能力：代加工工单要求 `processing`，代发清单要求 `direct_ship`，结算单要求 `settlement`；未开通对应能力时先回到客户门户配置启用能力或改选正确客户/导入类型。
11. 在客户履约账户手工提交代加工工单或代发订单前，也要确认客户已开通对应能力；未开通对应能力的客户不能通过内部 API 绕过页面写入工单或代发订单。
12. 手工调整托管库存前确认客户已开通 `inventory_custody` 能力；未开通托管库存能力的客户不能写库存物料、库存台账或余额。
13. 应用后刷新账户，核对托管库存、加工工单、代发订单、费用明细和结算批次。
14. 月结时先确认该客户已开通结算能力，再填写结算开始和结束日期；未开通结算能力的客户不能生成结算批次，开通后再生成月结并核对结算编号、期间、金额和状态。没有费用可结算时不会生成 0 元空结算批次，接口会返回 `no fees for settlement period`；重复生成同一周期月结不会清零已结费用或结算批次金额，其中草稿批次会把新增未结费用并入同一批次后重算总额。已确认或已结算的月结批次不能通过重复生成同周期月结改动；若结算后发现漏费，先按内部流程撤回或重开草稿，或在后续期间做调整，不要直接覆盖已结算批次。
15. 客户通过客户门户提交代发或代加工发货订单后，可从订单列表“履约客户订单”范围查看；该范围只包含已绑定启用登录的 ERP 履约账号且能力模板暴露 ERP 工作台的批发客户订单。在客户履约账户载入该客户后，“代发订单”区域也会显示客户门户提交的代发订单。
...
- 客户下拉框只出现已绑定启用登录的 ERP 履约账号、且能力模板暴露 ERP 工作台的批发客户；零售、电商、未绑定客户、绑定账号已禁用登录的客户和历史绑定指向非工作台模板或无法识别模板的客户不出现。
...
- 客户门户新提交的代发订单，无需 Excel 导入，也能在客户履约账户“代发订单”看到。
...
- 绑定 ERP 工作台账号提示模板不可用：检查客户门户配置的能力模板；零售商城客户不需要 ERP 工作台账号，无法识别的历史模板键也不能从客户履约内部 API 绕过绑定限制。
- 绑定 ERP 工作台账号提示 `login-enabled channel customer account required`：检查所选账号是否为渠道客户账号且登录未停用；先在用户权限页启用账号或改选其他已启用登录的渠道客户账号。
