...
2. 在 客户商品名 选择该客户，确认客户商品名是否已绑定正确商品档案；贴牌只改客户商品名，配方、包装、库存对象或生产方式变化才回到 商品档案 创建新商品档案并维护商品生产配置。客户需要一次性上架多款现有商品时，使用“批量添加商品档案”，系统会跳过同客户已存在的绑定并记录创建日志；客户商品名单个新增和批量新增都不选择分类模板，正式分组在客户商品名页启用分类模板 Tab 后勾选客户商品并移动分类。
