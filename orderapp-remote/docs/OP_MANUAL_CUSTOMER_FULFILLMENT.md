...
- 小程序页面底部品牌标识平时看不到：这是 `PR-591-MINI-PULL-BRAND-SELF-LOGIN-GUARD` 的预期交互。先滚动到实际内容末尾，再继续向上拉，才会出现 `Drived By` 与透明银灰棵凡四字字标；标识只读，不写客户、订单或操作日志。若完全不显示，核对微信小程序包是否已更新，不能只依据 ERP 服务器部署状态判断。
