...
- 正常打开、浏览或停留在页面内容底部时，品牌标识平时不可见；滚到页面底部后继续向上拉，才会显示 `Drived By` 与棵凡四字字标。
...
- 页面底部看不到 `Drived By`：这是正常状态；滚到内容末尾后继续向上拉才会出现。若完全不显示，先确认使用的是包含 PR-591 的小程序包，不能只依据 ERP 服务器部署状态判断微信版本。
