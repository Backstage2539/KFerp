...
3. 如果客户需要外部用户登录，直接在客户门户配置行的“外部用户”区域创建账号、设置密码并启用登录。ERP 内部顶部“客户账户”的当前客户下拉只要求客户已启用并打开“开通客户门户/工作台”，不要求先创建外部账号、密码或能力模板；没有外部账号/密码时客户只是不能登录小程序或客户外部工作台。创建、重置密码或启停登录后系统会自动刷新顶部客户列表，不需要整页刷新。一个客户只能保留一个启用绑定，同一外部用户不能 active 绑定第二个客户。
...
- 账号必须满足：员工记录启用、账号类型为渠道客户、登录未禁用、密码正确、存在 active 客户门户 ERP 绑定、绑定客户未停用、客户档案“开通客户门户/工作台”仍开启且客户门户能力模板有效。创建、重置密码和启停登录都在客户门户配置页完成。
...
- 后台配置中的 `miniapp_entry_mode` 仍保留给客户门户能力模板和兼容逻辑使用；客户侧主导航统一先进入首页，再从底部入口进入商城、订单中心、费用中心或个人中心。
...
- 费用中心默认展示当前客户全部未失效订单账单和待结算汇总，可用账期筛选切换本周、本月、本年或自定义开始/结束日期；顶部汇总展示应收总额、待结算金额、未付款订单数和已付款金额。
...
- 如果客户门户能力模板保存或 ERP 工作台绑定提示 `ERP workbench unavailable for capability template`，说明该模板不允许 ERP 工作台字段或账号绑定，或历史模板键无法识别；ERP 工作台字段会被拒绝，零售商城客户不绑定 ERP 工作台账号，客户门户配置页和客户履约内部 API 都不能绕过。
...
- 如果订单金额异常，熟豆先检查客户门户能力模板的小批量价格规则和商品阶梯价；生豆先检查已发布生豆豆单、商品配置、生产 BOM 成本快照和梯度模板，不要让客户通过小程序手动改价。
...
## 客户自有销售豆单
- 适用场景：客户希望把工厂供货豆单改成自己的品牌销售豆单，分享给自己的下游客户。该豆单只用于分享，不参与客户向工厂下单、ERP 录单、结算、费用中心或履约计价。
- 前置条件：客户门户能力模板启用“我的豆单”；工厂已经发布当前客户可见的工厂供货豆单；需要客户选择的阶梯价模板已在 ERP 商品配置中打开“允许客户转售豆单使用”。
- 客户进入小程序首页 → 我的豆单，页面会显示工厂供货豆单和“我的销售豆单”列表。首次制作时，系统默认打开最新工厂供货豆单作为来源。
- 在“我的销售豆单”编辑区选择来源供货豆单和授权阶梯价模板；版本号首版默认 `V1`，后续按当前客户自己的销售豆单版本递增，来源供货版本会单独保存用于追溯。
- 客户可勾选要展示的商品，填写品牌名、豆单说明、版本说明、统一加价、倍率加价、背景色、文字色、背景图、Logo、卡片/表格样式和每行卡片数。
- 单品可设置“上新”“推荐”标签，也可填写覆盖档位和单品价。覆盖价只影响本次客户销售豆单展示快照，不回写工厂供货豆单、阶梯价模板、商品档案或订单价格。
- 点击“保存草稿”只保存客户自己的草稿；点击“发布销售豆单”后，系统写入 `customer_resale` 发布快照、操作日志，并刷新“我的销售豆单”列表。
- 发布后点击“PDF”或“长图”生成分享文件。PDF 和长图都按版本和样式缓存；同一版本再次打开复用缓存，样式或内容变更后发布新版本再重新生成。
...
- 公共 SKU 小批量客户从小程序提交非 454g 规格现货/代发熟豆订单且未手填单价时，系统会按客户门户能力模板中的小批量规则匹配豆单重量档；低于 14lb 的订单按 15-28lb 档折算到当前规格，避免退回产品默认价。生豆订单按已发布生豆豆单价格档匹配。
...
- 生豆履约订单单价异常：检查是否已发布生豆豆单、商品是否出现在 `green_bean_sale_tiers` 中、商品配置、生产 BOM 成本快照和梯度模板是否正确。
...
- 账单页金额或未付款数量不符合预期：先确认是否已设置账期筛选；未设置账期时默认汇总当前客户全部未失效订单，再核对订单付款状态和收款方式；账单页按订单日期统计，不按实际到账时间统计。
