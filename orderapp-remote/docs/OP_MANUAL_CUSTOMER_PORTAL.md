...
## 入口
- 客户管理：客户档案、客户门户配置、客户门户能力模板、客户门户手册。
- 商品与配方：商城管理。
...
## 标准操作
1. 先在客户档案维护客户基础资料、客户类型和订单类型。客户类型支持自定义，渠道客户只是分类口径，不自动代表任何门户能力。
2. 需要给客户开通门户或工作台时，在客户档案打开“开通客户门户/工作台”开关并保存。未开通的客户不会出现在客户门户配置默认列表，也不会进入履约客户候选。
3. 进入客户门户配置，找到已开通的目标客户。
...
4. 按客户选择客户门户能力模板、主题和小程序首页；零售商城客户使用“零售商城客户”模板，不绑定 ERP 工作台账号。
5. 客户门户配置不维护豆单展示版本，也不维护代加工仓库。豆单按客户专属最新发布和公共兜底读取；客户仓库到“库存管理 / 仓库库存 / 仓库设置”里绑定客户。
6. 点击保存配置。
...
## 豆单版本展示与更新确认
- 客户门户配置页不再提供“豆单展示版本”开关。客户侧优先展示当前客户最新可用豆单；没有客户专属豆单时使用公共豆单。
- 如果业务需要冻结报价，应通过豆单发布和报价有效期流程处理，不在门户配置中为单个客户固定展示版本。
