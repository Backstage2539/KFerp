...
- 已迁移商品的商城、现货下单、代发和生产申请候选，只来自商品当前默认且已发布 BOM 的规格组。页面显示规格名称和库存单位，提交父商品与 BOM 规格身份；不再显示或接受旧派生子 SKU。
- 现货下单按规格库存单位直接录入数量和读取该规格的已发布价格，例如 2 袋就是 `2 × 元/袋`；不再把“227g袋”等规格名称反解析成克重，也不做商品销售单位换算。
