...
4. 按客户在门户配置页选择客户门户能力模板、主题和小程序首页；渠道客户常用“渠道代发/商品下单”模板，批发客户常用代加工履约或公共 SKU 小批量代发模板，零售/电商客户可使用“零售商城客户”模板但不默认开客户工作台。
5. 点击保存配置。
...
- 渠道客户是客户类型之一，适合代发、商品下单、平台渠道或经销商场景。渠道客户下单的终端收件人写入订单收件字段，不新增客户档案；常用收件信息维护在该客户共用地址簿，修改地址簿不影响历史订单快照。
...
- 首页保留按能力开放的客户服务，例如商城下单、商品下单、一件代发、生产工单和库存；订单/发货中心和费用中心不再作为首页卡片重复出现。
- 订单中心专门进入订单页，加载客户订单、商品明细、豆单版本、收件信息、订单金额、运费、生产/收款/发货状态、物流信息，并提供销售单和出库单 PDF 查看。订单页继续复用订单查询条件，可以按关键字、日期和状态筛选。
- 费用中心按账期统一展示 ERP 商品货款、加工费、代发服务费和运费，同时展示退款与调整。客户可下载 PDF/Excel、确认对账或针对具体费用项提出异议；确认对账不改变 ERP 付款状态。
...
## 商品下单商品可见范围与一件代发目录
- 两个入口都只显示 ERP 在“商品 → 商品价格表”按当前客户、商品类型和入口指定的具体发布版本内容。未指定的商品类型不显示；公共默认价格表、其他客户价格表和同表后续新版都不会自动补入，因此不能显示其他客户专属商品。
- 目录、分类、搜索、规格、数量档位和报价使用同一发布快照。手工构造表外商品、规格、价格表或客户 ID 时，服务端会拒绝且不生成订单。
- 门户客户配置仍负责开通业务能力，但不再维护“一件代发指定价格表”。价格表草稿中的旧“适用于一件代发”字段也不再决定授权。
- 客户侧只读显示报价所用表名与版本，不能切换。ERP 重新指定后，旧报价需重新确认；历史订单仍使用成交时快照。

## 小程序服务表单选择器
- 客户在小程序“一件代发”“代加工”“商品下单”服务页提交订单或加工申请时，通过商品/库存选择器选择可用项，不需要手填系统 ID。
- 一件代发和商品下单都使用指定价格表版本的商品与规格，并复用员工录单的分类、模糊搜索、多行增删、数量和单位逻辑。选择商品后在录单行内直接点击真实规格，不显示“默认规格”占位文字。生产工单不再提供投入物料选择器。
- 如果选择器为空，先检查当前客户是否有可用托管库存、是否有可见商品、商品是否启用，以及客户门户能力模板是否正确。

## 客户共用收件地址
- 客户在小程序“个人中心 → 收件地址”新增、编辑、删除或设置默认地址。同一客户绑定的多个账号读取同一份地址簿，不能查看或修改其他客户地址。
- 一件代发和现货下单主页面点击“收件客户”搜索已有地址，点击“维护”进入独立页面。解析或填写新信息后，“仅本单使用”只进入本单收件快照，“保存并使用”写入当前客户共用地址簿并带回本单；返回后已录商品保持不变。
- ERP 人员在客户档案的“收件地址”区域搜索和维护同一数据，可粘贴收件信息解析，也可明确将客户档案现有地址加入地址簿。历史订单不会自动导入，地址簿修改或删除也不会改写历史订单快照。
- 删除默认地址后，系统把该客户剩余地址中最近更新的一条设为默认；多人同时编辑旧版本时会提示刷新。新增、修改和删除均记录操作日志。

## 小程序履约订单后端定价
- 客户小程序“一件代发”“代加工发货”“商品下单”提交履约订单时，不允许客户手填单价或手动指定价格档位；阶梯价始终按数量自动匹配，也不信任请求体里的 `unit_price`、`shipping_amount` 或优惠字段。员工录单的跨门槛手选档只适用于员工入口。
- 一件代发和商品下单的单价、价格单位和版本来自 ERP 当前指定的具体发布快照；订单行保存快照中的最终价、价格单位、来源价格记录、库存单位和库存换算，不自动使用最新版本，也不回退公共表、商品默认价、旧商品阶梯价或客户侧提交价格。
- 两个录单入口显示当前表名和版本，并提供独立只读表格预览；预览按登录客户和 `direct_ship` / `product_order` 用途读取，不接受客户 ID、价格表 ID 或商品 ID 作为选表参数。
...
- 客户登录、首页、商城、服务、库存详情、工厂商品表、我的商品、价格表设置和个人中心等实际业务页面共用同一底部品牌标识；瞬时启动路由不展示，避免进入页面时闪现。
- 正常打开、浏览或停留在真实内容底部时，品牌标识平时不可见且不增加滚动区；到底后按住继续向上拉，才会临时显示 `Drived By` 与棵凡四字字标。松手后自动回弹隐藏，约 220ms 内收回，不会留下可滚动的空白尾页。
- 字标使用透明银灰单色和紧凑横向尺寸，不带原图背景或红色，不会悬浮遮挡商品、订单、表单、底部导航和手机安全区。
- 该标识只读，不跳转、不写客户或订单数据，也不产生操作日志。ERP 服务器已部署不代表微信正式版已更新，仍需分别确认固定包、开发者工具上传、审核和正式发布状态。
...
- 小程序一件代发、代加工和商品下单表单使用可读选择器，不能要求客户手填系统 ID。
- 小程序一件代发、代加工发货和商品下单订单价格以后端计算为准，客户侧不能手填或篡改单价。
...
- 商品下单只包含当前入口明确指定发布版本中的商品，不能补入公共表、其他客户表或其他版本商品。
...
- 费用中心只读取当前登录客户的统一账单，商品货款、加工费、代发服务费和运费按 ERP 原始来源去重，不能显示其他客户的订单、工单、费用或金额。
- 小程序登录页只显示 ERP 账号密码登录，不再展示微信一键登录；启用登录且已绑定客户门户的外部用户可登录，内部员工账号和未绑定账号不能登录。
- 小程序启动页不会停留在空白页。代加工客户底栏为“首页、发货中心、费用中心、个人中心”；发货中心查看订单、包裹和物流，费用中心查看 ERP 统一账单。其他客户继续显示普通订单中心。
...
- 商品下单看到了指定版本外商品：立即停止给客户展示，核对商品价格表页当前客户、商品类型和“商品下单价格表”指定；目录和提交都必须使用同一 publication ID 范围。
- 商品下单提交后提示无效请求：检查该类型是否已经指定、商品与规格是否在指定发布快照内，以及客户打开页面后指定是否发生变化；重新打开页面核价后再提交。
...
- 商城管理看不到某个商品：检查该商品是否为客户专属商品；商城公共目录只能上架公共商品，客户专属商品应走客户门户商品下单或客户豆单。
...
- 页面底部看不到 `Drived By`：这是正常状态；滚到内容末尾后按住继续向上拉才会出现，松手后自动回弹隐藏。若完全不显示或松手后留下空白尾页，先确认使用的是包含 PR-591 的小程序包，不能只依据 ERP 服务器部署状态判断微信版本。
...
- “费用中心”已升级为统一账单：展示商品货款、加工费、代发服务费、运费、退款与调整，并保留关联订单或工单、计费依据、数量、单价和金额。客户可下载 PDF/Excel、确认对账或针对具体费用项提出异议。
- 个人中心的“工厂商品表”“我的商品”同时要求 `product_order` 和 `bean_list`。代加工客户未开“商品下单”时入口隐藏；商品目录与分类、豆单详情和确认、PDF/PNG、转售价格表查看与编辑等客户直达接口也同时校验两项能力，缺任一项即拒绝。员工录单与员工商品接口不套用这组客户门禁。
...
## 生产申请、提前下单与库存详情（PR-668）

- 小程序“生产工单”列表点击单号进入独立详情；“新建工单”进入独立表单。商品、规格和数量变化后自动试算，多商品共用物料按整张申请合并校验。最大可生产量是当前可提交上限，不能把页面示例数量当作真实库存能力。
- 生产申请详情展示真实时间线、当前工序、申请数量、累计合格入库、订单有效预订、剩余可预订量、目标仓和关联订单。计划草稿仍显示待接单；生产工单实际生成后显示已接单；首道工序开始后显示开始生产；全部有效申请行入库满足后才显示生产完成。
- 提前下单预览按规格拆分为现货可用、工单可预订和本次可下单，并列出拟使用的现货、生产申请来源和提交后剩余额度。预览不写占用；提交成功后才显示已预订。代加工商品合计不足时整单不能提交，并提供生产申请入口。
- 库存详情按当前商品和 BOM 规格显示库存总量、订单占用、现货可用、未入库可预订、生产申请来源、成品批次、入库单、生产工单及关联代发订单。“使用现货下单”和“提交生产工单”都自动带入当前规格；每个来源单独展示。

## 连写收货信息与一件代发商品明细（PR-583）

- 适用角色与前置条件：已登录且获授“一件代发”能力的客户账号；客户已绑定成品仓，仓内至少有一个未冻结、未被其他申请预留的可用商品规格。
- 进入小程序“一件代发 → 新建发货”，把整段收货信息粘贴后点击“一键解析地址”。系统与 ERP 客户档案共用同一个解析接口，可识别没有空格的“省市区县 + 详细地址 + 姓名 + 手机号”；长自治县名称会完整保留。
- 解析完成后核对收件人、电话、省、市、区/县和详细地址。系统只在姓名可信时从详细地址末尾拆出姓名；无法可靠识别姓名时会保留完整地址并让姓名为空，避免误删地址，此时请手工补填收件人。解析动作只回填当前表单，不保存客户或发货数据。
- 商品明细打开时已经显示商品、规格、数量三个控件。点击商品框，在共享商品选择弹层中组合使用分类与搜索；可按商品名、客户显示名、别名、拼音、首字母、商品编码、SKU 或规格查找。目录仍只来自当前客户绑定成品仓的可用库存。
- 选中商品后系统自动带入默认规格；没有显式默认时带入该商品第一个可用规格。需要其他规格时直接点击当前行规格框改选，数量默认 1 并可修改。切换商品会替换旧规格但保留已填数量。
- 点击“新增商品”可增加多行；删除最后一行会恢复为空行。完全空白的附加行不会提交，但商品、规格或数量只填一部分，以及数量小于等于 0 时，必须补齐后才能预览或提交。相同 SKU/规格会按一项汇总数量。
- 常见异常：商品弹层为空时检查客户是否绑定成品仓及是否有正数可用库存；规格为空时检查商品是否仍启用且库存目录是否包含具体 SKU；提交前提示库存不足时刷新目录并重新选择，服务端会按当前冻结、预留和批次状态重新校验。

## PR-600：客户门户中的 BOM 规格

- 已迁移商品的商城、商品下单、代发和生产申请候选，只来自商品当前默认且已发布 BOM 的规格组。页面显示规格名称和库存单位，提交父商品与 BOM 规格身份；不再显示或接受旧派生子 SKU。
- 商品下单按规格库存单位直接录入数量和读取指定版本中该规格的价格，例如 2 袋就是 `2 × 元/袋`；不再把“227g袋”等规格名称反解析成克重，也不做商品销售单位换算。
- 数量直接按规格库存单位计算。例如选择“227g 袋装”规格、数量 100，即为 100 袋；配方中的 22.7kg 主料和 100 个袋材由对应规格 BOM 计算，不在客户页面做商品规格换算。
- 代加工预览与提交、代发预览与提交、商城下单及编辑回显都必须保持同一规格身份。服务端会再次确认该规格仍属于当前默认已发布 BOM；旧版本、其他 BOM 的同名规格或缺少规格的请求会被拒绝。
- 未迁移商品继续按旧规格合同工作。商品迁移不会自动执行，历史订单、价格发布、客户履约记录和操作日志不会改写。
