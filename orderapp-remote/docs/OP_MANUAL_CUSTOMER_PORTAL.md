...
- PR-381-CUSTOMER-PORTAL-TEMPLATE-BOUNDARY：客户档案不需要绑定能力模板，也不会自动推荐模板。保存客户档案只会创建或启用该客户的门户配置，并写入操作日志；能力模板在门户客户配置页选择和保存。关闭开关不会删除历史门户配置、订单、外部账号或操作日志，只停用门户/工作台可访问状态；再次打开后可回到门户客户配置恢复或重新选择模板。
