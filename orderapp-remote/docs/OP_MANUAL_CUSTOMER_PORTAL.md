...
11. 客户登录小程序后，如果配置为商城首页，会直接进入商城；重新登录或切换当前客户后，小程序也必须按当前客户的小程序首页模式进入服务首页或商城首页。商城页顶部保留“我的订单”，用于查看商城订单记录。
12. 如果同一小程序用户绑定多个客户，可在小程序首页、商城页或服务页顶部点击当前客户切换；切换后当前页面的临时表单或购物车会清空，并自动进入目标客户配置的服务首页或商城首页。
13. 如果需要清除本机小程序会话，可点击“退出登录”后重新登录；同一微信身份重新登录仍会回到该微信 openid 已绑定的客户。
...
- 零售商城客户具备商城能力但仍进入订单处理页：检查 `/api/mini/login`、`/api/mini/me` 或 `/api/mini/services/orders` 返回的 `miniapp_entry_mode` 是否为 `mall`；若仍为 `services`，需要修复客户门户 profile 或重新登录刷新会话。商城页进入“我的订单”等服务页时也必须保持 `mall`。
...
- 登录后总是进入 `13800138075` 或看不到切换入口：先核对 `/api/mini/me` 返回的 `bindings` 是否只有一个客户；只有同一小程序用户绑定了多个可用客户时才显示切换入口。若只是本机缓存旧 token，点“退出登录”后重新登录；若同一微信仍回到旧客户，需要在后台给该微信 openid 绑定正确客户或使用正确微信身份登录。
