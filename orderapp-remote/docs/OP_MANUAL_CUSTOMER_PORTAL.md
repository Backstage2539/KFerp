...
## 标准操作
1. PR-379-CHANNEL-PORTAL-WORKBENCH-SWITCH：先在客户档案确认客户类型，并按需打开“开通客户门户/工作台”。开关关闭时客户只作为 ERP 客户存在，不进入客户门户配置默认列表、履约客户候选或客户工作台入口；开关打开后必须选择能力模板。
2. 客户门户配置属于工厂内部设置。进入工厂总览后打开客户门户配置，默认只显示已开通门户/工作台的客户；需要给未开通客户配置时，先点“去客户档案开通”回到客户档案打开开关并选择模板。
...
4. 按客户选择能力模板、主题和小程序首页；渠道客户默认推荐“渠道代发/现货下单”模板，批发客户默认推荐代加工履约或公共 SKU 小批量代发模板，零售/电商客户可使用“零售商城客户”模板但不默认开客户工作台。
5. 点击保存配置。
...
## 客户档案开关和能力模板
- PR-379-CHANNEL-PORTAL-WORKBENCH-SWITCH：客户类型只描述业务身份，不能直接决定门户权限。客户是否出现在客户门户配置、客户账户下拉、履约客户候选和客户工作台入口，由“开通客户门户/工作台”开关和能力模板共同决定。
- 打开开关时必须选择能力模板；保存客户档案会创建或启用该客户的门户配置，并写入操作日志。关闭开关不会删除历史门户配置、订单、外部账号或操作日志，只停用门户/工作台可访问状态；再次打开时可恢复并重新选择模板。
...
- 如果在客户档案修改客户类型，系统不会再按类型自动改写能力模板或删除历史绑定。需要变更客户能力时，明确调整“开通客户门户/工作台”开关和能力模板，并在操作日志中核对变更记录。
...
- PR-379-CHANNEL-PORTAL-WORKBENCH-SWITCH：客户门户配置列表默认只出现已打开“开通客户门户/工作台”的客户；零售、电商、批发、渠道客户都必须先在客户档案打开开关并绑定有效能力模板。
...
- 关闭“开通客户门户/工作台”后，旧 ERP 工作台绑定不删除但不可再访问；重新打开并选择暴露 ERP 工作台或下单能力的模板后，客户才能恢复进入工作台。
...
- 客户门户配置找不到客户：检查客户档案是否启用、是否打开“开通客户门户/工作台”、是否保存了有效能力模板；未开通客户不出现在默认列表，需要从客户档案开通。
...
- 关闭客户门户/工作台后客户仍看到履约工作台：刷新客户上下文并检查客户档案 `portal_enabled`、`customer_portal_profiles.enabled` 和当前能力模板；开关关闭时即使历史绑定仍为 active，也不能继续进入工作台。
