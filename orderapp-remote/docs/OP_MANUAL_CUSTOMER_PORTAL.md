...
## 入口
- 工厂总览：客户门户配置、客户能力模板设置、客户门户手册。
- 客户账户模式不展示客户门户配置、能力模板、生产计划/开始生产和工作台模式手册；它只处理当前客户履约、订单、库存、SKU、豆单、BOM 和费用。
- 商品与配方：商城管理。
...
## 标准操作
1. PR-379-CHANNEL-PORTAL-WORKBENCH-SWITCH：先在客户档案确认客户类型，并按需打开“开通客户门户/工作台”。开关关闭时客户只作为 ERP 客户存在，不进入客户门户配置默认列表、履约客户候选或客户工作台入口；开关打开后只创建或启用门户配置，不在客户档案绑定能力模板。
2. PR-381-CUSTOMER-PORTAL-TEMPLATE-BOUNDARY：客户门户配置属于工厂内部设置。进入工厂总览后打开客户门户配置，默认只显示已开通门户/工作台的客户；需要给未开通客户配置时，先点“去客户档案开通”回到客户档案打开开关，再回到门户客户配置选择能力模板。
3. 如果客户需要外部用户登录，直接在客户门户配置行的“外部用户”区域创建账号、设置密码并启用登录。外部用户配置了手机号、密码并启用登录，且客户档案仍保持门户/工作台开关开启、模板暴露对应工作台或下单能力后，该客户会出现在 ERP 顶部“客户账户”的当前客户下拉和客户履约运营台选择器；创建、重置密码或启停登录后系统会自动刷新顶部客户列表，不需要整页刷新。一个客户只能保留一个启用绑定，同一外部用户不能 active 绑定第二个客户。
4. 按客户在门户配置页选择能力模板、主题和小程序首页；渠道客户常用“渠道代发/现货下单”模板，批发客户常用代加工履约或公共 SKU 小批量代发模板，零售/电商客户可使用“零售商城客户”模板但不默认开客户工作台。
5. 点击保存配置。
...
14. 客户登录后底部固定显示四个大入口：底部四个入口分别为“首页、订单中心、费用中心、个人中心”。首页保留原服务入口，但不再重复展示订单和费用入口；订单中心进入订单专页；费用中心进入结算专页；个人中心进入账号和个人信息页。
...
- 账号必须满足：员工记录启用、账号类型为渠道客户、登录未禁用、密码正确、存在 active 客户门户 ERP 绑定、绑定客户未停用、客户档案“开通客户门户/工作台”仍开启且能力模板有效。创建、重置密码和启停登录都在客户门户配置页完成。
...
## 客户档案开关和能力模板
- PR-379-CHANNEL-PORTAL-WORKBENCH-SWITCH：客户类型只描述业务身份，不能直接决定门户权限。客户是否出现在客户门户配置，由客户档案“开通客户门户/工作台”开关决定；客户账户下拉、履约客户候选和客户工作台入口，还必须满足门户客户配置中绑定的能力模板。
- PR-381-CUSTOMER-PORTAL-TEMPLATE-BOUNDARY：客户档案不需要绑定能力模板，也不会自动推荐模板。保存客户档案只会创建或启用该客户的门户配置，并写入操作日志；能力模板在门户客户配置页选择和保存。门户客户配置保存能力模板、门户启停、显示名、默认寄件人、代加工仓库和豆单展示版本时，都会写入操作日志，便于按客户追溯。关闭开关不会删除历史门户配置、订单、外部账号或操作日志，只停用门户/工作台可访问状态；再次打开后可回到门户客户配置恢复或重新选择模板。
...
- 后台配置中的 `miniapp_entry_mode` 仍保留给能力模板和兼容逻辑使用；客户侧主导航统一先进入首页，再从底部入口进入商城、订单中心、费用中心或个人中心。
- 首页保留原客户服务内容，例如商城下单、我的豆单、现货下单、一件代发、代加工和库存等能力入口；订单中心和费用中心不再作为首页卡片重复出现。
- 订单中心专门进入订单页，加载客户订单、商品明细、豆单版本、收件信息、订单金额、运费、生产/收款/发货状态、物流信息，并提供销售单和出库单 PDF 查看。订单页继续复用订单查询条件，可以按关键字、日期和状态筛选。
- 费用中心专门进入结算页，加载当前客户自己的账目汇总、订单账单、费用明细和结算单；即使还没有生成费用明细或结算单，只要当前客户已有订单，也应先看到订单应收。
- 费用中心默认按订单日期展示本月账期，可用账期筛选切换本周、本月、本年或自定义开始/结束日期；顶部汇总展示应收总额、未付款金额、未付款订单数和已付款金额。
- 费用中心列表只展示订单号、订单日期、应收金额、付款状态和收款方式。需要查看收件人、商品明细、物流、销售单或出库单时，点击订单号跳转订单页继续查看。
...
- 如果在客户档案修改客户类型，系统不会再按类型自动改写能力模板或删除历史绑定。需要变更客户能力时，先确认“开通客户门户/工作台”开关，再到门户客户配置调整能力模板，并在操作日志中核对变更记录。
...
- PR-379-CHANNEL-PORTAL-WORKBENCH-SWITCH：客户门户配置列表默认只出现已打开“开通客户门户/工作台”的客户；零售、电商、批发、渠道客户都必须先在客户档案打开开关，再在门户客户配置中绑定有效能力模板。
...
- 客户门户配置的能力模板摘要中，如果模板暴露客户侧 ERP 页面，按钮显示“打开客户档案”；点击后进入“客户档案”并打开当前客户抽屉，用于查看或补齐客户资料。
...
- 小程序启动页不会停留在空白页；底部四个入口固定为“首页、订单中心、费用中心、个人中心”。首页不重复显示订单/费用入口，订单中心能看到订单明细、销售单、出库单和物流信息，费用中心显示当前客户应收总额、未付款金额、未付款订单数、已付款金额、订单账单、费用明细和结算单，个人中心可退出登录和切换用户。
...
- 客户门户配置找不到客户：检查客户档案是否启用、是否打开“开通客户门户/工作台”；未开通客户不出现在默认列表，需要从客户档案开通。客户已出现但不能进入工作台时，再检查门户配置里是否保存了有效能力模板。
...
- 关闭客户门户/工作台后客户仍看到履约工作台：刷新客户上下文并检查客户档案 `portal_enabled`、`customer_portal_profiles.enabled` 和门户配置中的当前能力模板；开关关闭时即使历史绑定仍为 active，也不能继续进入工作台。
