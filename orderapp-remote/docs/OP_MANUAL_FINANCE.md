...
10. 在经营报告查看老板简报、利润明细、税费明细和来源明细，并导出 PDF、Excel 或会计交接/代账交接 Excel。订单收入来源明细会显示订单收款方式和收款凭证入口，交接 Excel 的 Drilldown 工作表也带出收款方式字段。
...
- 已付款、已收款或已支付订单的收款方式会同步进入财务来源明细；如订单上传了收款凭证，来源明细可直接打开凭证核对；经营报告“来源明细”和会计交接/代账交接 Excel 可按收款方式字段核对微信、支付宝、银行转账、现金等实际收款渠道。
...
- 来源明细缺少收款凭证：回到录单或订单编辑，确认订单已上传凭证且保存成功；已上传的凭证会随订单收入来源明细展示。
...
- 财务人员从“财务管理 → 代加工账单”进入账单处理；该入口按 `finance.read` 独立授权。没有 `settings.write` 时仍可处理账单，但页面不展示模板维护区；只有模板维护权限而没有财务权限时，页面只展示模板维护区。
- 继续使用“代加工模板”作为模板身份。模板发布后生成不可变版本和规则；费用类型可选烘焙、人工、物料、包装、代加工或仓储，名称可自定义；规则可按实际投入 kg、实际产出 kg、实际工序分钟、实际件数、每工单固定费或工厂提供物料实际成本计费。
- 账单操作顺序为“选择客户 → 选择已完工且未计费真实工单 → 选择已发布模板 → 预览 → 确认并推送”。客户和已发布计费模板从财务只读目录加载，预览要求 `finance.read`；确认、冲销、确认收款和付款要求 `finance.write`；模板发布仍要求 `settings.write`。
- 实际投入按每次完工记录汇总；同一次完工有多个产出规格时只计算一次投入。没有完工记录的历史工单依次兼容读取工序卡实际投入和工单当时保留的投入量。
- 烘焙、人工、包装取真实工单/工序卡实际值。物料费只统计来源归属为工厂的实际消耗；来源为当前客户库存或 WIP 的物料不重复收费。BOM 成品组件按实际消耗批次的成品库存单位成本计费；历史工单若只有工厂预留量而缺少实际消耗/退回数据，系统会阻止按实际成本预览并提示补齐数据，不会把预留量猜成实耗。
- 确认后把模板版本、工单、计费依据、数量、单价和金额冻结为账单行快照，并继续落入客户费用明细和结算批次。相同工单和规则幂等；已确认或已付款账单不可重算。登记付款只推进账单状态；冲销生成金额相反的关联冲销单；补收或退费生成关联调整单，原账单金额和计算快照始终不改。上述操作均写入操作日志。
- 客户小程序费用中心按当前客户和账期汇总商品货款、加工费、代发服务费和运费，支持 PDF/Excel 下载、确认对账和费用项异议；付款、退款与调整状态仍以 ERP 为准。
