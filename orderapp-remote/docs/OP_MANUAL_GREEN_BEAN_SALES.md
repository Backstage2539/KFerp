...
- 生豆价格计算模板试算为 0、提示未配置生产 BOM 或提示 published BOM 没有组件：先到 `生产管理 → 生产配置 → 生产 BOM`。完全没有 BOM 时点击新建，产出商品搜索“生豆”即可选择启用的生豆父商品或具体 Kg 等 SKU；维护真实组件并发布后再试算。已有空 published 版本时按提示检查有组件草稿。系统只用已发布组件成本，不会读取或自动发布草稿；直接销售生豆发布前还要核对商品形态、产出单位、组件比例、整体预期损耗和原料损耗，不要用名称、分类或旧成本汇总猜价。
