...
8. 到 商品价格管理 为生豆商品或客户生豆商品维护最终价格记录、价格单位和库存换算；需要阶梯报价时维护阶梯价格方案。产品价格表发布后会把最终价、价格单位、来源价格记录和库存换算固化到生豆价格档快照。
9. SKU设置的客户 SKU 列表可按形态、名称、产品类型、产品子类型过滤，确认生豆和熟豆在同一列表内区分清楚。
10. 进入 产品豆单，切换到“生豆豆单”，检查报价档、成本参考、最新通过生产质检信息和 PDF 预览；需要客户定价时，先在商品价格管理维护客户价格记录，再生成并发布客户价格表新版。页面提示“梯度按 KG，单价按元/KG”时，确认价格记录和快照中的价格单位、库存换算一致。
11. 发布或导出生豆豆单后，ERP 录单选择生豆产品；规格可选常用克数，也可选择“自定义克数”直接输入实际克数。录单会按该客户已发布商品价格表快照定价，缺少生豆价格档或库存换算时必须先回商品价格管理和产品豆单补齐并发布新版。
12. 小程序履约客户在服务页选择当前客户可见的生豆产品并提交订单；后端按已发布商品价格表快照计算单价。
...
## 常见问题
- 新增生豆失败提示需要绑定 BOM：确认是否误用了旧入口或旧 API；新版商品档案列表不应再出现生豆属性或绑定熟豆字段。历史数据需要补齐成本来源时，在生产 BOM 和商品价格管理里核对。
- 生豆产品价格表价格不对：检查生豆 SKU 或客户生豆商品是否有最终价格记录、价格单位、库存换算和已发布价格表快照，生产 BOM 成本快照或历史兼容来源是否正确，以及是否重新生成并发布产品价格表。若发布后又看到成本参考价（如 51.75），优先核对最新发布快照里的最终价、价格单位、来源价格记录和库存换算。
- 录单页展示的生豆梯度不对：确认该客户已发布最新商品价格表，并确认选择的是带“生豆”标记的 SKU；页面展示应读取已发布价格表快照，不会读取熟豆 SKU 的 `product_price_tiers`。
- 生豆豆单没有质检信息：检查对应商品、生产 BOM 或历史兼容来源是否有最新一条通过的生产质检记录。
- 录单里找不到生豆产品：检查产品是否启用、是否属于当前客户可见范围、是否保存为生豆形态。
- 历史兼容：旧手册中的“按模板 KG 档位填写销售单价”只用于解释旧生豆豆单草稿；PR-439 后正式交易价来自商品价格管理和已发布价格表快照。
