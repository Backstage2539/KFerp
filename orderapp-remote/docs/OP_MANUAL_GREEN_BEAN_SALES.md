...
8. 给生豆所属产品子类型绑定商品配置；商品配置里选择阶梯价模板、报价单位和单位换算，产品价格表会按绑定熟豆 BOM 中保存的原料成本快照生成 `green_bean_sale_tiers` 默认价。
...
- 生豆产品价格表价格不对：检查生豆 SKU 所属产品子类型是否绑定正确商品配置、商品配置里的阶梯价模板是否正确、绑定熟豆 BOM 成本快照是否正确、草稿中的档位手工价是否按模板单位填写，以及是否重新生成并发布产品价格表。若发布后又看到成本参考价（如 51.75），优先核对最新发布快照里的 `green_bean_sale_tiers.price_unit` 和 `price_per_kg`。
