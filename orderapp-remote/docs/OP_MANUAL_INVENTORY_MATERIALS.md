...
- PR-404 后页面只保留“商品 BOM列表”作为主列表。`新建生产 BOM` 放在工作区右上角；“商品 BOM列表”标题下方放“启用 / 已失效 / 全部”状态过滤和 BOM 名称、编号或商品搜索；列表上方先放“移动到分组”卡片，再放“增加分组”和分组 Tab。下方商品 BOM列表单独上下滚动，右侧编辑详情与列表主体顶部对齐。点击 BOM 名称打开抽屉编辑；操作列提供 `BOM版本`、`规格袋材映射`、`复制` 和红色 `失效`。复制来源 BOM 即使已失效也可以复制，新 BOM 默认启用。
...
- BOM 版本通过商品 BOM 列表行上的 `BOM版本` 按钮打开抽屉维护。已发布版本只读；需要调整配方或物料用量时，复制 BOM 或创建新版本草稿，编辑后发布，再把商品档案绑定到该已发布版本。页面底部不再单独摆放 BOM 版本 panel。
...
- 规格袋材映射通过商品 BOM 列表行上的 `规格袋材映射` 按钮打开抽屉维护。抽屉标题为“全局规格袋材映射”，表示该设置不是单个 BOM 专属，而是所有 BOM 共用；页面底部不再单独摆放规格袋材映射 panel。
