...
- PR-405 后页面只保留“商品 BOM列表”作为主列表。`新建生产 BOM` 放在工作区右上角；“商品 BOM列表”标题下方放“启用 / 已失效 / 全部”状态过滤和 BOM 名称、编号或商品搜索；列表上方先放“移动到分组”和“批量失效”卡片，再放“增加分组”和分组 Tab。下方商品 BOM列表单独上下滚动，右侧编辑详情与列表主体顶部对齐。点击 BOM 名称打开抽屉编辑；操作列提供 `BOM版本`、`规格袋材映射`、`复制` 和红色 `失效`。复制来源 BOM 即使已失效也可以复制，新 BOM 默认启用。
...
- 生产 BOM 页不再提供独立的“失效当前 BOM”按钮。需要失效 BOM 时，在商品 BOM列表勾选一个或多个 BOM 后点击“批量失效”，或在单行操作中点击红色“失效”；配方明细仍可查看。
