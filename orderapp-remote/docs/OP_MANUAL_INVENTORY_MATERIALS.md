...
## 标准操作
1. 在物料档案中维护物料名称、分类、库存单位、采购价与成本单价单位、默认采购价、行业字段和库存警戒线；默认采购价只作为兜底参考，不用于直接改历史批次成本。重量物料的库存数量可以使用 `g`，但采购价和批次单位成本统一按 `元/kg` 录入和展示。
...
- PR-538-MATERIAL-COST-UNIT-LOSS：`库存单位` 和 `采购价与成本单价单位` 是两个独立字段。库存单位用于库存数量、BOM 用量和生产耗用；成本单价单位只解释默认采购价、入库/批次单位成本和标准制造成本中的物料单价。重量物料即使库存单位为 `g`、`kg` 或 `lb`，成本单价单位都固定为 `kg`；件、袋、盒等非重量物料按库存单位计价。
- 新建物料时先选库存单位，页面自动计算成本单价单位；重量物料显示 `kg`，非重量物料与库存单位一致。保存后两个单位都只读，采购价标签按实际单位显示为 `采购价（元/kg）`、`采购价（元/件）` 等。若单位选错，不要直接修改已有物料，应新建正确档案并通过正式库存迁移流程处理，避免把历史 `54元/kg` 误解释成 `54元/g`。
- 历史物料升级时，系统只补齐采购价与成本单价单位：重量物料补为 `kg`，非重量物料补为当前库存单位；不会改采购价数值、批次、库存流水、BOM、工单或价格表快照。新建、编辑、批量失效等物料写操作继续在操作日志中追溯。
...
- 创建采购单时选择供应商和重量物料，填写采购克数、单价和备注；当前采购单接口按克记录数量，所以页面只允许选择重量物料，单价统一跟随采购价与成本单价单位 `元/kg`。件、袋、盒等离散物料不要在此入口建立采购单，改用原料入库按库存单位录入，避免出现“数量 g、成本元/个”的错账。
...
- 采购价与成本单价单位改不了：该单位在物料保存后与库存单位一样锁定。重量物料统一按 `kg` 计价；如果历史档案的单位选择错误，暂停继续录入成本，新建正确物料并让管理员核对迁移，不要直接改库。
- 库存是克但采购价显示元/kg：这是正确口径。库存单位管数量，采购价与成本单价单位管单价；采购价、批次成本和 BOM 成本试算都按该单位解释，不应把 `54元/kg` 显示为 `54元/g`。
