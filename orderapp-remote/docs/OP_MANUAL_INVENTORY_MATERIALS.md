...
- 商品档案归组使用 `product_catalog`，列表按业务分组路径（父组 / 子组）分段展示；勾选商品后通过“目标分组”移动到已有分组项。普通页面不显示迁移兼容用的“商品默认分组”等系统分组集名称；商品保存和复制不再写旧 `product_category_id`、`classification_template_id` 或旧分类模板 assignment。
