...
- 物料档案归类使用 `material_catalog`。点击 `设置分组模板` 多选本功能使用的模板后，列表会以模板为外层分组，同时展示全部已选模板的大类、小类和统一 `未分类`；模板、大类和小类都可收起。`分类与移动模板` 只选择本次移动的目标模板，不会切换或过滤列表。清空选择后按物料平铺展示且不能移动归类；取消选择不删除物料、模板、分类或既有归类。
