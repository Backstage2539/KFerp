...
- 商品档案归组使用 `product_catalog`，列表按用户维护的业务分组路径（父组 / 子组）分段展示；勾选商品后通过“目标分组”移动到已有分组项。普通页面不显示迁移兼容用的“商品默认分组”等系统分组集名称，也不把系统默认迁移分组里的旧分组项作为分组管理内容或“目标分组”候选；系统默认迁移归组在普通商品列表按未分组处理。商品保存和复制不再写旧 `product_category_id`、`classification_template_id` 或旧分类模板 assignment。
