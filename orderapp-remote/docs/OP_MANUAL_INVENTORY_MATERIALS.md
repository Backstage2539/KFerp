...
- PR-395-PRODUCT-PRICE-CLASSIFICATION：分类模板是商品档案列表和客户商品名列表的页面级分类视图，不是对象字段。商品档案页固定有“全部商品”Tab；点击“使用分类模板”启用一个模板后，会新增一个以该模板命名的商品 Tab。模板 Tab 只展示已归入该模板分类项的商品；未归类商品在“全部商品”Tab 通过“当前归类=未分类”查看。勾选商品后，在当前模板 Tab 下选择分类项并点击“移动到分类”。同一商品档案只能有一条有效归类；如果已经归类，需要先移出当前分类，才能归入其他分类模板或分类项。
