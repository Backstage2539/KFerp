...
- 操作列提供 `BOM版本`、`规格袋材映射`、`复制` 和红色 `失效`。复制来源 BOM 即使已失效也可以复制，新 BOM 默认启用。配方明细编辑仍遵守版本规则：已发布版本只读；需要调整物料时先复制为新版草稿，编辑草稿后发布。
...
- BOM 版本通过生产 BOM 列表行上的 `BOM版本` 按钮打开抽屉维护。已发布版本只读；需要调整配方或物料用量时，复制 BOM 或创建新版本草稿，编辑后发布，再把商品档案绑定到该已发布版本。页面底部不再单独摆放 BOM 版本 panel。
- PR-402 / PR-406：BOM 分组不再有默认分组。生产 BOM 列表固定有“全部分组”和“未分类”Tab，新增分组后会多一个分组 Tab。一个 BOM 只能属于一个分组 Tab；全部分组显示所有 BOM，未分类显示 `group_id=0` 的 BOM。管理分组入口用于新增、改名、删除和排序分组；勾选 BOM 后在列表工具区选择目标分组并移动，移动会直接覆盖旧分组；删除分组时，该组 BOM 回到未分类，不删除 BOM、不改变商品绑定。
...
- 规格袋材映射通过生产 BOM 列表行上的 `规格袋材映射` 按钮打开抽屉维护。抽屉标题为“全局规格袋材映射”，表示该设置不是单个 BOM 专属，而是所有 BOM 共用；页面底部不再单独摆放规格袋材映射 panel。
