...
5. 在 BOM 配方维护中设置商品所需物料、用量和启用版本。
   - PR-508-BOM-MATERIAL-LOSS-RATIO：BOM 中 `物料 + 比例 %` 行如果配置了 `原料损耗比`，库存准备、WIP 占用和生产消耗都会按损耗后的原料需求执行。例如产出基准 `1kg`、原料比例 `40%`、原料损耗 `20%` 时，需要准备 `0.5kg` 原料，而不是 `0.4kg`。固定用量、商品组件和非比例物料不使用该行级损耗。
