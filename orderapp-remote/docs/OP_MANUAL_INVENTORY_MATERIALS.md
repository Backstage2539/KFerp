...
- PR-382-SKU-UNIFIED-CREATE-COPY：“SKU复制”抽屉的交互和产品价格表生成选择器一致。顶部显示“选择分类和产品 X/Y 款”，有总复选框和“全选/清空”；产品类型、产品子类型、SKU 行都可勾选。复制时同名同分类覆盖目标 SKU 资料并保留目标 SKU ID；不存在则新增。BOM 里的成品组件、生豆绑定熟豆会重写为目标客户 SKU；若依赖 SKU 没有同批复制且目标客户没有同名同分类 SKU，复制会失败并提示先补齐依赖。复制结果提示新增 X 款、覆盖 Y 款、跳过 Z 款。停用 SKU 显示为已停用且不可选。
