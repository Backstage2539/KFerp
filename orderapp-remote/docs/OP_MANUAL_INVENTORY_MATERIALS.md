...
- 客户需要复用公共配置后再修改时，点击公共分类的“复制为客户分类”、公共 SKU 的“复制为客户SKU”，或把客户 SKU/公共 SKU 拖到公共分类中；系统会先派生客户自己的分类或 SKU，再允许改名、排序、维护 BOM 和绑定模板，不会直接改公共数据。
...
- 在商品分类树的二级分类行选择模板，保存后该分类下产品的成本试算和商用豆单预览会自动按模板生成梯度价格；客户分类选择公共梯度模板时，系统会先复制为客户模板再绑定，后续客户模板可单独改档位和利润率。
