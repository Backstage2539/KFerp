...
- 生产 BOM 归组使用 `production_bom`。生产 BOM 页面不再维护自己的大组、组内分类或小分类；需要复用某套通用分组时，先在 `分组管理` 维护分组项，再回到生产 BOM 工具区选择“可用分组”并点击 `使用分组`。使用后，顶部 Tab 和 `目标分组` 下拉才展示该套分组下的父组 / 子组；未被生产 BOM 启用的分组不展示。业务标签只显示父组 / 子组路径，不显示“商品分组 /”这类分组集名称。旧 `production_bom_groups` / `production_bom_group_categories` 只用于历史只读兼容。
...
- PR-442 / PR-450 后的当前口径：生产 BOM 分组不再有默认分组，也不再有 BOM 专用大组/组内分类。生产 BOM 列表固定有“全部分组”和“未分类”Tab；用户在 `分组管理` 维护可复用分组项后，需要先在生产 BOM 工具区点击 `使用分组` 启用某套分组，再把 BOM 移动到其中的分组项。顶部筛选 Tab 只显示已经被生产 BOM 启用的分组项。一个 BOM 的归组保存到 `business_group_assignments`，不改变产出商品、BOM 版本或配方明细。
...
- 生产 BOM 页面点击 `前往分组管理` 维护通用分组项。要让生产 BOM 使用某套通用分组，先在工具区的 `可用分组` 选择分组集并点击 `使用分组`；之后再选中 BOM，在 `目标分组` 中选择具体父组 / 子组并点击左侧 `移动到分组`。目标为具体分组项时写入 `business_group_assignments`，目标为 `未分组` 时移除该 BOM 的 `production_bom` 归组。新建、编辑或复制 BOM 只维护名称、产出商品、产出数量单位和状态，不写旧 `production_boms.group_id/group_category_id`。
