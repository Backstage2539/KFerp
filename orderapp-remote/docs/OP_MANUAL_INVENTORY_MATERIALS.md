...
- `客户商品名` 只维护客户侧展示名、系统生成的客户商品编号、品牌、排序和客户侧行业字段覆盖值。单个新增和批量添加商品档案时不手工填写客户商品编号；系统自动生成并在列表展示。新建客户商品抽屉不展示“进入价格表”开关，列表仍展示该状态用于追溯和后续配置。贴牌、客户命名和客户侧分类变化优先走客户商品名，不复制 BOM，也不在客户商品层修改生产数据。
...
- 需要为某个客户准备销售展示时，优先在“客户商品名”中绑定已有商品档案，只改客户对外名称、品牌和排序；客户商品编号由系统生成，客户侧分类在客户商品名页启用的分类模板 Tab 中归类。客户想批量使用商品档案时，点击过滤行右侧“新建客户商品”打开抽屉，再切到“批量添加商品档案”模式，在商品列表顶部按名称或编号搜索、多选后创建；已存在同客户同商品档案的客户商品名会跳过。只有配方、包装、库存对象、生产方式或成本口径变化时才创建新商品档案。
...
- 点击过滤行右侧 `新建客户商品` 打开抽屉。在“单个新增”模式选择绑定商品档案并填写客户商品名、品牌名、排序、启用状态和备注；客户商品编号由系统生成，不手工填写，抽屉内不展示“进入价格表”开关。客户商品名只负责客户侧展示和销售归属，不承载库存、生产 BOM、成本或成品批次。
- 点击“保存客户商品名”后，系统写入客户商品名记录并写操作日志；后续可在同一表格中停用，不能在客户商品名层编辑生产数据或 BOM。
- 如果客户要批量使用已有商品档案，在同一个 `新建客户商品` 抽屉中切到“批量添加商品档案”模式，在商品列表顶部按商品名称或编号搜索，多选后一次创建客户商品名。默认客户商品名=商品档案名称，客户商品编号由系统自动生成，品牌名为空；批量新增不选择产品类型、产品子类型、分类模板或“进入价格表”开关。同客户同商品档案已存在时自动跳过，并在结果里显示创建和跳过数量。实际创建的客户商品名都会写操作日志。
...
- PR-406 后页面只保留“商品 BOM列表”作为主列表，不再有顶部 `SKU归属` 或独立商品选择区。“商品 BOM列表”标题下方同一过滤行放商品过滤、状态过滤、BOM 名称/编号/商品搜索和 `批量失效`；商品过滤支持按商品编号/名称模糊搜索。分类 Tab 行左侧展示“全部分组 / 未分类 / 自定义分组”，右侧放 `新建商品 BOM`。BOM 列表可单独上下滚动，右侧配方明细与列表主体对齐。点击已绑定商品的 BOM 名称会加载该商品的配方明细；点击未绑定商品的 BOM 名称也会加载右侧配方明细，商品字段显示“未绑定商品”。
- 商品 BOM列表会合并已绑定商品的 BOM 和未绑定商品的生产 BOM。未绑定商品的 BOM 在商品列显示“未绑定商品”，仍可勾选、复制、失效、移动分组和查看配方明细；操作列提供 `BOM版本`、`规格袋材映射`、`复制` 和红色 `失效`。复制来源 BOM 即使已失效也可以复制，新 BOM 默认启用。
- 商品档案还没有生产 BOM 时，列表会显示“无生产 BOM / 未维护”。这不是不可编辑的终态，操作列会显示 `创建BOM`；点击后系统自动新建一条生产 BOM、生成 V001 已发布版本并绑定到该商品，随后即可在右侧维护配方明细。
...
- PR-402 / PR-406：BOM 分组不再有默认分组。商品 BOM列表固定有“全部分组”和“未分类”Tab，新增分组后会多一个分组 Tab。一个 BOM 只能属于一个分组 Tab；全部分组显示所有 BOM，未分类显示 `group_id=0` 的 BOM。管理分组入口用于新增、改名、删除和排序分组；勾选 BOM 后在列表工具区选择目标分组并移动，移动会直接覆盖旧分组；删除分组时，该组 BOM 回到未分类，不删除 BOM、不改变商品绑定。
