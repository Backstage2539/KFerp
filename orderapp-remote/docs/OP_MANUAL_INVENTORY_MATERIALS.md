...
- 商品档案归类使用 `product_catalog`。PR-453 的历史单模板选择入口已由 PR-584 更新：商品档案自动合并展示所有明确引用 `product_catalog` 的模板大类 / 小类，空大类和空小类也显示，未归类商品进入统一 `未分类`。商品档案不再显示分类过滤 Tab，商品表格不再显示独立 `分类` 列；分类归属通过列表分组标题表达。移动目标可选 `未分类`、大类或小类；PR-584 下移动时还要先选择目标模板，移动会覆盖旧归类并写操作日志。
...
- 适用角色：维护商品档案的商品/生产人员，以及维护分组模板的管理员。操作前先在 `设置 → 行业设置 → 行业字段模板` 准备需要组合使用的字段模板，再在 `设置 → 业务设置 → 分组模板` 为业务功能明确勾选需要引用的分组模板。
...
- 商品档案会合并展示所有明确引用 `product_catalog` 的分组模板。例如同时引用“商品-咖啡豆”和“商品-挂耳”时，两份模板的大类、小类都会出现；每个商品只显示一次，未归到任一当前引用模板的商品统一进入一个 `未分类`。
- 只有在分组模板中明确启用“商品档案”功能引用，模板才会进入商品档案的分类展示和移动目标。未引用任何分组模板时按平铺列表展示，并隐藏分类标题、分类收起和移动归类入口；仅创建模板但没有勾选功能引用，不会自动把该模板带入商品档案。
- 移动商品时先选择目标分组模板，再选择目标大类、小类或 `未分类`。移动继续表示商品当前唯一归类，不会让同一商品在多份模板下重复出现；移动和清除归类仍通过正式接口写入操作日志。
- 收起大类会隐藏全部后代分类标题和商品行；系统同时保留小类自身的折叠状态、各分类分页和已勾选商品。重新展开大类后恢复原状态，不需要重新选择商品或翻页。
- 取消某个功能引用只隐藏该模板在对应业务页面的分类展示和归类入口，不删除模板、分类、商品、既有对象归类或历史快照。以后重新启用引用时，仍可恢复查看原有当前归类；若需要真正删除分类，仍按分组模板删除流程处理并先确认影响范围。
- 结果校验：刷新商品档案后，行业字段顺序与所选模板顺序一致；分组区只出现明确引用模板；无引用时为普通平铺列表；大类收起时看不到任何后代小类标题或商品行。异常时先检查模板是否启用、功能引用是否保存成功，再到操作日志核对模板引用和商品配置保存记录。
...
## 标准操作
1. 在物料档案中维护物料名称、分类、库存单位、成本计价单位、默认采购价、行业字段和库存警戒线；默认采购价只作为兜底参考，不用于直接改历史批次成本。重量物料的库存数量可以使用 `g`，但采购价和批次单位成本统一按 `元/kg` 录入和展示。
...
## 物料档案
- PR-513 / PR-528 / PR-530：物料档案分类使用 `设置 → 业务设置 → 分组模板`。物料列表按所选模板的大类、小类和 `未分类` 展示，归类继续写入通用 `business_group_assignments`；物料页不直接维护分类结构。
...
- 物料档案里的单位来自 `设置 → 业务设置 → 全局单位字典` 中启用的单位，在新业务中统一解释为库存单位。物料档案只展示一个 `库存数量（库存单位）`；库存补录只输入一个目标库存数量，单位跟随该物料库存单位。库存调整单选择物料时同样只输入一个目标数量和原因，后台兼容旧 `g/件数` 库存字段。
...
- PR-538-MATERIAL-COST-UNIT-LOSS：`库存单位` 和 `成本计价单位` 是两个独立字段。库存单位用于库存数量、BOM 用量和生产耗用；成本计价单位只解释默认采购价、入库/批次单位成本和标准制造成本中的物料单价。重量物料即使库存单位为 `g`、`kg` 或 `lb`，成本计价单位都固定为 `kg`；件、袋、盒等非重量物料按库存单位计价。
- 新建物料时先选库存单位，页面自动给出可用成本计价单位；重量物料显示 `kg`，非重量物料与库存单位一致。保存后两个单位都只读，采购价标签按实际单位显示为 `采购价（元/kg）`、`采购价（元/件）` 等。若单位选错，不要直接修改已有物料，应新建正确档案并通过正式库存迁移流程处理，避免把历史 `54元/kg` 误解释成 `54元/g`。
- 历史物料升级时，系统只补齐成本计价单位：重量物料补为 `kg`，非重量物料补为当前库存单位；不会改采购价数值、批次、库存流水、BOM、工单或价格表快照。新建、编辑、批量失效等物料写操作继续在操作日志中追溯。
...
- PR-536-PRODUCT-INDUSTRY-TEMPLATE-ONLY：当前“复制为商品档案”只复制商品主档基础资料、单位模板引用、单位覆盖和库存相关主数据；不复制商品生产配置、工艺路线、预期损耗率、商品配置模板、价格阶梯、生产 BOM、行业字段模板、行业字段值、商品价格管理记录、价格表快照或客户引用关系。复制后如需行业字段，先在新商品配置中选择模板并填写；在新商品配置中重新配置工艺路线和预期损耗率；到生产 BOM 创建或复制 BOM 并把新商品设为产出；再到商品价格管理和商品价格表配置并发布价格。
...
- PR-538-MATERIAL-COST-UNIT-LOSS：新建生产 BOM 的整体预期损耗默认是 `0`（内部 `yield_rate=1`），不会再暗含 20% 整体损耗。BOM 的 `原料损耗比` 仍是独立配置：只需要原料损耗时，把整体预期损耗保持为 0；如果历史版本或业务确实同时配置两项，系统继续连续放大并在价格试算中警告，不能静默忽略其中一项。已发布历史 BOM 不会被批量改写。
...
- 创建采购单时选择供应商和重量物料，填写采购克数、单价和备注；当前采购单接口按克记录数量，所以页面只允许选择重量物料，单价统一跟随成本计价单位 `元/kg`。件、袋、盒等离散物料不要在此入口建立采购单，改用原料入库按库存单位录入，避免出现“数量 g、成本元/个”的错账。
...
## 库存作业
- 原料入库、生产领退、普通/成品转仓都通过库存作业的统一库存单据处理，物料或成品选择框支持名称/编号模糊搜索。
...
- 重量物料库存单位为 `g` 时，物料档案、采购/入库和批次成本仍显示 `元/kg`；物料采购价 `54` 不得显示成 `54元/g`。
...
- 成本计价单位改不了：成本计价单位在物料保存后与库存单位一样锁定。重量物料统一按 `kg` 计价；如果历史档案的单位选择错误，暂停继续录入成本，新建正确物料并让管理员核对迁移，不要直接改库。
- 库存是克但采购价显示元/kg：这是正确口径。库存单位管数量，成本计价单位管单价；价格试算会按 `cost_unit` 展示，不应把 `54元/kg` 显示为 `54元/g`。
