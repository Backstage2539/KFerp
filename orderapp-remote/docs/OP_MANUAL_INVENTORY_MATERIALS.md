...
## 标准操作
1. 在物料档案中维护物料名称、分类、库存单位、默认采购价、行业字段和库存警戒线；默认采购价只作为兜底参考，不用于直接改历史批次成本。
...
- 物料档案里的单位来自 `设置 → 系统设置 → 全局单位字典` 中启用的单位，在新业务中统一解释为库存单位。物料档案只展示一个 `库存数量（库存单位）`；库存补录只输入一个目标库存数量，单位跟随该物料库存单位。库存调整单选择物料时同样只输入一个目标数量和原因，后台兼容旧 `g/件数` 库存字段。
...
## 商品管理与商品档案
- PR-439-PRODUCT-PRICE-MASTER-REMODEL / PR-502-PRODUCT-UNIT-TEMPLATE-REFERENCE / PR-504-PARENT-PRODUCT-CHILD-SKU-UOM：商品档案是父商品入口，维护商品资料、默认单位模板、分组、客户引用和行业字段；具体销售/库存规格由子 SKU 表达，例如 `227g袋装`、`100g袋装`、`散装kg`。商品档案配置抽屉只读展示 BOM 使用摘要和价格摘要。价格摘要来自商品价格表快照，没有已发布快照时显示 `暂无价格表价格`。商品档案普通表单不再维护商品配置模板、报价单位、录单单位、阶梯价模板、计价方式、固定价、成本加成或利润率覆盖；这些旧字段仅用于历史数据兼容或迁移报告。
- PR-439 历史验收口径曾写“商品档案只维护商品资料、库存单位、规格、整数限制和行业字段”；PR-502 在这个口径上把“库存单位/默认销售单位”收敛为单位模板引用和高级覆盖，不恢复旧价格/配置模板入口。
- 商品档案配置抽屉的“销售规格 / SKU”区块用于维护同一父商品下的具体子 SKU。每个子 SKU 必须选择单位模板，可填写 SKU 名称、编码、条码、规格净含量和启停状态；普通商品档案不直接维护商品级单位换算。抽屉的“被哪些 BOM 使用”只读展示产出该子 SKU 或把该子 SKU 作为组件消耗的生产 BOM，并显示 `BOM状态`：`默认状态` 表示该 BOM 是产出商品当前默认 BOM；`启用状态` 表示 BOM 可继续作为生产/成本来源；`失效状态` 表示 BOM 已失效，仅用于历史追溯，不能作为新生产或新报价的有效来源。子 SKU 单位模板修改只影响后续新建或编辑的 BOM、工单和库存流转，不自动回改已发布 BOM 版本、历史工单、历史库存流水或历史价格表快照。
...
- PR-453 / PR-502：商品与配方菜单保持独立入口：`商品档案`、`客户商品`、`商品价格管理`、`商品价格表`；分组模板入口统一在 `设置 → 系统设置 → 分组模板`；`生产 BOM` 位于生产管理，`行业字段模板` 位于 `设置 → 行业设置`。旧 `商品配置和分类模板`、`阶梯价模板` 只作为历史兼容或迁移排查入口；`单位模板` 恢复为商品 UOM 主数据模板，可从商品档案引用和维护。进入 `商品档案` 后，在商品列表工具栏点击 `维护单位模板` 可进入单位模板维护页，维护完成后点击 `返回商品档案` 回到商品档案列表。
- `商品档案` 是库存、销售、成本、成品批次和价格对象。列表第一业务列是商品名；商品名本身就是配置入口，默认不在表格内编辑，点击商品名打开“商品档案配置”抽屉。
- `商品档案配置` 抽屉同屏维护基础信息、单位模板、销售规格 / SKU、工艺路线、预期损耗率和行业字段模板，并只读显示 BOM 使用摘要和价格摘要。BOM 摘要分别展示“产出该子 SKU 的 BOM”和“把该子 SKU 作为组件的 BOM”；同一 BOM 只展示一次，失效商品或失效 BOM 不展示。PR-430 后组件反查只读取每个生产 BOM 的当前有效版本，有草稿时看草稿，否则看最新发布版本，旧版本曾经用过该商品不会继续显示为当前使用关系。商品档案不再维护或更换生产 BOM；要修改制造结构，回到生产 BOM 中维护产出子 SKU、版本和组件清单。预期产出率不再作为用户维护字段；系统内部需要时按 `1 - 预期损耗率` 换算。
...
- 新增商品档案表单只填写父商品名称、单位模板、备注等基础信息；同类商品优先选择已有单位模板，模板会带出有效库存单位、默认销售单位和换算。具体规格在新增后的“商品档案配置”抽屉中用 `销售规格 / SKU` 维护；不要把 `袋/227g` 写成销售单位，也不要为每个克重复制一套单位模板。不再选择分类模板、产品类型、产品子类型、商品配置模板或阶梯价模板；也不把“贴牌”“受托加工”写成底层 SKU 类型。新增成功后系统会自动打开该商品的“商品档案配置”抽屉，便于立刻补齐子 SKU、工艺路线和行业字段；如某个子 SKU 需要生产，再到生产 BOM 新建或复制 BOM 并把它设为产出子 SKU。
...
- PR-366-GLOBAL-UNIT-TEMPLATES / PR-438-PRODUCT-TEMPLATE-DELETE-PRICE-LIST-UI / PR-439 / PR-500 / PR-501 / PR-502 / PR-504：基础单位在 设置 → 系统设置 → 全局单位字典 维护，例如 kg、g、lb、盒、袋、箱。单位模板是商品 UOM 主数据模板，只维护库存单位、默认销售单位、销售单位换算和整数规则，例如 `1 箱 = 12 袋`、`1 lb = 0.453592 kg`，历史模板也可解释 `1 盒 = 0.2 kg` 这类销售单位到库存单位换算；单位模板不表达 `227g`、`100g` 这类规格。创建新商品档案和新增子 SKU 时必须选择单位模板；模板修改后，对引用该模板且没有历史覆盖的子 SKU 实时生效。商品价格管理不维护单位换算；商品价格表只选择价格单位，并在发布时把子 SKU 当时的有效库存单位和换算固化到价格表快照。没有价格单位到库存单位换算时禁止发布或录单。
...
- 例子：先在 设置 → 系统设置 → 全局单位字典 确认基础单位“kg”“袋”“箱”存在，再在 商品档案 → 单位模板 新建“袋装商品单位”，库存单位 `袋`，默认销售单位 `袋`，换算 `1 箱 = 12 袋`，按需开启整数规则。到 商品档案 新建父商品“埃塞俄比亚 水洗”，在配置抽屉的 `销售规格 / SKU` 新增 `227g袋装` 子 SKU 并引用该单位模板；BOM 声明产出 `1 袋`，消耗 `0.227 kg` 熟豆和包装物料。需要按袋报价时，在 商品价格管理 维护价格计算模板，再到 商品价格表 选择该子 SKU 和价格单位袋并发布。发布时后端会把子 SKU 当时的有效单位模板和 `sku_snapshot` 固化到价格表快照。发布后，录单单位、订单价格和生产需求换算来自已发布快照；已发布价格表和历史订单不会被回改。
...
## BOM配置
- PR-417-MULTILEVEL-MANUFACTURING-BOM / PR-500-UNIT-MODEL-CONSOLIDATION：入口为“生产 BOM（制造主档）”，位于“生产管理”。生产 BOM 是生产端主档案，必须声明产出商品、产出数量、产出商品库存单位、组件清单、版本状态和可选工艺信息；产出单位由产出商品的有效库存单位自动带出并只读显示 `来源：商品档案库存单位`，商品档案不再主动绑定或编辑 BOM。
...
- 新建或编辑 BOM 时不能手工改产出单位；如果产出商品缺少显式库存单位，页面显示当前兜底单位并提示先到商品档案设置库存单位。历史 BOM 版本的产出单位和商品档案当前库存单位不一致时，详情会提示 `当前版本产出单位为 X，商品档案库存单位为 Y；历史版本不会自动回改`。
...
- 原料入库按库存单位录入。选择物料后，入库数量和成本字段会带出该物料库存单位；提交时系统保存库存单位数量并兼容旧克重入库数据。重量库存单位会归一到 `qty_g`，盒、袋、个等非重量库存单位会归一到 `qty_units`，并同步写入原料批次、库存批次、原料仓库位和库存流水。
...
- 库存调整单使用库存单位填写目标库存数量；原料、WIP、半成品和成品库存都归入库存单位口径，销售单位只在销售、价格和录单场景展示。
