...
- `商品档案配置` 抽屉同屏维护基础信息、商品配置模板、生产 BOM 版本、工艺路线、预期损耗率、行业字段模板和行业字段值，并提供“维护当前 BOM 明细”入口跳到生产 BOM 配方库。预期产出率不再作为用户维护字段；系统内部需要时按 `1 - 预期损耗率` 换算。
- `商品配置模板` 由商品档案直接引用，不再由产品子类型/分类引用。历史分类上的模板字段只作为旧数据 fallback。分类用于归类和价格表类型展示，不承载模板规则、BOM 或客户展示名。
- `行业字段模板` 是商品生产配置字段的定义来源。选择模板后，字段会在商品档案配置抽屉里按文本、长文本、数字、比例、选项、勾选、日期等普通表单展示和保存，普通用户不需要写 JSON。
...
- “商品配置模板”入口中维护商品配置模板、单位模板和阶梯价模板。商品配置模板可绑定阶梯价模板、工序模板、价格表生成规则和单位模板；配置完成后回到“商品档案”，在商品档案配置抽屉中选择该模板。
...
- “商品档案”列表默认展示工厂自营/公共商品档案。列表不再提供“生产配置 / 更换生产 BOM / 维护 BOM / BOM”等重复按钮；点击商品名进入配置抽屉。生产 BOM、工艺路线、预期损耗率、商品配置模板、行业字段和值都在抽屉内维护；需要编辑配方明细时点击“维护当前 BOM 明细”进入生产 BOM 配方库。列表中还可维护产品级利润率覆盖、产品失效和备注。
...
- 如果客户要批量使用已有商品档案，点击“批量添加商品档案”打开抽屉，按商品名称、编号、产品类型或产品子类型筛选，多选后一次创建客户商品名。默认客户商品名=商品档案名称、客户商品编号=商品编号、品牌名为空、进入价格表；同客户同商品档案已存在时自动跳过，并在结果里显示创建和跳过数量。实际创建的客户商品名都会写操作日志。
