...
- 分组列表顶部提供 `全部展开` 与 `全部收缩`：一次展开或收起本列表的所有模板、大类、小类/后代分类和未分类；没有分组模板时也适用于 `全部分类` 平铺区。只改变显示，不改查询条件、分类页码、勾选或业务数据。加载中、移动模式及没有分类时按钮禁用；无需编辑权限即可使用。
- 搜索结果中也可手动全部展开或全部收缩；清空并应用搜索后仍恢复搜索前的折叠状态。进入移动模式前的手动展开/收缩状态会在成功或取消移动后恢复。
- 适用页面：`商品 → 商品档案`、`库存管理 → 物料档案`，以及仓库库存中选中一个具体工厂仓库后的分组列表。适用角色和原页面权限不变；先按需选择本功能使用的分组模板。
- 商品档案输入搜索词后实时筛选；物料档案输入名称、编码或批次号后按回车或点击 `查询`。有结果时，系统自动收起没有结果的模板、分类和未分类，展开首条结果所在的完整父子路径，并把首条业务行滚动到屏幕内。父类自身没有结果但小类有结果时，父类仍保持展开。没有结果时只调整空分类的收起状态，不抢走当前焦点。
- 仓库库存只有在选中具体仓库且非客户库存上下文时使用上述分组交互。输入物品或批次并查询成功后才应用自动折叠和定位；查询失败时保留原列表和浏览状态。`全部仓库` 与客户库存仍使用原服务端分页平铺，不改为分类内分页。
- 清空并实际应用搜索后，恢复搜索前的分类折叠和滚动位置。物料或仓库仅清空输入框但尚未查询时，列表仍保持上一次已成功查询的结果状态。
- 每个分类筛选后的总数为 0～10 条时全部直接显示，不出现页码、跳页或每页条数；从 11 条开始显示该分类自己的分页。一个分类翻页、改变每页条数或全选当前页，不影响其他分类。
- 勾选业务行并点击 `移动到分类` 后，所有模板、大类、小类/后代分类和未分类标题同时可见，业务表、分页、搜索筛选和行操作统一收起或禁用。直接点击大类、小类/后代分类或 `未分类` 即移动；`全部分类` 与模板标题不可作为目标。
- 移动成功会清空勾选并恢复进入移动模式前的折叠和滚动位置；点击取消也恢复。若接口失败，勾选和移动模式保留，可检查提示后重试。移动继续调用既有归类接口并写操作日志，不改变商品、物料、库存数量、批次、成本、BOM 或历史快照。
- 结果校验：分别用“父类无直接命中但小类命中”、无结果、10 条分类、11 条分类和移动失败重试检查上述行为。若搜索未自动定位，物料/仓库先确认已经提交并成功返回查询；若移动失败，保留当前页面并按错误提示检查权限或分类是否仍有效，不要重复新建分类。
...
## 物料档案
- PR-639：一个物料档案只有一个归属方。新建时必须明确选择“本公司”或一个具体客户；客户使用可搜索单选，名称为主，联系人和电话用于辅助识别。当前工厂／客户视图不会替用户推断归属。
- 工厂员工按权限查看全部物料，并可按全部、本公司或具体客户筛选；客户视图与客户账号只显示归属于该客户的物料。列表和详情统一显示“物料归属”，不再显示使用范围、客户共享或多客户关联。
- 工厂采购并用于多个客户订单的物料仍归本公司；客户来料即使由工厂保管也归客户。同规格物料属于不同主体时分别建档、分别管理库存，不能共享、借用或合并数量。
- 点击 `复制` 会带入名称、库存单位、分类、规格和行业属性，并要求重新选择归属、生成独立编码；库存数量、成本余额、交易记录和已发布 BOM 绑定不复制。复制只是加快建档，不建立来源物料与新物料之间的替代关系。
- 只有无库存、无 BOM 和业务单据引用的物料可以在详情中调整归属。已经投入使用时，应新建目标归属的档案并通过正式库存或业务单据处理，不能通过改主档转移货权或实物。
...
- PR-645：物料失效前，各仓库、批次和客户库存的重量与件数都必须为 0，包括待检、WIP 以及负库存；不能用不同仓库或批次正负抵消后的合计数代替。若提示“仍有库存”，先在仓库核对批次，再通过库存调整单处理到 0，返回物料档案重新失效。未清零时操作失败，物料仍保持启用，不会留下成功失效日志。失效不会删除历史出入库单据或批次记录。
...
- PR-671：商品档案配置抽屉以“档案归属”显示真实商品归属。公共商品显示“工厂公共商品”；像 EFS 这样的客户引用只显示在客户引用区域，不会让公共商品看起来属于引用客户。新建后自动打开、列表点击商品名打开和连续切换商品时都应以当前商品档案身份显示。
- PR-671：商品价格表、分享预览、打印和 PDF 去掉每个字段重复的“属性”前缀。同一冻结行业字段模板下的多个“字段名：值”用分号连接成一行；不同模板分行显示，并保留模板/字段顺序。空值和关闭展示的字段隐藏，长内容自然换行。新的价格表快照保存模板身份；历史快照没有模板身份时继续按原有分行显示，不用当前模板回写历史。
...
- 商品档案只维护名称、分类、归属和行业字段等商品身份，不维护库存单位、销售规格、默认 SKU 或子 SKU。
- 商品行只读展示当前默认已发布 BOM 的规格名称、条码和库存单位。点击 `到 BOM 配置` 进入生产 BOM；新增、改名、改条码、改单位和调整默认规格都在 BOM 草稿中完成并重新发布。
- 新建商品无需先设置库存单位。建档后到 `生产配置 → 生产 BOM` 新建商品产出 BOM，至少填写一个规格，并从全局单位字典选择该规格的库存单位。
- 工厂／客户视图只改变商品列表和标题，不决定新商品归属。在任一视图点击“创建新商品档案”，都必须在表单中明确选择“工厂公共商品”或“客户专属商品”；客户专属商品还要选择归属客户，可填写客户商品名和客户货号。
- 工厂视图展示全部商品。“商品归属”支持“全部”“工厂公共商品”和客户名称模糊搜索；选择客户后展示该客户专属商品和已复制给该客户的公共商品。每行蓝色文字“复制到客户”可为多个客户分别设置客户商品名和货号。商品级供料方式已取消，库存来源在生产计划逐组件选择。
- “商品来源”显示工厂公共或客户专属，“商品归属”显示有效归属客户，无客户时显示“仅工厂”。最右侧“复制”只复制商品主档，不复制 BOM、价格或客户归属。
- 公共商品的客户名称只在对应客户视图、价格表和新订单快照中生效；公共商品原名称、BOM、规格与工艺路线保持不变。
- 没有默认已发布 BOM 规格时，商品显示 `未配置 BOM 规格`，保留档案但不能进入新价格表、录单、库存写入、排产或成本试算。不要用旧销售规格模板或复制子 SKU 绕过。
- 历史订单、价格表、库存流水和工单继续显示各自冻结的名称、规格、单位与价格快照，不从商品档案反查旧子 SKU。

## 物料唯一归属（PR-639）

- 物料页的“物料归属”支持本公司和客户名称模糊搜索；选择客户后只展示该客户拥有的独立物料档案。
- 新建完成后系统明确提示“已创建本公司物料”或“已创建客户「名称」的物料”。旧客户关联接口已经下线，历史关联只用于追溯，不决定归属或访问权限。
- BOM 物料行显示“名称＋归属”并引用具体物料 ID。工厂通用 BOM 只能使用本公司物料；客户 BOM 可以使用本公司物料及该客户物料，不能选择其他客户物料。
- 排产、预占、领料和消耗严格按 BOM 指定物料及其归属查找仓库、货主和批次。同名、同规格或由同一来源复制都不会自动替代；缺料时补充该归属库存，或明确修改 BOM 草稿并发布新版本。
...
商品库存身份统一为“主商品 + 当前默认已发布 BOM 规格 + 当前发布变体”，库存单位直接读取 BOM 规格。商品档案不再保存销售规格模板、默认 SKU、子 SKU 或迁移状态，旧写接口和 BOM 规格迁移入口已删除。

旧规格数据清理只由发布人员运行一次性命令。先在数据库主机生成并留存完整备份，核对文件大小和 SHA-256；`apply` 必须带入这三项备份凭证，随后才会在单事务中执行。任何未映射库存、在途订单、生产或已发布价格引用都会让整批清理停止，不能跳过。

物料仍使用物料档案库存单位。商品没有可用 BOM 规格时，新业务接口返回 `product_bom_spec_not_configured`；先完成 BOM 规格和完整配方并发布，不要修改历史单据快照或直接写数据库。

## PR-634 商品与分类复制到客户

适用角色：具备商品维护权限的内部人员。客户账号本次不增加改名权限。

1. 打开“商品档案”。使用顶部“复制全部商品到客户”、勾选后的“复制所选商品到客户”，或商品行“复制到客户”。
2. 在弹窗明确选择客户后确认。全部复制包含所有启用工厂公共商品，不受搜索、分类或分页限制；所选/单个复制只带入相关完整分类路径。
3. 留在当前页面，按提示在“商品归属”搜索客户名称。列表显示客户商品名和客户分类；点击商品名称修改该客户名称，点击“客户分类名称”修改其分类名称，编辑区会标出客户名称。
   - PR-669：客户商品名行下方会以小字标注“工厂名：xxx”。点击商品名打开配置抽屉后，“客户商品名”输入框只改该客户的引用名；工厂商品名在抽屉中只读展示，须回到“工厂公共商品”视图才能维护。保存时系统仅更新客户引用，不会把客户名写回工厂商品。
4. 去“商品价格表”，在“价格表归属”搜索同一客户，维护、预览并发布客户供货表。

结果核对：复制只新增客户引用和分类快照，共用原商品 ID、BOM、规格及组件，不产生库存或发布价格表。原行尾“复制”仍创建独立商品主档。
重复复制只补充缺失商品/分类，保留客户已经修改的名称、分类归属和顺序；已停用引用恢复。工厂后来改名不会覆盖客户名称。商品停用、归属其他客户或分类异常时整批失败，可根据错误检查商品后重试；成功复制、改名和迁移可在操作日志查询。

## PR-637 订单与客户目录操作更新

在“商品归属”选择客户后，复制商品的行操作为“删除”，勾选后可批量删除客户商品。确认框说明只删除客户绑定，工厂商品、BOM、其他客户商品和历史订单保留。再次复制可恢复原客户商品名及分类。
复制继承源档案的有效分类，源档案未分类的商品仍为未分类；不会根据名称推测商品类型。客户分类名称可独立修改，商品价格表读取同一份客户分类。
