...
## PR-453 / PR-455 分组模板新口径
- PR-453-GROUP-TEMPLATE-SYSTEM-SETTINGS / PR-455-GROUP-TEMPLATE-DELETE：商品档案、生产 BOM 和仓库库存统一使用 `系统设置 / 分组模板`。分组模板是系统级基础资料，只维护模板名、大类和小类；模板页不展示对象数量、对象列表、勾选对象或对象移动入口。模板没有启用/停用开关；不再使用的模板在编辑模板时点击 `删除模板`，删除后该模板、分类项、用途和对象归类会被移除，新业务下拉不再出现该模板。
- 商品档案归类使用 `product_catalog`。进入商品档案后先选择 `分组模板`，选择后才显示分类 Tab 和 `移动到分类`。列表按当前模板的大类 / 小类分段展示；父组和子组都可以形成分类标题，子组标题缩进显示并只显示子组名，完整父/子路径作为上下文，商品行跟随所在父组/子组缩进。移动目标可选 `未分类`、大类或小类；移动会覆盖旧归类并写操作日志。
- 生产 BOM 归类使用 `production_bom`。生产 BOM 页面不再维护自己的大组、组内分类或小分类，也不再显示 `使用分组`。用户先选择可用分组模板，再通过 `移动到分类` 把 BOM 移到 `未分类`、大类或小类。顶部分组 Tab 只展示当前所选模板下当前 BOM 列表实际使用过的分类项。旧 `production_bom_groups` / `production_bom_group_categories` 只用于历史只读兼容。
- 仓库库存归类对象是仓库，使用 `warehouse_inventory` 和仓库 code 作为对象引用。仓库库存页先选择 `库存分组模板`，仓库设置抽屉先选择模板再 `移动到分类`。分类只影响左侧仓库组织和筛选，不改变库存数量、批次、成本或追溯。
...
- PR-453 后的当前口径：生产 BOM 分类不再有默认分组，也不再有 BOM 专用大组/组内分类。生产 BOM 页面先选择 `分组模板`，选择后显示“全部分类”和“未分类”Tab，并追加当前所选模板下实际被当前 BOM 列表使用过的分类项。用户在 `系统设置 / 分组模板` 维护大类和小类，再回到生产 BOM 用 `移动到分类` 把 BOM 移到 `未分类`、大类或小类。一个 BOM 的归类保存到 `business_group_assignments`，不改变产出商品、BOM 版本或配方明细。
...
- 生产 BOM 页面点击 `前往分组模板` 到系统设置维护大类和小类。回到生产 BOM 后选择 `分组模板`，勾选 BOM，点击 `移动到分类`，并在 `目标分类` 中选择 `未分类`、大类或小类。目标为具体分类项时写入 `business_group_assignments`，目标为 `未分类` 时移除该 BOM 的 `production_bom` 归类。新建、编辑或复制 BOM 只维护名称、产出商品、产出数量单位和状态，不写旧 `production_boms.group_id/group_category_id`。
