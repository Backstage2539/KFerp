...
- 生产管理：工艺模板，用于把 SKU/BOM 与制造路线关联；商品与配方：行业字段模板，用于维护行业参数字段。
...
- PR-370-UNIT-TEMPLATE-DRAWER-SKU-CATEGORY-LAYOUT / PR-382：商品分类维护是低频配置，入口位于 SKU设置 → 商品配置 → 商品分类管理。进入后可在分类管理页搜索、折叠、排序、新增或删除当前归属自己拥有的产品类型/产品子类型；即使客户开启了公共商品分类，分类管理也不展示公共分类引用，避免误以为客户分类还未失效。产品类型标题和上下排序按钮在左侧，折叠/展开和删除动作在右侧；产品类型收起后会以单独一行边框展示，产品子类型缩进显示，便于区分父子层级。SKU 列表右上角不再放旧“分类设置”，只保留日常“新增SKU”和“SKU复制”。单位模板左侧是模板列表，右侧是新增或编辑表单；在单位模板页点击“全局单位字典”可用右侧抽屉维护基础单位。
- “客户SKU列表”默认展示公共SKU，可在同一列表维护工艺参数、BOM 预期产出率、BOM 来源、BOM 状态、BOM 入口、产品失效和最右侧备注；完整的预期损耗率在 BOM 配方维护中维护。
- 需要查看或初始化客户SKU时，在“SKU归属”里选择履约客户；选择列表来自履约客户候选。无专属 SKU 的履约客户可以直接新增 SKU，也可以点击“SKU复制”从公共 SKU 或其他客户复制一整套 SKU。
- 默认公共SKU时，点击“新增SKU”创建公共 SKU；选中履约客户后，同一个“新增SKU”按钮创建客户专属 SKU，表单使用顶部已选客户，不再重复选择客户。
- 新增 SKU 表单只填写 SKU 名称、备注、产品类型和产品子类型；不再区分公共 SKU、客户 SKU、定制 SKU、基础产品或产品形态。未选择产品子类型时进入停车场，后续可拖入正确子类型。
...
- PR-362-PRODUCT-CONFIG-TEMPLATE：SKU设置 的“商品配置”是统一模板入口，配置里维护阶梯价模板、工序模板、价格表生成规则，并引用单位模板。公共配置和客户配置都是独立模板，可点“复制为客户配置”后再调整；客户已经复制过某个公共配置后，该客户的商品配置模板列表会隐藏被派生的公共原模板，只显示客户自己的副本，避免误选或误改公共配置。产品子类型行直接选择商品配置，不再让用户填写 JSON，也不再在这里决定是否纳入产品价格表。
...
- 客户需要复用公共或其他客户的 SKU 与分类结构时，使用“SKU复制”把公共 SKU/其他客户 SKU 复制成当前客户 SKU；系统会自动补齐目标客户缺少的产品类型和产品子类型，同名同分类 SKU 按覆盖规则更新。商品分类管理里不再提供“复制为客户分类”，该页只维护当前归属已拥有的分类。客户需要复用公共阶梯价模板或公共商品配置时，仍分别使用“复制为客户模板”和“复制为客户配置”，复制后再修改，不会直接改公共数据；复制完成后当前客户模板列表会隐藏已经派生过的公共原模板。
- PR-382-SKU-UNIFIED-CREATE-COPY / PR-385-SKU-BOM-INHERITANCE：“SKU复制”抽屉的交互和产品价格表生成选择器一致。顶部显示“选择分类和产品 X/Y 款”，有总复选框和“全选/清空”；产品类型、产品子类型、SKU 行都可勾选。复制时同名同分类覆盖目标 SKU 资料并保留目标 SKU ID；不存在则新增。复制后的 SKU 默认不复制 BOM 明细，而是继承来源 SKU 当前有效 BOM；列表的“BOM来源”会显示来源 SKU 编号、名称和 BOM 版本。只有客户需要改配方时，进入 BOM 配方维护点击“派生自有 BOM”。复制结果提示新增 X 款、覆盖 Y 款、跳过 Z 款。停用 SKU 显示为已停用且不可选。
- PR-321/PR-322：客户自有分类和已归类客户 SKU 不会被隐藏或删除；产品子类型仍有效但父级误失效时，schema 修复会自动恢复父分类或迁移到同名 active 父分类。通过 SKU设置 停用 SKU 时，对应 active BOM 版本同步置为 disabled，历史 BOM 明细继续保留。
- 点击“公共SKU”回到公共SKU列表；客户SKU行同样支持维护备注、BOM、调整工艺参数/BOM 预期产出率或失效。列表筛选支持搜索关键词、产品类型和产品子类型；搜索可匹配商品名称、商品编号、历史兼容类型标签和备注。
...
- 需要维护客户SKU BOM 时，在“SKU归属”里选择客户；客户如果还没有专属 SKU，先在 SKU设置 中新增 SKU 或使用“SKU复制”生成客户 SKU。
- 客户 SKU BOM 商品列表按当前客户自有 SKU 优先，其次按订单使用次数排序；客户常用 SKU 会排在前面，公共 SKU 不会默认混入客户自有 SKU 列表。
- PR-385-SKU-BOM-INHERITANCE：客户通过“SKU复制”得到的新 SKU 默认继承来源 SKU 当前有效 BOM，BOM 来源列显示“继承：SKU-xx 来源名称 / BOM 版本”。继承 BOM 是只读的，预期损耗率、组件、版本启用和失效不能直接改；如果要为客户改配方，先点击详情顶部“派生自有 BOM”，系统会把当前有效 BOM 明细复制到该客户 SKU，并显示“自有 BOM，派生自：SKU-xx 来源名称 / BOM 版本”。派生来源编号、名称和版本号是快照，来源 SKU 改名或 BOM 升级不会改历史派生说明。
- “派生自有 BOM”适合受托加工、客户专属配方、客户专属包装等场景；贴牌只改商品名称、配方完全相同的场景，保持继承 BOM 即可，避免生成大量重复 BOM。
