...
- 生产 BOM 归组使用 `production_bom`。列表中的全部分组、未分类、分组标签和“移动到分组”都来自 `分组管理` 的通用分组项；生产 BOM 页面不再维护自己的大组、组内分类或小分类。移动生产 BOM 时，“目标分组”可选择分组管理中已经维护的业务分组项；即使该分组之前只用于商品档案、价格表或其他功能，保存 BOM 归组时系统也会自动补充 `production_bom` 用途并写操作日志。系统默认分组集名称不作为业务标签展示；旧 `production_bom_groups` / `production_bom_group_categories` 只用于历史只读兼容。
...
- PR-442 后的当前口径：生产 BOM 分组不再有默认分组，也不再有 BOM 专用大组/组内分类。生产 BOM 列表固定有“全部分组”和“未分类”Tab；用户在 `分组管理` 维护可复用分组项后，生产 BOM 列表会按这些分组项展示筛选入口。一个 BOM 的归组保存到 `business_group_assignments`，不改变产出商品、BOM 版本或配方明细。
...
- 生产 BOM 页面点击 `前往分组管理` 维护通用分组项。批量移动时，先选中 BOM，再在工具区选择 `目标分组` 并点击左侧 `移动到分组`；目标为具体分组项时写入 `business_group_assignments`，目标为 `未分组` 时移除该 BOM 的 `production_bom` 归组。新建、编辑或复制 BOM 只维护名称、产出商品、产出数量单位和状态，不写旧 `production_boms.group_id/group_category_id`。
