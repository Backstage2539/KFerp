...
- 客户商品页顶部提供客户选择、搜索、启用/停用/全部过滤、`新建客户商品` 和 `批量失效`，这些过滤对所有 Tab 生效。分类模板编辑时，保存/删除模板在右侧编辑区底部，分类项的阶梯价模板和单位模板并排；删除分类项后，该分类下对象回到该大类 Tab 的“未分类”。
...
- `商品配置和分类模板` 只维护商品配置模板和商品分类模板。`阶梯价模板`、`单位模板` 是商品与配方下的独立菜单功能。商品配置模板直接引用单位模板，并在计价方式为“按阶梯价模板”时引用阶梯价模板；商品分类模板和分类项也可引用阶梯价模板和单位模板，用于分类口径、价格表类型和不一致提示。实际价格/单位计算仍以商品档案或客户商品引用的商品配置模板为准。
...
- “商品配置和分类模板”入口只维护商品配置模板和商品分类模板。阶梯价模板、单位模板在商品与配方下是独立功能。商品配置模板维护计价方式、工序模板、价格表生成规则和单位模板；只有计价方式选择“按阶梯价模板”时，右侧才维护阶梯价模板。分类模板用于维护分类项集合。配置完成后回到“商品档案”或“客户商品”，通过“增加分类”把该模板启用为列表 Tab。
