...
- 生产 BOM 归组使用 `production_bom`。列表中的全部分组、未分类、分组标签和“移动到分组”都来自 `分组管理` 的通用分组项；生产 BOM 页面不再维护自己的大组、组内分类或小分类。系统默认分组集名称不作为业务标签展示；旧 `production_bom_groups` / `production_bom_group_categories` 只用于历史只读兼容。
