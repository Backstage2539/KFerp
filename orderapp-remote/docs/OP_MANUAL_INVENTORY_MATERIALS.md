...
## 商品管理与商品档案
- PR-439-PRODUCT-PRICE-MASTER-REMODEL / PR-502-PRODUCT-UNIT-TEMPLATE-REFERENCE：商品档案只维护商品资料、单位模板引用、库存单位/默认销售单位的高级覆盖、规格、整数限制和行业字段；同时只读展示 BOM 使用摘要和价格摘要。价格摘要来自商品价格表快照，没有已发布快照时显示 `暂无价格表价格`。商品档案普通表单不再维护商品配置模板、报价单位、录单单位、阶梯价模板、计价方式、固定价、成本加成或利润率覆盖；这些旧字段仅用于历史数据兼容或迁移报告。
- PR-439 历史验收口径曾写“商品档案只维护商品资料、库存单位、规格、整数限制和行业字段”；PR-502 在这个口径上把“库存单位/默认销售单位”收敛为单位模板引用和高级覆盖，不恢复旧价格/配置模板入口。
- 商品档案配置抽屉的“被哪些 BOM 使用”只读展示产出该商品或把该商品作为组件消耗的生产 BOM，并显示 `BOM状态`：`默认状态` 表示该 BOM 是产出商品当前默认 BOM；`启用状态` 表示 BOM 可继续作为生产/成本来源；`失效状态` 表示 BOM 已失效，仅用于历史追溯，不能作为新生产或新报价的有效来源。库存单位和整数库存也在商品档案配置抽屉维护；修改后只影响后续新建或编辑的 BOM、工单和库存流转，不自动回改已发布 BOM 版本、历史工单、历史库存流水或历史价格表快照。
...
- 新增商品档案表单只填写名称、单位模板、备注等基础信息；同类商品优先选择已有单位模板，模板会带出有效库存单位、默认销售单位和换算。只有例外 SKU 才打开“高级单位覆盖”维护库存单位、整数库存、销售单位和换算。不再选择分类模板、产品类型、产品子类型、商品配置模板或阶梯价模板；也不把“贴牌”“受托加工”写成底层 SKU 类型。新增成功后系统会自动打开该商品的“商品档案配置”抽屉，便于立刻补齐工艺路线和行业字段；如该商品需要生产，再到生产 BOM 新建或复制 BOM 并把它设为产出商品。
...
- PR-366-GLOBAL-UNIT-TEMPLATES / PR-438-PRODUCT-TEMPLATE-DELETE-PRICE-LIST-UI / PR-439 / PR-500 / PR-501 / PR-502：基础单位在 设置 → 系统设置 → 全局单位字典 维护，例如 kg、g、lb、盒、袋、箱。单位模板是商品 UOM 主数据模板，维护库存单位、默认销售单位、销售单位换算和整数规则，例如 `1 盒 = 0.2 kg`。创建新商品档案和商品档案配置抽屉优先选择单位模板；模板修改后，对未做商品级覆盖的商品实时生效。商品级“高级单位覆盖”只用于少量例外 SKU，并继续保留历史 JSON 里的其他键。商品价格管理不维护单位换算；商品价格表只选择价格单位，并在发布时把商品当时的有效库存单位和换算固化到价格表快照。没有价格单位到库存单位换算时禁止发布或录单。
...
- 例子：先在 设置 → 系统设置 → 全局单位字典 确认基础单位“kg”和“盒”存在，再在 商品档案 → 单位模板 新建“盒装200g”，库存单位 kg，默认销售单位盒，换算 `1 盒 = 0.2 kg`，按需开启整数规则。到 商品档案 新建“冻干速溶 200g 盒装”时直接引用该单位模板；如该 SKU 与模板不同，再打开“高级单位覆盖”维护例外。需要按盒报价时，在 商品价格管理 维护价格计算模板，再到 商品价格表 选择价格单位盒并发布。发布时后端会把商品当时的有效 `1 盒 = 0.2 kg` 固化到价格表快照。发布后，录单单位、订单价格和生产需求换算来自已发布快照；已发布价格表和历史订单不会被回改。
