...
- 操作列只保留 `复制` 和红色 `失效` 等列表级动作；点击 BOM 名称进入右侧编辑详情。复制来源 BOM 即使已失效也可以复制，新 BOM 默认启用。配方明细编辑仍遵守版本规则：已发布版本只读；需要调整物料时先在详情内复制为新版草稿，编辑草稿后发布。
...
- BOM 版本在右侧 BOM 编辑详情的 `BOM版本` 区维护。已发布版本只读；需要调整配方或物料用量时，在详情中复制为新版草稿，编辑后发布，再把商品档案绑定到该已发布版本。列表行不再单独提供 BOM版本抽屉，页面底部也不再单独摆放 BOM 版本 panel。
