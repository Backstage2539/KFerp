...
- 商品档案归类使用 `product_catalog`。PR-453 的历史单模板入口已由 PR-584 更新：在商品档案页面多选要使用的分组模板，页面合并展示全部已选模板的大类 / 小类，空大类和空小类也显示，未归类商品进入统一 `未分类`。商品档案不再显示分类过滤 Tab，商品表格不再显示独立 `分类` 列；分类归属通过列表分组标题表达。移动目标可选 `未分类`、大类或小类；移动时还要先选择目标模板，移动会覆盖旧归类并写操作日志。
- 物料档案归类使用 `material_catalog`。物料档案先多选本功能使用的分组模板，再从已选模板中切换当前分类视图和移动目标；清空选择后按物料平铺展示且不能移动归类。取消选择不删除物料、模板、分类或既有归类。
- 生产 BOM 归类使用 `production_bom`。生产 BOM 页面不再维护自己的大组、组内分类或小分类，也不再显示 `使用分组`。用户先多选本功能使用的分组模板，再从已选模板中切换当前分类视图；BOM 列表按当前模板完整大类 / 小类树和 `未分类` 自动整理，空分类也显示。通过 `移动到分类` 把 BOM 移到 `未分类`、大类或小类；清空选择后平铺且不能移动。旧 `production_bom_groups` / `production_bom_group_categories` 只用于历史只读兼容。
- 仓库库存归类对象是仓库，使用 `warehouse_inventory` 和仓库 code 作为对象引用。仓库库存页先多选本功能使用的库存分组模板，再从已选模板中切换当前分类视图和移动目标；左侧仓库列表按当前模板大类 / 小类和 `未分类` 自动整理，仓库行可勾选后批量移动。清空选择后仓库平铺且不能移动归类。页面不再固定分成 `普通仓库` / `客户仓库`；仓库类型和客户绑定只作为行内/抽屉信息保留。分类不改变库存数量、批次、成本或追溯。
...
- 未引用任何分组模板时按平铺列表展示，并隐藏分类标题、分类收起和移动归类入口。选择保存和取消选择都由商品档案发起并写操作日志；分组模板页面保存模板名称或分类树不会改变商品档案已经选择的模板。
...
- 结果校验：刷新商品档案后，行业字段顺序与所选模板顺序一致；分组区只出现已选模板；无选择时为普通平铺列表；大类收起时看不到任何后代小类标题或商品行。再进入商品价格表，确认商品类型与商品档案已选模板一一对应。异常时先检查模板是否启用、商品档案选择是否保存成功，再到操作日志核对模板选择和商品配置保存记录。
