...
## PR-508 端到端链路验收口径
- PR-509-E2E-RAW-MATERIAL-ORDER-PRODUCTION-FLOW：从原料开始验收时，先在物料档案确认库存单位，再通过原料入库生成原料批次、仓库库存和库存流水；后续商品 / SKU、销售规格模板和生产 BOM 必须能引用这些原料作为生产组件，不能只停留在物料档案静态资料。
...
5. 在 BOM 配方维护中设置商品所需物料、用量和启用版本。
   - PR-511-BOM-MATERIAL-LOSS-BOM-LEVEL：BOM 版本设置中打开 `原料损耗比` 后，组件消耗单位只能使用 `比例 %`；库存准备、WIP 占用和生产消耗都会按损耗后需求量执行。例如产出基准 `1kg`、原料比例 `40%`、原料损耗 `20%` 时，需要准备 `0.5kg` 原料，而不是 `0.4kg`。固定用量和商品组件需要先关闭原料损耗比。
...
## 物料档案
- PR-513-MATERIAL-CLASSIFICATION-TEMPLATE：物料档案分类使用 `设置 → 系统设置 → 分组模板`。在物料列表选择分组模板后，页面按该模板的大类、小类和 `未分类` 展示物料；勾选物料后选择目标分类并点击 `移动到分类`，会把本次物料归类写入通用 `business_group_assignments`（`usage_key=material_catalog`、`object_key=material`）。物料页不再直接新增分类或小分类；需要维护分类结构时进入系统设置的分组模板。旧物料大类/小类会迁移为 `物料档案默认分组` 以兼容历史数据。
...
- PR-416-MATERIALS-LIST-LAYOUT-BATCH-DEPRECATE / PR-513-MATERIAL-CLASSIFICATION-TEMPLATE：物料列表不再单独展示 `物料类型` 列，分类状态通过当前分组模板的大类、小类和 `未分类` 分组体现。搜索、状态过滤、查询、新建物料和批量失效位于 `物料列表` 上方；表头最左侧复选框用于当前列表或当前分组全选/取消全选。列表内容较宽时可横向滚动查看，不再把物料名称、单位、库存和状态挤在一起。
...
- PR-520-INVENTORY-UNIT-LOCK：物料档案库存单位只在新建时选择，保存后详情页只读展示。单位选错时不要把已有物料从 `g` 直接改成 `kg`；应新建正确库存单位的物料档案，再通过入库、库存调整或业务迁移处理库存。后端会拒绝编辑接口里的库存单位变化，避免历史库存和 BOM 用料含义被改写。
...
- 商品档案配置抽屉的 `销售规格模板明细` 是当前规格主展示区，逐行展示规格名称、`1 规格 = 库存数量 库存单位`、是否默认规格和已派生 SKU 编号；不再重复展示 `销售规格 / SKU` 大块说明。要增加 `227g袋装`、`100g袋装` 等规格，先维护销售规格模板，并在明细行点击 `默认规格` 选择新商品和价格表默认使用的规格，再保存引用该模板的父 SKU，系统按模板规格自动补派生子 SKU。模板规格移除后历史 SKU 不删除，默认不展示；打开 `显示历史规格` 后才追加显示 `模板已移除` 行，并提示 `历史 SKU 保留用于历史单据，不参与新建业务`。抽屉的“被哪些 BOM 使用”只读展示产出该子 SKU 或把该子 SKU 作为组件消耗的生产 BOM，并显示 `BOM状态`：`默认状态` 表示该 BOM 是产出商品当前默认 BOM；`启用状态` 表示 BOM 可继续作为生产/成本来源；`失效状态` 表示 BOM 已失效，仅用于历史追溯，不能作为新生产或新报价的有效来源。销售规格模板修改只影响后续新建或编辑的 BOM、工单和库存流转，不自动回改已发布 BOM 版本、历史工单、历史库存流水或历史价格表快照。
...
- PR-453 / PR-502 / PR-505 / PR-506-PRICE-LIST-SPEC-DEFAULT-ROW-ERRORS：商品与配方菜单保持独立入口：`商品档案`、`客户商品`、`商品价格管理`、`商品价格表`；分组模板入口统一在 `设置 → 系统设置 → 分组模板`；`生产 BOM` 位于生产管理，`行业字段模板` 位于 `设置 → 行业设置`。旧 `商品配置和分类模板`、`阶梯价模板` 只作为历史兼容或迁移排查入口；当前商品规格入口为 `销售规格模板`，可从商品档案引用和维护。进入 `商品档案` 后，在商品列表工具栏点击 `维护销售规格模板` 可进入模板维护页；模板明细支持设置默认规格，保存后默认销售单位使用选中规格名称。
- `商品档案` 是库存、销售、成本、成品批次和价格对象。列表第一业务列是商品名；商品名本身就是配置入口，默认不在表格内编辑，点击商品名打开“商品档案配置”抽屉。
- `商品档案配置` 抽屉同屏维护基础信息、销售规格模板、销售规格模板明细、工艺路线、预期损耗率和行业字段模板，并只读显示“库存单位：来自销售规格模板”、BOM 使用摘要和价格摘要。BOM 摘要分别展示“产出该子 SKU 的 BOM”和“把该子 SKU 作为组件的 BOM”；同一 BOM 只展示一次，失效商品或失效 BOM 不展示。PR-430 后组件反查只读取每个生产 BOM 的当前有效版本，有草稿时看草稿，否则看最新发布版本，旧版本曾经用过该商品不会继续显示为当前使用关系。商品档案不再维护或更换生产 BOM；要修改制造结构，回到生产 BOM 中维护产出子 SKU、版本和组件清单。预期产出率不再作为用户维护字段；系统内部需要时按 `1 - 预期损耗率` 换算。
...
- “商品档案”列表默认展示工厂自营/公共商品档案，并展示稳定商品编号；有 `product_code` 时显示该编号，没有显式编号时显示 `SKU-000xxx` 兼容编号。列表不再提供“生产配置 / 更换生产 BOM / 维护 BOM / BOM”等重复按钮，也不单独展示 `BOM 使用` 列；点击商品名进入配置抽屉。抽屉维护基础资料、商品分类、销售规格模板、工艺路线、预期损耗率和行业字段，并只读展示来自销售规格模板的库存单位、BOM 使用摘要和价格摘要。列表不再维护产品级利润率覆盖或商品内价格阶梯。
...
- PR-366-GLOBAL-UNIT-TEMPLATES / PR-438-PRODUCT-TEMPLATE-DELETE-PRICE-LIST-UI / PR-439 / PR-500 / PR-504 / PR-505：基础单位在 设置 → 系统设置 → 全局单位字典 维护，例如 kg、g、lb、盒、袋、箱。PR-505 后原单位模板入口改为 `销售规格模板`：模板维护库存单位、规格名称、销售单位、净含量、默认规格和启用状态，不再维护旧报价单位/录单单位。父 SKU 在商品档案引用销售规格模板，保存后系统按模板规格自动派生子 SKU；模板修改后，对引用该模板的父 SKU 补派生或标记历史子 SKU 状态。商品价格管理不维护单位换算；商品价格表按子 SKU 发布价格，并在发布时把子 SKU 销售单位、模板库存单位和 SKU 快照固化到价格表快照。
...
- 例子：先在 设置 → 系统设置 → 全局单位字典 确认基础单位“kg”“g”“袋”存在，再在 商品档案 → 销售规格模板 新建“咖啡豆袋装规格”，库存单位选择 `kg`，添加 `227g袋装 / 袋 / 227 g` 和 `100g袋装 / 袋 / 100 g`。到 商品档案 新建父 SKU“埃塞俄比亚 水洗”，引用该销售规格模板；保存后系统自动派生 `埃塞俄比亚 水洗 227g袋装` 和 `埃塞俄比亚 水洗 100g袋装` 两个子 SKU。需要生产时，生产 BOM 选择具体子 SKU，产出单位显示继承销售规格模板的 `kg`；克重和包装差异通过 BOM 组件清单表达。需要报价时，在 商品价格管理 维护价格计算模板，再到 商品价格表 选择对应子 SKU 发布价格。
...
## BOM配置
- PR-417-MULTILEVEL-MANUFACTURING-BOM / PR-500-UNIT-MODEL-CONSOLIDATION / PR-505-SALES-SPEC-TEMPLATE-DERIVED-SKU：入口为“生产 BOM（制造主档）”，位于“生产管理”。生产 BOM 是生产端主档案，必须声明产出商品、产出数量、产出商品库存单位、组件清单、版本状态和可选工艺信息；产出单位由产出商品的有效库存单位自动带出并只读显示来源。PR-505 后子 SKU 的有效库存单位来自销售规格模板，商品档案不再主动绑定或编辑 BOM。
