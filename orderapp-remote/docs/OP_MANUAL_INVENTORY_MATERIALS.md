...
商品库存身份使用“主商品 + BOM 规格”，库存单位从 BOM 规格读取，不再从销售规格模板或默认子 SKU 读取。商品档案行内可以查看默认 BOM 的规格并进入 BOM 配置；历史规格名称、条码、单位和业务快照由迁移档案保留，不能作为新库存身份继续使用。

物料仍使用自身库存单位。商品规格权威升级期间不要手工新建销售规格模板或子 SKU；相关写接口会返回 410，并引导到 BOM 配置。
