...
- `商品档案配置` 抽屉同屏维护基础信息、商品配置模板、分类模板、生产 BOM 版本、工艺路线、预期损耗率和行业字段模板，并提供“维护当前 BOM 明细”入口跳到生产 BOM 配方库。预期产出率不再作为用户维护字段；系统内部需要时按 `1 - 预期损耗率` 换算。
- PR-393-PRODUCT-CLASSIFICATION-TEMPLATES：商品档案只引用一个分类模板，不在商品档案配置抽屉里直接选择分类项。点击“配置分类”会打开第二层分类配置抽屉，在里面维护分类项和商品归类；删除分类项时，该分类下商品回到“未分类”。同一商品在同一分类模板内只能归属一个分类。
...
- `客户商品名` 只维护客户侧展示名、客户商品编号、品牌、分类模板、排序和是否进入价格表。贴牌、客户命名和客户编号变化优先走客户商品名，不复制 BOM。
- `商品配置模板` 只维护模板规则、单位模板、阶梯价模板和分类模板。分类模板用于定义可复用的分类项集合；商品档案或客户商品名引用模板后，通过“配置分类”抽屉维护对象归类。
...
- PR-386-PRODUCT-MODEL-OVERHAUL：用户侧把原 `SKU设置` 统一称为“商品管理”。“商品档案”是真实 SKU，用于库存、生产 BOM、成本、生产和成品批次；“客户商品名”是某客户名下的对外商品名称、编号、品牌和分类模板，绑定一个商品档案。工厂自营也按一个客户-like 归属维护客户商品名，避免把销售主体拆成两套逻辑。
- PR-363-SKU-SETTINGS-WORKBENCH-LAYOUT / PR-393：商品与配方分成“商品档案”“客户商品名”“商品配置模板”等独立页面。日常维护商品档案时进入“商品档案”；需要维护分类项和商品归类时，从商品档案配置或客户商品名页面点“配置分类”打开抽屉；需要新增或修改阶梯价模板、商品配置模板时进入“商品配置模板”。旧拖拽分类树不再作为新 UI 主入口。
- “商品档案”工作区中，商品分类和商品档案列表相邻展示；桌面宽度下左侧维护商品分类/停车场，右侧维护商品档案列表，窄屏下自动上下排列。阶梯价模板和商品配置不再夹在两者中间。
- “商品配置模板”入口中维护商品配置模板、单位模板、阶梯价模板和分类模板。商品配置模板可绑定阶梯价模板、工序模板、价格表生成规则和单位模板；分类模板用于维护分类项集合。配置完成后回到“商品档案”，在商品档案配置抽屉中选择商品配置模板和分类模板。
...
- “商品档案”列表默认展示工厂自营/公共商品档案。列表不再提供“生产配置 / 更换生产 BOM / 维护 BOM / BOM”等重复按钮；点击商品名进入配置抽屉。生产 BOM、工艺路线、预期损耗率、商品配置模板、分类模板和行业字段都在抽屉内维护；需要编辑配方明细时点击“维护当前 BOM 明细”进入生产 BOM 配方库。列表中还可维护产品级利润率覆盖、产品失效和备注。
- 需要为某个客户准备销售展示时，优先在“客户商品名”中绑定已有商品档案，只改客户对外名称、客户商品编号、品牌和客户侧分类模板归属；只有配方、包装、库存对象、生产方式或成本口径变化时才创建新商品档案。
...
- 点击“公共SKU”回到公共SKU列表；客户SKU行同样可通过商品名进入商品档案配置抽屉维护备注、生产 BOM、生产配置或失效。列表筛选支持搜索关键词、产品类型和产品子类型；搜索可匹配商品名称、商品编号、历史兼容类型标签和备注。
- 商品名称不再在列表单元格内直接编辑；需要改商品档案名称时点击商品名打开配置抽屉，在基础信息中修改并保存。
- 商品分类主流程改为分类模板。公共商品档案维护公共分类模板；客户商品名维护客户侧分类模板。新增、改名、删除、排序分类项，以及商品档案/客户商品名加入或移出分类，都在“配置分类”抽屉中完成，不再以拖拽分类树作为新 UI 主入口。
- 删除客户侧分类项后，分类内客户商品名会回到未分类；后续可以在同一客户归属下重新新增同名分类。旧 `product_categories` 分类树和客户公共分类开关只作为历史兼容，价格表、录单、成本和生产计划优先读取新分类模板归属。
- 客户上下文历史兼容：删除客户自己的商品分类后，旧分类树里的商品会回到未分类；重新新增同名分类时不得触发 `product_categories_customer_parent_name_uniq`。PR-393 后新 UI 主流程改用分类模板和配置分类抽屉，但旧分类树、客户自己的商品分类和 schema 修复仍保留兼容历史数据；历史 schema 修复会自动恢复父分类或把子分类迁到同名 active 父分类。
...
- 填写客户商品名、客户商品编号、品牌名、分类模板、排序和“进入价格表”。品牌名可留空，系统不会自动填工厂品牌。客户商品名不直接选择某个分类项，需要点击“配置分类”打开分类配置抽屉后加入对应分类。
- 点击“保存客户商品名”后，系统写入客户商品名记录并写操作日志；后续可在同一表格中编辑、停用或跳转到绑定商品档案的生产 BOM。
- 如果客户要批量使用已有商品档案，点击“批量添加商品档案”打开抽屉，按商品名称、编号、产品类型或产品子类型筛选，多选后一次创建客户商品名。默认客户商品名=商品档案名称、客户商品编号=商品编号、品牌名为空、进入价格表；分类模板不选时，系统按来源商品档案的分类模板复制/复用到客户侧，并把来源商品的分类按同名分类映射到客户模板，找不到同名分类时归到未分类。也可以在批量抽屉中手动选择客户侧分类模板。同客户同商品档案已存在时自动跳过，并在结果里显示创建和跳过数量。实际创建的客户商品名都会写操作日志。
- 客户侧分类配置只影响该客户商品名，不会回写商品档案分类模板，也不会影响其他客户。需要调整客户价格表分组时，在客户商品名页点击“配置分类”，维护客户侧分类项和客户商品名归类。
...
### 旧客户 SKU 收敛检查
- PR-386 Phase 5 / PR-393：商品管理会把新增动作拆成“创建客户商品名”和“创建新商品档案”。贴牌、客户命名、客户编号、客户侧分类或客户专属价格变化时，优先创建客户商品名；配方、包装、生产方式、库存对象或成本口径变化时，才创建新商品档案并维护生产 BOM。
