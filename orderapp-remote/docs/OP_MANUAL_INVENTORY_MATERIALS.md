...
- PR-453-GROUP-TEMPLATE-SYSTEM-SETTINGS / PR-455-GROUP-TEMPLATE-DELETE / PR-458-GROUP-TEMPLATE-BUSINESS-LISTING / PR-528-SEPARATE-GROUP-TEMPLATE-SYSTEM-SETTINGS：商品档案、生产 BOM 和仓库库存统一使用 `设置 / 分组模板` 独立页面。分组模板是系统级基础资料，只维护模板名、大类和小类；模板页不展示系统设置、对象数量、对象列表、勾选对象或对象移动入口。模板没有启用/停用开关；不再使用的模板在编辑模板时点击 `删除模板`，删除后该模板、分类项、用途和对象归类会被移除，新业务下拉不再出现该模板。
...
## 物料档案
- PR-513-MATERIAL-CLASSIFICATION-TEMPLATE / PR-528：物料档案分类使用 `设置 → 分组模板`。在物料列表选择分组模板后，页面按该模板的大类、小类和 `未分类` 展示物料；勾选物料后选择目标分类并点击 `移动到分类`，会把本次物料归类写入通用 `business_group_assignments`（`usage_key=material_catalog`、`object_key=material`）。物料页不再直接新增分类或小分类；需要维护分类结构时进入独立分组模板页面。旧物料大类/小类会迁移为 `物料档案默认分组` 以兼容历史数据。
...
- PR-453 / PR-502 / PR-505 / PR-506-PRICE-LIST-SPEC-DEFAULT-ROW-ERRORS / PR-528：商品与配方菜单保持独立入口：`商品档案`、`客户商品`、`商品价格管理`、`商品价格表`；分组模板入口统一在 `设置 → 分组模板`，不嵌入系统设置；`生产 BOM` 位于生产管理，`行业字段模板` 位于 `设置 → 行业设置`。旧 `商品配置和分类模板`、`阶梯价模板` 只作为历史兼容或迁移排查入口；当前商品规格入口为 `销售规格模板`，可从商品档案引用和维护。进入 `商品档案` 后，在商品列表工具栏点击 `维护销售规格模板` 可进入模板维护页；模板明细支持设置默认规格，保存后默认销售单位使用选中规格名称。
...
- PR-458 / PR-528 后的当前口径：生产 BOM 分类不再有默认分组，也不再有 BOM 专用大组/组内分类。生产 BOM 页面先选择 `分组模板`，列表会按模板完整大类 / 小类树和 `未分类` 自动整理，空分类也显示，不再通过分类 Tab 过滤。用户在独立 `设置 / 分组模板` 页面维护大类和小类，再回到生产 BOM 用 `移动到分类` 把 BOM 移到 `未分类`、大类或小类。一个 BOM 的归类保存到 `business_group_assignments`，不改变产出商品、BOM 版本或配方明细。
