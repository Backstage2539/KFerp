...
- PR-439-PRODUCT-PRICE-MASTER-REMODEL / PR-536-PRODUCT-INDUSTRY-TEMPLATE-ONLY：复制为商品档案只复制基础资料、库存单位和整数库存，不复制行业字段模板、行业字段值、BOM、价格、价格表快照或客户商品关系。需要新生产结构时，在生产 BOM 中复制或新建 BOM 并把新商品设为产出；需要新报价时，在商品价格管理和商品价格表发布新版。
...
- PR-370-UNIT-TEMPLATE-DRAWER-SKU-CATEGORY-LAYOUT / PR-382-SKU-UNIFIED-CREATE-COPY / PR-536-PRODUCT-INDUSTRY-TEMPLATE-ONLY 历史兼容：旧商品分类树、旧“新增SKU”和旧“SKU复制”是历史口径；旧 SKU复制 批量结果曾展示“新增 X 款、覆盖 Y 款、跳过 Z 款”。新 UI 中创建商品档案只保留顶部“创建新商品档案”按钮；“复制为商品档案”只复制来源商品档案的基础信息、库存单位、整数库存、工艺路线和预期损耗率，不复制行业字段模板、行业字段值、BOM、价格、价格表快照、客户商品关系或旧模板绑定；如生产定义也要复制，需在生产 BOM 中复制旧 BOM 并把新商品设为产出。历史 SKU 复制入口已下线。
