...
- `商品档案配置` 抽屉同屏维护基础信息、商品配置模板、工艺路线、预期损耗率和行业字段模板，并显示生产反查：哪些 BOM 可以生产该商品、哪些上层 BOM 把该商品作为组件消耗。商品档案不再维护或更换生产 BOM；要修改制造结构，回到生产 BOM 中维护产出商品、版本和组件清单。商品档案本身不再选择分类模板；分类模板只作为列表页启用的分类视图。预期产出率不再作为用户维护字段；系统内部需要时按 `1 - 预期损耗率` 换算。
...
- “商品档案”列表默认展示工厂自营/公共商品档案，并展示稳定商品编号；有 `product_code` 时显示该编号，没有显式编号时显示 `SKU-000xxx` 兼容编号。列表不再提供“生产配置 / 更换生产 BOM / 维护 BOM / BOM”等重复按钮；点击商品名进入配置抽屉。抽屉维护工艺路线、预期损耗率、商品配置模板和行业字段，并只读展示可生产 BOM 与被哪些 BOM 使用。列表中还可维护产品级利润率覆盖、产品失效和备注。
...
- 点击“公共SKU”回到公共SKU列表；客户SKU行同样可通过商品名进入商品档案配置抽屉维护备注、商品配置模板、工艺路线、行业字段或失效，并查看生产 BOM 反查。列表筛选以商品名称、商品编号、历史兼容类型标签和备注为主；搜索可匹配商品名称、商品编号、历史兼容类型标签和备注。产品类型/产品子类型只作为历史兼容字段保留。
...
- BOM 详情显示两个反查区：当前 BOM 产出商品会被哪些上层 BOM 使用；商品档案详情也只读显示“可生产 BOM”和“被哪些 BOM 使用”。不在商品档案里编辑 BOM。
...
- PR-374-COMPOSABLE-PRODUCT-PRICING 兼容口径：速溶咖啡条装原料、条包装膜、彩盒和标签仍通过 BOM 组件进入每盒成本；盒装行可用 `个/盒`、`unit_per_box`、`unit_per_bag` 或 `g_per_bag` 表达每报价单位用量。PR-417 后该例子拆成“条装速溶咖啡”和“10条盒装速溶咖啡”两个产出 BOM，不再让商品档案绑定 BOM。
...
- BOM 页面组件表单、袋材映射表单和版本备注使用会话内草稿；切到其他功能再回来会恢复，刷新浏览器后清空。
...
- 非最新版提示：一次性订单、合同约定配方或客户指定历史配方时，工单可继续选择旧的已发布 BOM 版本；系统不自动升级已冻结工单。商品档案只显示可生产 BOM 和上层使用关系。
