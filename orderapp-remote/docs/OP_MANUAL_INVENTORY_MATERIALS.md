...
- 商品档案归组使用 `product_catalog`，展示为“分组集 / 父组 / 子组”；商品保存和复制不再写旧 `product_category_id`、`classification_template_id` 或旧分类模板 assignment。
- 生产 BOM 归组使用 `production_bom`。列表中的全部分组、未分类、移动到分组和组内分类展示都来自分组管理；旧 `production_bom_groups` / `production_bom_group_categories` 只用于历史只读兼容。
- 仓库库存分组对象是仓库。仓库设置保存 `warehouse_inventory` 归组，使用仓库 code 作为对象引用；库存行仍按仓库、物品和批次展示，分组只影响左侧仓库组织和筛选，不改变库存数量、批次、成本或追溯。
