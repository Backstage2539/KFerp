...
- PR-370-UNIT-TEMPLATE-DRAWER-SKU-CATEGORY-LAYOUT / PR-382：商品分类维护是低频配置，入口位于 SKU设置 → 商品配置 → 商品分类管理。进入后可在分类管理页搜索、折叠、排序、新增或删除当前归属下的产品类型/产品子类型；产品类型标题和上下排序按钮在左侧，折叠/展开和删除动作在右侧。SKU 列表右上角不再放旧“分类设置”，只保留日常“新增SKU”和“SKU复制”。单位模板左侧是模板列表，右侧是新增或编辑表单；在单位模板页点击“全局单位字典”可用右侧抽屉维护基础单位。
...
- 客户需要复用公共配置后再修改时，点击公共分类的“复制为客户分类”、公共梯度模板的“复制为客户模板”、公共商品配置的“复制为客户配置”，或通过“SKU复制”把公共 SKU/其他客户 SKU 复制成当前客户 SKU；系统会先生成客户自己的分类、SKU、梯度模板或商品配置，再允许改名、排序、维护 BOM 和绑定模板，不会直接改公共数据。
