...
- 客户需要复用公共或其他客户的 SKU 与分类结构时，使用“SKU复制”把公共 SKU/其他客户 SKU 复制成当前客户 SKU；系统会自动补齐目标客户缺少的产品类型和产品子类型，同名同分类 SKU 按覆盖规则更新。商品分类管理里不再提供“复制为客户分类”，该页只维护当前归属已拥有的分类。客户需要复用公共梯度模板或公共商品配置时，仍分别使用“复制为客户模板”和“复制为客户配置”，复制后再修改，不会直接改公共数据。
...
- 在仓库列表下方的“绑定客户”区域选择客户并点击“保存绑定”；保存后该仓库会记录绑定客户，列表显示“绑定客户：客户名”。
- 需要解除绑定时，把绑定客户清空后保存。仓库绑定或解绑属于用户触发的库存配置变更，必须能在“操作日志”中按仓库查到旧客户和新客户。
- 门户客户配置不再维护“代加工仓库”。仓库绑定客户后，客户管理 → 门户客户配置 展开对应客户时只展示该客户已绑定的客户仓库；仓库归属统一回到仓库库存页修改。
