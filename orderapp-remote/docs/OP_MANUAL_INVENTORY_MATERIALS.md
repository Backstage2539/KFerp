...
- PR-366-GLOBAL-UNIT-TEMPLATES / PR-438-PRODUCT-TEMPLATE-DELETE-PRICE-LIST-UI / PR-439 / PR-500：基础单位在 设置 → 系统设置 → 全局单位字典 维护，例如 kg、g、lb、盒、袋、箱。创建新商品档案和商品档案配置抽屉都直接从全局单位字典选择库存单位，并可开启整数库存；没有手工选择时默认显示 `kg`。商品档案只维护一个库存单位和整数库存限制；报价单位、录单单位、销售包装单位和价格单位属于商品价格记录和价格表快照。价格单位到库存单位的换算必须在价格记录或快照中固化，没有换算时禁止发布或录单。
