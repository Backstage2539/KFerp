...
- PR-363-SKU-SETTINGS-WORKBENCH-LAYOUT：SKU设置页分成“商品资料”和“模板配置”两个工作区。日常新增 SKU、拖拽分类、查看停车场和维护客户SKU列表时停留在“商品资料”；需要新增或修改梯度模板、商品配置时切到“模板配置”。
- “商品资料”工作区中，商品分类和客户SKU列表相邻展示；桌面宽度下左侧维护商品分类/停车场，右侧维护 SKU 列表，窄屏下自动上下排列。梯度模板和商品配置不再夹在两者中间。
- “模板配置”工作区中，先维护梯度模板，再维护商品配置；商品配置可绑定梯度模板、工序模板、价格表生成规则和单位规则。配置完成后回到“商品资料”，在产品子类型行选择“更换商品配置”完成绑定。
...
- PR-362-PRODUCT-CONFIG-TEMPLATE：SKU设置 的“商品配置”是统一模板入口，配置里维护阶梯价模板、工序模板、价格表生成规则、库存单位、报价单位、录单单位、单位换算和整数单位。公共配置和客户配置都是独立模板，可点“复制为客户配置”后再调整。产品子类型行点击“更换商品配置”只选择模板，不再让用户填写 JSON，也不再在这里决定是否纳入产品价格表。
- 例子：先在“商品配置”新建“盒装速溶配置”，选择阶梯价模板，设置库存单位 kg、报价单位盒、录单单位盒，通过“新增换算”维护 `1 盒 = 0.2 kg`，开启整数单位并保存；再到“商品分类”里找到“速溶咖啡 / 冻干速溶”，选择该商品配置并保存。后续生成产品价格表、录单默认单位和生产折算会读取该配置；已发布价格表和历史订单不会被回改。
