...
- PR-374-COMPOSABLE-PRODUCT-PRICING：速溶咖啡等非烘焙产品的 BOM 可以按报价单位配置。例：速溶盒装直接进货条装原料，1 条 10g，1 盒 10 条；在物料档案建“速溶咖啡条装原料”，单位“条”，默认采购价填每条成本；在成品 SKU 的 BOM 中选择该物料，消耗单位选“个/盒”，用量填 10。盒子、标签等包材也用“个/盒”维护。成本核算会把这些 `个/盒`、`个/袋`、`克/袋` 用料汇总为每盒 BOM 成本，供产品价格表按阶梯价模板生成 `元/盒`。
