...
5. 在 BOM 配方维护中设置商品所需物料、用量和启用版本。
   - PR-511-BOM-MATERIAL-LOSS-BOM-LEVEL / PR-592-BOM-LOSS-GROSS-INPUT：BOM 版本设置中只维护一个 `原料损耗比`；库存准备、WIP 占用和生产消耗统一按 `净配方数量 ÷ (1 - 原料损耗率)` 执行。例如产出基准 `1kg`、原料比例 `40%`、原料损耗 `20%` 时，需要准备 `0.50kg` 原料。未配置损耗时仍为 `0.4kg`。
