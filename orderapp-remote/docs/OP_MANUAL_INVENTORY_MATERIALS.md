...
## 物料档案
- 工厂／客户视图只控制物料列表和标题。新建物料时必须显式选择“仅工厂使用”或“关联一个或多个客户”；选择客户时可多选，主档和关联在同一事务保存。视图不会自动把当前客户写入物料。
- 工厂视图显示全部物料，可按“已关联客户／仅工厂使用”筛选；每行“客户关联”可新增、停用或恢复关联。客户视图与客户账号只显示该客户有效关联的物料，不能通过查询参数取得其他客户物料。
- 物料客户关联只决定档案可见性和业务候选范围，不会转移库存或成本。实际数量仍按仓库与 `owner_customer_id` 区分；需要补录客户来料时使用客户库存单据并选择绑定客户仓库。
...
- 物料页的“物料归属”同样支持客户名称模糊搜索；选择客户后只展示已复制给该客户的物料。
- 物料行使用蓝色文字“复制到客户”。一个物料可以复制给多个客户；成功提示会引导回“物料归属”搜索客户名称。
- 物料归属只决定档案可见性和 BOM 候选，不移动库存、批次或成本。客户库存仍以仓库和货主为准。
