...
5. 在 BOM 配方维护中设置商品所需物料、用量和启用版本。
   - PR-510-BOM-MATERIAL-LOSS-BOM-LEVEL：BOM 版本设置中打开 `原料损耗比` 后，组件消耗单位只能使用 `比例 %`；库存准备、WIP 占用和生产消耗都会按损耗后需求量执行。例如产出基准 `1kg`、原料比例 `40%`、原料损耗 `20%` 时，需要准备 `0.5kg` 原料，而不是 `0.4kg`。固定用量和商品组件需要先关闭原料损耗比。
