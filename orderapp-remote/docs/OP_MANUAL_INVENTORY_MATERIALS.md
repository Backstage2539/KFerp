...
- PR-366-GLOBAL-UNIT-TEMPLATES / PR-438-PRODUCT-TEMPLATE-DELETE-PRICE-LIST-UI / PR-439 / PR-500 / PR-501 / PR-502 / PR-504：基础单位在 设置 → 系统设置 → 全局单位字典 维护，例如 kg、g、lb、盒、袋、箱。单位模板是商品 UOM 主数据模板，只维护库存单位、默认销售单位、销售单位换算和整数规则，例如 `1 箱 = 12 袋` 或 `1 lb = 0.453592 kg`；单位模板不表达 `227g`、`100g` 这类规格。创建新商品档案和新增子 SKU 时必须选择单位模板；模板修改后，对引用该模板且没有历史覆盖的子 SKU 实时生效。商品价格管理不维护单位换算；商品价格表只选择价格单位，并在发布时把子 SKU 当时的有效库存单位和换算固化到价格表快照。没有价格单位到库存单位换算时禁止发布或录单。
