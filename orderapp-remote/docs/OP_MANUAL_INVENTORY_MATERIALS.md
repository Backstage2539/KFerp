...
## 商品管理与商品档案
- PR-439-PRODUCT-PRICE-MASTER-REMODEL / PR-504-PARENT-PRODUCT-CHILD-SKU-UOM / PR-505-SALES-SPEC-TEMPLATE-DERIVED-SKU：商品档案是父 SKU 入口，维护商品资料、库存单位、销售规格模板、分组、客户引用和行业字段；具体销售/库存规格由模板自动派生的子 SKU 表达，例如 `227g袋装`、`100g袋装`、`散装kg`。商品档案配置抽屉只读展示 BOM 使用摘要和价格摘要。价格摘要来自商品价格表快照，没有已发布快照时显示 `暂无价格表价格`。商品档案普通表单不再维护商品配置模板、报价单位、录单单位、阶梯价模板、计价方式、固定价、成本加成或利润率覆盖；这些旧字段仅用于历史数据兼容或迁移报告。
- PR-439 历史验收口径曾写“商品档案只维护商品资料、库存单位、规格、整数限制和行业字段”；PR-502 在这个口径上把“库存单位/默认销售单位”收敛为单位模板引用和高级覆盖，不恢复旧价格/配置模板入口。
- 商品档案配置抽屉的“销售规格 / SKU”区块只读展示同一父 SKU 下由销售规格模板派生出的具体子 SKU；子 SKU 不手工新增，不配置库存单位，也不直接维护商品级单位换算。要增加 `227g袋装`、`100g袋装` 等规格，先维护销售规格模板，再保存引用该模板的父 SKU，系统按模板规格自动补派生子 SKU。抽屉的“被哪些 BOM 使用”只读展示产出该子 SKU 或把该子 SKU 作为组件消耗的生产 BOM，并显示 `BOM状态`：`默认状态` 表示该 BOM 是产出商品当前默认 BOM；`启用状态` 表示 BOM 可继续作为生产/成本来源；`失效状态` 表示 BOM 已失效，仅用于历史追溯，不能作为新生产或新报价的有效来源。销售规格模板修改只影响后续新建或编辑的 BOM、工单和库存流转，不自动回改已发布 BOM 版本、历史工单、历史库存流水或历史价格表快照。
...
- PR-453 / PR-502 / PR-505：商品与配方菜单保持独立入口：`商品档案`、`客户商品`、`商品价格管理`、`商品价格表`；分组模板入口统一在 `设置 → 系统设置 → 分组模板`；`生产 BOM` 位于生产管理，`行业字段模板` 位于 `设置 → 行业设置`。旧 `商品配置和分类模板`、`阶梯价模板` 只作为历史兼容或迁移排查入口；当前商品规格入口为 `销售规格模板`，可从商品档案引用和维护。进入 `商品档案` 后，在商品列表工具栏点击 `维护销售规格模板` 可进入模板维护页。
- `商品档案` 是库存、销售、成本、成品批次和价格对象。列表第一业务列是商品名；商品名本身就是配置入口，默认不在表格内编辑，点击商品名打开“商品档案配置”抽屉。
- `商品档案配置` 抽屉同屏维护基础信息、库存单位、销售规格模板、销售规格 / SKU、工艺路线、预期损耗率和行业字段模板，并只读显示 BOM 使用摘要和价格摘要。BOM 摘要分别展示“产出该子 SKU 的 BOM”和“把该子 SKU 作为组件的 BOM”；同一 BOM 只展示一次，失效商品或失效 BOM 不展示。PR-430 后组件反查只读取每个生产 BOM 的当前有效版本，有草稿时看草稿，否则看最新发布版本，旧版本曾经用过该商品不会继续显示为当前使用关系。商品档案不再维护或更换生产 BOM；要修改制造结构，回到生产 BOM 中维护产出子 SKU、版本和组件清单。预期产出率不再作为用户维护字段；系统内部需要时按 `1 - 预期损耗率` 换算。
...
- 新增商品档案表单填写父 SKU 名称、库存单位、销售规格模板、备注等基础信息；同类商品优先选择已有销售规格模板，模板会展示将自动派生的销售规格明细。具体子 SKU 由模板规格自动派生，不在商品档案里手工新增；不要把 `袋/227g` 写成销售单位，也不要为每个克重复制一套模板。不再选择分类模板、产品类型、产品子类型、商品配置模板或阶梯价模板；也不把“贴牌”“受托加工”写成底层 SKU 类型。新增成功后系统会自动打开该商品的“商品档案配置”抽屉，便于立刻确认派生子 SKU、工艺路线和行业字段；如某个子 SKU 需要生产，再到生产 BOM 新建或复制 BOM 并把它设为产出子 SKU。
...
- PR-366-GLOBAL-UNIT-TEMPLATES / PR-438-PRODUCT-TEMPLATE-DELETE-PRICE-LIST-UI / PR-439 / PR-500 / PR-504 / PR-505：基础单位在 设置 → 系统设置 → 全局单位字典 维护，例如 kg、g、lb、盒、袋、箱。PR-505 后原单位模板入口改为 `销售规格模板`：模板只维护规格名称、销售单位、净含量、默认规格和启用状态，不维护库存单位或销售单位换算。父 SKU 在商品档案维护库存单位并引用销售规格模板，保存后系统按模板规格自动派生子 SKU；模板修改后，对引用该模板的父 SKU 补派生或标记历史子 SKU 状态。商品价格管理不维护单位换算；商品价格表按子 SKU 发布价格，并在发布时把子 SKU 销售单位、父 SKU 库存单位和 SKU 快照固化到价格表快照。
...
- PR-371-SKU-UNIT-TEMPLATE-COMPACT-ACTIONS / PR-436-BOM-UNIT-DELETION-POLISH / PR-438-PRODUCT-TEMPLATE-DELETE-PRICE-LIST-UI / PR-505：商品管理顶部区域已压缩，当前归属和统计信息在一行内查看。销售规格模板页未选模板时，右下角按钮为“新增”；点击左侧模板进入编辑后右下角按钮为“保存”，并可删除该销售规格模板。点击左上角“新增销售规格模板”会清空表单回到新增状态。全局单位字典抽屉和 设置 → 系统设置 → 全局单位字典 也采用同样交互：点击“新增基础单位”清空表单，新增时按钮显示“新增”，编辑已有单位时显示“保存”，并可删除该基础单位。销售规格模板、全局单位字典、分组模板和商品配置模板删除后不再展示；删除不等于失效，历史商品配置、价格表、订单、BOM、工单和成本快照不回改，新建/编辑候选不再显示已删除记录。
- 例子：先在 设置 → 系统设置 → 全局单位字典 确认基础单位“kg”“g”“袋”存在，再在 商品档案 → 销售规格模板 新建“咖啡豆袋装规格”，添加 `227g袋装 / 袋 / 227 g` 和 `100g袋装 / 袋 / 100 g`。到 商品档案 新建父 SKU“埃塞俄比亚 水洗”，库存单位选择 `kg`，引用该销售规格模板；保存后系统自动派生 `埃塞俄比亚 水洗 227g袋装` 和 `埃塞俄比亚 水洗 100g袋装` 两个子 SKU。需要生产时，生产 BOM 选择具体子 SKU，产出单位显示继承父 SKU 的 `kg`；克重和包装差异通过 BOM 组件清单表达。需要报价时，在 商品价格管理 维护价格计算模板，再到 商品价格表 选择对应子 SKU 发布价格。
