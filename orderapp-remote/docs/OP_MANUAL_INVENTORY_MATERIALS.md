...
- PR-366-GLOBAL-UNIT-TEMPLATES / PR-438-PRODUCT-TEMPLATE-DELETE-PRICE-LIST-UI / PR-439 / PR-500 / PR-501：基础单位在 设置 → 系统设置 → 全局单位字典 维护，例如 kg、g、lb、盒、袋、箱。创建新商品档案和商品档案配置抽屉都直接从全局单位字典选择库存单位，并可开启整数库存；没有手工选择时默认显示 `kg`。商品档案维护一个库存单位，同时维护默认销售单位、销售单位换算和整数销售单位规则，例如 `1 盒 = 0.2 kg`。商品价格管理不维护单位换算；商品价格表只选择价格单位，并在发布时把商品档案当时的库存单位和换算固化到价格表快照。没有价格单位到库存单位换算时禁止发布或录单。
...
- 例子：先在 设置 → 系统设置 → 全局单位字典 确认基础单位“kg”和“盒”存在。到 商品档案 新建“冻干速溶 200g 盒装”，库存单位选择 kg，默认销售单位选择盒，在销售单位换算里维护 `1 盒 = 0.2 kg`，按需开启整数库存或整数销售单位限制；如需按盒报价，在 商品价格管理 维护价格计算模板，再到 商品价格表 选择价格单位盒并发布。发布时后端会把商品档案当时的 `1 盒 = 0.2 kg` 固化到价格表快照。发布后，录单单位、订单价格和生产需求换算来自已发布快照；已发布价格表和历史订单不会被回改。
