...
## PR-453 分组模板新口径
- PR-453-GROUP-TEMPLATE-SYSTEM-SETTINGS：商品档案、生产 BOM 和仓库库存统一使用 `系统设置 / 分组模板`。分组模板是系统级基础资料，只维护模板名、大类和小类；模板页不展示对象数量、对象列表、勾选对象或对象移动入口。
...
- 生产 BOM 归类使用 `production_bom`。生产 BOM 页面不再维护自己的大组、组内分类或小分类，也不再显示 `使用分组`。用户先选择启用的分组模板，再通过 `移动到分类` 把 BOM 移到 `未分类`、大类或小类。顶部分组 Tab 只展示当前所选模板下当前 BOM 列表实际使用过的分类项。旧 `production_bom_groups` / `production_bom_group_categories` 只用于历史只读兼容。
