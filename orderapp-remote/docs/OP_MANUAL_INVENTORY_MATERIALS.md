...
- PR-588 后四个业务列表即使未选择任何分组模板也保留共享分类工作区：左树只显示 `全部分类`，右侧按原列表平铺，`移动到分类` 禁用，左树底部的 `前往分组模板` 和 `设置分组模板` 仍可使用。选择模板后才增加模板、大类、小类和未分类节点。
- 商品档案归类使用 `product_catalog`。PR-584 允许在商品档案页面多选要使用的分组模板；PR-588 后页面左侧常驻分类结构，按“全部分类 → 模板 → 大类 → 小类”浏览，空分类和统一 `未分类` 都保留。勾选商品后点击 `移动到分类`，再直接点击左树中的大类、小类或未分类即完成移动，不再先选目标模板，也不二次确认；移动会覆盖旧归类并写操作日志。
- 物料档案归类使用 `material_catalog`。物料档案多选本功能使用的分组模板后，左侧分类结构合并展示全部已选模板；右侧继续保留名称/编码/批次号搜索和启用状态过滤。勾选物料后点击 `移动到分类`，再直接点击目标大类、小类或未分类；清空模板选择后左树回到 `全部分类`、右侧平铺且移动禁用，底部设置入口保留。取消选择不删除物料、模板、分类或既有归类。
- 生产 BOM 归类使用 `production_bom`。生产 BOM 页面不再维护自己的大组、组内分类或小分类，也不再显示 `使用分组`。左侧分类结构合并展示已选模板，右侧继续保留状态与 BOM 名称/编号搜索；勾选 BOM 后点击 `移动到分类`，再直接点击目标大类、小类或未分类。清空模板选择后左树仅显示 `全部分类`、右侧平铺且移动禁用，底部设置入口保留。旧 `production_bom_groups` / `production_bom_group_categories` 只用于历史只读兼容。
- PR-588 取代 PR-458 的仓库 code 归类口径：仓库库存仍用 `warehouse_inventory`，但当前归类对象是选中仓库内的物品/规格，使用 `object_key=warehouse_inventory_item` 和精确 `object_ref=<warehouse code>:<item_type>:<item_id>:<spec_g>`。warehouse code 只用于区分仓库命名空间，实际 identity 是物品/规格；同一 identity 的多个批次共享一次归类。外层仓库列表保持不变；选中具体仓库且非客户库存上下文时，右侧仓内物品显示分类工作区，可勾选物品/规格后移动到大类、小类或未分类。仓库先按 `q/warehouse/item_type/customer_id/page/limit` 取得 stock API 当前服务端页，左树分类只过滤该页 `rows`，不发送 `group_id/group_item_id`，`total/page/limit` 不变。全部仓库和客户库存上下文仅在分类层面平铺且不可勾选移动，既有 WIP/追溯等上下文能力不变；分类不改变库存数量、批次、成本或追溯。PR-442/PR-458 的 warehouse-code `group_id/group_item_id` 查询只保留历史兼容，不作为 PR-588 当前 UI。
...
- PR-588 后，未引用任何分组模板时左侧分类结构只显示 `全部分类`，右侧按平铺列表展示，`移动到分类` 禁用；仍可从左树底部进入模板设置。选择保存和取消选择都由商品档案发起并写操作日志；分组模板页面保存模板名称或分类树不会改变商品档案已经选择的模板。
- PR-595-UNIFIED-CATEGORY-MOVE-INTERACTION：移动商品时先勾选商品并点击 `移动到分类`，左侧分类树自动展开并提示 `请选择要移动到的分类`；直接点击目标大类、小类或 `未分类` 后立即移动。`全部分类` 和模板标题只用于浏览，不能作为目标。移动继续表示商品当前唯一归类，不会让同一商品在多份模板下重复出现；移动和清除归类仍通过正式接口写入操作日志。
...
- 结果校验：刷新商品档案后，行业字段顺序与所选模板顺序一致；分组区只出现已选模板；无选择时左树仅保留 `全部分类` 和底部模板入口，右侧为普通平铺列表且移动禁用；大类收起时看不到任何后代小类标题或商品行。再进入商品价格表，确认商品类型与商品档案已选模板一一对应。异常时先检查模板是否启用、商品档案选择是否保存成功，再到操作日志核对模板选择和商品配置保存记录。
...
- 生产 BOM 页面点击左树底部 `前往分组模板` 打开独立分组模板页面维护大类和小类。回到生产 BOM 后通过 `设置分组模板` 选择模板，左树自动出现模板里的大类/小类和 `未分类`；勾选 BOM 并点击 `移动到分类` 后，直接点击左树中的大类、小类或未分类立即移动，不再显示目标下拉或二次确认。目标为具体分类项时写入 `business_group_assignments`，目标为 `未分类` 时移除该 BOM 的 `production_bom` 归类。未选择模板时左树仅显示 `全部分类`、右侧平铺且移动禁用；新建、编辑或复制 BOM 只维护名称、产出商品、产出数量单位和状态，不写旧 `production_boms.group_id/group_category_id`。
