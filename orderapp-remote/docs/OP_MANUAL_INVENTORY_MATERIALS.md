...
- “商品档案”列表默认展示工厂自营/公共商品档案，可在同一列表维护工艺参数、生产 BOM 预期产出率、生产 BOM 使用方式、生产 BOM 状态、BOM 入口、产品失效和最右侧备注；完整的预期损耗率在 BOM 配方维护中维护。
...
- PR-382-SKU-UNIFIED-CREATE-COPY / PR-385 / PR-386：历史“SKU复制”仍保留兼容。复制后的商品档案默认不复制 BOM 明细，而是使用来源商品档案当前有效生产 BOM；列表的“生产 BOM”会显示“跟随默认 BOM：来源编号 来源名称 / BOM 版本”。只有客户需要改配方时，进入 BOM 配方维护点击“复制为单独维护 BOM”。复制结果提示新增 X 款、覆盖 Y 款、跳过 Z 款。停用商品显示为已停用且不可选。
...
## BOM配置
- 进入 BOM 配方维护后，顶部归属默认工厂自营/公共商品档案；商品 BOM 列表和商品选择显示该归属下可维护生产 BOM 的商品档案。
- PR-387-VIEW-CONTEXT：客户视图或订单视图只作为 BOM 页面筛选和跳转辅助。生产 BOM 仍属于商品档案，不属于客户商品名；如果客户只是贴牌改名，保持客户商品名绑定原商品档案即可。
- 生产 BOM 是商品档案的制造定义，不是客户商品名的展示资料。客户商品名只绑定商品档案，不能直接编辑 BOM；如果客户只是贴牌改名，保持绑定原商品档案即可。
- PR-385-SKU-BOM-INHERITANCE / PR-386：历史复制商品得到的新商品档案默认使用来源商品档案当前有效生产 BOM，生产 BOM 列显示“跟随默认 BOM：SKU-xx 来源名称 / BOM 版本”。跟随或固定的生产 BOM 是只读的，预期损耗率、组件、版本启用和失效不能直接改；如果要为当前商品档案改配方，点击详情顶部“复制为单独维护 BOM”，系统会把当前有效 BOM 明细复制到该商品档案，并显示“单独维护 BOM，基于：SKU-xx 来源名称 / BOM 版本”。来源编号、名称和版本号是快照，来源商品档案改名或 BOM 升级不会改历史说明。
- “复制为单独维护 BOM”适合受托加工、客户专属配方、客户专属包装等场景；贴牌只改客户商品名、配方完全相同的场景，保持绑定原商品档案和生产 BOM 即可，避免生成大量重复 BOM。
...
- 客户改配方：配方或理论损耗不同，创建新商品档案或对历史兼容商品执行“复制为单独维护 BOM”，后续成本、生产计划和工单用新生产 BOM。
- 固定 BOM 版本：一次性订单、合同约定配方或客户指定历史版本时，可把生产 BOM 固定到某个版本；工单开工后仍会冻结当时版本快照。
