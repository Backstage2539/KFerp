...
- `商品档案配置` 抽屉同屏维护基础信息、商品配置模板、工艺路线、预期损耗率和行业字段模板，并只读显示“被哪些 BOM 使用”。该列表展示产出该商品的有效生产 BOM，也展示把该商品当作组件消耗的上层有效生产 BOM，并用关系标签区分；同一 BOM 只展示一次，失效商品或失效 BOM 不展示。商品档案不再维护或更换生产 BOM；要修改制造结构，回到生产 BOM 中维护产出商品、版本和组件清单。商品档案本身不再选择分类模板；分类模板只作为列表页启用的分类视图。预期产出率不再作为用户维护字段；系统内部需要时按 `1 - 预期损耗率` 换算。
...
- BOM 详情显示当前 BOM 产出商品会被哪些上层 BOM 使用；商品档案详情只读显示“被哪些 BOM 使用”，同时列出产出该商品的有效 BOM 和把该商品作为组件消耗的上层有效 BOM。不在商品档案里编辑 BOM。
