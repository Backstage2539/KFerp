...
- PR-406 后页面只保留“商品 BOM列表”作为主列表，不再有顶部 `SKU归属` 或独立商品选择区。“商品 BOM列表”标题下方同一过滤行放商品过滤、状态过滤、BOM 名称/编号/商品搜索和 `批量失效`；商品过滤支持按商品编号/名称模糊搜索。分类 Tab 行左侧展示“全部分组 / 未分类 / 自定义分组”，右侧放 `新建商品 BOM`。BOM 列表可单独上下滚动，右侧编辑详情与列表主体对齐。点击已绑定商品的 BOM 名称会加载该商品的配方明细；未绑定商品的 BOM 名称会打开生产 BOM 档案编辑抽屉。
- 商品 BOM列表会合并已绑定商品的 BOM 和未绑定商品的生产 BOM。未绑定商品的 BOM 在商品列显示“未绑定商品”，仍可勾选、复制、失效和移动分组。点击 BOM 名称打开抽屉编辑；操作列提供 `BOM版本`、`规格袋材映射`、`复制` 和红色 `失效`。复制来源 BOM 即使已失效也可以复制，新 BOM 默认启用。
