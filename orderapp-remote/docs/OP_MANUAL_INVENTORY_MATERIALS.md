...
## 适用范围
物料档案、原料入库、原料批次、库存流水、库存批次、库存调整单、商品档案、客户商品、商品分类管理、商品价格管理、生产 BOM 制造主档。

## PR-442 分组管理新口径
- PR-442-BUSINESS-GROUP-OBJECT-UNIFICATION：商品档案、生产 BOM 和仓库库存统一使用 `分组管理`。分组是泛化主数据，不再让这些页面分别维护商品分类树、生产 BOM 专用大组/组内分类或库存专用分类表。
- 商品档案归组使用 `product_catalog`，列表按用户维护的业务分组路径（父组 / 子组）分段展示；父组和子组都可以形成分组标题，子组标题缩进显示并只显示子组名，完整父/子路径作为上下文，商品行跟随所在父组/子组缩进。勾选商品后通过“目标分组”移动到已有分组项，“目标分组”可选父组或子组；选择子组会把商品移动到该子类。普通页面不显示迁移兼容用的“商品默认分组”等系统分组集名称，也不显示“商品分组 /”这类分组集名称前缀，不把系统默认迁移分组里的旧分组项作为分组管理内容或“目标分组”候选；系统默认迁移归组在普通商品列表按未分组处理。商品保存和复制不再写旧 `product_category_id`、`classification_template_id` 或旧分类模板 assignment。
- 生产 BOM 归组使用 `production_bom`。生产 BOM 页面不再维护自己的大组、组内分类或小分类；需要复用某套通用分组时，先在 `分组管理` 维护分组项，再回到生产 BOM 工具区点击左侧 `使用分组`，把“可用分组”启用到生产 BOM。使用后，`目标分组` 下拉展示该套分组下的父组 / 子组；顶部分组 Tab 只展示当前 BOM 列表中实际归组使用过的分组项，没被 BOM 使用的分组项不展示。业务标签只显示父组 / 子组路径，不显示“商品分组 /”这类分组集名称。生产 BOM 表格不再单独展示“分组”列，分类查看由顶部 Tab 和当前列表范围体现。旧 `production_bom_groups` / `production_bom_group_categories` 只用于历史只读兼容。
- 仓库库存分组对象是仓库。仓库设置保存 `warehouse_inventory` 归组，使用仓库 code 作为对象引用；普通页面只显示库存分组项路径，不显示迁移兼容用的“仓库库存默认分组”名称。库存行仍按仓库、物品和批次展示，分组只影响左侧仓库组织和筛选，不改变库存数量、批次、成本或追溯。
- 商品价格表默认读取商品档案分组；价格表生成时的分组覆盖只固化到该价格表版本快照，不回写商品档案、BOM 或仓库分组。

## 入口
- 库存管理：物料档案/库存、原料入库、原料批次、库存流水、库存批次、库存调整单。
- 商品与配方：商品档案、客户商品、商品分类管理、商品价格管理、产品价格表。
...
- PR-397/PR-414/PR-439：商品与配方菜单保持独立入口：`商品档案`、`客户商品`、`商品分类管理`、`商品价格管理`、`产品价格表`；`生产 BOM` 已移动到生产管理，`行业字段模板` 已移动到 `设置 → 行业设置`。旧 `商品配置和分类模板`、`阶梯价模板`、`单位模板` 只作为历史兼容或迁移排查入口，不作为普通新业务入口。
...
- PR-366-GLOBAL-UNIT-TEMPLATES / PR-438-PRODUCT-TEMPLATE-DELETE-PRICE-LIST-UI / PR-439：基础单位在 设置 → 全局设置 → 全局单位字典 维护，例如 kg、g、lb、盒、袋、箱。商品档案只维护一个库存单位和整数库存限制；报价单位、录单单位、销售包装单位和价格单位属于商品价格记录和价格表快照。价格单位到库存单位的换算必须在价格记录或快照中固化，没有换算时禁止发布或录单。
...
- 例子：先在 设置 → 全局设置 → 全局单位字典 确认基础单位“kg”和“盒”存在。到 商品档案 新建“冻干速溶 200g 盒装”，库存单位选择 kg，按需开启整数库存限制；如需按盒报价，在 商品价格管理 为该商品维护最终价格记录，价格单位选择盒，并固化 `1 盒 = 0.2 kg` 的库存换算。发布商品价格表后，录单单位和订单价格来自已发布快照；已发布价格表和历史订单不会被回改。
...
- PR-442 / PR-450 后的当前口径：生产 BOM 分组不再有默认分组，也不再有 BOM 专用大组/组内分类。生产 BOM 列表固定有“全部分组”和“未分类”Tab；用户在 `分组管理` 维护可复用分组项后，需要先在生产 BOM 工具区点击 `使用分组` 启用某套分组，再把 BOM 移动到其中的分组项。顶部分组 Tab 只展示当前 BOM 列表中实际归组使用过的分组项；如果某个分组项还没有任何 BOM 归组，就不在 Tab 展示。一个 BOM 的归组保存到 `business_group_assignments`，不改变产出商品、BOM 版本或配方明细。
...
- 生产 BOM 页面点击 `前往分组管理` 维护通用分组项。要让生产 BOM 使用某套通用分组，先在工具区点击 `使用分组`，并在其右侧 `可用分组` 选择分组集；之后再选中 BOM，点击下一行左侧 `移动到分组`，并在其右侧 `目标分组` 中选择具体父组 / 子组。目标为具体分组项时写入 `business_group_assignments`，目标为 `未分组` 时移除该 BOM 的 `production_bom` 归组。新建、编辑或复制 BOM 只维护名称、产出商品、产出数量单位和状态，不写旧 `production_boms.group_id/group_category_id`。
