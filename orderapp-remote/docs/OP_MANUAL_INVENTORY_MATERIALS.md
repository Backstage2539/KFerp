...
- `行业字段模板` 是商品生产配置字段的定义来源。PR-397 后行业字段模板页是左侧模板列表、右侧模板编辑；新增字段只选“文本”或“下拉”，下拉预设用逗号分隔，不再让普通用户写 JSON。旧数字、比例、日期、勾选等字段继续兼容读取和保存。商品档案和客户商品名列表会显示“行业字段”列；客户商品名可以填写客户侧覆盖值，用于客户报价展示，未填写时继承绑定商品档案字段值。
