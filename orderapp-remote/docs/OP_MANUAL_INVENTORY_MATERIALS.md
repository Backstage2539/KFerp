...
- `商品生产配置` 归属商品档案，选择生产 BOM 版本、工艺路线，维护预期损耗率和要展示到产品价格表的产品信息字段。预期产出率不再作为用户维护字段；系统内部需要时按 `1 - 预期损耗率` 换算。
...
- BOM 版本在配方明细中维护。已发布版本只读；需要调整配方、包装、损耗或成本口径时，复制 BOM 或创建新版本草稿，编辑后发布，再把商品档案绑定到该已发布版本。
- PR-389：BOM 分组可在“生产 BOM（配方库）”页面点击“管理分组”新增、编辑排序或停用。普通分组筛选只显示启用分组；管理抽屉可查看已停用分组。停用分组只把配方移到未分组，不删除配方、不改变商品绑定。
- PR-389：BOM 版本特殊属性用于保存随配方版本冻结的产品信息。进入配方明细的“BOM版本与特殊属性”，选择草稿版本后维护 `special_attrs_schema_json` 字段定义和 `special_attrs_json` 字段值；已发布版本只读。需要修改已发布版本的特殊属性时，先“复制为新版草稿”，编辑后发布，再决定是否把商品档案绑定到新版本。
- `roast_level` 不再作为商品档案硬编码字段维护；咖啡烘焙度、服装布料克重、鲜果成熟度等都作为 BOM 版本特殊属性保存。旧商品档案里的 `special_attrs_json` 和 `roast_level` 不删除，只用于历史数据 fallback。
...
- PR-374-COMPOSABLE-PRODUCT-PRICING / PR-375 / PR-389：速溶咖啡等非烘焙产品的 BOM 可以按报价单位配置，预期损耗率和特殊属性由 BOM 版本维护。例：速溶盒装直接进货条装原料，1 条 10g，1 盒 10 条；在物料档案建“速溶咖啡条装原料”，单位“条”，默认采购价填每条成本；在 BOM 中选择该物料，消耗单位选“个/盒”，用量填 10，BOM 版本预期损耗率通常设为 0%。盒子、标签等包材也用“个/盒”维护。成本核算会把这些 `个/盒`、`个/袋`、`克/袋` 用料汇总为每盒 BOM 成本，工序模板成本也会计入产品价格和生产批次成本，供产品价格表按阶梯价模板生成 `元/盒`。
