...
- PR-598：`半成品标识` 可选择全部、半成品或非半成品，只过滤当前已加载的物料列表并重置本页分页。它不会恢复旧物料类型列，也不会改变只读的 `can_manufacture`；是否可制造仍只看默认已发布产出 BOM。
...
1. 在 `库存 → 物料档案` 打开物料详情，按业务需要维护 `是否半成品`。它只用于标识、筛选和展示；不勾选也能让任意有效物料成为普通 BOM 的产出对象。
2. `可制造能力` 是只读结果，只在该物料已有默认且已发布的产出 BOM 时显示可制造。勾选“是否半成品”不会自动产生 BOM，取消也不会撤销已有制造能力。
3. 在详情的 `产出该物料的 BOM` 查看制造该物料的配方；在 `使用该物料的 BOM` 查看把它作为投入的配方。点击名称进入生产 BOM，完成后使用页面左上角返回入口回到当前物料。
4. 如果要建立制造链，先保存物料，再到生产 BOM 新建普通 BOM，把产出类型选为“物料”、选择该物料、维护组件并发布，最后设为该物料的默认 BOM。

结果校验：`is_semi_finished` 仅反映“是否半成品”；`can_manufacture` 只反映默认已发布产出 BOM，两者不能互相替代。
