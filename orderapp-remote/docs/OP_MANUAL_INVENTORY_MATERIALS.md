...
- PR-453 / PR-502：商品与配方菜单保持独立入口：`商品档案`、`客户商品`、`商品价格管理`、`商品价格表`；分组模板入口统一在 `设置 → 系统设置 → 分组模板`；`生产 BOM` 位于生产管理，`行业字段模板` 位于 `设置 → 行业设置`。旧 `商品配置和分类模板`、`阶梯价模板` 只作为历史兼容或迁移排查入口；`单位模板` 恢复为商品 UOM 主数据模板，可从商品档案引用和维护。
