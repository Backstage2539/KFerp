...
- 需要为某个客户准备销售展示时，优先在“客户商品名”中绑定已有商品档案，只改客户对外名称、品牌、排序和是否进入价格表；客户商品编号由系统生成，客户侧分类在客户商品名页启用的分类模板 Tab 中归类。客户想批量使用商品档案时，点击过滤行右侧“新建客户商品”打开抽屉，再切到“批量添加商品档案”模式，按名称或编号搜索、多选后创建；已存在同客户同商品档案的客户商品名会跳过。只有配方、包装、库存对象、生产方式或成本口径变化时才创建新商品档案。
...
- 点击过滤行右侧 `新建客户商品` 打开抽屉。在“单个新增”模式选择绑定商品档案并填写客户商品名、品牌名、排序、是否进入价格表、启用状态和备注；客户商品编号由系统生成，不手工填写。客户商品名只负责客户侧展示和销售归属，不承载库存、生产 BOM、成本或成品批次。
- 点击“保存客户商品名”后，系统写入客户商品名记录并写操作日志；后续可在同一表格中停用，不能在客户商品名层编辑生产数据或 BOM。
- 如果客户要批量使用已有商品档案，在同一个 `新建客户商品` 抽屉中切到“批量添加商品档案”模式，按商品名称或编号搜索，多选后一次创建客户商品名。默认客户商品名=商品档案名称，客户商品编号由系统自动生成，品牌名为空，默认进入价格表；批量新增不选择产品类型、产品子类型或分类模板。同客户同商品档案已存在时自动跳过，并在结果里显示创建和跳过数量。实际创建的客户商品名都会写操作日志。
...
- PR-406 后页面只保留“商品 BOM列表”作为主列表，不再有顶部 `SKU归属` 或独立商品选择区。“商品 BOM列表”标题下方同一过滤行放商品过滤、状态过滤、BOM 名称/编号/商品搜索和 `批量失效`；商品过滤支持按商品编号/名称模糊搜索。分类 Tab 行左侧展示“全部分组 / 未分类 / 自定义分组”，右侧放 `新建商品 BOM`。BOM 列表可单独上下滚动，右侧编辑详情与列表主体对齐。
