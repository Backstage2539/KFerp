...
- 创建采购单时选择供应商和重量物料，填写采购克数、单价和备注；当前采购单接口以规范克记录到货数量，并换算到物料主档的 kg 库存，单价跟随唯一库存单位为 `元/kg`。件、袋、盒等离散物料不要在此入口建立采购单，改用原料入库按库存单位录入。
...
- 原料入库按库存单位录入。选择物料后，页面只读显示该物料主档单位，入库数量和成本都使用该单位，不再允许另选单位；提交时后端再次锁定并校验主档单位。重量 `kg` 只接受重量数量，盒、袋、个等非重量物料只接受件数，显式错单位或数量维度错误会整单拒绝；旧采购收货的 `qty_g` 且省略单位请求继续兼容。系统把重量归一到 `qty_g`、非重量数量归一到 `qty_units`，并同步写入原料批次、库存批次、原料仓库位和库存流水。
...
- 库存调整单使用物料主档库存单位填写目标库存数量；前端只读带出单位，后端拒绝显式提交其他单位或错误的重量/计件数量维度，省略新单位字段的旧 `target_g/target_units` 请求仍兼容。原料、WIP、半成品和成品库存都归入库存单位口径，销售单位只在销售、价格和录单场景展示。
