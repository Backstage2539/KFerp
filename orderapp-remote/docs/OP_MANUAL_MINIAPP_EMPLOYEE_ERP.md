...
2. 页面内容较短或已经滚动到底时，继续向上拉，底部才会展示 `Drived By` 与棵凡四字字标；松手或回到正常内容区域后不需要持续占用页面空间。
3. 棵凡字标采用透明银灰样式，没有原图底色和红色，尺寸适配手机宽度。标识属于正常页面内容，不悬浮覆盖录单按钮、商品列表、键盘或底部导航。
...
- 页面底部看不到 `Drived By`：这是正常状态；先滚动到页面内容末尾，再继续向上拉。若仍不显示，确认当前微信小程序包已经更新到包含 PR-591 的版本，而不是仅完成 ERP 服务器部署。
