...
- 备注为空时从收件信息提取收件人，以收件人作为零售客户名称，推断客户类型为 `retail`。
- 备注非空时先清理订单、样品单、标签和送货等业务后缀，再以备注客户主体聚合。赠品、库存和操作说明不能作为客户名称，无法安全提取时进入 `customer_remark_name_unresolved`。
- 备注客户只有 1 个规范收件地址时推断为 `wholesale`；有多个规范收件地址时推断为 `channel`。`规范收件地址数`、`收件地址样本`、`收件人名称`、`最新备注原文` 和 `历史备注` 是判定证据，审核时不得删除。
- 备注客户与备注为空的零售收件人分开识别，即使手机号相同也不自动合并。生产 ERP 已有客户保留正式客户类型；若与推断类型不同，审核状态为 `needs_review`。
- 同一客户有多个历史号码时，主电话取最近订单记录中的唯一有效号码；`历史号码`、`历史名称` 和来源订单键不得删除。最近记录有多个号码时必须人工选择。
- 新客户的 `customer_type` 按上述历史数据规则预填，但仍必须人工审核；历史来源名称与生产 ERP 选项精确匹配时才带入默认来源，唯一有效负责人可以安全带入，其他下拉值必须人工确认。
