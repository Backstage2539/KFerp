...
- PR-546 / PR-659：商品下拉顶部按当前所选已发布价格表显示真实商品分类，不再固定为“熟豆 / 挂耳 / 生豆 / 速溶咖啡”。分类和名称、拼音或首字母搜索同时生效；商品标签使用同一发布分类。点击商品下拉外部的规格、数量或页面空白处会自动收起；切换到另一行商品输入框时旧行下拉也会关闭。
