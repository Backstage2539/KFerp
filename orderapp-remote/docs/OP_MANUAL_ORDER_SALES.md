...
2. 维护商品档案，确认商品资料、商品分组、库存单位、客户引用、价格表快照和 BOM/成本追溯资料准确。
...
## 录单要点
- PR-439-PRODUCT-PRICE-MASTER-REMODEL / PR-500-UNIT-MODEL-CONSOLIDATION / PR-501-PRODUCT-UOM-SALES-CONVERSION / PR-502-PRODUCT-UNIT-TEMPLATE-REFERENCE / PR-504-PARENT-PRODUCT-CHILD-SKU-UOM：录单单位、价格和子 SKU 只读取已发布商品价格表快照，不再从客户商品或旧模板兜底决定报价单位/录单单位。商品有效单位规则来自子 SKU 引用的单位模板，历史商品级覆盖只作为兼容读取；商品价格表发布时把当时的 `sku_id/sku_snapshot` 和销售单位到库存单位换算固化到快照；价格表快照是什么子 SKU 和单位，订单行就是什么子 SKU 和单位。如果价格单位没有该子 SKU 的有效换算，价格表不得发布，录单也不得用该价格。商品有效库存单位只用于库存、生产、BOM 产出和成本口径，不直接决定订单行销售单位。
...
- 商品单位模板配置销售单位换算后，录单商品接口会带出产品类型、产品子类型、子 SKU、库存单位、默认销售单位、换算 JSON 和整数单位规则。前端选择商品时会把已发布价格表快照中的子 SKU 和价格单位带到数量栏；例如 `227g袋装` 是子 SKU，销售单位可以是 `袋` 或 `箱`，`1 箱 = 12 袋` 来自单位模板，`227g` 来自子 SKU 规格或 BOM，不写成 `袋/227g` 销售单位。系统用价格表快照中的单位换算保存库存单位数量，用于小计、库存、生产需求和销售单单位展示。
