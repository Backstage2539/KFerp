...
- PR-330-ORDER-ENTRY-PUBLIC-SKU-SCOPE：商品下拉还会读取 SKU 设置中的 `use_public_sku`。客户关闭“是否使用公共SKU”时，录单和修改订单不显示公共 SKU、其他客户 SKU 或其他客户豆单中的生豆 SKU；如果芬纳咖啡看到了岩师傅的 `孟连水洗A`、`红酒日晒-2026`，先检查芬纳是否误开启公共 SKU，关闭后重新进入录单。
...
- 销售单商品表中“备注”列来自录单商品明细的“条目备注”；为空时不影响生成。商品行“单价”为原价，最右列“优惠后价”为该行优惠后的金额。
...
- 销售单长商品名在商品列内换行；商品行最右列显示优惠后价；结算倒数第二行显示销售单备注，最后一行同排显示商品合计、优惠合计、运费和应收。
- 销售单/出库单预览显示“PREVIEW 预览版”，预览不会新增版本；确认生成 PDF 才新增正式历史版本。
