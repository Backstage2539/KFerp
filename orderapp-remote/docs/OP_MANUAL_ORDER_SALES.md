...
## PR-508 端到端链路验收口径
- PR-508-E2E-RAW-MATERIAL-ORDER-PRODUCTION-FLOW：下单验收必须使用可生产商品 / SKU 和已发布价格表快照。录单保存后，订单明细要保留具体销售规格、价格表版本和可进入生产计划的商品对象；库存不足时应能进入生产计划，而不是因父商品、历史模板或价格单位无法解析而丢失。
