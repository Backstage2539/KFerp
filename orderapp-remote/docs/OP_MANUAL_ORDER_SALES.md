...
8. 需要对外销售资料时，在订单列表打开销售单或出库单抽屉，先预览再生成 PDF 或图片版本；同一客户多张订单需要合并收款或合并出库资料时，在订单列表用统一选择框勾选后生成组合销售单或组合出库单。
...
- 商品明细标题区点击“选择豆单”打开豆单选择抽屉，可按熟豆豆单、生豆豆单、挂耳豆单分别选择已发布版本。标题区会把当前使用的三个豆单按行展示，方便核对完整版本号和发布时间。客户有某类型专属豆单时可选不同已发布版本，默认最新；客户没有专属豆单时该类型使用公共豆单，也可以选择公共豆单历史发布版本，默认最新公共豆单。
- PR-351-ORDER-ENTRY-PUBLIC-BEANLIST-NO-CUSTOMER：刚打开新建录单页、尚未选择客户时，商品明细标题区会先显示公共已发布豆单版本；如公共熟豆豆单已发布多个版本，可直接点“选择豆单”切换公共版本。选择客户后，系统会按客户专属豆单优先、无专属则公共豆单的规则重新同步。
...
- PR-356-CUSTOMER-PRODUCT-RULE-TEMPLATES：如果客户在 SKU设置 中绑定了客户产品规则模板或客户专属覆盖，录单仍只读取已发布产品价格表快照；规则模板影响的是生成和发布快照前的草稿内容。保存订单后，商品行记录实际命中的发布 ID、版本号、单价和单位快照，后续修改客户模板或覆盖不会静默改历史订单价格。
- 产品子类型或 SKU 配置了轻量单位规则后，录单商品接口会带出产品类型、产品子类型、库存单位、报价单位、录单单位、换算 JSON 和是否整数单位。前端选择商品时会把录单单位带到数量栏，例如显示“数量（盒）”；若 `unit_conversion_json` 中配置 `{"盒":{"kg":0.2}}`，系统会把“盒”作为一个默认规格选项，对应 200g，用于小计、库存/生产克重和销售单单位展示。
...
- 录单和编辑订单的商品行会显示当前选中豆单版本；能从豆单选到商品时不应显示“未记录”。切换熟豆、生豆或挂耳豆单版本后，已有商品行会按当前选择的豆单发布 ID 重新取价并刷新版本号。如果该行使用的豆单不是当前客户、当前豆单类型的最新发布版本，豆单版本会标红并显示感叹号，鼠标悬停或手机点击感叹号可看到“非新版本豆单”提示。新订单保存后会记录豆单发布 ID 和豆单版本号，历史订单详情中只在旧数据确实缺失时才可能显示未记录。
...
- 发货物流是发货核对字段。订单状态为已发货时，录单/编辑保存会强制要求选择物流公司和物流产品；物流公司和产品在“设置 → 物流设置”维护，系统默认初始化顺丰、顺丰小件和顺丰大件。
...
- PR-341-ORDER-ENTRY-STALE-BEANLIST-AUTOLINE：商品行使用旧版豆单时会标红感叹号提示“非新版本豆单”；商品明细按钮放在列表下方，选择商品后自动补一个空明细且全列表最多保留一个空明细。
...
- 已发货订单必须能追溯物流公司和物流产品；如果页面提示缺少物流配置，先到“设置 → 物流设置”补齐启用状态的公司和产品。
...
- PR-350-COMBINED-ORDER-DOCUMENTS：同一客户多张订单需要给客户发一张销售单收款时，在订单列表左侧统一选择框勾选这些订单，点击“组合销售单”。组合销售单会在顶部展示单据日期和关联订单，下面按订单号分组展示商品明细、小计和汇总应收。选择不同客户或少于两张订单时不能生成；预览不会新增版本，点击“确认生成 PDF”后才生成组合销售单正式版本并写入操作日志。
...
- PR-355-COMBINED-SALES-DOCUMENT-LAYOUT-PAGINATION：组合销售单发给客户时按普通销售单标题展示，头部看客户、客户公司、联系电话、单据日期、关联订单和地址，不显示组合单号或订单数；订单分组标题只显示订单号。销售单 PDF 内容很长时，说明和收款码会自动放到续页并显示页码；图片版本是一张长图，适合微信发送，不按 A4 分页。
...
- PR-358-PRODUCT-PRICE-LIST-ORDER-PRODUCTION：录单价格表版本数据会同时带旧 `list_type` 和新的产品类型字段。当前熟豆、生豆、挂耳版本选择仍按原逻辑展示；后续新增速溶咖啡等产品类型时，前端可按 `product_type_category_id/product_type_name` 选择对应产品价格表发布快照。
...
- PR-355-COMBINED-SALES-DOCUMENT-LAYOUT-PAGINATION：验收组合销售单时，重点看顶部客户信息是否清晰、是否只出现一个单据日期、关联订单是否完整、分组标题是否只保留订单号；长 PDF 应有续页页码，图片应为长图。
