...
- 商品明细直接在商品输入框搜索父商品，商品名称不追加规格；随后从“规格”查看并选择该商品档案的全部现有 SKU。这里“价格表中的规格”指已发布、可自动取价并可保存的具体 SKU；未定价规格仍可看见，但页面会明确提示并阻止保存，需先补齐价格表或改选有价规格。新发布价格不跨同父规格按重量换算取价。选择一个商品后，系统会在明细列表底部自动补一个空明细行；已有空明细时不会因修改商品反复新增空行，仍可在列表下方点击“新增明细”手工加行。
...
## PR-544 录单从当前价格表选择商品规格
- 录单商品下拉按父商品展示。例如只显示“乌拉嘎”，不会再把“乌拉嘎18g”“乌拉嘎100g”当成多个商品。选择商品后，独立“规格”下拉展示商品档案中的 18g、100g 等全部现有规格；当前价格表未发布的规格会显示缺价提示并阻止保存。
- 商品档案中的 SKU 才是录单规格：规格下拉展示该父商品全部现有 SKU，不会混入系统全局的 36g、80g 等规格，也不能临时输入自定义克数。当前价格表决定哪些规格可以自动取价和保存；需要新增规格时，先维护商品规格，再生成、发布包含该 SKU 的新版价格表。
- 默认规格依次使用商品档案 `default_sku_id`、规格默认标记和第一项。若默认规格不在当前价格表内，系统保留该规格并提示缺价，不会静默改成另一个 SKU；录单人员需补齐价格表或人工改选有价规格。
- 切换价格表版本后，如果当前规格仍在商品档案但不在新版本中，系统保留父商品和规格、清空自动价并提示缺价；只有 SKU 已从商品档案停用或移除时才清空规格。必须补齐价格表或人工选择有价规格后才能保存，系统不会静默换成另一规格。
...
- 例如当前咖啡豆价格表 V3.0.19 只有白月光瑰夏、风味孟连、果皮茶、黑巧炸弹四个商品时，商品下拉也只能看到这四个父商品；每个父商品的规格下拉展示其全部现有 SKU，未在 V3.0.19 定价的规格会提示缺价；V3.0.18 的其他父商品不会自动出现。
- 确实需要按旧表补录时，在“选择价格表”中找到历史兼容组并明确选择旧版本。完成后可重新选“不使用该历史价格表”，旧商品会从后续新明细候选中移除。
- 历史订单编辑继续显示保存时冻结的商品、规格、价格表版本和成交价，不需要为了查看历史单据重新启用旧表。复制历史订单属于新订单；若旧规格仍在商品档案但不在当前价格表，页面保留规格、清空自动价并提示补价或重新选择。
- 切换页面后恢复尚未保存的新订单草稿时，系统会按当前客户重新同步分类价格表并复核每条商品明细。若草稿仍引用已停用的旧全局价格表，系统会保留仍有效的商品规格但清除旧自动价，要求选择当前价格表或补齐定价，不会让旧价在当前版本标题下继续保存。
