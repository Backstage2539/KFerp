...
## 录单要点
- PR-439-PRODUCT-PRICE-MASTER-REMODEL / PR-500-UNIT-MODEL-CONSOLIDATION：录单单位和价格只读取已发布商品价格表快照，不再从商品档案、客户商品或旧模板兜底决定报价单位/录单单位。价格表快照是什么单位，订单行就是什么单位；如果价格单位没有固化到库存单位的换算，价格表不得发布，录单也不得用该价格。商品档案库存单位只用于库存、生产、BOM 产出和成本口径，不直接决定订单行销售单位。
