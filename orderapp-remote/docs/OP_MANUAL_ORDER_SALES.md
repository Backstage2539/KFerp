...
## 录单要点
- PR-439-PRODUCT-PRICE-MASTER-REMODEL / PR-500-UNIT-MODEL-CONSOLIDATION / PR-501-PRODUCT-UOM-SALES-CONVERSION / PR-502-PRODUCT-UNIT-TEMPLATE-REFERENCE：录单单位和价格只读取已发布商品价格表快照，不再从客户商品或旧模板兜底决定报价单位/录单单位。商品有效单位规则来自商品引用的单位模板，历史商品级覆盖只作为兼容读取；商品价格表发布时把当时的销售单位到库存单位换算固化到快照；价格表快照是什么单位，订单行就是什么单位。如果价格单位没有商品有效换算，价格表不得发布，录单也不得用该价格。商品有效库存单位只用于库存、生产、BOM 产出和成本口径，不直接决定订单行销售单位。
...
- 商品单位模板配置销售单位换算后，录单商品接口会带出产品类型、产品子类型、库存单位、默认销售单位、换算 JSON 和整数单位规则。前端选择商品时会把已发布价格表快照中的价格单位带到数量栏；若商品有效 `unit_conversion_json` 中配置 `{"盒":{"kg":0.2}}` 并已发布为价格表快照，系统会把“盒”作为可用销售单位，对应 200g，用于小计、库存/生产克重和销售单单位展示。
