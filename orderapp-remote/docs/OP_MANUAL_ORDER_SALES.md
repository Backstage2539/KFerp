...
- 订单失效使用软失效，不删除订单行。点击订单列表或抽屉中的“失效”后，该订单会从默认 ERP 订单列表、履约客户订单和小程序订单/账单中隐藏；需要查看或恢复时，把失效筛选切到“已失效”或“全部”，再点击“恢复订单”。
- 已失效订单不能勾选发货处理；恢复后才会重新进入默认列表、履约客户订单范围和小程序订单/账单。
...
- 订单失效后，默认订单列表看不到该订单；切换到“已失效”能看到并恢复；恢复后默认列表、履约客户订单和小程序订单/账单重新展示同一订单。
...
- 履约客户或小程序看不到某个订单：先在 ERP 订单列表把失效筛选切到“全部”或“已失效”，确认订单是否已失效；已失效订单默认不会进入履约客户订单、小程序订单和小程序账单。
