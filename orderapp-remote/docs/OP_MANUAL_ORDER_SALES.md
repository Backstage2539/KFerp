...
- 商品明细直接在商品输入框搜索父商品，商品名称不追加规格；随后从“规格”查看并选择该商品档案的全部现有 SKU。当前价格表已发布的规格会按数量自动匹配本规格件数梯度；未定价规格仍可看见，但页面会明确提示并阻止保存，需先补齐价格表或改选有价规格。新发布价格不跨同父规格按重量换算取价。选择一个商品后，系统会在明细列表底部自动补一个空明细行；已有空明细时不会因修改商品反复新增空行，仍可在列表下方点击“新增明细”手工加行。
