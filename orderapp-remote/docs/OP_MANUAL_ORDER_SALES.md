...
- 保存订单会记录本单使用的价格表发布 ID 和版本号；商品行显示 `报价来源：价格表 {版本}`，用于后续追溯当时的报价和展示口径。ERP 录单只使用“工厂供货”用途的已发布价格表，客户转售分享价格表不会出现在版本选项或订单自动价中。同一客户/公共范围多次部分发布时，每个派生 SKU 分别使用最近一次包含自己的快照；新版已包含该 SKU 但价格为空时不会回退旧价。订单详情和销售单页面还会只读展示 `报价来源`、`生产来源` 两块：报价来源来自价格表快照，生产来源来自工单/BOM/工序卡快照；生产来源变化只影响成本、毛利和追溯，不自动改已成交价格。
