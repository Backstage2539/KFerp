...
3. 进入录单页面，选择客户、商品、规格或单位、数量、订单日期、订单来源、发货和付款信息；熟豆商品仍按重量规格录单，挂耳商品选择“袋”或“盒”单位后按发布的挂耳价格梯度报价。当收款状态选择“已付款/已收款/已支付”时必须同步选择收款方式。如单个商品有贴标、磨粉、特殊包装等说明，在该商品明细行填写“条目备注”；如该行需要优惠，可选择减免数额、折扣或免费。
...
- 生豆商品录单时会保留 `green_bean` 形态；客户生豆 SKU 没有自身生豆价目时，系统会用绑定熟豆 BOM 的启用梯度作为自动价兜底，保存后价格来源可追溯到绑定熟豆梯度。
...
- 生豆订单使用绑定熟豆梯度兜底时，订单明细仍显示生豆形态，金额不为 0，价格来源能追溯到绑定熟豆梯度。
...
- 价格不符合预期：检查商品价格、成本发布记录、规格是否有专属梯度、总重量折算磅数或 KG 数是否落在预期梯度内，以及当前单价单位是元/磅还是元/kg。曲奇拼配这类 KG 梯度产品要按总 KG 核对；例如 1000g × 25 袋应按 25kg 落入 24-49kg 梯度，81.91 元/kg 展示为 82 元/kg，小计 2050。生豆 SKU 如果自身没有生豆价目，还要检查是否已绑定熟豆 BOM，以及绑定熟豆是否有启用梯度。
