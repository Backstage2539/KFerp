...
- 商品明细直接在商品输入框搜索父商品，商品名称不追加规格；随后从“规格”选择当前价格表中该商品实际发布的具体 SKU，再按数量自动匹配该规格的件数梯度。新发布价格不跨同父规格按重量换算取价。选择一个商品后，系统会在明细列表底部自动补一个空明细行；已有空明细时不会因修改商品反复新增空行，仍可在列表下方点击“新增明细”手工加行。
...
- 录单商品下拉按父商品展示。例如只显示“乌拉嘎”，不会再把“乌拉嘎18g”“乌拉嘎100g”当成多个商品。选择商品后，再从独立“规格”下拉选择当前价格表中的 18g、100g 等具体规格。
- 价格表中的规格才是录单规格：规格下拉只显示当前已选择且已发布价格表里该父商品实际有有效价格的 SKU；不会混入系统全局的 36g、80g 等规格，也不能临时输入自定义克数。需要新增规格时，先维护商品规格并生成、发布包含该 SKU 的新版价格表。
- 商品档案默认规格已进入当前价格表时会自动选中；默认规格不在表内时自动使用该价格表第一个有效规格。自动选择后仍要核对规格、单价单位和价格表版本。
- 切换价格表版本后，如果当前规格不在新版本中，系统保留父商品但清空规格和自动价，并提示重新选择。必须人工选择新版本中存在的规格后才能保存，系统不会静默换成另一规格。
- 保存时后端会再次核对父商品、具体 SKU、规格和发布版本。即使手工修改了单价，也不能使用不属于当前价格表的规格。订单商品名称快照保持父商品名或客户显示名，规格在独立字段冻结。
...
- 例如当前咖啡豆价格表 V3.0.19 只有白月光瑰夏、风味孟连、果皮茶、黑巧炸弹四个商品时，商品下拉也只能看到这四个父商品及其已发布规格；V3.0.18 的其他商品不会自动出现。
- 确实需要按旧表补录时，在“选择价格表”中找到历史兼容组并明确选择旧版本。完成后可重新选“不使用该历史价格表”，旧商品会从后续新明细候选中移除。
- 历史订单编辑继续显示保存时冻结的商品、规格、价格表版本和成交价，不需要为了查看历史单据重新启用旧表。复制历史订单属于新订单；若旧规格不在当前价格表，页面会清空规格和自动价并要求重新选择。
- 切换页面后恢复尚未保存的新订单草稿时，系统会按当前客户重新同步分类价格表并复核每条商品明细。若草稿仍引用已停用的旧全局价格表，系统会要求重新选择当前价格表中的规格，不会让旧价在当前版本标题下继续保存。
