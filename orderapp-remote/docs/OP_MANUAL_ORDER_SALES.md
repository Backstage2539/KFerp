...
- PR-440-PRODUCT-GROUP-PRICE-REMODEL / PR-620-ORDER-ENTRY-PRODUCT-CUSTOMER-REFERENCE：新业务不再有独立客户商品主数据；客户料号和客户显示名来自商品档案的 `客户引用` 子表。`客户引用` 不定义价格、单位、BOM 或分组，但会把商品投影到对应客户的录单目录，使该客户已发布工厂供货价格表能够精确匹配商品；最终价仍只来自已发布价格表快照。若同一客户和商品仍有旧客户商品别名，录单保持旧别名优先并只显示一条商品。
