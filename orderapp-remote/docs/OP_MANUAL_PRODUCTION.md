...
PR-559-PRODUCTION-CONFIG-WIP-ISSUE-UX：生产 BOM 并入 `生产配置` 第 4 个 Tab。工单开工前由统一 WIP 覆盖计算检查冻结物料需求；released 工单即使尚未创建占用记录，也会在执行枢纽逐物料显示需求、可用和缺口。点击“生产领料”会打开一个多物料库存单据，显示真实工单号并自动带出全部短缺物料、单一数量输入和库存单位。

PR-560-PRODUCTION-ISSUE-COMPACT-DIAGNOSTICS：生产领料的多物料明细按统一列头紧凑显示，恢复草稿时重新读取当前 WIP 覆盖。原料仓库存不足、质检冻结或超过当前剩余 WIP 缺口时，页面用中文列出受影响物料及需要、可用/可领、缺少和库存单位。
...
15. 在生产工单中查看商品规格、订单信息、计划数量、BOM/工艺路线、工艺参数、预期损耗率、冻结生产配置、工序执行汇总、冻结工位产能、计划分钟、计划工序成本、组件/原料摘要、多层展开树、WIP 占用、WIP 已占/已耗/剩余占用，需要时打印工单。PR-493-PLAN-WORKORDER-SPLIT-EDIT：工单状态为 `released` 且所有工序卡仍是 `pending` 时，可以在生产工单页点击 `编辑拆分`，补充或调整工位产能和承担产量；保存后系统重建 pending 工序卡并记录操作日志。工单开始生产后，拆分入口不可用，不能覆盖已执行工序卡。PR-499 / PR-559 / PR-560：工单号或 `执行枢纽` 按钮会打开工单执行枢纽，先看 readiness、逐物料 WIP 需求/可用/缺口和质检状态；红色“WIP库存不足”必须先点击“生产领料”处理。多物料领料在同一列头下逐行显示；若恢复的过期草稿超过当前剩余 WIP 缺口，页面按当前可领上限刷新。原料仓不足或提交期间缺口再次变化时，按中文逐物料提示调整，补足并刷新后再开始生产。历史 `roast_level` 等字段只作为工艺参数兼容展示，不作为咖啡烘焙主列。上层工单和下层工单保持独立，不把所有层级混成一张不可追踪的大工单。
