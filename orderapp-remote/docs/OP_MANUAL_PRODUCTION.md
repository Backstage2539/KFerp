...
## 生产日志：查看结果与批次追溯（PR-661）

入口：生产管理 → 生产日志。适用于有生产查看权限的员工及管理人员。这里展示已经写入的完工记录；执行中的任务仍在工位视图处理。本页只读，不修改历史报工或库存。

1. 在顶部按商品、订单号、生产批次或成品批次搜索；也可选择商品、开始/结束日期和完整完成人姓名。日期按北京时间包含所选开始日和结束日。
2. 点击“查询”应用条件。“清除筛选”恢复全部日志；“刷新记录”沿用上一次已查询条件。修改尚未查询的条件时有提示，翻页也沿用已查询条件。
3. 浅绿摘要显示本次查询记录总数与本页已记录产出重量。件数与重量分别展示；本页重量不是全厂或全部查询记录的合计。
4. 列表按完成时间、记录顺序倒序排列，采用服务端分页，可查看超过原 200 条限制的较早记录。空结果可清除/调整条件；失败保留输入和上一次结果。
5. 点击“查看记录”，在右侧按本次结果、人员与时间、库存变化、订单与批次、本次用料阅读明细。窄屏显示卡片，点击后定位到详情，“返回记录列表”回到列表。
6. 日期、筛选、页码和选中记录保留在链接中；刷新可恢复当前页选中记录。从“操作说明”进入手册后，左上角可返回原日志。离开后从批次追溯页左上角“返回生产日志”可恢复本次来源上下文；该返回入口只在本次操作中保留，刷新目标页后清除。
7. 点击成品或物料批次进入仓库库存追溯面板，批次自动带入；点击追溯查询查看既有库存与来源链。没有批次号的旧记录明确提示缺失，不提供空跳转。
8. 投料、产出及物料重量按存储的克数换算为 g / kg，包材数量按其数量单位展示。库存前后是当次完成时的历史快照，不是当前库存。
9. 未记录有效投料时产出率标为“待核对”；旧记录有完成数量但没有包装规格/可靠产出重量时标为“数量待核对”，详情保留原值并提示历史单位缺失。不得用当前商品规格猜测或回写旧记录；如需处理，请先核对原始单据。

接口沿用 `GET /api/produce/logs`，补充 `q/page/limit/work_order_id` 与 `total/page/limit/total_pages`；旧查询参数继续接受。没有新增业务写接口或数据库迁移。

## 生产排班与工位自动派工（PR-657 / PR-658，取代逐任务配人）

**入口与角色**

- 生产管理员在“生产配置 → 工位”维护人员资格，在“生产管理 → 生产排班”维护每周出勤和当日工位调整。
- 生产员工进入“生产管理 → 工位视图”，默认看到“我的今日工位”；管理员可以切换全厂总览。
- 计划员可从尚未开工的生产计划商品或工位任务进入“撤回并重新安排”。

**首次配置工位人员**

1. 打开工位，在人员区域选择一名启用员工为“主负责人”，替补员工选中即加入顺序列表，无需再点“添加”。可上移、下移、移除，保存后重新读取，列表展示主负责人及已保存的替补顺序。主负责人和替补不能重复；启用工位必须配置主负责人。
2. 保存前查看未来七天负责人影响。页面按现有排班显示主负责人、替补、人工调整或“无人值班”；保存只改变今天及未来的自动判断，历史执行人与报工不变。
3. 工序不再配置人员，旧工序人员关系只保留历史。日常任务也不再逐条选择执行人或时间。

**安排一周出勤**

1. 打开“生产排班”，默认进入北京时间本周周一至周日。员工日格有“上班、休息、未排班”三种状态；未排班不会参与工位负责人计算。
2. 可逐格修改，也可选择日期批量设为上班、休息或未排班。复制上一周时先显示预览，确认无误后再保存。
3. 切换“工位安排”检查每个工位每天的唯一负责人。系统顺序为：有效的当日人工调整、当天上班的主负责人、按顺序第一个当天上班的替补。一个员工可同时负责多个工位。
4. 点击某天某工位的“调整负责人”，面板列出当天上班的全部启用内部员工，标明主负责人、替补或其他员工。选择其他员工仅形成当天该工位的临时安排，不加入长期替补名单；“恢复自动安排”删除该日该工位的人工调整，不修改工位配置。
5. 右侧处理未排班、无人值班、失效调整及待交接项。修改出勤、临时负责人或恢复自动安排后会自动更新预览并标记“未保存”。点击底部“保存排班”核对受影响工位和任务，再“确认保存排班”。保存前员工、工位或任务范围发生变化时需要重新核对。页面变脏时离开会提示；并发修改要求重新读取；重复点击不会重复写入。
6. 第一次保存某周排班后，该周自动派工门禁生效。某工位当天没有合格上班人员时显示“无人值班”，新任务不能开工。名单外的当日临时负责人可按有效安排开工，不受长期替补名单限制。
7. 版本显示 `V0` 且底部提示“本周排班尚未保存”时，表示这一周还没有保存过排班；先完成员工出勤和工位人员配置，再保存本周排班。

**员工请假与整日换人（PR-658）**

1. 把 A 当天的“上班”改为“休息”或“未排班”，自动负责的工位立即在预览中重新匹配主负责人和替补。当天人工指定给 A 的工位也自动解除指定，显示解除原因和接替员工；点击保存后统一生效。
2. 有替补上班时显示“替补自动接班”；没有人可接时显示具体原因，如“主负责人休息，替补未排班”。员工被外部停用等无效调整仍提示处理，不会静默清除。
3. 在“工位安排”点击“整日换人”，选择日期、原员工和接替员工。接替人必须是当天上班的启用内部员工；默认全选原员工工位，可以取消部分。默认保持原员工出勤，“同时将原员工当天设为休息”需主动勾选。
4. 预览会列出原负责人到新负责人的变化、未开始任务数和待交接任务数。请假前的工位范围保留在补位清单，保存并刷新后仍可从“最近保存的补位清单 → 统一调整接替人”继续调整。
5. 一次确认保存整批生效，失败保留输入。未开始任务跟随新负责人；正在执行的任务保留实际执行人，接替人仍需到工位“接手工位”。

**员工在工位执行**

1. 员工登录后在“我的今日工位”查看自己当天负责的全部工位。每个工位显示当前任务、下一任务、商品规格、本批数量、单位和批次；没有任务时仍显示“今日负责此工位，暂无任务”。休息与未排班分别提示。
2. 未开始任务自动跟随当天工位负责人，不领取任务、不选择执行人。未来计划日期尚未到的任务不提前进入；未完成的待执行任务次日继续跟随新一天负责人。
3. 点击“开始本任务”时，系统重新校验本人就是当日负责人、工位资格、本批 WIP 齐套、质检和前序工序。成功后把实际负责人冻结到任务并写操作日志；重复点击不重复开工。
4. “本次任务齐套”使用绿色；真实缺料使用橙色并提供可用处理入口；资料不足显示“用料待核对”。领料、质检、报工、异常和完工入库沿用本任务口径。
5. 管理员临时换班后，未开始任务立即跟随新负责人。若工位仍有执行中或暂停任务，新负责人点击“接手工位”一次确认，系统把该工位所有未结束执行任务交给接班人，并保存每张任务的原负责人、接班人、时间和操作人。

**撤回未开工需求并合并重排**

1. 从计划详情商品行或工位待执行任务点击“撤回并重新安排”。整张计划处于“已提交”或“生产中”都可以发起，系统只接收仍未开工的独立商品需求；同一计划里已经开工的其他商品继续执行。预览列出原订单、数量、工单、工位、关联上下游，以及已在 WIP 的实际库存说明。
2. 勾选需要合并的新增待计划需求。页面分别显示原数量、新增数量和新总量；例如 4kg 与 36kg 合并为 40kg。
3. 共用上游导致其他未开工商品必须一起撤回时，页面会展开范围并列明新增总量。任何关联任务已开工、耗料或产出时，本次操作被阻止并显示具体原因。
4. 点击“撤回并创建新草稿”后，原需求释放、原任务撤回和新草稿创建在一个事务完成。失败不改变原安排；重复请求返回同一结果。已领入 WIP 的实物仍留在 WIP，只释放尚可释放的预留。
5. 成功后直接打开统一生产计划草稿，在“本次生产安排”重新选择工位、产能和批次，再保存并提交。旧工单显示“已撤回重排”和新计划链接；一张计划只撤回部分商品时，其他不相关商品继续执行。

**异常处理与结果核对**

- 外部变更导致人工调整员工停用时，显示“调整已失效”，需回到排班处理；主动修改出勤为休息或未排班则按上述请假规则自动补位。
- 无人值班时先补排当天合格员工，或修改工位人员资格；不能通过旧逐任务配人接口绕开。
- 保存排班、工位配置、人工换班、工位交接和撤回重排都可在“操作日志”按对象、日期和操作人查询。
- 生产排班只确定人员归属，不自动启动设备、不耗料、不报工；实际库存和生产条件仍在开工事务核对。

## 历史流程：逐任务单负责人（PR-656，已由 PR-657 取代）

**工序维护谁能做、默认由谁做；排程确认这一次何时做、具体谁做；工位沿用安排执行。**

1. 在“生产配置 → 工序”勾选启用的可执行员工，再从中选择一名默认负责人。启用工序必须有候选人员和默认负责人；配置未完成时可保存为停用。新建任务自动带入有效默认负责人，修改默认负责人只影响后续新任务和明确执行的未分配补齐，不覆盖已有安排。
...
5. 点击“核对本次安排”查看任务和负责人，再确认保存。冲突检查覆盖本次任务之外、不同工位的有效任务；存在负责人时间重叠时确认可兼顾后保存。保存时重新校验，冲突变化需重新核对。版本变化需重新读取任务；失败保留输入，整批成功或整批不写入，重复请求不会重复保存。
6. 右侧显示工位已占用分钟。未维护可用工时显示“待配置”，不会展示空闲率；可从“更多 → 维护可用工时”按工位、日期和班次维护。
7. 工位视图和工单详情沿用同一名负责人。临时调整负责人会保留时间、班次、备注和工位，并写入操作日志；领取任务也校验员工是否可执行本工序。已开始的任务保留历史安排，不因新增配置要求中断执行；结束任务不可修改。旧协作关系仅作历史留存，不再参与派工或冲突计算。
...
工位任务列表同时核对工单和工序状态，已取消或已完成工单不会进入待执行队列。每条任务显示商品规格、本批数量、库存单位和批次；烘焙任务分别显示本批投料与目标产出。展开用料时，已齐套使用绿色文字，真实缺料使用橙色，冻结资料不完整显示“用料待核对”。

同一工序拆成多批时，物料按完整冻结批次顺序分摊取整尾差，各批合计必须等于总需求。例如 1,749g 分两批为 875g 与 874g，刷新、筛选或前一批完成都不会重新均分。只有服务端确认仍有可领来源时才显示“领料”；物料已经在 WIP 时显示“本次任务已齐套”，仍缺料但没有可领来源时提示补充来源或等待上游，不打开空白单据。

“开始本任务”按当前工单防重复。同一销售订单的其他工单正在生产不会阻止本工单；当前工单重复点击仍只建立一次运行记录。负责人、工位、本批用料、质检和前序工序仍会在事务内重新核对，阻断时按页面提示处理。

## 按任务执行与完工入库（PR-654）

入口：生产管理 → 生产流程。计划员从“生产工单”查看调度状态；现场人员从“我的今日工位”直接执行当天负责工位的任务；质检员从“生产质检”处理检查；仓库从“完工入库”核对并过账。页面顶部的“操作说明”打开本手册。

1. **生产工单**默认显示未结束工单。列表只保留产品/规格与工单号、目标数量、当前状态、工序进度、当前待办和快捷操作。进入“查看工单”后，按工序及“第 N 批/共 N 批”查看执行人、工位、阻塞和进度；配方、成本及订单追溯按需展开。
2. **自动负责人**来自当天工位排班。工单详情与工位任务显示同一负责人，常规操作不再逐任务分配或领取；临时换人在生产排班按当天工位调整，执行中任务需确认交接。仓库、质检和调度只在阻塞时显示为待协同岗位。
...
常见待办：无人值班时回到生产排班安排当天合格人员；待领料时查看每项物料的需求、WIP 可用和缺口后打开领料；等待前序时核对上一工序已报工产出；待质检时进入对应批次检查。只有真实生产异常显示“异常”。旧工单无法可靠恢复任务用料时会要求人工确认，历史已执行工单继续保留原库存和成本口径。
...
## 入口
- 生产管理：工位视图、生产配置、生产流程、生产排班、生产日志、生产手册。`生产流程` 用五个 Tab 集中生产计划、生产工单、工序记录、生产质检和完工入库；生产手册固定在生产管理菜单最底部。
...
- 工位视图仅保留工位选择、任务队列、人员和当前工序操作；其他生产功能通过全局菜单访问。工单列表用于查询，工序记录用于只读复盘，工单详情汇总状态，进入工位定位现场任务。
...
1. 开工前进入生产验收，确认仓库、原料批次、WIP、生产工单、WIP 占用、生产日志、质检和成品追溯检查项。
   - PR-495-PRODUCTION-WORKSTATION-OVERVIEW / PR-496-PRODUCTION-FLOW-PHASE1-OPTIMIZATION / PR-499-PRODUCTION-EXECUTION-HUB-PHASE2 / PR-562-PRODUCTION-EXECUTION-JOB-CARD-CONSOLIDATION：日常调度进入生产排班维护员工出勤和工位负责人，工位视图按当天负责人展示现在做、下一件和不能做原因。工单详情用于汇总阻塞与进度；开始、暂停、继续、完成本工序、报异常和呼叫补料均在工位执行。工序记录只用于查看和复盘。
...
26. 完成生产遇到 WIP 批次质检待处理或不通过，或 WIP 不足时，系统会拒绝本次完工并保持工单继续生产；页面在行内或完成面板内展示原因、影响对象和动作入口。处理方式是从工位任务或工单详情打开库存作业，系统会带入 WIP tab、工单、工序卡、running item、物料和缺口数量上下文；若是质检冻结，先从枢纽或生产中打开生产质检复核对应批次，确认合格或更换合格 WIP 批次后再提交完工。
...
## 制造三期生产协调
- PR-480-MANUFACTURING-PHASE3-SCHEDULE-CAPACITY：旧“生产排程工作台”的逐任务排程由 PR-657 的周排班取代。当前只维护员工每天上班/休息/未排班和当日工位调整，变更均可在操作日志追溯。
- PR-481-MANUFACTURING-PHASE3-SCHEDULING-WORKBENCH：旧任务排程写入口已停用，旧链接进入“生产排班”。BOM、工艺路线、工位和批次继续由生产计划确定，任务自动跟随当天工位负责人。
- PR-482-MANUFACTURING-PHASE3-MRP-SUGGESTIONS：MRP 建议已从排程页移除，原接口保留兼容；缺料从任务进入工位或工单详情处理，真实领料仍到库存作业创建并过账库存单据。
...
- PR-667：申请预览给出当前物料支持的最大可生产量；提交事务会重新锁定复核并立即建立原料/包材预订。超过上限整张申请失败，并发提交也不能超过同一批次可用量。客户显示状态收敛为“待接单、已接单、开始生产、生产完成”，内部仍保留细分执行状态。
- PR-667：代加工申请的未入库产出可以按申请先后被同客户一件代发订单预订。生产看板必须扣除订单已经用成品和在制覆盖的数量，不能重复补产。完工入库按订单先后把在制承诺转换到本次成品批次；部分入库不提前完成订单，最终减产形成明确缺口。
- PR-667：仍有有效订单产出承诺的代加工工单不能取消或缩减。取消订单会释放对应成品/在制占用但不删除代加工申请；跨客户、跨商品或跨 BOM 规格的产出不可互相冲抵。
- PR-668：待计划需求新增“需求来源”筛选；客户生产申请显示来源“客户工单”、客户、申请单号、客户货权目标仓和 BOM 规格。客户生产申请要求新增生产，现货不冲减申请数量。进入草稿仍为客户侧“待接单”，实际提交计划并生成生产工单后才为“已接单”。
- PR-668：生产工单执行枢纽显示客户申请、计划、BOM 快照、目标仓、物料预订、真实状态时间线、当前工序和关联代发订单。首道工序尚未实际开始时，即使工单已准备执行也不提前显示“开始生产”。
