...
- 商品与配方：商品档案、客户商品、商品配置模板和价格表。商品档案是库存/销售对象，只读查看该商品被哪些上层 BOM 作为组件使用，不再作为 BOM 绑定入口。
...
2. 生产前维护商品档案、物料、生产 BOM、工艺模板和设备产能。生产 BOM 必须声明“产出商品 + 产出数量 + 组件清单”，组件可以是物料，也可以是另一个有库存的半成品/成品商品。商品档案不编辑 BOM，只作为库存/销售对象被 BOM 产出或消耗。
   - PR-419：商品档案中的“被哪些 BOM 使用”只表示当前商品被上层 BOM 当组件消耗；产出当前商品的 BOM 在生产 BOM 列表或 BOM 详情中查看。生产 BOM 列表点击 BOM 名称直接打开设置抽屉，不再使用单独“编辑”按钮。
   - PR-420：编辑或新建生产 BOM 时，产出商品和商品组件下拉主标签显示 `SKU编号 商品名`，例如 `SKU-000518 初晓`，用于区分同名商品并支持按 SKU 编号搜索。
