...
16. 在工序卡中查看每个工序的顺序、计划投入、实际投入、实际产出、实际损耗、实际损耗率、是否记录损耗和异常原因；如完工后需要补录或修正工序实际数据，点击“保存实际”，系统会写入操作日志并回写工单工序汇总。
...
- 进入 `生产管理 -> 工艺路线` 维护路线；路线只定义步骤怎么跑，例如烘焙、裁剪、缝制、去皮、包装，不选择 SKU、BOM 或 BOM 版本。进入 `生产管理 -> 工序` 维护工序主数据，进入 `生产管理 -> 工位/设备` 维护工位、设备、默认工时和费率。BOM 版本草稿选择一条工艺路线，发布后成为生产计划/工单使用的路线快照来源。
- 工艺路线的每个工序可配置工位、默认设备、默认工时、是否记录损耗和质检项。
...
- 工艺路线保存、发布、停用，工序/工位主数据保存或停用，行业字段模板保存、停用，以及工序卡实际损耗更新，都会写操作日志，可在操作日志按 `process_route`、`manufacturing_operation`、`manufacturing_workstation`、`industry_field_template` 或 `job_card` 追溯。
