...
1. 生成生产计划草稿后，在当前计划或计划详情的“组件来源仓”逐行选择。选项同时显示仓库、货主和当前可用量；客户仓固定使用该客户货主，共享在制仓按批次货主分别列出。
2. 所有最终库存输入都必须选择来源。同一工单的生豆、包材和其他物料可以来自不同仓库；由上游工单生产的组件沿用上游目标仓和货主，不重复选择。
3. 提交计划会重新校验可用量，并把 BOM 组件身份、BOM 版本、来源仓、货主、库存快照和具体批次冻结到工单预留。任何一行未选择或不足时整张计划保持草稿。
4. 工单领料单从冻结批次生成，不能修改为其他仓库、货主或批次。开工和完工耗用再次核对冻结范围；所选客户批次不足时不会使用同物料的工厂库存补足。
