...
1. 在 `生产配置 -> 生产 BOM` 打开 `BOM 规格模板`。模板按版本维护规格键、名称、库存单位、默认规格、主投入占位、包材、其他组件、工艺路线和损耗；发布前必须保证至少一个规格、规格键唯一且恰好一个默认规格。
2. 新建商品产出 BOM 时选择一个已发布 BOM 规格模板版本和一个主投入物料。系统复制为 BOM 专属规格组；模板后续修改不会改变已复制 BOM。物料产出 BOM 继续使用单配方。
3. 商品 BOM 的每个规格固定产出 `1 × 规格库存单位`，独立维护完整配方。规格组整组保存、整组发布；任一规格失败时整组不写入。
4. 每个配方只允许一种模式：比例模式全部为物料 `比例 %`；固定模式按每个组件自己的库存单位录入。损耗开启时必须大于 0 且强制比例模式；固定模式可录入 `0.227kg 熟豆 + 1个袋子`。
5. 已切换商品的订单需求直接按规格单位进入计划，不再把销售规格转换为库存数量；BOM 组件内部的 g/kg 重量换算继续生效。历史子 SKU 与已冻结工单继续按旧合同执行。

```mermaid
flowchart LR
  T[已发布 BOM 规格模板] --> C[选择主投入物料并复制]
  C --> G[BOM 专属规格组草稿]
  G --> V[逐规格完整配方校验]
  V --> P[整组原子发布]
  P --> D[默认 BOM 提供可售规格]
