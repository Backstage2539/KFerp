...
- PR-487-PRODUCTION-PLAN-CAPACITY-SPLITS / PR-512-PRICING-RULE-TRIAL-SOURCE-COST：PR-486 旧口径更正为：工艺路线只定义工序顺序和价格试算用的 `计划工序成本`，不保存工位、工位产能、标准批量、标准分钟/批、小时费率、计划批次数或计划分钟。工序主数据不维护工时；工位/设备不维护加工时间，只保留默认小时费率作为兜底。
- 进入 `生产管理 -> 工艺路线` 维护路线；路线定义步骤怎么跑，例如烘焙、裁剪、缝制、去皮、包装，并可在每道路线工序填写 `计划工序成本` 供商品价格试算读取。路线不选择 SKU、BOM、BOM 版本、工位或工位产能。进入 `生产管理 -> 工序` 维护工序主数据，进入 `生产管理 -> 工位/设备` 维护工位、设备、默认小时费率和工位产能。BOM 版本草稿选择一条工艺路线，发布后成为价格试算、生产计划/工单使用的路线快照来源。
