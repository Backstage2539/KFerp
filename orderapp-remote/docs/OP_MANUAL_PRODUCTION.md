...
生产计划主页面按业务判断分成三步：`选需求 → 核对缺口 → 安排生产`。

1. **选需求**：按真实商品档案归组，同一商品下展开规格。可搜索商品、客户或订单号；商品、规格和全部需求支持全选、取消与半选。无法生产的需求显示“配置异常”和具体原因，先按提示补齐资料后再选择。
2. **核对缺口**：顶部先给出本次生产结论。“商品缺口”按商品规格显示订单需求、现货覆盖、待生产数量和关联订单；“生产用料”已经包含全部上游层级，只显示需要数量、现货覆盖、在产覆盖、剩余缺口和处理方式。`需制造` 表示系统会建立上游生产任务，`待补料` 表示需先补充不可制造的用料，`在产覆盖` 和 `库存满足` 不需要重复安排。
3. **安排生产**：系统保存生产计划草稿，再选择领料仓和货主，安排每道工序的工位、批量和计划数量，最后提交生成工单。草稿可以点“修改需求”返回第一步；重新核对后仍保存到原计划号，只重新计算变化的部分。若库存或在产供应发生变化，草稿会保留并提示刷新。

计划记录放在页面下方，供查询、查看详情、调整草稿拆分和批量提交。主工作台不显示 BOM 层级、版本号或上下游依赖术语；冻结 BOM、来源仓、订单行和工单追溯仍可在计划详情查看。
...
PR-556-PRODUCTION-PLAN-DRAFT-SPLIT-UX / PR-585-BOM-SINGLE-MATERIAL-LOSS：生产计划的 BOM 摘要不再显示整体产出率。只有实际解析到的 BOM 版本启用了“原料损耗比”时，摘要才显示 `BOM原料损耗`；具体 SKU 继承父商品 BOM 时读取同一父 BOM 版本。PR-649 起，草稿先进入“安排生产”步骤，由计划员主动打开生产安排并核对后提交。
...
- 进入生产计划页时默认只看 `待计划` 需求；需要复盘已提交或已完成需求时，把需求状态切到 `全部` 或对应状态。切到全部后左侧标题显示为 `生产需求`，避免把生产中或已完成的需求误认为还能重复排产。创建生产计划前，当前计划工作台只显示计划预览；生成草稿后可从步骤条第 3 步或草稿详情点击 `编辑拆分`，进入完整的产能拆分工作区。
