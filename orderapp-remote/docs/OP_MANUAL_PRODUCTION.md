...
- PR-487-PRODUCTION-PLAN-CAPACITY-SPLITS / PR-512-PRICING-RULE-TRIAL-SOURCE-COST / PR-514-WORKSTATION-COST-COMPONENTS / PR-516-STANDARD-COST-DEFAULT-CAPACITY：PR-486 旧口径更正为：工位/设备维护机器、人工和其他小时成本；工位产能维护标准批量、单位和标准分钟；工艺路线只维护工序顺序、是否记录损耗和质检项，不选择生产计划实际工位产能，也不维护计划工序成本。PR-516 后，路线工序可额外选择 `标准成本默认产能`，仅用于标准制造成本和价格试算。工序主数据不维护工时，也不维护小时费率。
...
- 进入 `生产管理 -> 工艺路线` 维护路线；路线定义步骤怎么跑，例如烘焙、裁剪、缝制、去皮、包装。每道路线工序选择工序、维护工序名称快照、是否记录损耗和质检项，并可选择 `标准成本默认产能`。该默认产能只用于报价/标准制造成本，页面会展示 `小时费率 × 标准分钟 / 60 / 标准产出 = 元/库存单位` 折算说明；它不锁定生产计划实际设备。BOM 版本草稿选择一条工艺路线，发布后成为价格试算、生产计划/工单使用的路线快照来源；实际使用哪台设备、哪条产能和多少成本，仍在生产计划草稿或未开工工单的 `工序产能拆分` 中决定。
