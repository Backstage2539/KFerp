...
- 生产管理：工艺路线/工艺模板，用于维护制造步骤；商品与配方：商品档案的商品生产配置，用于绑定生产 BOM 版本、工艺路线、预期损耗率和商品生产设定字段。
...
- 行业字段模板用于沉淀不同业务的参数字段；商品生产配置字段是普通用户维护这些参数和值的入口，不要求用户手写 JSON。PR-393 后商品档案配置只能保存所选行业字段模板中已有的字段，不能临时新增或删除字段定义；字段定义调整必须回到行业字段模板。
