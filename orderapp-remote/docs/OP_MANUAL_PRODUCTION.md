...
- PR-487-PRODUCTION-PLAN-CAPACITY-SPLITS / PR-512-PRICING-RULE-TRIAL-SOURCE-COST / PR-514-WORKSTATION-COST-COMPONENTS：PR-486 旧口径更正为：工位/设备维护机器、人工和其他小时成本；工位产能维护标准批量、单位和标准分钟；工艺路线通过选择工位产能自动折算价格试算用的 `计划工序成本`。工序主数据不维护工时，也不维护小时费率。
...
- 进入 `生产管理 -> 工艺路线` 维护路线；路线定义步骤怎么跑，例如烘焙、裁剪、缝制、去皮、包装。每道路线工序选择 `工位产能` 后，系统带出工位小时成本、标准批量和标准分钟，并按 `小时成本 × 标准分钟/60 ÷ 标准批量` 自动折算 `计划工序成本`，供商品价格试算读取。BOM 版本草稿选择一条工艺路线，发布后成为价格试算、生产计划/工单使用的路线快照来源。
