...
- 生产管理：生产 BOM、工艺路线/工艺模板，用于维护配方库和制造步骤；商品与配方：商品档案的商品生产配置，用于绑定生产 BOM 版本、工艺路线、预期损耗率和商品生产设定字段。
...
2. 生产前维护商品档案、商品生产配置、生产 BOM、物料和设备产能；如果该商品档案有固定制造路线，在商品生产配置中选择工艺路线。客户商品名只用于销售侧展示，生产计划会解析到绑定的商品档案。
...
9. 开始生产前确认计划投料、BOM 原料需求、商品生产配置预期损耗率、WIP 可用量和建议领到 WIP 数量；如需调整排产，可在生产建议中直接修改推荐机器、每锅数量和锅数，系统会实时重算最终投料数和预计成品(kg)。系统内部按 `目标产出 / (1 - 预期损耗率)` 计算投料，不再把预期产出率作为用户维护字段。
   - PR-388：生产计划按商品档案当前绑定的生产 BOM 版本读取物料组成。BOM 发布新版不会自动升级已绑定商品；如果商品仍引用旧版本，页面只提示“当前引用 Vxxx，最新 Vyyy”。已开工工单继续使用开工时冻结的 BOM 版本和原料快照，不受后续 BOM 升级影响。
...
11. 点击开始生产后，系统生成生产中记录和生产工单，冻结当时的客户商品名快照、商品档案快照、商品生产配置快照、生产 BOM、原料快照和工艺路线/工艺模板快照，并建立 WIP 软占用。重复点击或用旧生产计划再次提交时，系统会返回 `production already started`，不会重复生成生产中记录、生产工单或 WIP 占用。
12. 在生产工单中查看商品规格、订单信息、生产建议、商品生产配置预期损耗率、冻结生产配置、冻结工艺路线、工序执行汇总、原料摘要、WIP 占用、WIP 已占/已耗/剩余占用，需要时打印工单。
...
- 行业字段模板用于沉淀不同业务的参数字段；商品生产配置字段是普通用户维护这些参数和值的入口，不要求用户手写 JSON。PR-398 后行业字段模板改为左侧模板列表、右侧模板编辑；用户不再维护行业键，字段键同时作为显示名；新增字段只支持文本和下拉，文本字段可填默认文本，下拉字段用空格分隔选项。商品档案配置只能保存所选行业字段模板中已有的字段，不能临时新增或删除字段定义；字段定义调整必须回到行业字段模板。
...
- PR-394：商品档案页和客户商品名页的分类模板 Tab 只影响列表分组、价格表展示和人工查找，不改变生产对象。商品在某个分类模板下重新归类后，已经生成的订单、生产计划、工单和成品批次不会回改；新生产计划仍按订单行冻结的客户商品名和商品档案解析生产 BOM。
