...
PR-473-GENERIC-MANUFACTURING-ABSTRACTION + PR-487-PRODUCTION-PLAN-CAPACITY-SPLITS：生产主流程统一使用 `商品 / BOM / 工艺路线 / 工序 / 工位 / 生产计划 / 工单 / 工序卡`。咖啡、包装盒、童装等行业差异通过 BOM、工艺路线、工序、工位、工位产能和商品生产配置表达；设备产能、单批标准和批次数进入草稿生产计划的工序产能拆分，不写进工艺路线。
...
10. PR-475-PRODUCTION-PLAN-ERPNEXT-WORKBENCH + PR-487-PRODUCTION-PLAN-CAPACITY-SPLITS：创建草稿成功后，右侧当前生产计划立即显示计划号、中文状态和计划行，并显示 `工序产能拆分`。草稿计划可按计划行和工艺路线工序添加拆分行，选择工位产能并填写本次批次数；例如同一烘焙工序可拆成 `布勒 18kg` 5 批和 `智烘 4kg` 2 批。系统按工位产能带出的标准批量、标准分钟/批和小时费率预览计划投入、计划分钟和计划工序成本。点击 `提交当前计划生成工单` 前会保存当前拆分；已提交工单、生产中、已完成和已取消计划不可重复提交或修改拆分。
...
13. 未选择库存不足项目时，不会开始生产，也不会生成生产中记录、生产工单或 WIP 占用。已选项目没有手工填写投料数时，系统按生产计划的默认投料数启动；设备、单批标准和批次数只在草稿生产计划的工序产能拆分中维护，不在工艺路线中维护。只有 `released` 工单允许开始生产；`draft/running/completed/cancelled` 工单不能再次开始。
...
- 创建生产计划后，在草稿计划的 `工序产能拆分` 中选择本次生产使用的工位产能并填写批次数。计划数量、计划分钟和计划工序成本由拆分行计算；提交生产计划时冻结到工单和工序卡，后续修改工位产能主数据不会回改历史工单。
- 拆分计算规则：计划数量为 `批次数 * 标准批量`；计划分钟为 `批次数 * 标准分钟/批`；计划工序成本为 `计划分钟 / 60 * 小时费率`。例如计划拆分为 `布勒 18kg` 5 批、15 分钟/批、300 元/小时，得到 90kg、75 分钟、375 元工序成本；再加 `智烘 4kg` 2 批则额外得到 8kg、对应分钟和成本。
