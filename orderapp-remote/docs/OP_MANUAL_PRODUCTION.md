...
2. **分配执行人**只影响当前工序批次任务，不改变整单调度负责人。批量安排统一在生产排程完成，工单详情和工位视图可临时调整；有权限且符合工序条件的员工可领取任务。仓库、质检和调度只在阻塞时显示为待协同岗位。
...
## 入口
- 生产管理：工位视图、生产配置、生产流程、生产排程、生产日志、生产手册。`生产流程` 用五个 Tab 集中生产计划、生产工单、工序记录、生产质检和完工入库；生产手册固定在生产管理菜单最底部。
...
1. 开工前进入生产验收，确认仓库、原料批次、WIP、生产工单、WIP 占用、生产日志、质检和成品追溯检查项。
   - PR-495-PRODUCTION-WORKSTATION-OVERVIEW / PR-496-PRODUCTION-FLOW-PHASE1-OPTIMIZATION / PR-499-PRODUCTION-EXECUTION-HUB-PHASE2 / PR-562-PRODUCTION-EXECUTION-JOB-CARD-CONSOLIDATION：日常调度进入生产排程，工位视图按工位展示现在做、下一件和不能做原因。工单详情用于汇总人员、阻塞与进度；开始、暂停、继续、完成本工序、报异常和呼叫补料均在工位执行。工序记录只用于查看和复盘。
...
## 制造三期生产协调
- PR-480-MANUFACTURING-PHASE3-SCHEDULE-CAPACITY：生产排程工作台现按 PR-655 的工序任务安排流程操作，计划时间、班次、人员、备注和可用工时变更均可在操作日志追溯。
- PR-481-MANUFACTURING-PHASE3-SCHEDULING-WORKBENCH：排程仅安排已经生成的工序任务，不选择 BOM 或工艺路线。当前工作区已精简为任务列表，参见本手册顶部 PR-655 操作说明。
