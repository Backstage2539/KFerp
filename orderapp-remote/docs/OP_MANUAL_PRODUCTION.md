...
2. 生产前维护商品档案、物料、生产 BOM、工艺路线、工序和工位/设备。生产 BOM 必须声明“产出商品 + 产出数量 + 组件清单”，组件可以是物料，也可以是另一个有库存的半成品/成品商品。商品档案不编辑 BOM，只作为库存/销售对象被 BOM 产出或消耗；商品档案只可在该商品已有的产出 BOM 中设置一个默认 BOM 主表，不选择默认版本。
   ...
   - PR-500-UNIT-MODEL-CONSOLIDATION / PR-504-PARENT-PRODUCT-CHILD-SKU-UOM / PR-505-SALES-SPEC-TEMPLATE-DERIVED-SKU：BOM 产出商品实际选择具体子 SKU，产出单位自动取该子 SKU 继承到的父 SKU 库存单位；销售规格模板可以派生多个子 SKU，但 BOM、工单、WIP、完工入库和库存流水只读库存单位。`袋/227g` 不作为销售单位，`227g袋装` 是子 SKU 规格，BOM 的克重和包装差异通过组件清单表达。新建或编辑生产 BOM 时只选择产出子 SKU 和产出数量，不手工填写产出单位；前端保存时即使携带旧 `output_unit` 字段，后端也会重新读取产出子 SKU 有效库存单位写入草稿版本。组件表单会显示组件库存单位，消耗单位从全局单位字典选择。物料单位不作为新业务概念。
...
## 客户商品到生产执行
- 录单、客户门户和履约侧选择的是客户商品或客户侧商品引用；生产计划收到订单后必须解析到绑定的具体子 SKU，再选择能产出该子 SKU 的生产 BOM、BOM 最新可用版本、BOM 版本工艺路线、商品生产配置和子 SKU 有效库存单位。PR-505 后销售单位来自销售规格模板派生出的子 SKU，库存单位来自父 SKU；销售单位不进入 BOM 主数据，也不从商品价格管理重新推导生产数量。
