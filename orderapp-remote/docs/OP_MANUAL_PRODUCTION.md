...
2. 生产前维护商品档案和物料，并在 `生产管理 / 生产配置` 的工艺路线、工序、工位/设备、生产 BOM 四个 Tab 维护制造主数据。生产 BOM 必须声明“产出商品 + 产出数量 + 组件清单”，组件可以是物料，也可以是另一个有库存的半成品/成品商品。商品档案不编辑 BOM，只作为库存/销售对象被 BOM 产出或消耗；商品档案只可在该商品已有的产出 BOM 中设置一个默认 BOM 主表，不选择默认版本。
   ...
   - PR-585-BOM-SINGLE-MATERIAL-LOSS：BOM 版本设置区只维护一个 `原料损耗比`。公式统一为 `实际原料需求 = 计划投料基准 × 配方比例 × (1 + 原料损耗比)`；例如生产基准 `1kg`、A 原料比例 `40%`、原料损耗 `20%`，A 原料需求为 `1 × 40% × 1.2 = 0.48kg`。未配置原料损耗时，需求就是净配方数量；`合计比例` 只统计净配方比例，不含损耗。
...
12. PR-585-BOM-SINGLE-MATERIAL-LOSS + PR-562-PRODUCTION-EXECUTION-JOB-CARD-CONSOLIDATION：在生产工单页筛选 `released`，点击工单号或“执行枢纽”，确认计划投料、BOM组件需求、BOM原料损耗、WIP可用量和建议领到 WIP 数量后，在执行枢纽点击 `开始生产`。新计划按 `目标产出 × (1 + BOM原料损耗率)` 计算理论投料；未配置 BOM 原料损耗时投料等于目标产出。旧 `yield_rate`、商品 `expected_loss_rate` 和设备整批不得再次放大物料需求。
   ...
   - 例如14件454g成品需求为6356g，BOM原料损耗18%时理论投料为 `6356 × 1.18 = 7500.08g`，为保证用料覆盖按整克向上取整为7501g；智烘按4kg标准批量安排8kg只属于拆分产能，不会把物料需求改成8000g或10000g。
...
- 客户看到的状态由真实生产计划、工单和工序卡派生，包括待排产、已排产、已下达、生产中、暂停、部分完成、已完成和取消；不要只改申请头状态代替真实执行。
   - 生产 BOM 的列表分类来自 `设置 / 业务设置 / 分组模板` Tab，不是生产 BOM 自己维护的大组/小类。先维护分组模板的大类和小类，再回到生产 BOM 页面点击 `设置分组模板` 打开抽屉，勾选要使用的分组模板并 `保存模板选择`；保存后 BOM 列表按模板完整大类 / 小类树和 `未分类` 自动整理，空分类也显示，不再提供分类过滤 Tab。未选择分组模板时列表平铺展示；分组模板选择属于低频操作收在抽屉里，列表顶部只保留紧凑的分类与移动工具栏，`设置分组模板` 入口同时在工具栏和未选模板的空状态中提供。
   - PR-588 起，“生产 BOM列表”标题下不再重复显示接口和主档说明。前往分组模板、分类与移动模板、移动到分类、目标分类、已选数量和设置分组模板等分组处理在桌面端保持一行，窄屏最多两行。列表初始只展开第一顶层分组；打开另一模板或 `未分类` 时，原展开组自动收起。页码和每页条数只服务当前展开组的 BOM 行，分类标题和收起组不占分页条数；切组、状态或搜索后从第 1 页开始。没有选择模板时仍按平铺列表分页。
