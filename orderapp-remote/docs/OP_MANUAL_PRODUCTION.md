...
15. 在生产工单中查看商品规格、订单信息、计划数量、BOM/工艺路线、工艺参数、预期损耗率、冻结生产配置、工序执行汇总、冻结工位产能、计划分钟、计划工序成本、组件/原料摘要、多层展开树、WIP 占用、WIP 已占/已耗/剩余占用，需要时打印工单。PR-493-PLAN-WORKORDER-SPLIT-EDIT：工单状态为 `released` 且所有工序卡仍是 `pending` 时，可以在生产工单页点击 `编辑拆分`，补充或调整工位产能和承担产量；保存后系统重建 pending 工序卡并记录操作日志。工单开始生产后，拆分入口不可用，不能覆盖已执行工序卡。PR-499 / PR-559 / PR-560 / PR-561：工单号或 `执行枢纽` 按钮会打开工单执行枢纽，先看 readiness、逐物料 WIP 需求/可用/缺口和质检状态；红色“WIP库存不足”时点击“生产领料”处理。多物料领料在同一列头下逐行显示，缺口为首次默认建议值；仓管可按 60Kg 等实际批量修改，库存单位为 g 时填写 60000。最近一次已提交批量会作为下次默认值；恢复旧草稿时保留原数量并提示当前建议量，提交只受真实原料仓合格 FIFO/指定批次库存限制。超出工单需求的部分留在 WIP，不会自动写成生产消耗；生产消耗最多只能达到工单尚未消耗的冻结需求。原料仓不足时按中文逐物料提示调整。历史 `roast_level` 等字段只作为工艺参数兼容展示，不作为咖啡烘焙主列。上层工单和下层工单保持独立，不把所有层级混成一张不可追踪的大工单。
