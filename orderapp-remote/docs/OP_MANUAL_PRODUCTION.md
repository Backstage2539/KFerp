...
1. 开工前进入生产验收，确认仓库、原料批次、WIP、生产工单、WIP 占用、生产日志、质检和成品追溯检查项。
   - PR-495-PRODUCTION-WORKSTATION-OVERVIEW：日常调度先进入 `生产视图`。页面按今日任务汇总待处理、执行中、异常、工位负载、阻塞原因和下一步处理人；需要处理时可直接打开工单、打开库存作业、打开质检，或在右侧保存工位/负责人/优先级分配。现场执行先进入 `工位视图`，按工位查看 `现在做`、`下一件` 和 `不能做原因`；可在任务行执行开始、暂停、继续、完成本工序、部分完成、报异常和呼叫补料。报异常和呼叫补料会把工序卡暂停并记录异常原因，补料或复核完成后再继续或完成。
