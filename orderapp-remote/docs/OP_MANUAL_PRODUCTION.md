...
9. 新建生产计划时，先勾选库存不足商品；可点击 `生成计划` 预览缺口、投料数和物料需求，也可直接点击 `创建生产计划`，系统会先按所选商品自动生成计划再创建正式单据。计划行冻结商品快照、客户/订单来源、缺口数量、选中 BOM 版本、工艺路线和组件需求快照。创建计划只形成 `draft` 计划，不创建生产中记录、生产日志或 WIP 占用。
...
12. 未选择库存不足项目时，不会开始生产，也不会生成生产中记录、生产工单或 WIP 占用。已选项目没有手工填写投料数时，系统按生产计划的默认投料数启动；操作员在烘焙建议中修改机器、每锅数量或锅数后，使用修改后的投料数。只有 `released` 工单允许开始生产；`draft/running/completed/cancelled` 工单不能再次开始。
...
14. 在生产工单中查看商品规格、订单信息、生产建议、预期损耗率、冻结生产配置、冻结工艺路线、工序执行汇总、组件/原料摘要、多层展开树、WIP 占用、WIP 已占/已耗/剩余占用，需要时打印工单。上层工单和下层工单保持独立，不把所有层级混成一张不可追踪的大工单。
