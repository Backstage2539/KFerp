...
## 入口
- 生产管理：生产视图、工位视图、生产配置、生产 BOM、生产流程、生产排程、生产日志、生产手册。`生产流程` 用五个 Tab 集中生产计划、生产工单、工序卡、生产质检、生产验收；生产手册固定在生产管理菜单最底部。顶部生产切换条使用生产流程统一入口，并保留 `生产中` 兼容状态页和统一完成面板。
- 生产管理：`生产配置` 用三个 Tab 维护工艺路线、工序、工位/设备；`生产 BOM` 维护产出商品、组件清单和多层展开。生产成本不再作为常规菜单或顶部切换入口，但工单执行枢纽和追溯上下文仍可打开既有成本明细。
...
2. 生产前维护商品档案、物料、生产 BOM，并在 `生产管理 / 生产配置` 的工艺路线、工序、工位/设备三个 Tab 维护制造主数据。生产 BOM 必须声明“产出商品 + 产出数量 + 组件清单”，组件可以是物料，也可以是另一个有库存的半成品/成品商品。商品档案不编辑 BOM，只作为库存/销售对象被 BOM 产出或消耗；商品档案只可在该商品已有的产出 BOM 中设置一个默认 BOM 主表，不选择默认版本。
