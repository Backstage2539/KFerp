...
## 客户商品到生产执行
- 录单、客户门户和履约侧选择的是客户商品或客户侧商品引用；生产计划收到订单后必须解析到绑定的具体子 SKU，再选择能产出该子 SKU 的生产 BOM、BOM 最新可用版本、BOM 版本工艺路线、商品生产配置和子 SKU 有效库存单位。PR-505 后销售单位来自销售规格模板派生出的子 SKU，库存单位来自销售规格模板；销售单位不进入 BOM 主数据，也不从商品价格管理重新推导生产数量。
