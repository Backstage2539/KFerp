...
2. 生产前维护商品档案和物料，并在 `生产管理 / 生产配置` 的工艺路线、工序、工位/设备、生产 BOM 四个 Tab 维护制造主数据。生产 BOM 必须声明“产出商品 + 产出数量 + 组件清单”，组件可以是物料，也可以是另一个有库存的半成品/成品商品。商品档案不编辑 BOM，只作为库存/销售对象被 BOM 产出或消耗；商品档案只可在该商品已有的产出 BOM 中设置一个默认 BOM 主表，不选择默认版本。
   ...
   - PR-500-UNIT-MODEL-CONSOLIDATION / PR-504-PARENT-PRODUCT-CHILD-SKU-UOM / PR-505-SALES-SPEC-TEMPLATE-DERIVED-SKU / PR-553-PRODUCTION-PLAN-PARENT-BOM-UNIT-CONVERSION：新建 BOM 时仍可选择具体子 SKU 作为产出；若多个销售规格只是同一父商品配方按库存单位缩放，也可以只在父商品维护通用 BOM，由没有自身 BOM 的子 SKU 在生成生产计划时继承。产出单位自动取 BOM 来源商品的有效库存单位；计划、工单、WIP、完工入库和库存流水仍冻结并产出订单上的具体子 SKU。`袋/227g` 不作为销售单位，`227g袋装` 是子 SKU 规格。新建或编辑生产 BOM 时不手工填写产出单位；前端保存时即使携带旧 `output_unit` 字段，后端也会重新读取产出商品有效库存单位写入草稿版本。组件表单会显示组件库存单位，固定用量的消耗单位跟随该库存单位。
   - PR-592-BOM-LOSS-GROSS-INPUT / PR-593-BOM-LOSS-PACKAGING-UNITS / PR-594-PRICING-BOM-DEBUG-WORKFLOW：BOM 版本设置区只维护一个 `原料损耗比`，表示损耗占总投料比例。开启后先选择“有损耗的配方”或“无损耗的配方”：前者按 `比例 %` 录入并应用当前 BOM 原料损耗，后者包含包材、固定用量物料和商品组件，按其个、件、袋、盒等库存单位录入且不参与损耗；两类组件可在同一 BOM 共存。比例原料公式统一为 `实际原料需求 = 计划投料基准 × 配方比例 ÷ (1 - 原料损耗率)`；例如生产基准 `1kg`、A 原料比例 `40%`、原料损耗 `20%`，A 原料需求为 `1 × 40% ÷ 0.8 = 0.50kg`。未配置原料损耗时，需求就是净配方数量；`合计比例` 只统计净配方比例，不含损耗。
...
- 生产成本异常：检查 BOM 唯一的原料损耗、投料数量、物料采购价和成本计价单位、标准工序成本快照及实际产出。重量物料库存单位可以是 `g`，但成本单价应按 `元/kg`；20% 原料损耗应只按 `净配方 × 1.2` 计算一次，旧整体产出率或商品预期损耗不得参与当前结果。
...
- 客户看到的状态由真实生产计划、工单和工序卡派生，包括待排产、已排产、已下达、生产中、暂停、部分完成、已完成和取消；不要只改申请头状态代替真实执行。
   - 生产 BOM 的列表分类来自 `设置 / 业务设置 / 分组模板`，不是生产 BOM 自己维护的大组/小类。先维护分组模板的大类和小类，再回到生产 BOM 页面点击 `设置分组模板` 打开抽屉，勾选要使用的分组模板并 `保存模板选择`；分组模板选择属于低频操作，入口保留在共享分类工作区底部。
   - PR-588-PRICING-TRIAL-PARENT-PICKER-BOM-ACCORDION：删除“生产 BOM列表”标题下重复的接口和主档说明，并压缩分组维护、移动和设置操作占用的空间；多模板列表初始只展开第一顶层分组，页码和每页条数只服务当前展开组的 BOM 行。PR-595-UNIFIED-CATEGORY-MOVE-INTERACTION 后，分类浏览和目标选择改由左树承载，不再显示 `分类与移动模板` 或 `目标分类` 下拉；状态、BOM 名称/编号搜索及分类内分页语义继续保留。
   - PR-595-UNIFIED-CATEGORY-MOVE-INTERACTION：共享分类工作区始终显示；未选择模板时左树仅显示 `全部分类`，右侧平铺、`移动到分类` 禁用，底部 `前往分组模板` 和 `设置分组模板` 入口保留。选择模板后左侧分类结构按“全部分类 → 模板 → 大类 → 小类”合并展示，右侧继续保留状态和 `按 BOM 名称或编号搜索`。勾选 BOM 后点击 `移动到分类`，右侧列表置灰、左树提示 `请选择要移动到的分类` 并自动展开；直接点击大类、小类或未分类后立即移动，不显示目标下拉且不二次确认。`全部分类` 和模板标题不可作为目标。成功后清空勾选，失败时保留勾选和移动模式供重试；成功或取消会恢复进入移动前的分类、展开状态和滚动位置。
