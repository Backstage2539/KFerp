...
4. 全局设置用于控制账号模式下的菜单入口和全局单位字典；“客户账户模式隐藏履约运营台”默认开启，只隐藏客户账户模式里的内部履约运营台入口，不删除履约运营台页面、客户侧工作台或履约 API。全局单位字典维护 kg、盒、箱等基础单位，供 SKU设置 的单位模板引用。编辑已有基础单位时可删除该单位，删除为软失效：新建/编辑候选不再显示，历史单位模板、价格表、订单、BOM、工单和成本快照不回改。
...
- 全局单位字典删除后历史单据还显示该单位：这是预期行为；删除只阻止新选择，不回改历史业务快照。
