...
3. `分组模板` 维护模板名称、编码、排序、备注、大类/小类和“功能引用”多选；商品、物料、BOM、仓库仍在各自业务页面查看分类和移动归类。`全局单位字典` 维护 kg、盒、箱等基础单位，供 SKU 单位模板和阶梯价模板引用；删除单位不回改历史单位模板、价格表、订单、BOM、工单和成本快照。
...
## PR-584-PRODUCT-MULTI-GROUP-TEMPLATES 分组模板功能引用与审计
- 适用角色：具备业务设置写权限的管理员。进入 `设置 → 业务设置 → 分组模板`，在左侧选择已有模板或新建模板，在编辑区勾选“功能引用”多选项。一个模板可以供多个功能引用，同一功能也可以同时引用多份模板。
- 可选功能包括商品档案、物料档案、生产 BOM、仓库库存和商品价格表等实际接入场景。只有明确启用引用的模板才会进入对应业务页面；仅保存模板名称和分类、但没有勾选功能引用的模板，不再作为通用候选自动出现。
- 保存时，模板资料、功能引用列表和操作日志作为一次完整业务写入处理。保存成功后刷新模板，确认勾选项仍存在；再进入对应业务页面，确认只看到明确引用的模板。操作日志应能按分组模板名称或模板编号查到保存人、时间以及引用功能的变化。
- 取消引用只会隐藏对应业务入口中的模板分类和移动归类能力，不会删除模板、分类、既有对象归类或历史快照，也不会把业务对象自动移动到其他模板。以后重新勾选同一功能引用时，原有归类可以重新显示。
- 若要删除模板或分类，不要用“取消功能引用”代替删除。删除仍走分组模板既有删除流程，并按页面提示处理当前引用分类的对象；价格表、订单、BOM、工单等历史快照不回改。
- 常见异常：业务页面没有出现刚保存的模板时，先确认模板和对应功能引用都处于启用状态，再刷新业务页面；出现不应展示的模板时，检查该模板是否仍保留相同功能的 active 引用。配置保存失败或操作日志没有成功记录时，不继续批量归类，先让管理员排查事务和权限。
