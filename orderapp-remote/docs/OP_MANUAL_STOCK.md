...
PR-560-PRODUCTION-ISSUE-COMPACT-DIAGNOSTICS：工单库存动作的多物料明细按统一列头紧凑排列，桌面端一行一个物料。原料仓库存不足、质检冻结和超过当前剩余 WIP 缺口均使用中文逐物料说明；恢复旧草稿时按最新缺口刷新可领数量。
...
2. 快捷动作打开同一库存单据抽屉，并显示 `工单号：WO-...`。生产领料会一次带入全部当前短缺物料；桌面端按统一列头一行显示一个物料，物料、原料仓、WIP 仓和库存单位只读，每行只填写一个“领用数量”。可删除本次暂不处理的行，允许分批领料。
3. 首次领料默认当前剩余缺口；同一工单同一物料再次领料时，默认值不超过最近一次已提交领料量和当前剩余缺口。已有草稿时恢复草稿并重新读取当前 WIP 覆盖；若过期草稿数量已经大于当前剩余 WIP 缺口，页面将数量收敛到当前可领上限，保存或提交后才更新草稿。已取消单据不作为数量记忆来源。
...
- 领料数量超过当前剩余 WIP 缺口：重新打开生产领料，页面会按最新缺口刷新旧草稿；若在打开后其他人又完成领料，提交仍会用中文列出本次领用和当前可领数量，按新上限调整后再提交。
