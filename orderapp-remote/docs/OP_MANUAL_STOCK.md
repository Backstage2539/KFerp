...
3. 选择 `领料到WIP`，填写工单 ID、工序卡 ID、物料 ID、物料名称、出库仓 `原料仓`、入库仓 `WIP在制仓`、数量和备注，提交后系统生成 Stock Entry 单据并写操作日志。
   - PR-503-PRODUCT-UNIT-TEMPLATE-MULTI-SALES-UOM：库存作业、WIP、完工入库和库存流水都按物料或商品的库存单位录入和展示。商品销售单位只在订单/价格表快照中冻结换算，不进入库存单据；例如销售 `盒` 会先在订单/生产需求中折算成 `kg`、`袋` 等库存单位后再进入生产和库存。
