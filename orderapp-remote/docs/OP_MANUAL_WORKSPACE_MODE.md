...
flowchart TD
  ...
  B -->|围绕一个客户| D["当前视图：客户"]
  B -->|围绕一张订单| E["当前视图：订单"]
  D --> F["客户商品、价格表、录单、订单、库存、费用默认过滤"]
  E --> G["订单列表锁定订单；录单、履约和追溯默认使用订单客户"]
  C --> H["全局订单、生产、库存、财务、设置和需求管理"]
  F --> I["商品档案仍是执行对象；客户商品只是客户侧展示对象"]
  G --> I
```

## 标准操作
1. 处理工厂整体运营时，保持“当前视图：工厂总览”。左侧菜单按订单销售、生产管理、库存管理、商品、财务管理、设置、系统、需求管理展开。
...
7. 商品管理在客户视图下默认进入“客户商品”：客户商品绑定商品档案，只维护客户对外名称、编号、品牌和展示分类，不直接编辑 BOM。
8. 产品价格表在客户视图下锁定“价格表归属”为当前客户；候选只来自该客户启用且纳入价格表的客户商品。品牌名为空时不会自动填工厂品牌。
9. 生产 BOM 在客户视图下仍是生产管理主档案；它声明产出商品和组件清单，不属于客户商品，也不由商品档案绑定维护。
10. 录单在客户视图下默认选中当前客户；订单视图下默认使用该订单客户。保存订单时仍由后端校验客户商品、商品档案、价格表和跨客户范围。
11. 生产计划和工单仍是工厂执行入口；客户或订单视图只提供过滤标签和追溯辅助，不把生产管理变成客户专属生产页面。

## 商品模型边界
- 商品档案：真实 SKU，用于库存、销售、成本、生产产出、成品/半成品批次和工单反查。
- 客户商品：某客户名下对外展示的名称、编号、品牌和分类，绑定一个商品档案。
- 生产 BOM：生产端制造定义，声明产出商品、产出数量、组件清单、版本和可选工艺信息。
- 产品价格表：按价格表归属客户读取客户商品，发布时冻结客户商品和商品档案双快照。
- 工厂自营：可作为一个客户维护客户商品、价格表和订单，但执行侧仍使用商品档案和生产 BOM。
...
- 客户视图进入商品管理时默认显示客户商品；不能在客户商品里直接编辑生产 BOM。
- 客户视图进入产品价格表时，价格表归属锁定为当前客户，预览卡片、生成抽屉、PDF/发布候选只包含当前客户客户商品。
...
- 客户价格表内容不对：先确认当前视图客户，再检查该客户是否维护客户商品并勾选“进入价格表”。
- BOM 看起来不是客户自己的：这是正常边界。BOM 是生产主档；如果只是贴牌，多个客户商品可以绑定同一个商品档案，并由同一个产出 BOM 生产。
