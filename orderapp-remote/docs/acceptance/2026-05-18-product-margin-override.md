...
- 单个公共SKU填写利润率后，成本试算和商用豆单预览使用该值覆盖二级分类绑定的梯度模板利润率。
- 清空产品级利润率覆盖后，产品恢复继承二级分类梯度模板利润率。
...
- SKU设置页“客户SKU列表 · 公共SKU”展示“利润率覆盖”输入列，留空表示继承分类模板。
- 操作手册已同步说明产品级利润率覆盖、二级分类梯度模板继承关系和清空恢复规则。
