...
- “商品分类 · 客户SKU”只提供“是否使用公共商品分类”开关；“客户SKU列表 · 客户 SKU”只提供“是否使用公共SKU”开关；开启后分别复制当前公共商品分类和公共 SKU 给该客户，用于后续生成客户自定义豆单。
- 复制公共 SKU 时必须保留公共 SKU 原名；旧库 `products.name` 全局唯一约束由 schema 启动修复移除，不能报 `products_name_key`。
...
- 后端复制公共 SKU 曾为绕开旧库 `products.name` 全局唯一约束而改名；当前要求保留公共 SKU 原名，因此需要从 schema 层移除旧约束。
...
- 新增 `POST /api/product-settings/customer-public-copy`：
  - `use_public_sku=true` 时复制当前公共 SKU 为客户自己的 SKU，保留公共 SKU 原名、基础 SKU 关联、价格档、BOM、挂耳配置、生豆 BOM 绑定和分类位置。
