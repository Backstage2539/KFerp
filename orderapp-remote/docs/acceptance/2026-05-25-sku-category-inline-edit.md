...
4. 点击产品类型工具栏减号后，每个可编辑产品类型左侧显示红色减号；点击红色减号可删除该产品类型。
5. 每个产品类型左侧显示上下箭头，可调整大类顺序；搜索状态下排序按钮禁用，避免按过滤后的列表错排。
6. 每个大类标题下方显示子类型加号和减号；加号新增子类型，减号进入该大类的子类型删除模式，并在子类型左侧显示红色减号。
