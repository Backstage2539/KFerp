...
8. 切到某个履约客户，复制公共商品配置模板；确认页面仍在该客户上下文，公共模板仍可见且只新增一条客户模板。重复复制同一公共模板时返回已有客户模板。
...
- 客户 SKU 的商品配置页仍保持 `客户 #1 SKU` 上下文，公共模板和 `客户复制速溶配置测试` 同时可见，公共模板可继续复制为客户配置。
