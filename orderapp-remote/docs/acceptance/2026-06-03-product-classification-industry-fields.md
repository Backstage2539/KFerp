# PR-397 商品分类交互、客户商品、产品价格表 SQL 与行业字段

## 范围
- 商品档案/客户商品分类工具区拆为“增加分类”和“移动分类”两张卡片。
- 客户商品补齐搜索、启停过滤、批量停用和客户行业字段覆盖值。
- 产品价格表候选查询修复 `classification_template_id` 歧义，并优先使用客户商品行业字段覆盖值。
...
- API RED：新增接口/仓储/SQL 断言覆盖客户商品 `active/q`、批量停用、客户商品行业字段、分类项删除回未分类、产品价格表 SQL 无歧义，初始失败。
