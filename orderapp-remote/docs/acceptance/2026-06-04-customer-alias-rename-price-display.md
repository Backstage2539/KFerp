# PR-410 客户商品重命名与商品编号展示验收记录

## 范围
- 客户商品配置中，旧“品牌名”改为“重命名”；底层 `brand_name` 字段继续作为兼容存储字段。
- 客户商品列表删除独立品牌名列，客户商品列优先展示重命名。
- 客户范围产品价格表、PDF/预览和发布内容优先使用重命名作为客户商品展示名称。
- 商品档案列表展示稳定商品编号，优先 `product_code`，否则使用 `SKU-000xxx` 兼容编号。

## 验收点
- [x] 客户商品抽屉显示 `重命名`，不再显示 `品牌名`。
- [x] 客户商品列表没有 `品牌名` 列；填写重命名后主名称显示重命名。
- [x] 客户产品价格表候选和 PDF 组装逻辑使用 `重命名 > 客户商品 > 商品档案名`。
