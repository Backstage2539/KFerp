...
- 商品档案配置抽屉只读展示“被哪些 BOM 使用”，不再编辑生产 BOM 绑定；PR-419 后该列表只展示把当前商品作为组件消耗的上层 BOM，不再展示产出当前商品的 BOM、关系前缀或版本号。
...
- 商品没有自己的产出 BOM 时，也能在商品档案中看到被上层 BOM 使用的关系；商品有产出 BOM 时，产出关系在生产 BOM 中查看，不再混入商品档案使用关系列表。
