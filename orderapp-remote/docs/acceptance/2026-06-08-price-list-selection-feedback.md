...
- Follow-up deploy: browser-comment follow-up was pushed through `develop`; `origin/develop=e1a1b7f15f65b0b66de4fefd498841173863bee6` deployed to the development stack. Backup: `root@1.12.242.58:/opt/stacks/erp/orderapp.backup.deploy-20260608121235`. Deploy script ran Vue shell build, miniapp typecheck/build, miniapp `build:mp-weixin`, Docker image build, and container-internal `go test ./...`.
- Follow-up smoke: `erp_orderapp` was Up, `erp_postgres` was healthy, unauthenticated `https://erp.qacoohee.com/app/` returned `303` to `/app/orders`, authenticated `https://erp.qacoohee.com/app/vue-shell/?view=costing` returned `200`, support API exposed `PR-449-PRICE-LIST-SELECTION-FEEDBACK`, and deployed source exposed `price-list-page-config`, `price-list-rules-dialog`, `价格表配置`, and `DEV-449-PRICE-LIST-PAGE-CONFIG`.
- Deployed browser follow-up: `https://erp.qacoohee.com/vue-shell?view=costing&pr449_followup=1` at 599x752 rendered 商品数 `2` and `选择分类和产品 2/2 款`; main page had `price-list-page-config=1`, old `.collapsible-bean-section=0`, persistent `.price-list-model-panel=0`, `Price List / Item Price 生成规则` once, `价格表配置` button count `1`, and `计价模式规则` button count `1`. Clicking `价格表配置` opened `aria-label="价格表配置"` and did not duplicate generation rules in the drawer. Clicking `计价模式规则` opened a modal with `商品 > 子类 > 父类 > 价格表` and `group_source=price_list`; page text no longer contained `模板继承规则`; console errors `0`. Screenshot: `/tmp/pr449-deployed-price-list-page-config-followup.png`.
- Pending: Van product acceptance.
