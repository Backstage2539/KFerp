...
## 当前口径更新
- PR-459 已覆盖本记录中的发布售价快照反推行为。当前价格计算模板试算缺 BOM/工序成本时不再反推历史发布售价，`BOM+工序成本` 显示 0，并显示明确警告。
...
- 试算抽屉和结果区删除 `售价后附加成本`。
- 报价单位改为全局单位字典下拉。
- 历史 PR-456 曾支持 `PR439-20260606182321 熟豆下单商品` 无 BOM/工序成本时按已发布价格快照反推；PR-459 后不再反推。
...
- PR-459 后，选择缺 BOM/工序成本的商品时不再反推，BOM+工序成本显示 0 并显示警告。
