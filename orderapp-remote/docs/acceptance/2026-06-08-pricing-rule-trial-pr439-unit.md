...
## 当前口径更新
- PR-460 已覆盖本记录中的发布售价快照反推行为。当前价格计算模板试算缺 BOM/工序成本时不再反推历史发布售价，`BOM+工序成本` 显示 0，并显示明确警告。
- 本记录下方的 `88.5/kg` 和发布售价快照内容只作为 PR-456 当时部署验收的历史证据，不作为当前验收口径。

## 范围
- 商品价格管理 `价格计算模板试算` 抽屉删除 `重新试算` 按钮。
- 试算抽屉和结果区删除 `售价后附加成本`。
- 报价单位改为全局单位字典下拉。
- 历史 PR-456 曾支持 `PR439-20260606182321 熟豆下单商品` 无 BOM/工序成本时按已发布价格快照反推；PR-460 后不再反推。
...
- PR-460 后，选择缺 BOM/工序成本的商品时不再反推，BOM+工序成本显示 0 并显示警告。
