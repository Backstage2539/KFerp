...
- Follow-up implementation commit `origin/develop=d95e3b50c34bc4b8ae3b236c2bfddd0d5721f0f3` deployed to development; backup `root@1.12.242.58:/opt/stacks/erp/orderapp.backup.deploy-20260608141222`.
- Follow-up browser DOM acceptance: child group `意式拼配豆` header uses `--classification-group-indent: 40px` and its product row uses `--classification-item-indent: 42px` / computed padding `42px`; parent group `BOM-咖啡熟豆` header uses `--classification-group-indent: 16px` and its product row uses `--classification-item-indent: 18px` / computed padding `18px`; console errors 0. Screenshot: `/tmp/pr451-deployed-product-indent.png`.
