...
- DEPLOY development: `./deploy_orderapp.sh` deployed `origin/develop=c8f27a053b11e576fbe4a8a5f98f743de311947b`; backup `root@1.12.242.58:/opt/stacks/erp/orderapp.backup.deploy-20260613005126`.
- SMOKE development: `erp_orderapp` is up; `/app/vue-shell?view=producePlan` and `/app/vue-shell?view=jobCards` returned 200 under auth; `/app/api/production-plans?limit=1`, `/app/api/produce/unproduced?limit=1&demand_status=unplanned`, and `/app/api/produce/job-cards?limit=1` returned 200; req API exposed `PR-491-PRODUCTION-DEMAND-STATUS-JOBCARD-CONTEXT`; deployed Vue asset contains `demand_status`, `job-card-work-order-drawer`, and `已进入生产计划的需求不可重复生成计划`.
