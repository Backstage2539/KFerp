...
- 商品档案配置抽屉只保留 `销售规格模板明细` 主展示区，默认隐藏 `template_removed` 历史 SKU；打开 `显示历史规格` 后才展示 `模板已移除` 行，并提示历史 SKU 仅用于历史单据。
- 商品价格表、BOM、录单、生产日志和库存调整的新建候选默认过滤 `template_removed` 派生 SKU；历史详情仍可按 SKU 编号回显。
