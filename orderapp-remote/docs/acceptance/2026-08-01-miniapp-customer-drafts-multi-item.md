...
- 小程序：合并环境隔离后共 19 个测试文件、111 项测试全部通过；`vue-tsc --noEmit` 与 development/production 环境门禁下的 `mp-weixin` 构建通过。覆盖多行独立增删改、草稿完整恢复、失效客户/商品保留并阻止误提交、手工单价、同 SKU 多客户别名身份贯通、快捷客户维护并发保护及全页面环境标识。
