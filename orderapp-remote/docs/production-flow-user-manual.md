...
> 先判断成品库存。库存充足的订单标记为“无需生产”后直接发货；库存不足才把原料领到 WIP，在生产计划里开始生产，按工单生产，最后在“生产中”完成入库。
...
| 排产 | 生产管理 -> 生产计划/开始生产 | 只选择库存不足的商品生成计划；库存充足订单标记为无需生产；查看建议领到 WIP 数量 | 缺口商品进入生产；库存充足订单可去订单列表直接发货 |
...
1. 查看需要生产的商品。
2. 选择要生产的行。
3. 库存充足的订单不进生产工单，生产状态设为“无需生产”，发货状态仍保持“未发货”。
4. 只对库存不足的行点击“生成计划”。
5. 检查烘焙建议、建议投料、BOM 原料需求。
6. 确认投料克重。
7. 点击开始生产。
...
| 成品库存已经够，订单状态怎么处理 | 不需要进生产工单 | 生产状态设为“无需生产”；发货状态保持“未发货”，回填快递单号后系统标记“已发货” |
