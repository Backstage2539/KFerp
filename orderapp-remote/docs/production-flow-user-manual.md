...
flowchart LR
  ...
  B --> C["领料到 WIP<br/>库存作业"]
  C --> D["库存判断<br/>确认是否使用成品批次/库存余额"]
  D -->|使用库存批次| J["订单列表<br/>库存待发货"]
  D -->|不使用或库存不足| E["开始生产<br/>仅处理缺口商品"]
  E --> F["查看/打印工单<br/>生产工单"]
...
> 先判断成品库存。库存充足时录单会提示可用批次；如果旧库存余额没有 FP 批次，会提示 `LEGACY-FP-商品ID-规格`。确认使用后订单进入“库存待发货”并可直接发货；不使用现有批次或库存不足时才把原料领到 WIP，在生产计划里开始生产，按工单生产，最后在“生产中”完成入库。
...
保存订单前，如果订单商品的成品批次库存或历史成品库存余额足够，系统会列出建议使用的批次或库存余额：

1. 选择“使用以上批次”：订单生产状态进入“库存待发货”，不生成生产工单，后续到订单列表“只看可发货”里直接生成顺丰发货。
2. 选择“不使用”：系统不占用这些成品批次，订单按库存不足处理，后续会出现在生产计划的库存不足区。

如果商品只有旧的 `finished_inventory` 余额、没有 `FP-...` 成品批次，提示里会显示 `LEGACY-FP-商品ID-规格`。它代表系统库存余额，不代表可追溯的生产批次；确认使用后同样可直接进入顺丰发货。

确认使用库存时，保存订单只做预留，不马上扣物理库存。生成顺丰发货 Excel 仍然不扣库存；在订单抽屉或顺丰回传 Excel 里回填快递单号、订单变成“已发货”时，系统才扣减对应 `FP-...` 批次或 `LEGACY-FP-...` 对应的成品库存余额，并写库存流水。重复回填同一订单不会重复扣库存。

这个选择是为了处理“库存虽然够，但这批不想发给这个客户”的情况，例如要保留某个批次、客户指定新鲜烘焙或质量/账务原因需要重新生产。
...
| 排产 | 生产管理 -> 生产计划/开始生产 | 只选择库存不足的商品生成计划；库存充足区只做提示；查看建议领到 WIP 数量 | 缺口商品进入生产；库存待发货订单可去订单列表直接发货 |
...
| 复盘 | 库存管理 -> 仓库库存 / 出库日志 | 查库存、查 FP 成品批次追溯；查看并下载出库单 | 看到成品用了哪些原料批次，也能追到订单出库凭证 |
...
1. 查看库存不足和库存充足两个区块。
2. 库存不足（需生产）标题左侧复选框可全选或全不选；只勾选部分行时显示半选减号。
3. 库存充足区只提示，不允许勾选进入生产工单。
4. 如果录单时拒绝使用库存批次，这类订单会被放到库存不足区，即使物理库存仍然显示充足。
5. 已发货订单不进入生产计划；如果历史订单生产状态为空但发货状态已是“已发货”，系统也会排除。
6. 库存待发货但尚未回填快递单号的订单，会占用可用成品库存，避免同一库存被后续订单重复使用。
7. 只对库存不足的行点击“生成计划”。
8. 检查烘焙建议、建议投料、BOM 原料需求。
9. 确认投料克重。
10. 点击开始生产。
...
1. 左侧选择质检类型：工单质检、原料质检或产品质检。
2. 当前是工单质检时点击“选择工单”；当前是原料质检时点击“选择原料批次”；当前是产品质检时点击“选择产品批次”。
3. 右侧抽屉只显示当前类型对应的对象，选择后系统会回填单据号/批次号和物料/产品名称。
4. 填检查结果：通过、待处理、不合格。
5. 可选填指标 JSON 和备注。
6. 保存。
...
| WIP看起来有库存，但开始生产仍提示不足 | WIP 库存可能已被其他工单软占用 | 到“仓库库存 -> WIP占用”处理异常占用 |
| 物料档案显示有库存，但生产仍提示不足 | 总库存充足不等于 WIP 充足 | 查“仓库库存”，确认该物料在 WIP 有余额 |
| 成品库存已经够，订单状态怎么处理 | 录单保存时确认是否使用批次或库存余额 | 使用后生产状态为“库存待发货”；不使用则进入生产计划的库存不足区 |
...
- 生产要用的原料已经领到 WIP。
- 生产验收页没有关键异常。
- 库存充足订单已在录单时确认是否使用批次或库存余额；使用库存的订单显示“库存待发货”，可在订单列表直接发货。
