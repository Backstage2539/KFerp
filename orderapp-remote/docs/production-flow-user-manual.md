...
> 先判断成品库存。库存充足时录单会提示可用批次；确认使用后订单进入“库存待发货”并可直接发货；不使用现有批次或库存不足时才把原料领到 WIP，在生产计划里开始生产，按工单生产，最后在“生产中”完成入库。
...
保存订单前，如果订单商品的成品批次库存足够，系统会列出建议使用的批次：
...
| 成品库存已经够，订单状态怎么处理 | 录单保存时确认是否使用批次 | 使用批次后生产状态为“库存待发货”；不使用则进入生产计划的库存不足区 |
...
- 生产要用的原料已经领到 WIP。
- 生产验收页没有关键异常。
- 库存充足订单已在录单时确认是否使用批次；使用批次的订单显示“库存待发货”，可在订单列表直接发货。
