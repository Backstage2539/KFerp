...
test('BOM view exposes recipe library and removes inherited/locked source actions', async () => {
  ...
  assert.match(source, /特殊属性/)
  assert.match(source, /special_attrs_schema_json/)
  assert.match(source, /special_attrs_json/)
  assert.match(source, /include_inactive=1/)
  assert.match(source, /production-bom-groups\/\$\{group\.id\}\/disable/)
