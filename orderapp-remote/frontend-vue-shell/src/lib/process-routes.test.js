...
test('process route operation row supports standard cost default capacity without actual production capacity', () => {
  const source = fs.readFileSync(new URL('../views/ProcessTemplatesView.vue', import.meta.url), 'utf8')

  assert.match(source, /\/api\/manufacturing-workstation-capacities/)
  assert.match(source, /标准成本默认产能/)
  ...
  assert.match(source, /小时成本\s*×\s*标准分钟\s*\/\s*60\s*\/\s*标准产出/)
