...
test('SKU settings opens SKU creation and SKU copy behind drawers while category management is a config tab', () => {
  ...
  assert.match(script, /const normalizedSkuFilters = computed\(\(\) => normalizeVisibleSkuFilters\(skuFilters\.value, unfilteredDisplaySkuRows\.value\)\)/)
  assert.match(script, /const filteredDisplaySkuRows = computed\(\(\) => filterSkuRows\(unfilteredDisplaySkuRows\.value, normalizedSkuFilters\.value\)\)/)
  assert.match(script, /const skuDisplayTotal = computed\(\(\) => filteredDisplaySkuRows\.value\.length\)/)
  ...
  assert.match(script, /const displaySkuRows = computed\(\(\) => slicePageRows\(filteredDisplaySkuRows\.value, \{/)
  assert.match(script, /const skuPrimaryCategoryOptions = computed\(\(\) => primaryCategoryOptions\(unfilteredDisplaySkuRows\.value\)\)/)
  assert.match(script, /const skuSecondaryCategoryOptions = computed\(\(\) => secondaryCategoryOptions\(unfilteredDisplaySkuRows\.value, normalizedSkuFilters\.value\.primaryCategory\)\)/)
