...
test('SKU settings opens SKU creation and SKU copy behind drawers while category management is a config tab', () => {
  ...
  assert.match(script, /const skuVisibleTableState = ref\(skuTableState\(\[\], skuFilters\.value, \{/)
  ...
  assert.match(script, /const skuRenderRows = computed\(\(\) => skuVisibleTableState\.value\.rows\)/)
  assert.match(script, /const skuRenderTotal = computed\(\(\) => skuVisibleTableState\.value\.total\)/)
  assert.match(script, /function syncVisibleSkuTableState\(\)/)
  assert.match(script, /const sourceRows = currentSkuSourceRows\.value/)
  assert.match(script, /const tableState = skuTableState\(sourceRows, normalizedFilters, \{/)
  assert.match(script, /skuVisibleTableState\.value = tableState/)
