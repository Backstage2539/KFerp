...
test('SKU settings keeps only the product creation drawer while business groups drive the category workspace', () => {
  ...
  assert.match(script, /const currentSkuSourceRows = computed\(\(\) => customerCatalogProductRows\(\(/)
  ...
  assert.match(script, /applyWorkspaceCustomerContext\(\)\s+syncVisibleSkuTableState\(\)\s+await loadCustomerCatalog\(\)\s+pruneSelectedProducts\(filteredSkuRows\.value\)/)
