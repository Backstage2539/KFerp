...
test('SKU settings opens SKU creation and SKU copy behind drawers while category management is a config tab', () => {
  ...
  assert.match(template, /:total="skuRenderTotal"/)
  ...
  assert.match(script, /const filteredDisplaySkuRows = computed\(\(\) => filterSkuRows\(currentSkuSourceRows\.value, normalizedSkuFilters\.value\)\)/)
  ...
  assert.match(script, /const displaySkuRows = computed\(\(\) => sliceVisibleSkuRows\(/)
  assert.match(script, /const skuPrimaryCategoryOptions = computed\(\(\) => primaryCategoryOptions\(currentSkuSourceRows\.value\)\)/)
  assert.match(script, /const skuSecondaryCategoryOptions = computed\(\(\) => secondaryCategoryOptions\(/)
  assert.match(script, /const skuRenderRows = computed\(\(\) => \{/)
  assert.match(script, /const skuRenderTotal = computed\(\(\) => \{/)
  assert.match(script, /return currentSkuSourceRows\.value\.length/)
  assert.match(script, /function syncVisibleSkuTableState\(\)/)
  assert.doesNotMatch(script, /displaySkuRows\.value = pageState\.rows/)
