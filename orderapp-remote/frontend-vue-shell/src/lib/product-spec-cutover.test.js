...
test('product settings renders current BOM specs read-only without migration controls', () => {
  const source = fs.readFileSync(new URL('../views/ProductSettingsView.vue', import.meta.url), 'utf8')
  assert.match(source, /BOM 规格（只读）/)
  assert.match(source, /productProductionBomSpecs/)
  assert.match(source, /到 BOM 配置/)
  assert.match(source, /未配置 BOM 规格/)
  assert.doesNotMatch(source, /\/bom-spec-migration|BOM 规格迁移|mutateProductBomSpecMigration/)
  assert.doesNotMatch(source, /显示历史规格|设为默认规格|个历史规格/)
