...
test('BOM settings keeps customer-scoped drafts without restoring the legacy SKU scope selector', () => {
  const view = source('../views/BomView.vue')

  assert.match(view, /isWorkspaceCustomerLocked/)
  assert.match(view, /function bomFormDraftKey\(\)/)
  assert.match(view, /const customerID = Number\(props\.customerContextId \|\| 0\)/)
  assert.match(view, /\/api\/production-boms\?status=all/)
...
test('bean list page locks scope to the current customer in customer workspace', () => {
  ...
  assert.match(view, /isWorkspaceCustomerLocked/)
  assert.match(view, /v-if="!isWorkspaceCustomerLocked"\s+class="price-list-toolbar-scope"/)
  assert.match(view, /v-else\s+class="price-list-toolbar-scope locked-scope"/)
