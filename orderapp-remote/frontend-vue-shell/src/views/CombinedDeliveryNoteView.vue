<template>
  <div class="combined-document-page" :class="{ embedded: props.embedded }">
    ...
    <section class="panel">
      ...
      <table>
        <thead>
          <tr><th>订单号</th><th>单据日期</th><th>订单日期</th><th>出库日期</th><th>收货人</th><th>快递单号</th><th>商品数</th></tr>
        </thead>
        <tbody>
          <tr v-for="group in preview?.snapshot?.groups || []" :key="group.order_id">
            ...
            <td>{{ group.posting_date || '-' }}</td>
            <td>{{ group.receiver_name || '-' }}</td>
            <td>{{ group.tracking_no || '-' }}</td>
            <td>{{ group.items?.length || 0 }}</td>
          </tr>
          <tr v-if="!(preview?.snapshot?.groups || []).length"><td colspan="7" class="muted">暂无关联订单</td></tr>
        </tbody>
      </table>
    </section>
...
const deliveryNotePreviewPDFUrl = computed(() => query.value ? `/api/orders/combined/delivery-note-preview.pdf?${query.value}&v=${previewPDFRefreshKey.value}` : '')
const relatedOrderNos = computed(() => (preview.value?.order_nos || preview.value?.snapshot?.order_nos || []).join('、'))

async function loadPreview() {
  if (!canPreview.value) {
    preview.value = null
    error.value = '请选择同一客户且已发货的至少两个订单'
    return
  }
  previewLoading.value = true
  error.value = ''
  try {
    preview.value = await apiGet(`/api/orders/combined/delivery-note-preview?${query.value}`)
    previewPDFRefreshKey.value += 1
  } catch (err) {
    preview.value = null
    error.value = err.message || '加载组合出库单预览失败'
  } finally {
...
async function generate() {
  ...
  message.value = ''
  try {
    const data = await apiSend('/api/orders/combined/delivery-notes', {
    ...
    message.value = `已生成组合出库单 V${data.version_no}`
    await loadPreview()
  } catch (err) {
    error.value = err.message || '生成组合出库单失败'
  } finally {
