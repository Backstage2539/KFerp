  ...
  <div class="page">
    ...
    <div class="grid">
      <section class="panel table-wrap">
        ...
        <table>
          <thead>
            <tr><th>工序</th><th>编码</th><th>默认工时</th><th>状态</th><th>更新时间</th><th>操作</th></tr>
          </thead>
          <tbody>
            <tr v-for="row in operations" :key="row.id" :class="{ active: row.id === form.id }" @click="editOperation(row)">
              ...
              <td><button class="text danger" type="button" :disabled="row.status === 'inactive'" @click.stop="deactivateOperation(row)">停用</button></td>
            </tr>
            <tr v-if="!operations.length"><td colspan="6" class="muted">暂无工序</td></tr>
          </tbody>
        </table>
      </section>
...
.grid { display: grid; grid-template-columns: minmax(420px, .9fr) minmax(420px, .8fr); gap: 14px; align-items: start; }
