  ...
  <div class="page">
    ...
    <section class="panel">
      <div class="table-wrap">
        <table>
          <thead><tr><th>批次号</th><th>物料</th><th>供应商</th><th>入库单</th><th>入库(g)</th><th>剩余(g)</th><th>成本</th><th>状态</th><th>时间</th><th>备注</th></tr></thead>
          <tbody>
            <tr v-for="row in rows" :key="row.id">
              <td>{{ row.batch_code }}</td><td>{{ row.material_name }}</td><td>{{ row.supplier || '-' }}</td><td>#{{ row.receipt_id }}</td>
              <td>{{ row.qty_g }}</td><td>{{ row.remaining_g }}</td><td>{{ money(row.unit_cost) }}</td><td>{{ row.status }}</td><td>{{ row.received_at }}</td><td>{{ row.note }}</td>
            </tr>
            <tr v-if="!rows.length"><td colspan="10" class="muted">暂无原料批次</td></tr>
          </tbody>
        </table>
      </div>
    </section>
