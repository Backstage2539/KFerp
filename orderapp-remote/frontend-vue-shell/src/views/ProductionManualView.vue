  ...
  <div class="manual-page">
    <section class="intro">
      <div>
        ...
        <h2>从原料入库到直接发货</h2>
        <p class="lead">这里是新员工的流程导航：库存充足就把订单标记为无需生产后直接发货；库存不足才进入生产计划、工单、完工和追溯。</p>
      </div>
    </section>
...
const dailyRows = [
  ...
  { when: '排产', where: '生产管理 -> 生产计划/开始生产', action: '只选择库存不足的商品生成计划；库存充足的订单标记为无需生产', result: '缺口商品进入生产，库存充足订单可去订单列表直接发货' },
...
const operationSteps = [
  ...
  },
  {
    ...
    items: ['先看缺口(g)，库存充足的行不需要生产', '库存充足订单把生产状态设为无需生产，发货状态仍保持未发货', '只勾选库存不足的行生成计划', '检查建议投料、原料需求和建议领到 WIP 数量', '点击开始生产', '系统冻结 BOM 和原料快照并建立 WIP 占用'],
  },
  {
    label: '第 5 步',
    title: '处理 WIP 占用',
...
const faqs = [
  ...
  { q: '成品库存已经够，订单状态怎么处理', a: '生产状态设为无需生产，表示不用进生产工单；发货状态继续保持未发货，直到回填快递单号后系统标记已发货。' },
