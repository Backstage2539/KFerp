  ...
  <div class="manual-page">
    <section class="intro">
      <div>
        ...
        <h2>从原料入库到直接发货</h2>
        <p class="lead">这里是新员工的流程导航：库存充足先在录单时确认是否使用成品批次；确认使用后进入库存待发货并直接发货；不使用库存或库存不足才进入生产计划、工单、完工和追溯。</p>
      </div>
    </section>
...
const dailyRows = [
  ...
  { when: '排产', where: '生产管理 -> 生产计划/开始生产', action: '只选择库存不足的商品生成计划；库存充足区只提示，录单确认使用批次的订单进入库存待发货', result: '缺口商品进入生产，库存待发货订单可去订单列表直接发货' },
...
const faqs = [
  ...
  { q: '成品库存已经够，订单状态怎么处理', a: '录单保存时确认是否使用建议批次。选择使用后订单进入库存待发货；选择不使用则不占用库存，进入生产计划的库存不足区。' },
...
const beforeChecklist = [
  ...
  '生产要用的原料已经领到 WIP',
  '生产验收页没有关键异常',
  '库存充足订单已在录单时确认是否使用批次；库存待发货订单可直接发货',
