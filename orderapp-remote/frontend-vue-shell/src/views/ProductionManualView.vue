  ...
  <div class="manual-page">
    <section class="intro">
      <div>
        ...
        <h2>从原料入库到直接发货</h2>
        <p class="lead">这里是新员工的流程导航：库存充足先在录单时确认是否使用成品批次或库存余额；确认使用后进入库存待发货并直接发货；不使用库存或库存不足才进入生产计划、工单、完工和追溯。</p>
      </div>
    </section>
...
const dailyRows = [
  ...
  { when: '排产', where: '生产管理 -> 生产计划/开始生产', action: '只选择库存不足的商品生成计划；库存充足区只提示，录单确认使用批次或库存余额的订单进入库存待发货', result: '缺口商品进入生产，库存待发货订单可去订单列表直接发货' },
...
const operationSteps = [
  ...
  },
  {
    ...
    items: ['先看库存不足和库存充足两个区块', '库存充足区只提示，不进入生产计划', '库存不足标题左侧复选框可全选或全不选，部分选择时显示减号', '录单拒绝使用库存批次的订单会进入库存不足区', '检查建议投料、原料需求和建议领到 WIP 数量', '点击开始生产', '系统冻结 BOM 和原料快照并建立 WIP 占用'],
  },
  {
    label: '第 5 步',
    title: '处理 WIP 占用',
  ...
  },
  {
    ...
    title: '质检和完成生产',
    route: '生产管理 -> 生产中',
    items: ['需要时先到生产质检，点选择工单记录工单检查结果', '填写实际产出件数', '如有散重，填写散重克数', '本次只完成一部分时勾选部分完工并填写本次消耗投料', '选择成品入库仓', '点击完成'],
  },
  {
    label: '第 8 步',
    title: '成品转仓',
...
const faqs = [
  ...
  { q: 'WIP看起来有库存，但开始生产仍提示不足', a: '可能已被其他工单占用。到仓库库存的 WIP占用抽屉查看、调整或释放异常占用。' },
  { q: '物料档案显示有库存，但生产仍提示不足', a: '总库存充足不等于 WIP 充足。确认该物料在 WIP 有余额。' },
  { q: '成品库存已经够，订单状态怎么处理', a: '录单保存时确认是否使用建议批次或库存余额。选择使用后订单进入库存待发货；选择不使用则不占用库存，进入生产计划的库存不足区。' },
...
const beforeChecklist = [
  ...
  '生产要用的原料已经领到 WIP',
  '生产验收页没有关键异常',
  '库存充足订单已在录单时确认是否使用批次或库存余额；库存待发货订单可直接发货',
  '库存不足商品已确认 WIP 和原料准备情况',
  '生产计划里已经开始生产',
  '生产工单已打印或现场可查看',
]
