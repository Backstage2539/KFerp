<template>
  <div class="manual-page">
    ...
    <section class="panel">
      ...
      <p class="plain">入口：库存管理 -> 仓库库存，输入 `FP-...` 成品批次查询。</p>
    </section>
...
const mainFlow = [
  ...
  { title: '开始生产', note: '冻结 BOM 和原料快照' },
  { title: '生产工单', note: '查看烘焙建议并打印' },
  { title: '完成生产', note: '扣 WIP 原料并入成品仓' },
...
const dailyRows = [
  ...
  { when: '排产', where: '生产管理 -> 生产计划/开始生产', action: '选择要生产的商品，确认投料', result: '生成生产中记录和生产工单' },
  { when: '生产中', where: '生产管理 -> 生产工单', action: '看烘焙建议、原料需求，打印工单', result: '现场按工单执行' },
  { when: '生产后', where: '生产管理 -> 生产中', action: '填实际产出，选择成品入库仓', result: '扣 WIP 原料，生成成品库存' },
...
const operationSteps = [
  ...
  },
  {
    ...
    items: ['选择要生产的行', '检查建议投料和原料需求', '点击开始生产', '系统冻结 BOM 和原料快照'],
  },
  {
    label: '第 4 步',
    title: '查看和打印生产工单',
  ...
  },
  {
    ...
    title: '完成生产',
    route: '生产管理 -> 生产中',
    items: ['填写实际产出件数', '如有散重，填写散重克数', '选择成品入库仓', '点击完成'],
  },
  {
    label: '第 6 步',
    title: '成品转仓',
