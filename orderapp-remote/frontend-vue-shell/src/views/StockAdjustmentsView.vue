...
const error = ref('')
const ok = ref('')
const form = reactive({ item_type: 'material', item_id: 0, spec_g: 0, target_g: 0, target_units: 0, reason: '' })
const currentOptions = computed(() => form.item_type === 'material' ? materials.value : products.value)
async function loadOptions() {
  loading.value = true
  error.value = ''
  try {
    const [mat, prod] = await Promise.all([apiGet('/api/materials?limit=500'), apiGet('/api/products')])
...
.form-grid { display:grid; grid-template-columns:120px minmax(220px,1.4fr) 100px 130px 110px; gap:10px; align-items:end; }
