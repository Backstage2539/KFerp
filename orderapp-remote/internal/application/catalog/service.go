...
type ProductSettingsProduct struct {
	...
	BOMSpecs                    []BOMSpecOption `json:"bom_specs,omitempty"`
}
...
type ProductSettingsData struct {
	...
	// Product unit templates are legacy sales-spec metadata.  Keep the Go
	// field for replay/compatibility, but omit it from the active settings DTO;
	// BOM specifications now own product units and identities.
	ProductUnitTemplates         []ProductUnitTemplate         `json:"product_unit_templates,omitempty"`
...
func (s *Service) ProductSettings(ctx context.Context) (ProductSettingsData, error) {
	...
	// PR-608: the active product-settings contract no longer projects sales
	// unit templates.  The repository method remains available only for
	// historical replay/rollback; global unit definitions are returned below.
...
func productSettingsProduct(p Product) ProductSettingsProduct {
	...
	return ProductSettingsProduct{
		...
		BOMSpecs:                    append([]BOMSpecOption(nil), p.BOMSpecs...),
	}
}
