...
func TestCreateProductCustomerReferenceDefaultsFactorySupply(t *testing.T) {
	...
	if _, err := svc.CreateProduct(context.Background(), CreateProductCommand{
		Actor: "tester", Name: "客户商品A", CustomerID: 42,
	}); err != nil {
...
func TestCreateProductDropsLegacyProductUnitAuthority(t *testing.T) {
	repo := &fakeRepo{}
	svc := NewService(repo)
	...
	if repo.create.UnitRuleOverrideJSON != `{}` {
		t.Fatalf("new product unit override=%q, want BOM-owned product units", repo.create.UnitRuleOverrideJSON)
	}
}
