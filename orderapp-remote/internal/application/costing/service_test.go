...
func TestGenerateBeanListPublicationPDFSavesAndReusesAsset(t *testing.T) {
	...
	if repo.savedBeanListAsset.PublicationID != 7 || repo.savedBeanListAsset.AssetType != "pdf" || repo.savedBeanListAsset.CacheKey != "bean-list-preview-style-v3:7:V3.0.5" {
...
func TestGenerateBeanListPublicationPDFRegeneratesStaleCacheKey(t *testing.T) {
	...
	if repo.savedBeanListAsset.CacheKey != "bean-list-preview-style-v3:7:V3.0.5" || string(repo.savedBeanListAsset.Payload) != "%PDF-preview-style" {
		t.Fatalf("saved asset = %+v", repo.savedBeanListAsset)
	}
	if file.CacheKey != "bean-list-preview-style-v3:7:V3.0.5" || string(file.Payload) != "%PDF-preview-style" {
