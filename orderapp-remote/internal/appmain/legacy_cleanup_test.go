...
func TestGoRoutesDoNotKeepLegacyTemplateBranches(t *testing.T) {
	err := filepath.WalkDir("internal/appmain", func(path string, entry os.DirEntry, err error) error {
		if err != nil {
			return err
		}
		if entry.IsDir() {
...
func TestCompanyStaffAPIUsesApplicationService(t *testing.T) {
	body, err := os.ReadFile("internal/appmain/company_staff.go")
	if err != nil {
		t.Fatal(err)
	}
	src := string(body)
...
func TestProduceBatchAPIUsesApplicationServiceForReadModels(t *testing.T) {
	body, err := os.ReadFile("internal/appmain/produce_batch_api.go")
	if err != nil {
		t.Fatal(err)
	}
	src := string(body)
...
func TestDeductPreviewContractTestIsNotSkipped(t *testing.T) {
	body, err := os.ReadFile("internal/appmain/produce_batch_deduct_preview_test.go")
	if err != nil {
		t.Fatal(err)
	}
	src := string(body)
