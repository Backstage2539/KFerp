...
func TestProductionFlowRoutesAndSchemaAreSplitOut(t *testing.T) {
	...
	for _, path := range []string{"internal/appmain/production_flow_routes.go", "internal/appmain/production_flow_schema.go"} {
...
func TestProductionFlowMonolithIsRemoved(t *testing.T) {
	...
	if _, err := os.Stat("internal/appmain/production_running_repository.go"); err != nil {
...
func TestProductionRoutesCallApplicationService(t *testing.T) {
	body, err := os.ReadFile("internal/appmain/production_flow_routes.go")
	if err != nil {
		t.Fatal(err)
	}
	content := string(body)
...
func TestSalesRepositoryDoesNotParseSaveOrderFormArrays(t *testing.T) {
	body, err := os.ReadFile("internal/appmain/sales_order_repository.go")
	if err != nil {
		t.Fatal(err)
	}
	content := string(body)
...
func TestMaterialsAPIUsesApplicationService(t *testing.T) {
	body, err := os.ReadFile("internal/appmain/materials_api.go")
	if err != nil {
		t.Fatal(err)
	}
	content := string(body)
