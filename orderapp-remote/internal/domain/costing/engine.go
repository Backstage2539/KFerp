...
func ValidateProductInput(params Parameters, in ProductInput) (ProductInput, error) {
	...
	if len(in.WholesaleKgMarginRates) == 0 {
		in.WholesaleKgMarginRates = []float64{0.5421052631578949, 0.3842105263157895, 0.27894736842105267, 0.2, 0.12, 0.045}
	}
	if len(in.WholesaleDripMultipliers) == 0 {
		in.WholesaleDripMultipliers = []float64{2.2, 1.8, 1.6, 1.5}
	}
	return in, nil
}
