...
func TestProductionStockLedgerSchemaFailsClosedWhenBOMSpecIdentityIndexCannotBeCreated(t *testing.T) {
	...
	defer pool.Close()
	schema := fmt.Sprintf("pr600_production_stock_schema_failclosed_%d", time.Now().UnixNano())
	...
	if err := ensureStockLedgerTables(ctx, pool, schema); err == nil {
		t.Fatal("production schema accepted an uninstalled BOM-spec stock source identity index")
	}
}

func TestProductionStockLedgerSchemaKeepsBOMSpecIdentityIndexOnRepeat(t *testing.T) {
	dsn := strings.TrimSpace(os.Getenv("ORDERAPP_TEST_DATABASE_URL"))
	if dsn == "" {
		dsn = strings.TrimSpace(os.Getenv("DATABASE_URL"))
	}
	...
	defer pool.Close()
	schema := fmt.Sprintf("pr600_production_stock_schema_repeat_%d", time.Now().UnixNano())
	if _, err := pool.Exec(ctx, "CREATE SCHEMA "+schema); err != nil {
		t.Fatal(err)
	}
	defer pool.Exec(ctx, "DROP SCHEMA "+schema+" CASCADE")
	...
	if !unique {
		t.Fatal("production stock source identity index must be unique")
	}
}
