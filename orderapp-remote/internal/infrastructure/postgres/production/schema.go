...
CREATE INDEX IF NOT EXISTS work_center_capacity_calendar_lookup_idx ON %s.work_center_capacity_calendar(work_date, work_center);
	...
	for _, stmt := range []string{
		...
		fmt.Sprintf(`ALTER TABLE %s.production_plan_items ADD COLUMN IF NOT EXISTS inventory_unit TEXT NOT NULL DEFAULT ''`, schema),
		...
		fmt.Sprintf(`ALTER TABLE %s.work_orders ADD COLUMN IF NOT EXISTS inventory_unit TEXT NOT NULL DEFAULT ''`, schema),
		fmt.Sprintf(`ALTER TABLE %s.work_orders ADD COLUMN IF NOT EXISTS planned_inventory_qty NUMERIC(18,9) NOT NULL DEFAULT 0`, schema),
		fmt.Sprintf(`ALTER TABLE %s.work_orders ADD COLUMN IF NOT EXISTS sales_spec_snapshot_json JSONB NOT NULL DEFAULT '{}'::jsonb`, schema),
		fmt.Sprintf(`ALTER TABLE %s.work_orders ADD COLUMN IF NOT EXISTS bom_inherited BOOLEAN NOT NULL DEFAULT false`, schema),
		...
		fmt.Sprintf(`ALTER TABLE %s.job_cards ADD COLUMN IF NOT EXISTS cost_method TEXT NOT NULL DEFAULT 'time'`, schema),
		fmt.Sprintf(`ALTER TABLE %s.job_cards ADD COLUMN IF NOT EXISTS piece_rate NUMERIC(14,4) NOT NULL DEFAULT 0`, schema),
		...
		fmt.Sprintf(`ALTER TABLE %s.job_cards ALTER COLUMN started_at DROP NOT NULL`, schema),
		fmt.Sprintf(`ALTER TABLE %s.job_cards ALTER COLUMN started_at DROP DEFAULT`, schema),
