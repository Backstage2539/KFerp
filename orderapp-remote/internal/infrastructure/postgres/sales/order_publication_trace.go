...
func orderPublicationTraceSnapshot(ctx context.Context, q interface {
	...
	err := q.QueryRow(ctx, fmt.Sprintf(`SELECT COALESCE(NULLIF(to_jsonb(b)->'config_json'->'publication_batch'->>'table_name',''),b.product_type_name,''),b.owner_type,b.owner_key,
