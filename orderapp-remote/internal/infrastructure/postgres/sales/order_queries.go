...
func fetchOrders(ctx context.Context, pool *pgxpool.Pool, schema string, query salesapp.OrderListQuery) ([]salesapp.OrderRow, bool, error) {
	...
	sql := fmt.Sprintf(`
		SELECT
			...
			CASE WHEN o.portal_service_code IN ('direct_ship','product_order','processing_ship') THEN COALESCE(o.receiver_name,'') ELSE COALESCE(NULLIF(o.receiver_name,''), NULLIF(c.contact,''), c.name, '') END AS receiver_name,
			CASE WHEN o.portal_service_code IN ('direct_ship','product_order','processing_ship') THEN COALESCE(o.receiver_phone,'') ELSE COALESCE(NULLIF(o.receiver_phone,''), c.phone, '') END AS receiver_phone,
			CASE WHEN o.portal_service_code IN ('direct_ship','product_order','processing_ship') THEN COALESCE(o.receiver_address,'') ELSE COALESCE(NULLIF(o.receiver_address,''), c.address, '') END AS receiver_address,
