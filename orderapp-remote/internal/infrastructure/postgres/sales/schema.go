...
func ensureOrderItemUnitPricingColumns(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	stmts := []string{
		...
		fmt.Sprintf(`ALTER TABLE %s.order_items ADD COLUMN IF NOT EXISTS material_source_mode TEXT NOT NULL DEFAULT 'factory'`, schema),
		...
		fmt.Sprintf(`UPDATE %s.order_items SET material_source_mode='factory' WHERE material_source_mode IS NULL OR material_source_mode=''`, schema),
