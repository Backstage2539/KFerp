...
func TestProductionBomAPIsExposeGroupsCopyVersionsAndBinding(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{method: http.MethodGet, path: "/api/production-bom-groups?include_inactive=1", want: []string{`"name":"常用配方"`, `"name":"停用分组"`}},
		...
		{method: http.MethodPost, path: "/api/production-bom-groups/1/disable", want: []string{`"ok":true`}},
	...
	if !repo.includeInactiveProductionBomGroups {
		t.Fatal("include_inactive query flag was not passed to group repository")
	}
	...
	if repo.disabledProductionBomGroupID != 1 {
		t.Fatalf("disabled group id = %d, want 1", repo.disabledProductionBomGroupID)
	}
