...
func TestProductionBomAPIsExposeGroupsCopyVersionsAndBinding(t *testing.T) {
	repo := &apiFakeRepo{
		productionBomGroups: []bomapp.ProductionBomGroup{{ID: 1, Name: "常用配方", Active: true, SortOrder: 10}},
		...
		productionBomDetail: bomapp.ProductionBomDetail{
			...
			Versions: []bomapp.ProductionBomVersion{{ID: 101, BomID: 11, VersionNo: "V003", Status: "published", YieldRate: 0.82, IsLatest: true}},
		},
	...
	for _, tc := range []struct {
		...
		{method: http.MethodGet, path: "/api/production-boms/11", want: []string{`"versions"`, `"version_no":"V003"`, `"is_latest":true`}},
