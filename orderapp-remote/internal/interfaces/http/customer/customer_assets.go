...
func insertCustomerAsset(ctx context.Context, pool *pgxpool.Pool, schema, actor string, customerID int64, kind string, objectKey string, contentType string, bytes int64, sha string) (int64, error) {
	...
	support.AuditInsert(ctx, pool, schema, actor, "customer_asset", &customerID, "upload", support.StrPtr("kind"), nil, support.StrPtr(kind), support.AuditMeta{"asset_id": id, "object_key": objectKey, "bytes": bytes, "content_type": contentType})
	return id, nil
}
...
func deleteCustomerAssetByID(ctx context.Context, pool *pgxpool.Pool, schema, actor string, assetID int64) (customerID int64, kind string, objectKey string, err error) {
	...
	support.AuditInsert(ctx, pool, schema, actor, "customer_asset", &customerID, "delete", support.StrPtr("asset_id"), support.StrPtr(fmt.Sprintf("%d", assetID)), nil, support.AuditMeta{"kind": kind})
