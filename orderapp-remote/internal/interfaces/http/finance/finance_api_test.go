...
func TestFinanceExpenseAndClosingAPI(t *testing.T) {
	e, _ := newFinanceTestEcho()
	body := strings.NewReader(`{"date":"2026-05-02","category":"房租","amount":3800,"allocation":"period_expense"}`)
	req := httptest.NewRequest(http.MethodPost, "/api/finance/expenses", body)
	req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	rec := httptest.NewRecorder()
	e.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK || !strings.Contains(rec.Body.String(), `"category":"房租"`) {
...
func (s *fakeFinanceService) CreateExpense(_ context.Context, cmd appfinance.CreateExpenseCommand) (appfinance.Expense, error) {
	return appfinance.Expense{ID: 1, Date: cmd.Date, Month: "2026-05", Category: cmd.Category, Amount: cmd.Amount, Allocation: cmd.Allocation}, nil
}

func (s *fakeFinanceService) ListExpenses(context.Context, string) ([]appfinance.Expense, error) {
	return []appfinance.Expense{{ID: 1, Date: "2026-05-02", Month: "2026-05", Category: "房租", Amount: 3800, Allocation: appfinance.AllocationPeriodExpense}}, nil
}
