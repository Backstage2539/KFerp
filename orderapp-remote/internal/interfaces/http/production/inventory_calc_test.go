...
func TestInvNormalize(t *testing.T) {
	spec := int64(454)
	q, err := invNormalize(spec, InvQty{Units: 5, LooseG: 10})
	if err != nil {
		t.Fatal(err)
	}
	...
	q, err = invNormalize(spec, InvQty{Units: 5, LooseG: 470}) // 470=454+16
	if err != nil {
		t.Fatal(err)
	}
...
func TestInvDeductEnough(t *testing.T) {
	spec := int64(454)
	remain, deducted, gap, err := invDeduct(spec, InvQty{Units: 0, LooseG: 500}, 454)
	if err != nil {
		t.Fatal(err)
	}
	if gap != 0 {
...
func TestInvDeductInsufficient(t *testing.T) {
	spec := int64(454)
	remain, deducted, gap, err := invDeduct(spec, InvQty{Units: 1, LooseG: 0}, 1000)
	if err != nil {
		t.Fatal(err)
	}
...
func TestInvDeductZero(t *testing.T) {
	spec := int64(454)
	remain, deducted, gap, err := invDeduct(spec, InvQty{Units: 2, LooseG: 10}, 0)
	if err != nil {
		t.Fatal(err)
	}
	if deducted != 0 || gap != 0 {
