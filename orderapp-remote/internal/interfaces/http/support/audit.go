...
func InlineUpdateOrder(ctx context.Context, pool *pgxpool.Pool, schema string, orderID int64, actor string, req *InlineUpdateRequest) error {
	...
	if !eqIntPtr(curOrderType, nextOrderType) {
		...
		if err := AuditInsertTx(ctx, tx, schema, actor, "order", &orderID, "update", StrPtr("order_type_id"), oldS, newS, AuditMeta{"order_id": orderID}); err != nil {
			return err
		}
	}
	if !eqIntPtr(curPay, nextPay) {
		...
		if err := AuditInsertTx(ctx, tx, schema, actor, "order", &orderID, "update", StrPtr("pay_status_id"), oldS, newS, AuditMeta{"order_id": orderID}); err != nil {
			return err
		}
	}
	if !eqIntPtr(curShip, nextShip) {
		...
		if err := AuditInsertTx(ctx, tx, schema, actor, "order", &orderID, "update", StrPtr("ship_status_id"), oldS, newS, AuditMeta{"order_id": orderID}); err != nil {
			return err
		}
	}
	if !eqIntPtr(curProc, nextProc) {
		...
		if err := AuditInsertTx(ctx, tx, schema, actor, "order", &orderID, "update", StrPtr("process_status_id"), oldS, newS, AuditMeta{"order_id": orderID}); err != nil {
			return err
		}
	}
	if !eqStrPtr(curNotes, nextNotesPtr) {
		...
		if err := AuditInsertTx(ctx, tx, schema, actor, "order", &orderID, "update", StrPtr("notes"), oldS, newS, AuditMeta{"order_id": orderID}); err != nil {
			return err
		}
	}
