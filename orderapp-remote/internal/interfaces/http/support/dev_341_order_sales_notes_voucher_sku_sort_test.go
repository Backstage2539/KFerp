...
func TestDev341OrderSalesNotesVoucherSkuSortRequirementSeeds(t *testing.T) {
	src := string(readOrderAppFileForTest(t, filepath.Join("internal", "interfaces", "http", "support", "req_store.go")))
	for _, want := range []string{
		"PR-341-ORDER-SALES-NOTES-VOUCHER-SKU-SORT",
		"DEV-341-ORDER-SALES-NOTES-VOUCHER-SKU-SORT",
		"UT-341-ORDER-SALES-NOTES-VOUCHER-SKU-SORT",
		"API-341-ORDER-SALES-NOTES-VOUCHER-SKU-SORT",
		"REV-341-ORDER-SALES-NOTES-VOUCHER-SKU-SORT",
	} {
...
func TestDev341OrderSalesNotesVoucherSkuSortWiring(t *testing.T) {
	checks := []struct {
		rel     string
		markers []string
	}{
...
func TestDev341OrderSalesNotesVoucherSkuSortDocs(t *testing.T) {
	checks := []struct {
		rel     string
		markers []string
	}{
	...
	for _, check := range checks {
		...
		for _, want := range check.markers {
			if !strings.Contains(src, want) {
				t.Fatalf("%s missing PR-341 documentation marker %q", check.rel, want)
			}
		}
	}
}
