...
func TestDev474ProductionPlanBulkSubmitFilterContracts(t *testing.T) {
	for rel, wants := range map[string][]string{
		filepath.Join("internal", "interfaces", "http", "support", "req_store.go"): {
			"PR-474-PRODUCTION-PLAN-BULK-SUBMIT-FILTER",
			"DEV-474-PRODUCTION-PLAN-FILTER-API",
			"DEV-474-PRODUCTION-PLAN-BULK-SUBMIT",
			"DEV-474-VUE-PLAN-SELECTION-FILTERS",
			"DEV-474-DOCS-ACCEPTANCE",
			"UT-474-PRODUCTION-PLAN-BULK-SUBMIT-FILTER",
			"API-474-PRODUCTION-PLAN-BULK-SUBMIT-FILTER",
			"REV-474-PRODUCTION-PLAN-BULK-SUBMIT-FILTER",
		},
		...
		for _, want := range wants {
			if !strings.Contains(src, want) {
				t.Fatalf("%s missing PR-474 marker %q", rel, want)
			}
		}
	}
}
