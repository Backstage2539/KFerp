...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_product", code: "PR-145", title: "顺丰发货命名统一；出库日志整合到库存管理并可查看和下载出库单", status: "review", assignee: "VA", evidence: "codex/outbound-log-stock-sf-20260503"},
		{table: "req_dev", code: "DEV-145-01", title: "订单列表顺丰导出入口统一展示为顺丰发货，保留既有导出兼容接口", status: "done", assignee: "Codex", evidence: "OrdersView 顺丰发货; deliveryMethodDisplayName 顺丰发货"},
		{table: "req_dev", code: "DEV-145-02", title: "库存管理新增出库日志页面，按出库单文档列出订单、客户、出库仓、快递、版本和状态，并支持查看抽屉和下载出库单", status: "done", assignee: "Codex", evidence: "GET /api/stock/outbound-logs; StockOutboundLogsView.vue"},
		{table: "req_unit", code: "UT-145-01", title: "源码守卫覆盖出库日志库存菜单、顺丰发货命名、出库单查看下载入口和需求种子", status: "done", assignee: "Codex", evidence: "dev_145_outbound_logs_stock_test.go"},
		{table: "req_api", code: "API-145-01", title: "API 测试覆盖 GET /api/stock/outbound-logs 返回出库单文档、顺丰发货显示名、查看和下载 URL", status: "done", assignee: "Codex", evidence: "TestStockAPIRoutes outbound logs; TestListOutboundLogsReturnsDeliveryNoteDocumentsForInventory"},
		{table: "req_review", code: "REV-145-01", prCode: "PR-145", title: "验收：库存管理可打开出库日志，看到已生成出库单，点击查看出库单打开抽屉，点击下载出库单得到 PDF；订单列表显示顺丰发货", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
