...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_dev", code: "DEV-157-04", title: "Vue/Vite 客户履约账户工作台：按客户上传 Excel、解析校验、应用批次、查看库存/工单/代发/费用/结算", status: "done", assignee: "Codex", evidence: "CustomerFulfillmentView.vue; menu key customerFulfillment; /api/customer-fulfillment wrappers"},
		...
		{table: "req_review", code: "REV-157-01", prCode: "PR-157", title: "验收：按誉观山三份样本文件导入后，可看到客户托管生豆、客户产品库存、加工工单、代发订单、费用和月结批次", status: "review", assignee: "VA", evidence: "本地验收：三类 Excel parser、parse 不改业务数据、apply 幂等写库存/工单/订单/费用/月结、Vue/Vite 页面无 template debt；待 Van ERP 样本验收"},
	} {
