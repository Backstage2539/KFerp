...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_dev", code: "DEV-CUSTOMER-PORTAL-P0-02", title: "新增 customerportal Postgres schema/repository，包含 mini_users、mini_sessions、customer_portal_profiles、customer_portal_user_bindings、customer_service_capabilities", status: "done", assignee: "Codex", evidence: "go test ./internal/infrastructure/postgres/customerportal ./internal/appmain -count=1; go test ./... -count=1"},
		...
		{table: "req_unit", code: "UT-CUSTOMER-PORTAL-P0-01", title: "单元测试覆盖客户门户服务能力聚合、schema 表定义、miniapp 能力入口显示逻辑和需求种子", status: "done", assignee: "Codex", evidence: "go test ./internal/application/customerportal ./internal/infrastructure/postgres/customerportal ./internal/interfaces/http/support -count=1; npm test --prefix miniapp"},
