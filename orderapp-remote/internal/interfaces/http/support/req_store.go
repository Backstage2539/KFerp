...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_product", code: "PR-077", title: "成本参数按分类展示并带小字说明，成本核算页可右侧快速设置", status: "review", assignee: "VA", evidence: "codex/erpnext-p0-p3-20260427"},
		{table: "req_dev", code: "DEV-077-01", title: "成本参数设置页按基础换算、生产包装、商用熟豆、零售熟豆、挂耳分类展示并显示说明", status: "done", assignee: "Codex", evidence: "CostingSettingsPanel.vue + costing-settings.js"},
		{table: "req_dev", code: "DEV-077-02", title: "成本核算页提供右侧快速参数抽屉，保存参数后刷新当前试算", status: "done", assignee: "Codex", evidence: "CostingView.vue drawer + CostingSettingsPanel"},
		{table: "req_unit", code: "UT-077-01", title: "成本参数分类、说明、排序和未知参数兜底覆盖", status: "done", assignee: "Codex", evidence: "node --test src/lib/costing-settings.test.js"},
		{table: "req_api", code: "API-077-01", title: "成本设置仍复用 GET /api/costing/settings 与 POST /api/costing/settings/:key", status: "done", assignee: "Codex", evidence: "go test ./internal/interfaces/http/costing -run TestCostingSettingsAPI -count=1"},
		{table: "req_review", code: "REV-077-01", prCode: "PR-077", title: "验收：成本参数有分类和说明，成本核算页可不跳转快速修改参数", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
	} {
