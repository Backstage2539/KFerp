...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_product", code: "PR-138", title: "订单和出库单分享到微信必须直接分享文件，不发送链接", status: "review", assignee: "VA", evidence: "codex/wechat-share-files-20260503"},
		{table: "req_dev", code: "DEV-138-01", title: "统一前端分享 helper 使用 /api/share-resources 返回的 file_url 下载 Blob，构造 File 后通过 Web Share API files 分享", status: "done", assignee: "Codex", evidence: "shareResourceToWechat"},
		{table: "req_dev", code: "DEV-138-02", title: "销售单和出库单分享按钮只提示直接分享文件；浏览器不支持文件分享时提示下载后手动发送，不再复制分享链接", status: "done", assignee: "Codex", evidence: "SalesOrderView; DeliveryNoteView; sales-order-user-manual; delivery-note-user-manual"},
		{table: "req_unit", code: "UT-138-01", title: "单测覆盖文件分享 payload、canShare 不支持分支、不复制链接和 Vue 文案源码守卫", status: "done", assignee: "Codex", evidence: "sales-order.test.js; dev_138_wechat_file_share_test.go"},
		{table: "req_api", code: "API-138-01", title: "API 回归沿用统一 /api/share-resources 流程，响应必须提供 file_url 并可返回对应 PDF 或 PNG 文件", status: "done", assignee: "Codex", evidence: "TestExternalShareResourceAPIUsesOneFlowForSalesOrderAndDeliveryNote"},
		{table: "req_review", code: "REV-138-01", prCode: "PR-138", title: "验收：点击销售单 PDF、销售单图片或出库单分享到微信后，微信收到的是 PDF/PNG 文件而不是链接", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
