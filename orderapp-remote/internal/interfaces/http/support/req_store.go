...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_product", code: "PR-149", title: "库存待发货订单在回填快递单号时正式扣成品库存，并修复已发货历史订单误入生产计划", status: "review", assignee: "VA", evidence: "codex/order-stock-shipment-deduction-20260503"},
		{table: "req_dev", code: "DEV-149-01", title: "新增订单库存出库扣减记录，回填快递单号时扣减 FP 批次或 LEGACY-FP 对应 finished_inventory 并写库存流水，保证幂等", status: "done", assignee: "Codex", evidence: "order_stock_deductions; deductOrderAllocatedStockTx; sales_order_shipment ledger"},
		{table: "req_dev", code: "DEV-149-02", title: "生产计划排除已发货订单，并把库存待发货未发货预留量从可用成品库存中扣除，避免重复占用", status: "done", assignee: "Codex", evidence: "fetchUnproducedNeeds ship_status filter; reserved allocations excluding deductions"},
		{table: "req_unit", code: "UT-149-01", title: "单元/集成测试覆盖已发货空生产状态订单不进入生产计划、库存发货扣减幂等和需求种子", status: "done", assignee: "Codex", evidence: "TestProducePlanExcludesShippedOrdersWithBlankProcessStatus; TestOrdersShippingTrackingAPIDeductsReservedLegacyFinishedInventoryOnce; TestOrdersSingleShippingTrackingAPIDeductsReservedFinishedBatch"},
		{table: "req_api", code: "API-149-01", title: "API 测试覆盖回填快递单号触发 FP/LEGACY-FP 库存扣减和库存流水；生产计划只显示拒绝用库存订单自身缺口", status: "done", assignee: "Codex", evidence: "POST /api/orders/shipping-tracking; POST /api/orders/:id/shipping-tracking; GET /api/produce/unproduced"},
		{table: "req_review", code: "REV-149-01", prCode: "PR-149", title: "验收：使用库存的订单发货后库存减少且流水可查；SO-20260503-0001 取消使用库存后生产计划只按自身 2 件处理，不再被已发货旧单放大到 82 件", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
