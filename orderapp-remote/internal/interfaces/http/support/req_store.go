...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_product", code: "PR-141", title: "订单号编辑抽屉：订单号与编辑入口合并，点击订单号直接在抽屉编辑订单", status: "review", assignee: "VA", evidence: "codex/order-number-edit-drawer-20260503"},
		{table: "req_dev", code: "DEV-141-01", title: "订单列表移除独立编辑跳转，在订单号抽屉内嵌录单编辑表单，保存后刷新当前列表和抽屉数据", status: "done", assignee: "Codex", evidence: "OrdersView OrderEntryView embedded order-edit-panel"},
		{table: "req_unit", code: "UT-141-01", title: "源码守卫覆盖订单号编辑抽屉、移除独立编辑链接、录单页嵌入模式保存不跳转和需求种子", status: "done", assignee: "Codex", evidence: "dev_141_order_number_edit_drawer_test.go"},
		{table: "req_api", code: "API-141-01", title: "API 回归沿用现有 GET /api/order/form 和 POST /api/order 编辑保存接口，列表刷新仍走 GET /api/orders", status: "done", assignee: "Codex", evidence: "go test ./internal/interfaces/http/sales -count=1"},
		{table: "req_review", code: "REV-141-01", prCode: "PR-141", title: "验收：订单列表点击订单号打开抽屉即可编辑订单；列表和抽屉中不再出现独立编辑跳转入口；保存后停留在当前列表", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
