...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_product", code: "PR-106", title: "订单发货闭环：快递录单生成发货批次，回填快递单号后订单自动标记已发货", status: "review", assignee: "VA", evidence: "codex/p1-ops-20260428"},
		{table: "req_dev", code: "DEV-106-01", title: "新增订单发货批次和批次订单表，生成快递 Excel 时写入 shipment_id/shipment_no 和订单寄件人", status: "done", assignee: "Codex", evidence: "order_shipments; CreateOrderShipment"},
		{table: "req_dev", code: "DEV-106-02", title: "新增快递单号回填 API，按发货批次订单写入 tracking_no 并把订单发货状态更新为已发货", status: "done", assignee: "Codex", evidence: "POST /api/orders/shipping-tracking"},
		{table: "req_unit", code: "UT-106-01", title: "单测覆盖发货批次命令规范化、重复订单去重和单号回填命令默认操作人", status: "done", assignee: "Codex", evidence: "TestServiceOwnsShipmentClosureUseCases"},
		{table: "req_api", code: "API-106-01", title: "API 测试覆盖快递 Excel 创建发货批次、回填单号后订单已发货", status: "done", assignee: "Codex", evidence: "TestOrdersShippingExcelAPICreatesShipmentRecord; TestOrdersShippingTrackingAPIMarksOrdersShipped"},
		{table: "req_review", code: "REV-106-01", prCode: "PR-106", title: "验收：生产完成订单生成快递 Excel 后出现发货批次号，回填单号后订单状态为已发货", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
		{table: "req_product", code: "PR-107", title: "采购入库 P1：供应商、采购单、采购收货与库存批次/流水打通", status: "review", assignee: "VA", evidence: "codex/p1-ops-20260428"},
		{table: "req_dev", code: "DEV-107-01", title: "新增 purchase application/postgres/http 模块，维护供应商、采购单和采购收货记录", status: "done", assignee: "Codex", evidence: "internal/application/purchase; internal/infrastructure/postgres/purchase; /api/purchase/*"},
		{table: "req_dev", code: "DEV-107-02", title: "采购收货复用库存原料入库能力，生成原料批次、库存批次、原料仓库存和库存流水，并更新物料采购价", status: "done", assignee: "Codex", evidence: "CreatePurchaseReceipt + stock.ReceiveMaterial"},
		{table: "req_dev", code: "DEV-107-03", title: "Vue/Vite 新增采购入库页面和采购权限菜单，支持供应商、采购单和一键收货", status: "done", assignee: "Codex", evidence: "PurchaseView; menu-ia purchase"},
		{table: "req_unit", code: "UT-107-01", title: "单测覆盖采购供应商/采购单命令规范化和采购收货调用库存入库与采购价更新", status: "done", assignee: "Codex", evidence: "TestPurchaseServiceCreatesSupplierAndPurchaseOrder; TestPurchaseReceiptReceivesStockAndUpdatesMaterialPurchasePrice"},
		{table: "req_api", code: "API-107-01", title: "API 测试覆盖 POST /api/purchase/receipts 返回库存批次号并传递操作人", status: "done", assignee: "Codex", evidence: "TestPurchaseReceiptAPIReceivesMaterial"},
		{table: "req_review", code: "REV-107-01", prCode: "PR-107", title: "验收：采购页可建供应商/采购单，收货后原料批次和库存流水增加，物料采购价更新", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
		{table: "req_product", code: "PR-108", title: "账号治理 P1：用户权限页支持员工登录启停和管理员重置密码", status: "review", assignee: "VA", evidence: "codex/p1-ops-20260428"},
		{table: "req_dev", code: "DEV-108-01", title: "登录密码表新增 login_disabled/must_reset_password，登录与 Bearer 会话解析拒绝停用账号", status: "done", assignee: "Codex", evidence: "employee_login_passwords columns; resolveEmployeeBySessionToken"},
		{table: "req_dev", code: "DEV-108-02", title: "新增账号列表、账号启停和密码重置 API，要求 auth.manage 权限并写操作日志", status: "done", assignee: "Codex", evidence: "GET /api/auth/accounts; POST /api/auth/account-state; POST /api/auth/password/reset"},
		{table: "req_dev", code: "DEV-108-03", title: "用户权限 Vue 页面增加账号列，可切换可登录/已停用并重置员工密码", status: "done", assignee: "Codex", evidence: "UserPermissionsView account controls"},
		{table: "req_unit", code: "UT-108-01", title: "单测覆盖 support 鉴权接口和登录账号治理编译路径", status: "done", assignee: "Codex", evidence: "go test ./internal/interfaces/http/support"},
		{table: "req_api", code: "API-108-01", title: "API 级验证账号治理接口受 auth.manage 保护，停用账号不能继续登录或解析会话", status: "done", assignee: "Codex", evidence: "go test ./internal/interfaces/http/support; deployment smoke"},
		{table: "req_review", code: "REV-108-01", prCode: "PR-108", title: "验收：管理员可停用/启用员工登录并重置密码；停用员工无法登录和继续访问", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
	} {
