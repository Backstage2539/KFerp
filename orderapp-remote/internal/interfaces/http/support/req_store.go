...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_dev", code: "DEV-161-02", title: "销售单设置页公章大小上限扩到 120mm，滑块或数字框变更后自动保存到 seal-position 接口", status: "done", assignee: "Codex", evidence: "SalesOrderSettingsView seal_width_mm @change; salesOrderSealMaxWidthMM"},
		...
		{table: "req_product", code: "PR-163", title: "销售单设置页公章下移后预览不能消失，设置页必须能看见并继续调整当前位置", status: "review", assignee: "VA", evidence: "codex/sales-order-seal-settings-preview-visibility-20260509; OP_MANUAL_ORDER_SALES.md"},
		{table: "req_dev", code: "DEV-163-01", title: "销售单设置页公章预览改为可滚动 A4 坐标画布，视窗高度随当前公章 Y 坐标和高度撑开，避免短预览框裁掉公章", status: "done", assignee: "Codex", evidence: "SalesOrderSettingsView seal-position-stage; salesOrderSealSettingsViewportHeight"},
		{table: "req_unit", code: "UT-163-01", title: "单元测试覆盖设置页公章视窗高度 helper、可滚动 A4 画布源码守卫、手册和需求种子", status: "done", assignee: "Codex", evidence: "sales-order.test.js; dev_163_sales_order_settings_seal_preview_visibility_test.go"},
		...
		{table: "req_review", code: "REV-163-01", prCode: "PR-163", title: "验收：销售单设置页把公章往下拖后，预览区域仍能看到公章并继续拖动；保存后重新生成 PDF 或图片使用最新位置", status: "review", assignee: "VA", evidence: "待 Van ERP 后台验收；手册 OP_MANUAL_ORDER_SALES.md"},
		...
		{table: "req_dev", code: "DEV-167-03", title: "成本核算和豆单预览带出 bom_status/warnings，BOM已失效时在列表和豆单区域显示警示，不静默发布", status: "done", assignee: "Codex", evidence: "costing.ProductInput warnings; CostingView warning-chip"},
		...
		{table: "req_product", code: "PR-169", title: "产品设置把商品基础信息和客户 SKU 列表合并为客户SKU列表，默认展示公共SKU，并可切换已有自定义 SKU 的客户", status: "review", assignee: "VA", evidence: "codex/product-settings-sku-list-20260509; OP_MANUAL_INVENTORY_MATERIALS.md"},
		...
		{table: "req_dev", code: "DEV-169-02", title: "客户SKU列表默认展示公共SKU；客户选择列表只存在有自定义SKU的客户，选择后只展示该客户自定义SKU", status: "done", assignee: "Codex", evidence: "ProductSettingsView customerSkuCustomers; customProductCustomerIDs"},
		...
		{table: "req_review", code: "REV-169-01", prCode: "PR-169", title: "验收：进入产品设置看到客户SKU列表默认公共SKU；选择已有自定义SKU的客户后仅显示该客户SKU；无自定义SKU客户不出现在选择列表", status: "review", assignee: "VA", evidence: "待 Van ERP 后台验收；REQUIREMENTS.md; ACCEPTANCE_TESTS.md; OP_MANUAL_INVENTORY_MATERIALS.md"},
		{table: "req_product", code: "PR-170", title: "产品设置客户上下文置顶，选择客户SKU后客户自己的商品分类、豆单和价格试算必须同步切换", status: "review", assignee: "VA", evidence: "codex/product-settings-customer-context-20260509; OP_MANUAL_INVENTORY_MATERIALS.md; OP_MANUAL_COSTING.md"},
		...
		{table: "req_dev", code: "DEV-170-03", title: "产品设置页把客户上下文传给成本核算，客户自己的豆单和客户自己的价格试算按同一 SKU归属过滤", status: "done", assignee: "Codex", evidence: "CostingView customerContextId; visibleCostingItems"},
		...
		{table: "req_review", code: "REV-170-01", prCode: "PR-170", title: "验收：产品设置顶部先选择 SKU归属；公共和每个客户看到各自商品分类；客户豆单和价格试算跟随同一客户上下文", status: "review", assignee: "VA", evidence: "待 Van ERP 后台验收；REQUIREMENTS.md; ACCEPTANCE_TESTS.md; OP_MANUAL_INVENTORY_MATERIALS.md; OP_MANUAL_COSTING.md"},
