...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_product", code: "PR-135", title: "销售单公章拖动必须稳定，点击公章不跳走；修改公章位置后新生成图片使用最新公章位置", status: "review", assignee: "VA", evidence: "codex/sales-order-seal-drag-image-20260502"},
		{table: "req_dev", code: "DEV-135-01", title: "销售单设置页公章只在按住公章本身时拖动，按拖动位移计算坐标，避免把点击点当成左上角导致跳走", status: "done", assignee: "Codex", evidence: "beginSalesOrderSealDrag; moveSalesOrderSealDrag; SalesOrderSettingsView"},
		{table: "req_dev", code: "DEV-135-02", title: "销售单设置页公章拖动松手自动保存坐标；销售单预览保存后提示需要重新生成图片或 PDF 后下载", status: "done", assignee: "Codex", evidence: "saveSealPosition; POST /api/settings/sales-order/seal-position"},
		{table: "req_unit", code: "UT-135-01", title: "单测覆盖公章拖动保留点击偏移、不跳走，源码守卫覆盖设置页松手自动保存和重新生成提示", status: "done", assignee: "Codex", evidence: "sales-order.test.js; dev_135_sales_order_seal_drag_image_test.go"},
		{table: "req_api", code: "API-135-01", title: "API 测试覆盖保存公章坐标后确认生成销售单图片，新图片快照写入最新 x/y/width", status: "done", assignee: "Codex", evidence: "TestSalesOrderImageGenerationUsesSavedSealPosition"},
		{table: "req_review", code: "REV-135-01", prCode: "PR-135", title: "验收：点击并拖动公章不会跳走；松手后刷新预览坐标不丢；重新生成并下载图片后公章位置跟随最新设置", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
