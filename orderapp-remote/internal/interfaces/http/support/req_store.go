...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_dev", code: "DEV-137-03", title: "分享页免登录但创建分享链接必须鉴权并具备订单写权限，避免未授权生成外部链接", status: "done", assignee: "Codex", evidence: "isPublicUnauthenticatedPath(/share/); AuthorizationMiddleware /api/share-resources"},
		...
		{table: "req_api", code: "API-137-01", title: "API 测试覆盖同一 /api/share-resources 流程生成销售单 PDF、销售单图片和出库单 PDF 分享链接，公开分享页与文件可访问", status: "done", assignee: "Codex", evidence: "TestExternalShareResourceAPIUsesOneFlowForSalesOrderAndDeliveryNote"},
		{table: "req_review", code: "REV-137-01", prCode: "PR-137", title: "验收：销售单和出库单已生成版本后可分享到微信；客户打开分享链接无需登录；同一套外部分享逻辑适配不同资源类型", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
		...
		{table: "req_product", code: "PR-139", title: "出库单公章设置和订单抽屉优化：出库单复用销售单公章，订单列表快递信息合并与订单状态合并，单号在订单抽屉回填", status: "review", assignee: "VA", evidence: "codex/delivery-note-seal-order-drawer-20260503"},
		{table: "req_dev", code: "DEV-139-01", title: "出库单预览和 PDF 复用销售单公章资产、坐标和透明背景处理，并提供出库单页公章设置入口", status: "done", assignee: "Codex", evidence: "DeliveryNoteSnapshot.Seal; DeliveryNoteRenderer; DeliveryNoteView; CompanySealSettingsView"},
		{table: "req_dev", code: "DEV-139-02", title: "订单列表收窄为订单、快递信息和订单状态等关键列；点击订单打开抽屉展示寄件人、单号、收款、发货、生产和发票状态，并支持单订单回填单号", status: "done", assignee: "Codex", evidence: "OrdersView orderDetailDrawerOpen/status-stack; POST /api/orders/:id/shipping-tracking"},
		{table: "req_unit", code: "UT-139-01", title: "单测覆盖单订单单号回填服务校验、出库单公章 PDF 渲染、Vue 源码守卫和需求种子", status: "done", assignee: "Codex", evidence: "TestServiceNormalizesSingleOrderTracking; TestRenderDeliveryNotePDFEmbedsConfiguredSeal; dev_139_delivery_note_seal_order_drawer_test.go"},
		{table: "req_api", code: "API-139-01", title: "API 测试覆盖出库单预览返回公章、订单列表返回最近发货寄件人、订单抽屉单号回填后标记已发货", status: "done", assignee: "Codex", evidence: "TestDeliveryNotePreviewIncludesConfiguredSeal; TestOrdersListIncludesLatestShipmentSender; TestOrdersSingleShippingTrackingAPIMarksOrderShipped"},
		{table: "req_review", code: "REV-139-01", prCode: "PR-139", title: "验收：出库单页可设置并拖动公章，新生成出库单 PDF 带公章；订单列表不再堆叠过多列，点击订单抽屉可查看快递和四类状态，并直接回填单号", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
