...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_product", code: "PR-143", title: "手机从 /app 登录时只需一次外层 order 认证和一次系统账号登录，不因根路径跳转反复登录", status: "review", assignee: "VA", evidence: "codex/mobile-login-prefix-20260503"},
		{table: "req_dev", code: "DEV-143-01", title: "登录页短信/密码 API 请求和登录成功回跳必须保留当前 /app 前缀，避免离开手机 BasicAuth 保护空间", status: "done", assignee: "Codex", evidence: "templates/login.html appPath"},
		{table: "req_dev", code: "DEV-143-02", title: "Vue/Vite API 客户端和退出回登录逻辑必须根据当前 URL 自动补 /app 前缀，页面内返回/跳转不离开入口前缀", status: "done", assignee: "Codex", evidence: "frontend-vue-shell/src/api/client.js appURL/apiURL; App.vue redirectToLogin"},
		{table: "req_dev", code: "DEV-143-03", title: "后端旧入口和 Vue shell 跳转必须使用可解析到当前前缀根的相对 Location，兼容 /app/* 和根路径访问", status: "done", assignee: "Codex", evidence: "support.PrefixRelativeLocation; VueShellRedirectWith; legacy redirects"},
		{table: "req_unit", code: "UT-143-01", title: "单测覆盖登录页前缀保留、Vue API URL 前缀保留、无 token 回登录保留前缀、旧入口相对重定向和需求种子", status: "done", assignee: "Codex", evidence: "auth_middleware_test.go; client.test.js; dev_143_mobile_login_prefix_test.go"},
		{table: "req_api", code: "API-143-01", title: "API/handler 级测试覆盖 legacy routes Location 为相对地址并解析到 Vue shell，订单 API redirect_url 保留当前前缀语义", status: "done", assignee: "Codex", evidence: "go test ./internal/interfaces/http/support ./internal/interfaces/http/sales ./internal/interfaces/http/production -count=1"},
		{table: "req_review", code: "REV-143-01", prCode: "PR-143", title: "验收：手机打开 https://erp.qacoohee.com/app/ 后输入一次 order 和一次系统账号即可进入；刷新/页面跳转/退出回登录不再要求重复认证", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
