...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_dev", code: "DEV-164-02", title: "订单保存后端支持同规格专属梯度优先和跨规格豆单重量梯度回退，显式梯度 ID 跨规格时按元/磅计算", status: "done", assignee: "Codex", evidence: "postgres/sales Repository.SaveOrder weight-tier fallback"},
		{table: "req_unit", code: "UT-164-01", title: "单元测试覆盖 1000g × 30 袋前端按 454g 豆单 49lb+ 档自动得出 48 元/磅和正确小计", status: "done", assignee: "Codex", evidence: "order-entry.test.js syncWholesaleTierPrice falls back to bean-list weight tiers"},
		{table: "req_api", code: "API-164-01", title: "API 测试覆盖 /api/order 保存 1000g × 30 袋时写入匹配到的 price_tier_id、48 元/磅单价和折算小计", status: "done", assignee: "Codex", evidence: "TestOrderAPISavesWholesale1000gByBeanListWeightTier; dev_164_order_weight_tier_pricing_test.go"},
		{table: "req_review", code: "REV-164-01", prCode: "PR-164", title: "验收：录单选择榛巧类商品，规格 1000g、数量 30 袋，页面立即显示 49lb+ 档单价和 30000g 折算小计，保存后订单金额一致", status: "review", assignee: "VA", evidence: "待 Van ERP 后台验收；REQUIREMENTS.md; ACCEPTANCE_TESTS.md; OP_MANUAL_ORDER_SALES.md"},
