...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_product", code: "PR-131", title: "生产质检对象选择抽屉：按仓库库存式工作台布局选择工单质检、原料质检和产品质检对象", status: "review", assignee: "VA", evidence: "codex/production-quality-drawer-20260502"},
		...
		{table: "req_dev", code: "DEV-131-02", title: "生产质检页右侧抽屉复用工单、原料批次和库存批次 API 选择对象并回填单据号/批次号和名称", status: "done", assignee: "Codex", evidence: "quality-inspections.js; QualityInspectionsView targetDrawerOpen"},
