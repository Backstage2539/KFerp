...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_product", code: "PR-130", title: "生产质检对象选择抽屉：按仓库库存式工作台布局选择工单质检、原料质检和产品质检对象", status: "review", assignee: "VA", evidence: "codex/production-quality-drawer-20260502"},
		{table: "req_dev", code: "DEV-130-01", title: "生产质检页改为工作台布局，左侧切换工单质检、原料质检、产品质检，表单保留结果、指标和备注", status: "done", assignee: "Codex", evidence: "QualityInspectionsView workspace/type-panel"},
		{table: "req_dev", code: "DEV-130-02", title: "生产质检页右侧抽屉复用工单、原料批次和库存批次 API 选择对象并回填单据号/批次号和名称", status: "done", assignee: "Codex", evidence: "quality-inspections.js; QualityInspectionsView targetDrawerOpen"},
		{table: "req_unit", code: "UT-130-01", title: "单测覆盖质检对象 helper、Vue 工作台抽屉源码守卫、需求种子和生产手册更新", status: "done", assignee: "Codex", evidence: "quality-inspections.test.js; dev_130_quality_drawer_test.go"},
		{table: "req_api", code: "API-130-01", title: "API 验证复用 GET /api/produce/work-orders、GET /api/stock/material-batches、GET /api/stock/batches 作为质检对象来源，保存仍走 POST /api/produce/quality-inspections", status: "done", assignee: "Codex", evidence: "TestManufacturingGapAPIs; TestStockAPIRoutes; QualityInspectionsView API wiring"},
		{table: "req_review", code: "REV-130-01", prCode: "PR-130", title: "验收：生产质检页点击选择质检对象后右侧弹出工单/原料/产品三类候选，选择后可保存质检记录", status: "todo", assignee: "VA", evidence: "待 Van 服务器验收"},
	} {
