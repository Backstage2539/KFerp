...
func seedReqWorkflowA(ctx context.Context, pool *pgxpool.Pool, schema string) error {
	...
	for _, row := range []reqSeedRow{
		...
		{table: "req_product", code: "PR-151", title: "新增正式环境：未来线上发布默认走正式环境，开发环境发布链路保留不动", status: "review", assignee: "VA", evidence: "codex/production-environment-20260503"},
		{table: "req_dev", code: "DEV-151-01", title: "发布脚本新增 production/development 环境 profile，默认 production，正式环境要求 main/origin/main 并写入 /opt/stacks/erp-production", status: "done", assignee: "Codex", evidence: "deploy_orderapp.sh --print-plan; deploy/production"},
		{table: "req_dev", code: "DEV-151-02", title: "开发环境保留旧 develop/origin/develop 和 /opt/stacks/erp 目标，只能通过 development 显式发布", status: "done", assignee: "Codex", evidence: "deploy_orderapp.sh development"},
		{table: "req_unit", code: "UT-151-01", title: "单测/源码守卫覆盖正式环境需求种子、默认 production 发布计划、development 发布计划和发布文档", status: "done", assignee: "Codex", evidence: "dev_151_production_environment_test.go"},
		{table: "req_api", code: "API-151-01", title: "API 级脚本接口测试覆盖 --print-plan 输出，验证正式/开发环境目标分支和服务器目录不会混用", status: "done", assignee: "Codex", evidence: "./deploy_orderapp.sh --print-plan; ./deploy_orderapp.sh --print-plan development"},
		{table: "req_review", code: "REV-151-01", prCode: "PR-151", title: "验收：执行发布计划可看到默认正式环境 production；开发环境必须显式 development 且目标仍为 /opt/stacks/erp", status: "todo", assignee: "VA", evidence: "待 Van 验收发布计划"},
		{table: "req_product", code: "PR-152", title: "正式环境登录修复：旧 token 或 API 401 不能反复弹出浏览器原生 order 密码框，admin 默认账号登录后必须正常显示工作台", status: "review", assignee: "VA", evidence: "codex/production-environment-20260503"},
		{table: "req_dev", code: "DEV-152-01", title: "BasicAuth 中间件区分页面访问与 API/Bearer 失败：页面可触发外层 BasicAuth，API 或 Bearer 失效只返回 JSON 401", status: "done", assignee: "Codex", evidence: "shouldAdvertiseBasicAuthChallenge; auth_middleware.go"},
		{table: "req_unit", code: "UT-152-01", title: "单测覆盖 API 请求、/app/api 请求和失效 Bearer token 不返回 BasicAuth challenge，页面请求仍允许外层提示", status: "done", assignee: "Codex", evidence: "TestBasicAuthChallengeIsNotAdvertisedForAPIOrBearerFailures"},
		{table: "req_api", code: "API-152-01", title: "公网 API 验证失效 Bearer 的 /app/api/auth/me 返回 401 JSON 且无 WWW-Authenticate；admin/admin 登录后 /api/auth/me 和首屏 API 返回 200", status: "done", assignee: "Codex", evidence: "postdeploy curl /app/api/auth/me; /app/api/order/form; browser smoke"},
		{table: "req_review", code: "REV-152-01", prCode: "PR-152", title: "验收：浏览器有旧 token 或系统账号过期时不再反复弹 order 密码框；打开 /app/login 使用 admin/admin 后能看到工作台菜单", status: "todo", assignee: "VA", evidence: "待 Van 验收"},
		{table: "req_product", code: "PR-153", title: "公网入口可在正式环境和开发环境之间显式切换，切换只操作 Caddy，不停止两边的 orderapp/postgres", status: "review", assignee: "VA", evidence: "codex/production-environment-20260503"},
		{table: "req_dev", code: "DEV-153-01", title: "部署脚本新增 --switch-public production|development，按目标环境停止非目标 Caddy 并启动目标 Caddy", status: "done", assignee: "Codex", evidence: "deploy_orderapp.sh --switch-public"},
		{table: "req_dev", code: "DEV-153-02", title: "部署文档、README、AGENTS 和本地部署 skill 记录正式/开发公网入口切换规则和手动兜底命令", status: "done", assignee: "Codex", evidence: "DEPLOYMENT.md; README.md; AGENTS.md; kferp-deploy skill"},
		{table: "req_unit", code: "UT-153-01", title: "单测/源码守卫覆盖公网切换脚本参数、切换文档和需求种子，防止切换逻辑丢失", status: "done", assignee: "Codex", evidence: "TestPublicEnvironmentSwitchRequirementSeeds; TestDeploymentDocsDescribePublicSwitching"},
		{table: "req_api", code: "API-153-01", title: "API/运维级验证执行 --switch-public development 后公网命中开发环境，需求表含 PR-150 且不含正式试运行 PR-151", status: "done", assignee: "Codex", evidence: "./deploy_orderapp.sh --switch-public development; /app/api/req/product?limit=500"},
		{table: "req_review", code: "REV-153-01", prCode: "PR-153", title: "验收：公网从正式环境切回开发环境后，开发环境 Caddy 绑定 443，正式环境 Caddy 停止，双方应用和数据库保留运行", status: "todo", assignee: "VA", evidence: "待 Van 验收"},
