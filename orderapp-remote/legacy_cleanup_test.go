...
func TestGoRoutesDoNotKeepLegacyTemplateBranches(t *testing.T) {
	err := filepath.WalkDir(".", func(path string, entry os.DirEntry, err error) error {
		...
		if entry.IsDir() {
			if path == "docs" || path == "frontend-vue-shell" || strings.HasPrefix(path, "internal/") {
...
func TestCompanyStaffAPIUsesApplicationService(t *testing.T) {
	body, err := os.ReadFile("company_staff.go")
	if err != nil {
		t.Fatal(err)
	}
	src := string(body)
...
func TestProduceBatchAPIUsesApplicationServiceForReadModels(t *testing.T) {
	body, err := os.ReadFile("produce_batch_api.go")
	if err != nil {
		t.Fatal(err)
	}
	src := string(body)
...
func TestDeductPreviewContractTestIsNotSkipped(t *testing.T) {
	body, err := os.ReadFile("produce_batch_deduct_preview_test.go")
	if err != nil {
		t.Fatal(err)
	}
	src := string(body)
