...
SCENARIOS = [
    Scenario(
        ...
        steps=[
            ...
            Step("read product catalog group template selection", "GET", "/api/business-group-feature-selections/product_catalog"),
            Step("add generated group to product catalog selection", "PUT", "/api/business-group-feature-selections/product_catalog", write=True),
            Step("read production BOM group template selection", "GET", "/api/business-group-feature-selections/production_bom"),
            Step("add generated group to production BOM selection", "PUT", "/api/business-group-feature-selections/production_bom", write=True),
            Step("read warehouse inventory group template selection", "GET", "/api/business-group-feature-selections/warehouse_inventory"),
            Step("add generated group to warehouse inventory selection", "PUT", "/api/business-group-feature-selections/warehouse_inventory", write=True),
...
def run_generated_main_flow(
    ...
    add_generated_group_to_feature_selections(client, context, cleanup_stack, results, failures)
...
def add_generated_group_to_feature_selections(
    client: Client,
    context: dict[str, Any],
    cleanup_stack: list[CleanupAction],
    results: list[dict[str, Any]],
    ...
    if group_id <= 0:
        failures.append("cannot select generated group: missing generated group")
        return
    features = (
    ...
    for feature_key, label in features:
        path = f"/api/business-group-feature-selections/{feature_key}"
        current = request_step(
            client,
            context,
            results,
            failures,
            f"read {label} group template selection",
            "GET",
            path,
        )
        if not isinstance(current, dict):
            continue
        original_ids = ordered_positive_ids(current.get("group_template_ids"))
        ...
        saved = request_step(
            ...
            results,
            failures,
            f"add generated group to {label} selection",
            "PUT",
            path,
            body={"feature_key": feature_key, "group_template_ids": selected_ids},
        )
        if not isinstance(saved, dict):
            continue
        cleanup_stack.append(
            CleanupAction(
                f"restore {feature_key} group template selection",
                "PUT",
                path,
                {"feature_key": feature_key, "group_template_ids": original_ids},
            )
        )
...
def ordered_positive_ids(values: Any) -> list[int]:
    out: list[int] = []
    seen: set[int] = set()
    for value in values if isinstance(values, list) else []:
        item_id = int_value(value)
